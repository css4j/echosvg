	...
	protected boolean readRecords(DataInputStream is) throws IOException {
		...
		while (functionId > 0) {
			...
			case WMFConstants.META_EXTTEXTOUT: {
				...
				/*
				 * get the StringRecord, having decoded the String, using the current charset
				 * (which was given by the last META_CREATEFONTINDIRECT)
				 */
			...
			case WMFConstants.META_TEXTOUT: {
				...
				/*
				 * get the StringRecord, having decoded the String, using the current charset
				 * (which was givben by the last META_CREATEFONTINDIRECT)
				 */
				mr = new MetaRecord.ByteRecord(bstr);
				mr.numPoints = recSize;
				mr.functionId = functionId;
			...
			case WMFConstants.META_SETWINDOWEXT: {
				...
				// sets the width, height of the image if the file does not have an APM (in this
				// case it is retrieved
				// from the viewport)
				if (!isAldus) {
					this.width = vpW;
					this.height = vpH;
			...
			case WMFConstants.META_CREATEPENINDIRECT: {
				...
				/**
				 * sometimes records generated by PPT have a recSize of 6 and not 5 => in this
				 * case only we have to read a last short element
				 **/
				// int height = readShort( is );
				if (recSize == 6)
					readShort(is);
		...
		// sets the characteristics of the image if the file does not have an APM (in
		// this case it is retrieved
		// from the viewport). This is only useful if one wants to retrieve informations
		// about the file after
		// decoding it.
		if (!isAldus) {
			right = (int) vpX;
			left = (int) (vpX + vpW);
