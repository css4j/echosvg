 ...
 * encoding.
 * <p>
 * This class is used as a replacement for {@link java.net.URL}. This is done
 * for several reasons. First, unlike {@link java.net.URL} this class will
 * accept and parse as much of a URL as possible, without throwing a
 * {@link java.net.MalformedURLException}. This makes it useful for simply
 * parsing a URL string (hence its name).
 * </p>
 * <p>
 * Second, it allows for extension of the URI schemes supported by the parser.
 * EchoSVG uses this to support the
 * <a href='http://www.ietf.org/rfc/rfc2397'><code>data:</code> URL scheme
 * (RFC2397)</a>.
 * </p>
 * <p>
 * Third, by default it checks the streams that it opens to see if they are GZIP
 * compressed, and if so it automatically uncompresses them (avoiding opening
 * the stream twice in the process).
 * </p>
 * <p>
 * It is worth noting that most real work is defered to the
 * {@link ParsedURLData} class to which most methods are forwarded. This is done
 * because it allows a constructor interface to {@link ParsedURL} (mostly for
 * compatability with core {@link URL}), in spite of the fact that the real
 * implemenation uses the protocol handlers as factories for protocol specific
 * instances of the {@link ParsedURLData} class.
 * </p>
 *
...
public class ParsedURL {
	...
	/**
	 * Returns the shared instance of HandlersMap. This method is also responsible
	 * for initializing the handler map if this is the first time it has been
	 * requested since the class was loaded.
	 */
	 ...
	 * <code>null</code> or no match is found in the handlers map it returns the
	...
	/**
	 * Registers a Protocol handler by adding it to the handlers map. If the given
	 * protocol handler returns <code>null</code> as it's supported protocol then it
	 * is registered as the default protocol handler.
	 * 
	...
	/**
	 * This is a utility function others can call that checks if is is a GZIP stream
	 * if so it returns a GZIPInputStream that will decode the contents, otherwise
	 * it returns (or a buffered version of is) untouched.
	 * 
	...
	/**
	 * Construct a ParsedURL from the given java.net.URL instance. This is useful if
	 * you already have a valid java.net.URL instance. This bypasses most of the
	 * parsing and hence is quicker and less prone to reinterpretation than
	 * converting the URL to a string before construction.
	 *
	...
	/**
	 * Return a string rep of the URL (can be passed back into the constructor if
	 * desired).
	 */
	@Override
	...
	/**
	 * Returns the URL that was ultimately used to fetch the resource represented by
	 * this <code>ParsedURL</code>. For HTTP URLs, this will result in the
	 * post-redirect URL being returned. If there was no redirect, or if this isn't
	 * an HTTP URL, the original URL is returned (the same string as
	 * {@link #toString()}).
	 */
	...
	/**
	 * Implement Object.equals. Relies heavily on the contained ParsedURLData's
	...
	/**
	 * Implement Object.hashCode. Relies on the contained ParsedURLData's
	...
	/**
	 * Sets the user agent associated with this url (null clears any associated user
	 * agent).
	 */
	...
	/**
	 * Returns the host for this URL, if any, <code>null</code> if there isn't one
	...
	/**
	 * Returns the port on the host to connect to, if it was specified in the url
	...
	/**
	 * Returns the path for this URL, if any (where appropriate for the protocol
	 * this also includes the file, not just directory). Note that getPath appears
	...
	/**
	 * Returns the content type if available. This is only available for some
	 * protocols.
	 */
	...
	/**
	 * Returns the content type's type/subtype, if available. This is only available
	 * for some protocols.
	 */
	...
	/**
	 * Returns the content type's charset parameter, if available. This is only
	...
	/**
	 * Returns the content encoding if available. This is only available for some
	 * protocols.
	 */
	...
	/**
	 * Attempt to open the stream checking for common compression types, and
	 * automatically decompressing them if found.
	 */
	 ...
	 * automatically decompressing them if found.
	 * 
	 * @param mimeType The expected mime type of the content in the returned
	 *                 InputStream (mapped to Http accept header among other
	 *                 possibilities).
	 */
	 ...
	 * automatically decompressing them if found.
	 * 
	 * @param mimeTypes The expected mime types of the content in the returned
	 *                  InputStream (mapped to Http accept header among other
	 *                  possabilities).
	 */
	 ...
	 * @param mimeTypes The expected mime types of the content in the returned
	 *                  InputStream (mapped to Http accept header among other
	 *                  possabilities). The elements of the iterator must be
	 *                  strings.
	 */
	...
	/**
	 * Parse out the protocol from a url string. Used internally to select the
	 * proper handler, all other parsing is done by the selected protocol handler.
	 */
	...
	/**
	 * Factory method to construct an appropriate subclass of ParsedURLData, for a
	 * sub url.
	 * 
