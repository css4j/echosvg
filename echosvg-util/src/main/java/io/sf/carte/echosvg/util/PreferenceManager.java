...
public class PreferenceManager {
	 ...
	 * 
	 * @param prefFileName the name of the preference file.
	 * @param defaults     where to get defaults value if the value is not specified
	 *                     in the file.
	 */
	...
	/**
	 * Sets a <code>String</code> representing the directory where
	 * <code>PreferenceManager</code> instances should look for preferences files.
	 * The default value is <code>null</code> which means the automatic mechanism
	 * for looking for preferences is used.
	 * 
	 * @see #load
	 */
	...
	/**
	 * Returns a <code>String</code> representing the directory where
	 * <code>PreferenceManager</code> instances should look for preferences.
	 * 
	 * @see #load
	...
	/**
	 * Loads the preference file. If the file has already been previously sucessfuly
	 * loaded or saved, it will first try to reaload it from this location.
	 * Otherwise, it will try to find the file in the following order: in the
	 * directory set by {@link #setPreferenceDirectory} if it exists, in the user
	...
	public void load() throws IOException {
		...
		if (fullName == null) {
			if (PREF_DIR != null) {
				try {
					fis = new FileInputStream(fullName = PREF_DIR + FILE_SEP + prefFileName);
			...
			if (fullName == null) {
				try {
					fis = new FileInputStream(fullName = USER_HOME + FILE_SEP + prefFileName);
				} catch (IOException e3) {
					try {
						fis = new FileInputStream(fullName = USER_DIR + FILE_SEP + prefFileName);
	...
	/**
	 * Saves the preference file. If it has previously sucessfuly been loaded or
	 * save it will save it at the same location. In other cases it will save it in
	 * the directory set by {@link #setPreferenceDirectory} if has been set and
	 * exists, otherwise in the user home directory.
	 * 
	 * @exception IOException if an error occured when writing the file or if is
	 *                        impossible to write the file at all available
	 *                        locations.
	 * @see #load
	 */
	public void save() throws IOException {
		...
		if (fullName == null) {
			if (PREF_DIR != null) {
				try {
					fos = new FileOutputStream(fullName = PREF_DIR + FILE_SEP + prefFileName);
				} catch (IOException e2) {
					fullName = null;
				}
			}
			if (fullName == null) {
				try {
					fos = new FileOutputStream(fullName = USER_HOME + FILE_SEP + prefFileName);
				} catch (IOException e3) {
					fullName = null;
					throw e3;
				}
	...
	/**
	 * Sets a String array preference. If null or size null removes previous
	 * preference.
	 */
	...
	/**
	 * Sets an array of URLs property. If null or size null removes previous
	 * preference.
	 */
	...
	/**
	 * Sets an array of Files property. If null or size null removes previous
	 * preference.
	 */
	...
	public void setInteger(String key, int value) {
		if (getDefault(key) != null && (Integer) getDefault(key) != value)
