...
public class ParsedURLData implements Serializable {
	...
	public static InputStream checkGZIP(InputStream is) throws IOException {
		...
		if (((data[0] & 0x0F) == 8) && ((data[0] >>> 4) <= 7)) {
			...
			int chk = ((data[0] & 0xFF) * 256 + (data[1] & 0xFF));
			if ((chk % 31) == 0) {
				try {
	...
	protected void extractContentTypeParts(String userAgent) {
		...
		for (;;) {
			...
			if (contentType.charAt(i++) == 'c') {
				if (contentType.charAt(i++) != 'h')
					continue;
				if (contentType.charAt(i++) != 'a')
					continue;
				if (contentType.charAt(i++) != 'r')
					continue;
				if (contentType.charAt(i++) != 's')
					continue;
				if (contentType.charAt(i++) != 'e')
					continue;
				if (contentType.charAt(i++) != 't')
					continue;
				if (contentType.charAt(i++) != '=')
					continue;
				int j = i;
	...
	/**
	 * Open the stream and check for common compression types. If the stream is
	 * found to be compressed with a standard compression type it is automatically
	 * decompressed.
	 * 
	 ...
	 *                  possability). The elements of the iterator must be strings
	 *                  (may be null)
	 */
	...
	/**
	 * Open the stream and returns it. No checks are made to see if the stream is
	 * compressed or encoded in any way.
	 * 
	 * @param userAgent The user agent opening the stream (may be null).
	 * @param mimeTypes The expected mime types of the content in the returned
	 *                  InputStream (mapped to Http accept header among other
	...
	protected InputStream openStreamInternal(String userAgent, Iterator<String> mimeTypes,
			Iterator<String> encodingTypes) throws IOException {
		if (stream != null)
			return stream;
