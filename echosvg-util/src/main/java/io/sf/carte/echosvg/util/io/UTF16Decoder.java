...
public class UTF16Decoder extends AbstractCharDecoder {
	...
	/**
	 * Creates a new UTF16Decoder. It is assumed that the byte-order mark is
	 * present.
	 * 
	 ...
	 * 
	 * @param is The stream to decode.
	 * @param be Whether or not the given stream's byte-order is big-endian.
	 */
	...
	public int readChar() throws IOException {
		...
		int c = (bigEndian) ? (((b1 & 0xff) << 8) | (b2 & 0xff)) : (((b2 & 0xff) << 8) | (b1 & 0xff));
		if (c == 0xfffe) {
			charError("UTF-16");
		}
		return c;
