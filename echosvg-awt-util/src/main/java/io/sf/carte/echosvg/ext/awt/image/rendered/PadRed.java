...
public class PadRed extends AbstractRed {
	...
	/**
	 * Construct A Rendered Pad operation. If the pad is smaller than the original
	 * image size then this devolves to a Crop.
	 *
	...
	public PadRed(CachableRed src, Rectangle bounds, PadMode padMode, RenderingHints hints) {
		...
		if (DEBUG) {
			System.out.println("Src: " + src + " Bounds: " + bounds + " Off: " + src.getTileGridXOffset() + ", "
	...
	protected void handleZero(WritableRaster wr) {
		...
		// Draw #2
		if (DEBUG) {
			System.out.println("WrR: [" + ar.x + "," + ar.y + "," + ar.width + "," + ar.height + "] s rcR: " + srcR);
		...
		// Draw #3
		if (DEBUG) {
			System.out.println("WrR: [" + ar.x + "," + ar.y + "," + ar.width + "," + ar.height + "] srcR: " + srcR);
