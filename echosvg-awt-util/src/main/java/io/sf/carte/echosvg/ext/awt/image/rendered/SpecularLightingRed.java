...
public class SpecularLightingRed extends AbstractTiledRed {
	...
	public SpecularLightingRed(double ks, double specularExponent, Light light, BumpMap bumpMap, Rectangle litRegion,
			double scaleX, double scaleY, boolean linear) {
		this.ks = ks;
		...
		init((CachableRed) null, litRegion, cm, sm, litRegion.x, litRegion.y, null);
	}
		...
		if (light instanceof SpotLight) {
			...
			for (i = 0; i < h; i++) {
				...
				for (j = 0; j < w; j++) {
					...
					a = (int) (mult * Math.pow(N[0] * L[0] + N[1] * L[1] + N[2] * L[2], specularExponent) + 0.5);
