...
public abstract class MultipleGradientPaint implements Paint {
	...
	/**
	 * Indicates (if the gradient starts or ends inside the target region) to use
	 * the terminal colors to fill the remaining area. (default)
	 */
	public static final CycleMethodEnum NO_CYCLE = new CycleMethodEnum();

	/**
	 * Indicates (if the gradient starts or ends inside the target region), to cycle
	 * the gradient colors start-to-end, end-to-start to fill the remaining area.
	 */
	public static final CycleMethodEnum REFLECT = new CycleMethodEnum();

	/**
	 * Indicates (if the gradient starts or ends inside the target region), to cycle
	 * the gradient colors start-to-end, start-to-end to fill the remaining area.
	 */
	 ...
	 *
	 * @throws NullPointerException     if arrays are null, or gradientTransform is
	 *                                  null
	 *
	 * @throws IllegalArgumentException if fractions.length != colors.length, or if
	 *                                  colors is less than 2 in size, or if an
	 *                                  enumerated value is bad.
	 */
	public MultipleGradientPaint(float[] fractions, Color[] colors, CycleMethodEnum cycleMethod,
