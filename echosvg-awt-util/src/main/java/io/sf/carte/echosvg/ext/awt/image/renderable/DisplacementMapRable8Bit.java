...
import io.sf.carte.echosvg.ext.awt.image.rendered.DisplacementMapRed;
...
public class DisplacementMapRable8Bit extends AbstractColorInterpolationRable implements DisplacementMapRable {
	...
	/**
	 * Defines which channel in the second source is used to displace along the x
	 * axis
	 */
	private ARGBChannel xChannelSelector;

	/**
	 * Defines which channel in the second source is used to displace along the y
	 * axis.
	 */
	private ARGBChannel yChannelSelector;

	public DisplacementMapRable8Bit(List<Filter> sources, double scale, ARGBChannel xChannelSelector,
	...
	public RenderedImage createRendering(RenderContext rc) {
		...
		aoiR = new Rectangle2D.Double(aoiR.getX() - scale / 2, aoiR.getY() - scale / 2, aoiR.getWidth() + scale,
		...
		CachableRed cr = new DisplacementMapRed(GraphicsUtil.wrap(displacedRed), GraphicsUtil.wrap(mapRed),
				xChannelSelector, yChannelSelector, scaleX, scaleY, rh);
		//
		// Apply the non scaling part of the transform now,
		// if different from identity.
		//
		AffineTransform resAt = new AffineTransform(sx / atScaleX, shy / atScaleX, shx / atScaleY, sy / atScaleY, tx,
				ty);
