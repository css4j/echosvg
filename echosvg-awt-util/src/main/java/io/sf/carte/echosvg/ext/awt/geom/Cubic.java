...
public class Cubic extends AbstractSegment {
	...
	@Override
	public Object clone() {
		return new Cubic(new Point2D.Double(p1.x, p1.y), new Point2D.Double(p2.x, p2.y), new Point2D.Double(p3.x, p3.y),
				new Point2D.Double(p4.x, p4.y));
	}

	public Segment reverse() {
		return new Cubic(new Point2D.Double(p4.x, p4.y), new Point2D.Double(p3.x, p3.y), new Point2D.Double(p2.x, p2.y),
	...
	private void getMinMax(double p1, double p2, double p3, double p4, double[] minMax) {
		...
		for (int r = 0; r < roots; r++) {
			...
			if ((tv <= 0) || (tv >= 1))
				continue;
			tv = ((1 - tv) * (1 - tv) * (1 - tv) * p1 + 3 * tv * (1 - tv) * (1 - tv) * p2 + 3 * tv * tv * (1 - tv) * p3
					+ tv * tv * tv * p4);
			if (tv < minMax[0])
				minMax[0] = tv;
			else if (tv > minMax[1])
	...
	protected int findRoots(double y, double[] roots) {
		double[] eqn = { p1.y - y, 3 * (p2.y - p1.y), 3 * (p1.y - 2 * p2.y + p3.y), 3 * p2.y - p1.y + p4.y - 3 * p3.y };
	...
	public Point2D.Double evalDt(double t) {
		double x = 3 * ((p2.x - p1.x) * (1 - t) * (1 - t) + 2 * (p3.x - p2.x) * (1 - t) * t + (p4.x - p3.x) * t * t);
		double y = 3 * ((p2.y - p1.y) * (1 - t) * (1 - t) + 2 * (p3.y - p2.y) * (1 - t) * t + (p4.y - p3.y) * t * t);
		return new Point2D.Double(x, y);
	}

	@Override
	public Point2D.Double eval(double t) {
		double x = ((1 - t) * (1 - t) * (1 - t) * p1.x + 3 * (t * (1 - t) * (1 - t) * p2.x + t * t * (1 - t) * p3.x)
				+ t * t * t * p4.x);
		double y = ((1 - t) * (1 - t) * (1 - t) * p1.y + 3 * (t * (1 - t) * (1 - t) * p2.y + t * t * (1 - t) * p3.y)
				+ t * t * t * p4.y);
		return new Point2D.Double(x, y);
	}

	/**
	 * Subdivides this Cubic curve into two curves at t = 0.5. can be done with
	 * getSegment but this is more efficent.
	 * 
	...
	public Segment getSegment(double t0, double t1) {
		...
		Point2D.Double np2 = new Point2D.Double(np1.x + dt * dp1.x / 3, np1.y + dt * dp1.y / 3);

		Point2D.Double np4 = eval(t1);
		Point2D.Double dp4 = evalDt(t1);

		Point2D.Double np3 = new Point2D.Double(np4.x - dt * dp4.x / 3, np4.y - dt * dp4.y / 3);
	...
	/*
	 * public static boolean epsEq(double a, double b) { final double eps =
	 * 0.000001; return (((a + eps) > b) && ((a-eps) < b)); }
	 * 
	 * public static void sub(Cubic orig, Cubic curr, double t, double inc, int lev)
	 * { Cubic left=new Cubic(); Cubic right=new Cubic(); curr.subdivide(left,
	 * right); Point2D.Double ptl = left.eval(.5); Point2D.Double ptr =
	 * right.eval(.5); Point2D.Double pt1 = orig.eval(t-inc); Point2D.Double pt2 =
	 * orig.eval(t+inc); int steps = 100; Point2D.Double l, r, o; for (int i=0;
	 * i<=steps; i++) { l = left.eval(i/(double)steps); o =
	 * orig.eval(t-(2*inc)*(1-i/(double)steps)); if (!epsEq(l.x, o.x) || !epsEq(l.y,
	 * o.y)) System.err.println("Lf Pt: [" + l.x + "," + l.y + "] Orig: [" + o.x +
	 * "," + o.y +"]"); r = right.eval(i/(double)steps); o =
	 * orig.eval(t+(2*inc*i/(double)steps)); if (!epsEq(r.x, o.x) || !epsEq(r.y,
	 * o.y)) System.err.println("Rt Pt: [" + r.x + "," + r.y + "] Orig: [" + o.x +
	 * "," + o.y +"]"); } if (lev != 0) { sub(orig, left, t-inc, inc/2, lev-1);
	 * sub(orig, right, t+inc, inc/2, lev-1); } }
	 * 
	 * public static void evalCubic(Cubic c) {
	 * 
	 * int steps = 1000000; Point2D.Double oldP = c.eval(0); Point2D.Double newP;
	 * double len = 0; for (int i=1; i<=steps; i++) { newP =
	 * c.eval(i/(double)steps); double dx = newP.x-oldP.x; double dy =
	 * newP.y-oldP.y; len += Math.sqrt(dx*dx + dy*dy); oldP = newP; }
	 * System.err.println("Length(.1): " + c.getLength(.001) + " x " + count); count
	 * = 0; System.err.println("Length    : " + c.getLength() + " x " + count);
	 * count = 0; System.err.println("D  Len    : " + len); }
	 * 
	 * public static void main(String args[]) { Cubic c;
	 * 
	 * c = new Cubic(0,0, 10,10, 20,-10, 30,0); sub(c, c, .5, .25, 3); evalCubic(c);
	 * 
	 * c = new Cubic(0,0, 1,0, 2,-1, 3,0); sub(c, c, .5, .25, 3); evalCubic(c); }
	 */
}
