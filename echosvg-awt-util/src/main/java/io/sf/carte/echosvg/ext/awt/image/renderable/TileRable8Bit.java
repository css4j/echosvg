...
import io.sf.carte.echosvg.ext.awt.image.rendered.TileRed;
...
public class TileRable8Bit extends AbstractColorInterpolationRable implements TileRable {
	...
	/**
	 * Controls whether the tileRegion clips the source or not
	 */
	...
	public RenderedImage createRendering(RenderContext rc) {
		...
		// System.out.println("tileScaleX/tileScaleY : " + tileScaleX + " / " +
		// tileScaleY);
		...
		if ((tiledArea.width == Integer.MAX_VALUE) || (tiledArea.height == Integer.MAX_VALUE)) {
			tiledArea = new Rectangle(Integer.MIN_VALUE / 4, Integer.MIN_VALUE / 4, Integer.MAX_VALUE / 2,
					Integer.MAX_VALUE / 2);
		}
		...
		// io.sf.carte.echosvg.test.gvt.ImageDisplay.showImage("Tile", tiledRed);
	...
	public Rectangle2D getActualTileBounds(Rectangle2D tiledRect) {
		...
		if ((tileRect.getWidth() <= 0) || (tileRect.getHeight() <= 0) || (tiledRect.getWidth() <= 0)
	 ...
	 *
	 * The tile has its origin in the upper left corner of the tiled region. That
	 * tile is separated into 4 areas: top-left, top-right, bottom-left and
	 * bottom-right. Each of these areas is mapped to some input area from the
	 * source. If the source is smaller than the tiled area, then a single rendering
	 * is requested from the source. If the source's width or height is bigger than
	 * that of the tiled area, then separate renderings are requested from the
	 * source.
	 *
	 */
	public RenderedImage createTile(RenderContext rc) {
		...
		// System.out.println("dx / dy / w / h : " + dx + " / " + dy + " / " + w + " / "
		// + h);
		...
		if (B.getWidth() > 0 && B.getHeight() > 0) {
			...
			if (devB.width > 0 && devB.height > 0) {
				...
				// System.out.println("BRed : " + BRed.getMinX() + " / " + BRed.getMinY() + " /
				// " + BRed.getWidth() + " / " + BRed.getHeight());
			}
		}
		...
		if (C.getWidth() > 0 && C.getHeight() > 0) {
			...
			if (devC.width > 0 && devC.height > 0) {
				...
				// System.out.println("CRed : " + CRed.getMinX() + " / " + CRed.getMinY() + " /
		...
		if (D.getWidth() > 0 && D.getHeight() > 0) {
			...
			if (devD.width > 0 && devD.height > 0) {
				...
				Shape aoi = D;
				if (overflow) {
					aoi = new Rectangle2D.Double(D.x - tileWidth + w - dx, D.y - tileHeight + h - dy, tiledWidth,
							tiledHeight);
				}
		...
		BufferedImage realTileBI = new BufferedImage(realTileRectDev.width, realTileRectDev.height,
