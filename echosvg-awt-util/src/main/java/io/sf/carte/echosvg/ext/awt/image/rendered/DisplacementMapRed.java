...
public class DisplacementMapRed extends AbstractRed {
	 ...
	 * @param xChannel defines the channel of off whose values will be on X-axis
	 *                 operation
	 * @param yChannel defines the channel of off whose values will be
	 * @param scaleX   defines the scale factor of the filter operation on the X
	 *                 axis.
	 * @param scaleY   defines the scale factor of the filter operation on the Y
	 *                 axis
	 * @param rh       the rendering hints
	 */
	public DisplacementMapRed(CachableRed image, CachableRed offsets, ARGBChannel xChannel, ARGBChannel yChannel,
			float scaleX, float scaleY, RenderingHints rh) {
		...
		init(image, rect, cm, image.getSampleModel(), rect.x, rect.y, null);
	...
	public Raster getTile(int tileX, int tileY) {
		...
		if (USE_NN)
			filterNN(mapRas, dest, xinfo.tile, xinfo.off, yinfo.tile, yinfo.off);
		else if (image.getColorModel().isAlphaPremultiplied())
			filterBL(mapRas, dest, xinfo.tile, xinfo.off, yinfo.tile, yinfo.off);
		else
			filterBLPre(mapRas, dest, xinfo.tile, xinfo.off, yinfo.tile, yinfo.off);
	...
	public TileOffsets getXOffsets(int xTile) {
		...
		int x0 = getTileGridXOffset() + xTile * tw - maxOffX - image.getTileGridXOffset();
		int x1 = x0 + width - 1;
	...
	public TileOffsets getYOffsets(int yTile) {
		...
		int y0 = getTileGridYOffset() + yTile * th - maxOffY - image.getTileGridYOffset();
		int y1 = y0 + height - 1;
	...
	public void filterBL(Raster off, WritableRaster dst, int[] xTile, int[] xOff, int[] yTile, int[] yOff) {
		...
		final int dstOff = dstDB.getOffset() + dstSPPSM.getOffset(dst.getMinX() - dst.getSampleModelTranslateX(),
				dst.getMinY() - dst.getSampleModelTranslateY());

		offSPPSM = (SinglePixelPackedSampleModel) off.getSampleModel();
		final int offOff = offDB.getOffset() + offSPPSM.getOffset(dst.getMinX() - off.getSampleModelTranslateX(),
			...
			for (x = xStart; x < xEnd; x++, dp++, ip++) {
				...
				if ((yt == yt1)) {
					...
					if ((xt == xt1)) {
						...
						imgPix = ((DataBufferInt) image.getTile(xt1, yt).getDataBuffer()).getBankData()[0];
						pel10 = imgPix[xOff[x0 + 1] + yOff[y0]];
						pel11 = imgPix[xOff[x0 + 1] + yOff[y0 + 1]];
						xt = xt1;
					}
					...
					if ((xt == xt1)) {
						...
						imgPix = ((DataBufferInt) image.getTile(xt, yt1).getDataBuffer()).getBankData()[0];
						pel01 = imgPix[xOff[x0] + yOff[y0 + 1]];
						pel11 = imgPix[xOff[x0 + 1] + yOff[y0 + 1]];
						yt = yt1;
					} else {
						...
						imgPix = ((DataBufferInt) image.getTile(xt1, yt1).getDataBuffer()).getBankData()[0];
						pel11 = imgPix[xOff[x0 + 1] + yOff[y0 + 1]];
				...
				sp1 = (pel11 >> 8) & 0xFF00;
				pel1 = (sp0 + (((sp1 - sp0) * xFrac + 0x4000) >> 15)) & 0xFFFF;
				newPel |= (((pel0 << 15) + (pel1 - pel0) * yFrac + 0x00400000) & 0x7F800000) >>> 7;
				...
				sp1 = (pel11) & 0xFF00;
				pel1 = (sp0 + (((sp1 - sp0) * xFrac + 0x4000) >> 15)) & 0xFFFF;
				newPel |= (((pel0 << 15) + (pel1 - pel0) * yFrac + 0x00400000) & 0x7F800000) >>> 15;
				...
				sp1 = (pel11 << 8) & 0xFF00;
				pel1 = (sp0 + (((sp1 - sp0) * xFrac + 0x4000) >> 15)) & 0xFFFF;
				newPel |= (((pel0 << 15) + (pel1 - pel0) * yFrac + 0x00400000) & 0x7F800000) >>> 23;
	 ...
	 * @param dst stores the filtered image. If null, a destination will be created.
	 *            img and dst can refer to the same Raster, in which situation the
	...
	public void filterNN(Raster off, WritableRaster dst, int[] xTile, int[] xOff, int[] yTile, int[] yOff) {
		...
		offSPPSM = (SinglePixelPackedSampleModel) off.getSampleModel();
		final int offOff = offDB.getOffset() + offSPPSM.getOffset(off.getMinX() - off.getSampleModelTranslateX(),
