...
/**
 * This extension of the <code>java.awt.Graphics2D</code> abstract class is
 * still abstract, but it implements a lot of the <code>Graphics2D</code> method
 * in a way that concrete implementations can reuse.
 *
 * This class uses a <code>GraphicContext</code> to store the state of its
 * various attributes that control the rendering, such as the current
 * <code>Font</code>, <code>Paint</code> or clip.
 *
 * Concrete implementations can focus on implementing the rendering methods,
 * such as <code>drawShape</code>. As a convenience, rendering methods that can
 * be expressed with other rendering methods (e.g., <code>drawRect</code> can be
 * expressed as <code>draw(new Rectangle(..))</code>), are implemented by
 * <code>AbstractGraphics2D</code>
 *
...
public abstract class AbstractGraphics2D extends Graphics2D implements Cloneable {
	 ...
	 * state of this <code>Graphics2D</code> graphic context attributes.
	 */
	 ...
	 * at <code>x</code> and <code>x&nbsp;+&nbsp;width&nbsp;-&nbsp;1</code>. The top
	 * and bottom edges are at <code>y</code> and
	 * <code>y&nbsp;+&nbsp;height&nbsp;-&nbsp;1</code>. The resulting rectangle
	 * covers an area <code>width</code> pixels wide by <code>height</code> pixels
	 ...
	 * 0&nbsp;degrees is at the 3&nbsp;o'clock position. A positive value indicates
	 * a counter-clockwise rotation while a negative value indicates a clockwise
	 * rotation.
	 * <p>
	 * The center of the arc is the center of the rectangle whose origin is
	 * (<i>x</i>,&nbsp;<i>y</i>) and whose size is specified by the
	 ...
	 * point.
	 * <p>
	 * This method draws the polygon defined by <code>nPoint</code> line segments,
	 ...
	 * 1&nbsp;&le;&nbsp;<i>i</i>&nbsp;&le;&nbsp;<code>nPoints</code>. The figure is
	 ...
	 * current font and color. The baseline of the first character is at position
	 ...
	 * current color. The iterator has to specify a font for each character. The
