 ...
 * feTurbulence filter of the SVG specification. What follows is my high-level
 * description of how the noise is generated. This is not contained in the SVG
 * spec, just the algorithm for doing it. This is provided in the hope that
 * someone will figure out a clever way to accelerate parts of the function.
 *
 * gradient contains a long list of random unit vectors. For each point we are
 * to generate noise for we do two things. first we use the latticeSelector to
 * 'co-mingle' the integer portions of x and y (this allows us to have a
 * one-dimensional array of gradients that appears 2 dimensional, by using the
 * co-mingled index).
 *
 * We do this for [x,y], [x+1,y], [x,y+1], and [x+1, y+1], this gives us the
 * four gradient vectors that surround the point (b00, b10, ...)
 *
 * Next we construct the four vectors from the grid points (where the gradient
 * vectors are defined) [these are rx0, rx1, ry0, ry1].
 *
 * We then take the dot product between the gradient vectors and the grid point
 * vectors (this gives the portion of the grid point vector that projects along
 * the gradient vector for each grid point). These four dot projects are then
 * combined with linear interpolation. The weight factor for the linear
 * combination is the result of applying the 's' curve function to the
 * fractional part of x and y (rx0, ry0). The S curve function get's it's name
 * because it looks a bit like as 'S' from 0-&gt;1.
 *
...
public final class TurbulencePatternRed extends AbstractRed {
	...
	static final class StitchInfo {
		...
		void assign(StitchInfo stitchInfo) {
			this.width = stitchInfo.width;
			this.height = stitchInfo.height;
			this.wrapX = stitchInfo.wrapX;
			this.wrapY = stitchInfo.wrapY;
		...
		/*
		 * Adjustst the StitchInfo for when the frequency has been doubled.
		 *
		 * width = tileWidth*baseFrequencyX height = tileHeight*baseFrequencyY minY =
		 * tileY*baseFrequencyY + PerlinN wrapX = tileX*baseFrequencyX + PerlinN + width
		 * wrapY = tileY*baseFrequencyY + PerlinN + height
		 *
		 */
		void doubleFrequency() {
			width *= 2;
			height *= 2;
			wrapX *= 2;
			wrapY *= 2;
	...
	/**
	 * Defines the tile for the turbulence function, if non-null turns on stitching,
	 * so frequencies are adjusted to avoid discontinuities in case frequencies do
	 * not match tile boundaries.
	 */
	...
	/**
	 * Produces results in the range [1, 2**31 - 2]. Algorithm is: r = (a * r) mod m
	 * where a = 16807 and m = 2**31 - 1 = 2147483647 See [Park & Miller], CACM vol.
	 * 31 no. 10 p. 1195, Oct. 1988 To test: the algorithm should produce the result
	 * 1043618065 as the 10,000th generated number if the original seed is 1.
	 */
	...
	/**
	 * Generate a pixel of noise corresponding to the point vec0,vec1. See class
	 * description for a high level discussion of method. This handles cases where
	 * channels <= 4.
	 * 
	...
	private void noise2(final double[] noise, double vec0, double vec1) {
		...
		case 4:
			noise[3] = lerp(sy,
					lerp(sx, rx0 * gradient[b0 + 6] + ry0 * gradient[b0 + 7],
							rx1 * gradient[b1 + 6] + ry0 * gradient[b1 + 7]),
					lerp(sx, rx0 * gradient[b0 + 8 + 6] + ry1 * gradient[b0 + 8 + 7],
							rx1 * gradient[b1 + 8 + 6] + ry1 * gradient[b1 + 8 + 7]));
		case 3:
			noise[2] = lerp(sy,
					lerp(sx, rx0 * gradient[b0 + 4] + ry0 * gradient[b0 + 5],
							rx1 * gradient[b1 + 4] + ry0 * gradient[b1 + 5]),
					lerp(sx, rx0 * gradient[b0 + 8 + 4] + ry1 * gradient[b0 + 8 + 5],
							rx1 * gradient[b1 + 8 + 4] + ry1 * gradient[b1 + 8 + 5]));
		case 2:
			noise[1] = lerp(sy,
					lerp(sx, rx0 * gradient[b0 + 2] + ry0 * gradient[b0 + 3],
							rx1 * gradient[b1 + 2] + ry0 * gradient[b1 + 3]),
					lerp(sx, rx0 * gradient[b0 + 8 + 2] + ry1 * gradient[b0 + 8 + 3],
							rx1 * gradient[b1 + 8 + 2] + ry1 * gradient[b1 + 8 + 3]));
		case 1:
			noise[0] = lerp(sy,
					lerp(sx, rx0 * gradient[b0 + 0] + ry0 * gradient[b0 + 1],
							rx1 * gradient[b1 + 0] + ry0 * gradient[b1 + 1]),
					lerp(sx, rx0 * gradient[b0 + 8 + 0] + ry1 * gradient[b0 + 8 + 1],
							rx1 * gradient[b1 + 8 + 0] + ry1 * gradient[b1 + 8 + 1]));
		}
	}

	/**
	 * This version of the noise function implements stitching. If any of the
	 * lattice is on the right or bottom edge, the function uses the the latice on
	 * the other side of the tile, i.e., the left or right edge.
	 * 
	 * @param noise      The place to put the generated noise.
	 * @param vec0       The X coordiate to generate noise for
	 * @param vec1       The Y coordiate to generate noise for
	 ...
	 */
	private void noise2Stitch(final double[] noise, final double vec0, final double vec1,
		...
		case 4:
			noise[3] = lerp(sy,
					lerp(sx, rx0 * gradient[b00 + 6] + ry0 * gradient[b00 + 7],
							rx1 * gradient[b10 + 6] + ry0 * gradient[b10 + 7]),
					lerp(sx, rx0 * gradient[b01 + 6] + ry1 * gradient[b01 + 7],
							rx1 * gradient[b11 + 6] + ry1 * gradient[b11 + 7]));
		case 3:
			noise[2] = lerp(sy,
					lerp(sx, rx0 * gradient[b00 + 4] + ry0 * gradient[b00 + 5],
							rx1 * gradient[b10 + 4] + ry0 * gradient[b10 + 5]),
					lerp(sx, rx0 * gradient[b01 + 4] + ry1 * gradient[b01 + 5],
							rx1 * gradient[b11 + 4] + ry1 * gradient[b11 + 5]));
		case 2:
			noise[1] = lerp(sy,
					lerp(sx, rx0 * gradient[b00 + 2] + ry0 * gradient[b00 + 3],
							rx1 * gradient[b10 + 2] + ry0 * gradient[b10 + 3]),
					lerp(sx, rx0 * gradient[b01 + 2] + ry1 * gradient[b01 + 3],
							rx1 * gradient[b11 + 2] + ry1 * gradient[b11 + 3]));
		case 1:
			noise[0] = lerp(sy,
					lerp(sx, rx0 * gradient[b00 + 0] + ry0 * gradient[b00 + 1],
							rx1 * gradient[b10 + 0] + ry0 * gradient[b10 + 1]),
					lerp(sx, rx0 * gradient[b01 + 0] + ry1 * gradient[b01 + 1],
	 ...
	 * 'turbFunctionResult', as defined in the spec. This is special case for 4
	 * bands of output.
	 *
	 ...
	 */
	private int turbulence_4(double pointX, double pointY, final double[] fSum) {
		double n, ratio = 255;
	 ...
	 */
	private void turbulence(final int[] rgb, double pointX, double pointY, final double[] fSum,
			final double[] noise) {
		fSum[0] = fSum[1] = fSum[2] = fSum[3] = 0;
		double ratio = 255;
		pointX *= baseFrequencyX;
	 ...
	 * @param pointX     x coordinate of the point to process.
	 * @param pointY     y coordinate of the point to process.
	 * @param fSum       array used to avoid reallocating double array for each
	 *                   pixel
	 * @param noise      array used to avoid reallocating double array for each
	 *                   pixel
	 * @param stitchInfo The stitching information for the noise function
	 */
	private void turbulenceStitch(final int[] rgb, double pointX, double pointY, final double[] fSum,
			final double[] noise, StitchInfo stitchInfo) {
		double ratio = 1;
		pointX *= baseFrequencyX;
		pointY *= baseFrequencyY;
		fSum[0] = fSum[1] = fSum[2] = fSum[3] = 0;
	...
	/**
	 * This is the heart of the turbulence calculation. It returns
	 * 'turbFunctionResult', as defined in the spec. This handles the case where we
	 * are generating 4 channels of noise.
	 * 
	 * @param pointX x coordinate of the point to process.
	 * @param pointY y coordinate of the point to process.
	 * @param fSum   array used to avoid reallocating double array for each pixel
	 ...
	 */
	private int turbulenceFractal_4(double pointX, double pointY, final double[] fSum) {
	 ...
	 * @param noise  array used to avoid reallocating double array for each pixel
	 */
	private void turbulenceFractal(final int[] rgb, double pointX, double pointY, final double[] fSum,
			final double[] noise) {
		double ratio = 127.5;
		int nOctave;
		fSum[0] = fSum[1] = fSum[2] = fSum[3] = 127.5;
	 ...
	 * @param stitchInfo The stitching information for the noise function
	 */
	private void turbulenceFractalStitch(final int[] rgb, double pointX, double pointY, final double[] fSum,
			final double[] noise, StitchInfo stitchInfo) {
		double ratio = 127.5;
		int nOctave;
		fSum[0] = fSum[1] = fSum[2] = fSum[3] = 127.5;
	...
	public WritableRaster copyData(WritableRaster dest) {
		...
		int dstOff = dstDB.getOffset()
				+ sppsm.getOffset(minX - dest.getSampleModelTranslateX(), minY - dest.getSampleModelTranslateY());
	...
	/**
	 * @param baseFrequencyX x-axis base frequency for the noise function along the
	 *                       x-axis
	 * @param baseFrequencyY y-axis base frequency for the noise function along the
	 *                       x-axis
	 * @param numOctaves     number of octaves in the noise function. Positive
	 *                       integral value.
	 * @param seed           starting number for the pseudo random number generator
	 * @param isFractalNoise defines whether the filter performs a fractal noise or
	 *                       a turbulence function.
	 * @param tile           defines the tile size. May be null if stitchTiles is
	 *                       false. Otherwise, should not be null.
	 ...
	 */
	public TurbulencePatternRed(double baseFrequencyX, double baseFrequencyY, int numOctaves, int seed,
			boolean isFractalNoise, Rectangle2D tile, AffineTransform txf, Rectangle devRect, ColorSpace cs,
			boolean alpha) {
		...
		if (tile != null) {
			...
			if (stitchInfo.width == 0)
				stitchInfo.width = 1;
			if (stitchInfo.height == 0)
				stitchInfo.height = 1;
		...
		if (alpha)
			...
			cm = new DirectColorModel(cs, 24, 0x00FF0000, 0x0000FF00, 0x000000FF, 0x0, false, DataBuffer.TYPE_INT);
