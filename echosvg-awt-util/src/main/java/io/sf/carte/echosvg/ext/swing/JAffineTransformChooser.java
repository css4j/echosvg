...
/**
 * The <code>JAffineTransformChooser</code> is a pane that contains controls to
...
public class JAffineTransformChooser extends JGridBagPanel {
	...
	protected void build() {
		Component txyCmp = buildPanel(Resources.getString(LABEL_TRANSLATE), Resources.getString(LABEL_TX), txModel,
				Resources.getString(LABEL_TY), tyModel, "", "", true);

		Component sxyCmp = buildPanel(Resources.getString(LABEL_SCALE), Resources.getString(LABEL_SX), sxModel,
				Resources.getString(LABEL_SY), syModel, Resources.getString(LABEL_PERCENT),
				Resources.getString(LABEL_PERCENT), true);
	...
	protected Component buildRotatePanel() {
		...
		Component anglePanel = buildPanel(Resources.getString(LABEL_ROTATE), Resources.getString(LABEL_ANGLE),
				rotateModel, null, null, Resources.getString(LABEL_DEGREE), null, false);

		Component centerPanel = buildPanel("", Resources.getString(LABEL_RX), rxModel, Resources.getString(LABEL_RY),
				ryModel, null, null, false);
	...
	protected Component buildPanel(String panelName, String tfALabel, Document tfAModel, String tfBLabel,
			Document tfBModel, String tfASuffix, String tfBSuffix, boolean setBorder) {
		JGridBagPanel panel = new JGridBagPanel();
	...
	public void setPanelBorder(JComponent panel, String panelName) {
		...
		border = BorderFactory.createCompoundBorder(border,
				BorderFactory.createEmptyBorder(topPad, leftPad, bottomPad, rightPad));
	 ...
	 *
	 * @return null if the dialog was cancelled. Otherwise, the value entered by the
	 *         user.
	 */
	...
	/**
	 * Creates a new modal dialog box that can be used to configure an
	 * <code>AffineTransform</code>
	 *
	 * @param cmp   the dialog's parent component
	 * @param title the dialog's title
	...
	/*
	 * Class which builds a AffineTransform chooser dialog consisting of a
	 * JAffineTransformChooser with "Ok", "Cancel", and "Reset" buttons.
	 *
