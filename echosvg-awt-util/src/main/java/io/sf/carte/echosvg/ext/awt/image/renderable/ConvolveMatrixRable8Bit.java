 ...
 *
 * Known limitations: Does not support bias other than zero - pending 16bit
 * pathway Does not support edgeMode="wrap" - pending Tile code.
 *
...
public class ConvolveMatrixRable8Bit extends AbstractColorInterpolationRable implements ConvolveMatrixRable {
	 ...
	 * then device space will be used.
	 */
	@Override
	...
	public RenderedImage createRendering(RenderContext rc) {
		...
		// Grow the region in usr space.
		{
			...
			r = new Rectangle2D.Double(Math.floor(rx0), Math.floor(ry0), Math.ceil(rx1 - Math.floor(rx0)),
		...
		// io.sf.carte.echosvg.test.gvt.ImageDisplay.printImage
		// ("Padded Image", ri,
		...
		r = devRect;
		r = new Rectangle2D.Double(Math.floor(r.getX() - kx), Math.floor(r.getY() - ky),
				Math.ceil(r.getX() + r.getWidth()) - Math.floor(r.getX()) + (kw - 1),
				Math.ceil(r.getY() + r.getHeight()) - Math.floor(r.getY()) + (kh - 1));
		...
		BufferedImageOp op = new ConvolveOp(kernel, ConvolveOp.EDGE_NO_OP, rh);
		...
		if (!preserveAlpha) {
			...
			cm = new DirectColorModel(ColorSpace.getInstance(ColorSpace.CS_LINEAR_RGB), 24, 0x00FF0000, 0x0000FF00,
					0x000000FF, 0x0, false, DataBuffer.TYPE_INT);

			// Create an image with that color model
			BufferedImage tmpSrcBI = new BufferedImage(cm,
					cm.createCompatibleWritableRaster(wr.getWidth(), wr.getHeight()), cm.isAlphaPremultiplied(), null);

			// Copy the color data (no alpha) to that image
			// (dividing out alpha if needed).
			GraphicsUtil.copyData(srcBI, tmpSrcBI);

			// io.sf.carte.echosvg.test.gvt.ImageDisplay.showImage
			// ("tmpSrcBI: ", tmpSrcBI);
			...
			destBI = new BufferedImage(dstCM, dstCM.createCompatibleWritableRaster(wr.getWidth(), wr.getHeight()),
			...
			WritableRaster dstWR = Raster.createWritableRaster(
					cm.createCompatibleSampleModel(wr.getWidth(), wr.getHeight()), destBI.getRaster().getDataBuffer(),
					new Point(0, 0));
			...
			Rectangle dstRect = new Rectangle(srcRect.x - phaseShiftX, srcRect.y - phaseShiftY, srcRect.width,
					srcRect.height);
			GraphicsUtil.copyBand(wr, srcRect, wr.getNumBands() - 1, destBI.getRaster(), dstRect,
