...
public class MorphologyOp implements BufferedImageOp, RasterOp {
	...
	private int radiusY;
	/*
	 * Determine whether to do the dilation or erosion operation. Will do dilation
	 * when it's true and erosion when it's false.
	 */
	...
	/**
	 * @param radiusX    defines the radius of filter operation on X-axis. Should
	 *                   not be negative. A value of zero will disable the effect of
	 *                   the operation on X-axis, as described in the SVG
	 *                   specification.
	 * @param radiusY    defines the radius of filter operation on Y-axis. Should
	 *                   not be negative. A value of zero will disable the effect of
	 *                   the operation on Y-axis, as described in the SVG
	 *                   specification.
	 * @param doDilation defines whether to do dilation or erosion operation. Will
	 *                   do dilation when the value is true, erosion when false.
	 */
	...
	private void checkCompatible(SampleModel model) {
		...
		if (!(model instanceof SinglePixelPackedSampleModel))
			throw new IllegalArgumentException(
					"MorphologyOp only works with Rasters " + "using SinglePixelPackedSampleModels");
		...
		for (int i = 0; i < bitOffsets.length; i++) {
			if (bitOffsets[i] % 8 != 0)
				throw new IllegalArgumentException("MorphologyOp only works with Rasters using 8 bits " + "per band : "
						+ i + " : " + bitOffsets[i]);
		}
	}
	...
	/*
	 * This method compares the two input variables according to the doDilation
	 * boolean variable.
	 */
	...
	/*
	 * This method deals with the condition that the Kernel is wider than the Image
	 */
	private void specialProcessRow(Raster src, WritableRaster dest) {
		...
		final int srcOff = srcDB.getOffset() + sppsm.getOffset(src.getMinX() - src.getSampleModelTranslateX(),
				src.getMinY() - src.getSampleModelTranslateY());

		sppsm = (SinglePixelPackedSampleModel) dest.getSampleModel();
		final int dstOff = dstDB.getOffset() + sppsm.getOffset(dest.getMinX() - dest.getSampleModelTranslateX(),
	...
	/*
	 * This method deals with the condition when the Kernel is higher than the
	 * image.
	 */
	...
	/**
	 * Filters src and writes result into dest. If dest if null, then a Raster is
	 * created. If dest and src refer to the same object, then the source is
	 * modified.
	 * <p>
	 * The filtering kernel(the operation range for each pixel) is a rectangle of
	 * width 2*radiusX+1 and height radiusY+1
	 * <p>
	 * 
	 * @param src  the Raster to be filtered
	 * @param dest stores the filtered image. If null, a destination will be
	 *             created. src and dest can refer to the same Raster, in which
	 *             situation the src will be modified.
	 */
	@Override
	...
	/**
	 * This implementation of filter does the morphology operation on a
	 * premultiplied alpha image. This tends to muddy the colors. so something that
	 * is supposed to be a mostly transparent bright red may well become a muddy
	 * opaque red. Where as I think it should become a bright opaque red. Which is
	 * the result you would get if you were using unpremult data.
	 */
	@Override
	public BufferedImage filter(BufferedImage src, BufferedImage dest) {
		...
		if (!isCompatible(src.getColorModel(), src.getSampleModel())) {
			src = new BufferedImage(src.getWidth(), src.getHeight(), BufferedImage.TYPE_INT_ARGB_PRE);
		...
		if (dest == null) {
			...
			finalDest = dest;
		} else if (!isCompatible(dest.getColorModel(), dest.getSampleModel())) {
			dest = createCompatibleDestImage(src, null);
	...
	/*
	 * This commented out implementation of filter does the morphology operation on
	 * unpremultiplied alpha image data. This tends to leave colors bright.
	 */
	/*
	 * public BufferedImage filter(BufferedImage src, BufferedImage dest){ if (src
	 * == null && dest == null) throw new
	 * NullPointerException("Source image should not be null");
	 * 
	 * BufferedImage origSrc = src; BufferedImage finalDest = dest;
	 * 
	 * if (!isCompatible(src.getColorModel(), src.getSampleModel())) { src = new
	 * BufferedImage(src.getWidth(), src.getHeight(), BufferedImage.TYPE_INT_ARGB);
	 * GraphicsUtil.copyData(origSrc, src); } else if (src.isAlphaPremultiplied()) {
	 * ColorModel srcCM, srcCMUnpre; srcCM = src.getColorModel(); srcCMUnpre =
	 * GraphicsUtil.coerceColorModel(srcCM, false); src = new
	 * BufferedImage(srcCMUnpre, src.getRaster(), false, null);
	 * 
	 * GraphicsUtil.copyData(origSrc, src); }
	 * 
	 * 
	 * if (dest == null) { dest = new BufferedImage(src.getWidth(), src.getHeight(),
	 * BufferedImage.TYPE_INT_ARGB); finalDest = dest; } else if
	 * (!isCompatible(dest.getColorModel(), dest.getSampleModel())) { dest = new
	 * BufferedImage(src.getWidth(), src.getHeight(), BufferedImage.TYPE_INT_ARGB);
	 * } else if (dest.isAlphaPremultiplied()) { ColorModel dstCM, dstCMUnpre; dstCM
	 * = dest.getColorModel(); dstCMUnpre = GraphicsUtil.coerceColorModel(dstCM,
	 * false); dest = new BufferedImage(dstCMUnpre, finalDest.getRaster(), false,
	 * null); }
	 * 
	 * // We now have two compatible images. We can safely filter the rasters
	 * filter(src.getRaster(), dest.getRaster());
	 * 
	 * // Check to see if we need to 'fix' our source (divide out alpha). if
	 * ((src.getRaster() == origSrc.getRaster()) && (src.isAlphaPremultiplied() !=
	 * origSrc.isAlphaPremultiplied())) { GraphicsUtil.copyData(src, origSrc); }
	 * 
	 * // Check to see if we need to store our result... if ((dest.getRaster() !=
	 * finalDest.getRaster()) || (dest.isAlphaPremultiplied() !=
	 * finalDest.isAlphaPremultiplied())){ // Coerce our source back the way it
	 * was... System.out.println("Dest: " + dest.isAlphaPremultiplied() +
	 * " finalDest: " + finalDest.isAlphaPremultiplied());
	 * 
	 * GraphicsUtil.copyData(dest, finalDest); } return finalDest; }
	 */
}
