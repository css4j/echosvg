...
final class RadialGradientPaintContext extends MultipleGradientPaintContext {
	 ...
	 *
	 * @param cm           {@link ColorModel} that receives the <code>Paint</code>
	 *                     data. This is used only as a hint.
	 *
	 * @param deviceBounds the device space bounding box of the graphics primitive
	 *                     being rendered
	 *
	 * @param userBounds   the user space bounding box of the graphics primitive
	 *                     being rendered
	 *
	 * @param t            the {@link AffineTransform} from user space into device
	 *                     space (gradientTransform should be concatenated with
	 *                     this)
	 *
	 * @param hints        the hints that the context object uses to choose between
	 *                     rendering alternatives
	 *
	 * @param cx           the center point in user space of the circle defining the
	 *                     gradient. The last color of the gradient is mapped to the
	 *                     perimeter of this circle X coordinate
	 *
	 * @param cy           the center point in user space of the circle defining the
	 *                     gradient. The last color of the gradient is mapped to the
	 ...
	 *                     color gradient
	 *
	 * @param fx           the point in user space to which the first color is
	 *                     mapped X coordinate
	 *
	 * @param fy           the point in user space to which the first color is
	 *                     mapped Y coordinate
	 *
	 ...
	 *
	 * @param colorSpace   which colorspace to use for interpolation, either SRGB or
	 *                     LINEAR_RGB
	 *
	 */
	public RadialGradientPaintContext(ColorModel cm, Rectangle deviceBounds, Rectangle2D userBounds, AffineTransform t,
			RenderingHints hints, float cx, float cy, float r, float fx, float fy, float[] fractions, Color[] colors,
	 ...
	 *
	 * @param x The x coordinate of the area in device space for which colors are
	 *          generated.
	 * @param y The y coordinate of the area in device space for which colors are
	 *          generated.
	 * @param w The width of the area in device space for which colors are
	 *          generated.
	 * @param h The height of the area in device space for which colors are
	 *          generated.
	 */
	@Override
	...
	/**
	 * This code works in the simplest of cases, where the focus == center point,
	 * the gradient is noncyclic, and the gradient lookup method is fast (single
	 * array index, no conversion necessary).
	 *
	 */
	private void fixedPointSimplestCaseNonCyclicFillRaster(int[] pixels, int off, int adjust, int x, int y, int w,
			int h) {
	...
	/**
	 * Fill the raster, cycling the gradient colors when a point falls outside of
	 * the perimeter of the 100% stop circle.
	 *
	 * This calculation first computes the intersection point of the line from the
	 * focus through the current point in the raster, and the perimeter of the
	 * gradient circle.
	 *
	 * Then it determines the percentage distance of the current point along that
	 * line (focus is 0%, perimeter is 100%).
	 *
	 * Equation of a circle centered at (a,b) with radius r: (x-a)^2 + (y-b)^2 = r^2
	 * Equation of a line with slope m and y-intercept b y = mx + b replacing y in
	 * the cirlce equation and solving using the quadratic formula produces the
	 * following set of equations. Constant factors have been extracted out of the
	 * inner loop.
	 *
	 */
	private void cyclicCircularGradientFillRaster(int[] pixels, int off, int adjust, int x, int y, int w, int h) {
		...
		for (j = 0; j < h; j++) { // for every row
			...
			for (i = 0; i < w; i++) {
				...
				if (((X - focusX) > -0.000001f) && ((X - focusX) < 0.000001f)) {
					solutionX = focusX;
	...
	/**
	 * Fill the raster, cycling the gradient colors when a point falls outside of
	 * the perimeter of the 100% stop circle. Use the anti-aliased gradient lookup.
	 *
	 * This calculation first computes the intersection point of the line from the
	 * focus through the current point in the raster, and the perimeter of the
	 * gradient circle.
	...
	private void antiAliasFillRaster(int[] pixels, int off, int adjust, int x, int y, int w, int h) {
		...
		for (j = 0; j < h; j++) { // for every row
			...
			for (i = 1; i <= w; i++) {
				...
				// Get the color at this point
				pixels[indexer + i] = indexGradientAntiAlias((float) ((g00 + g01 + g10 + g11) / 4),
						(float) Math.max(Math.abs(g11 - g00), Math.abs(g10 - g01)));
