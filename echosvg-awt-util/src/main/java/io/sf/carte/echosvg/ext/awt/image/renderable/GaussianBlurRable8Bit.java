...
import io.sf.carte.echosvg.ext.awt.image.rendered.GaussianBlurRed8Bit;
...
public class GaussianBlurRable8Bit extends AbstractColorInterpolationRable implements GaussianBlurRable {
	...
	public Rectangle2D getBounds2D() {
		...
		float radX = 3 * dX / 2;
		float radY = 3 * dY / 2;
		return new Rectangle2D.Float((float) (src.getMinX() - radX), (float) (src.getMinY() - radY),
				(float) (src.getWidth() + 2 * radX), (float) (src.getHeight() + 2 * radY));
	}
	...
	public RenderedImage createRendering(RenderContext rc) {
		...
		int outsetX, outsetY;
		if ((sdx < 10) && (sdy < 10) && eps_eq(sdx, sdy) && eps_abs_eq(sx / scaleX, sy / scaleY)) {
		...
		try {
			...
			r = aoi.getBounds2D();
			r = new Rectangle2D.Double(r.getX() - outsetX / scaleX, r.getY() - outsetY / scaleY,
					r.getWidth() + 2 * outsetX / scaleX, r.getHeight() + 2 * outsetY / scaleY);
		}
	...
	public Shape getDependencyRegion(int srcIndex, Rectangle2D outputRgn) {
		if (srcIndex != 0)
			...
			float radX = 3 * dX / 2;
			float radY = 3 * dY / 2;
			outputRgn = new Rectangle2D.Float((float) (outputRgn.getMinX() - radX),
					(float) (outputRgn.getMinY() - radY), (float) (outputRgn.getWidth() + 2 * radX),
	...
	public Shape getDirtyRegion(int srcIndex, Rectangle2D inputRgn) {
		...
		if (srcIndex == 0) {
			...
			float radX = 3 * dX / 2;
			float radY = 3 * dY / 2;
			inputRgn = new Rectangle2D.Float((float) (inputRgn.getMinX() - radX), (float) (inputRgn.getMinY() - radY),
					(float) (inputRgn.getWidth() + 2 * radX), (float) (inputRgn.getHeight() + 2 * radY));
