...
public class Quadradic extends AbstractSegment {
	...
	@Override
	public Object clone() {
		return new Quadradic(new Point2D.Double(p1.x, p1.y), new Point2D.Double(p2.x, p2.y),
				new Point2D.Double(p3.x, p3.y));
	}

	public Segment reverse() {
		return new Quadradic(new Point2D.Double(p3.x, p3.y), new Point2D.Double(p2.x, p2.y),
	...
	public Segment getSegment(double t0, double t1) {
		...
		Point2D.Double np2 = new Point2D.Double(np1.x + .5 * dt * dp1.x, np1.y + .5 * dt * dp1.y);
	...
	/**
	 * Subdivides this Quadradic curve into two curves at t = 0.5. can be done with
	...
	/**
	 * Subdivides this Quadradic curve into two curves at t. can be done with
	 * getSegment but this is more efficent.
	 * 
	 * @param s0 if non-null contains portion of curve from 0-&gt;.5
	 * @param s1 if non-null contains portion of curve from .5-&gt;1
	...
	/*
	 * public static boolean epsEq(double a, double b) { final double eps = 0.00001;
	 * return (((a + eps) > b) && ((a-eps) < b)); }
	 * 
	 * public static void sub(Quadradic orig, Quadradic curr, double t, double inc,
	 * int lev) { Quadradic left=new Quadradic(); Quadradic right=new Quadradic();
	 * curr.subdivide(left, right); Point2D.Double ptl = left.eval(.5);
	 * Point2D.Double ptr = right.eval(.5); Point2D.Double pt1 = orig.eval(t-inc);
	 * Point2D.Double pt2 = orig.eval(t+inc); int steps = 100; Point2D.Double l, r,
	 * o; for (int i=0; i<=steps; i++) { l = left.eval(i/(double)steps); o =
	 ...
	 * 
	 * int steps = 1000000; Point2D.Double oldP = q.eval(0); Point2D.Double newP;
	 * double len = 0; for (int i=1; i<=steps; i++) { newP =
	 * q.eval(i/(double)steps); double dx = newP.x-oldP.x; double dy =
	 * newP.y-oldP.y; len += Math.sqrt(dx*dx + dy*dy); oldP = newP; }
	 * System.err.println("Length(.1): " + q.getLength(.001) + " x " + count); count
	 * = 0; System.err.println("Length    : " + q.getLength() + " x " + count);
	 * count = 0; System.err.println("D  Len    : " + len); }
	 * 
	 * public static void main(String args[]) { Quadradic q;
	 * 
	 * q = new Quadradic(0,0, 10,10, 30,0); sub(q, q, .5, .25, 3); evalQuad(q);
	 * 
	 * q = new Quadradic(0,0, 1,2, 3,0); sub(q, q, .5, .25, 3); evalQuad(q);
	 * 
	 * 
	 * }
	 */
