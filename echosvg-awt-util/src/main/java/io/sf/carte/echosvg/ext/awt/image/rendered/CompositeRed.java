...
/**
 * This is an implementation of an affine operation as a RenderedImage. Right
 * now the implementation makes use of the AffineBufferedImageOp to do the work.
 * Eventually this may move to be more tiled in nature.
 *
	...
	public CompositeRed(List<CachableRed> srcs, CompositeRule rule) {
		...
		if (rule.getRule() == CompositeRule.RULE_ARITHMETIC) {
			...
			while (i.hasNext()) {
				...
				if ((r.x != myBounds.x) || (r.y != myBounds.y) || (r.width != myBounds.width)
	...
	public void genRect(WritableRaster wr) {
		...
		while (i.hasNext()) {
			...
			if (first) {
				...
				if ((r.x < crR.x) || (r.y < crR.y) || (r.x + r.width > crR.x + crR.width)
				...
				if (crR.intersects(r)) {
					...
					WritableRaster smWR = wr.createWritableChild(smR.x, smR.y, smR.width, smR.height, smR.x, smR.y,
							null);
