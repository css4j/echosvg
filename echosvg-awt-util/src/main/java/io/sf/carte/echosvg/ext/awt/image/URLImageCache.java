 ...
 *
 * <p>
 * Adding an image is two fold. First you add the ParsedURL, this lets the cache
 * know that someone is working on this ParsedURL. Then when the completed
 * RenderedImage is ready you put it into the cache.
 * </p>
 * <p>
...
public class URLImageCache extends SoftReferenceCache {
	...
	/**
	 * Check if <code>request(url)</code> will return with a Filter (not putting you
	 * on the hook for it). Note that it is possible that this will return true but
	 * between this call and the call to request the soft-reference will be cleared.
	 * So it is still possible for request to return NULL, just much less likely
	 * (you can always call 'clear' in that case).
	 */
	...
	/**
	 * Check if <code>request(url)</code> will return immediately with the Filter.
	...
	/**
	 * If this returns null then you are now 'on the hook'. to put the Filter
	 * associated with ParsedURL into the cache.
	 */
	...
	/**
	 * Clear the entry for ParsedURL. This is the easiest way to 'get off the hook'.
	...
	/**
	 * Associate bi with purl. bi is only referenced through a soft reference so
	 * don't rely on the cache to keep it around. If the map no longer contains our
	 * purl it was probably cleared or flushed since we were put on the hook for it,
	 * so in that case we will do nothing.
	 */
