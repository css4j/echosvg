 ...
 *
 * Ideally this would also support a better dither option than just the JDK's
 * pattern dither.
 *
 * The algorithm used is the 'Median Cut Algorithm' published by Paul Heckbert
 * in early '80s.
 *
...
public class IndexImage {
	...
	private static class Counter {
		...
		/**
		 * contains the 'packed' rgb-color for this point. Must not change after
		 * construction!
		 */
		final int val;
		...
		/**
		 * convert the color-point of this counter to an rgb-array. To avoid creating
		 * lots of arrays, the caller passes the array to store the result.
		 *
	...
	/**
	 * Used to define a cube of the colorspace. The cube can be split approximagely
	...
	private static class Cube {
		...
		/**
		 * the colors-array is not modified - in fact, all cubes use the same
		 * colors-array. The Counter contains the rgb-color-code and the count of pixels
		 * with this color.
		 */
		...
		private boolean contains(int[] val) {
			...
			return (((min[RED] <= vRed) && (vRed <= max[RED])) && ((min[GRN] <= vGrn) && (vGrn <= max[GRN]))
					&& ((min[BLU] <= vBlu) && (vBlu <= max[BLU])));
		}

		/**
		 * Splits the cube into two parts. This cube is changed to be one half and the
		 * returned cube is the other half. This tries to pick the right channel to
		 * split on.
		 */
		Cube split() {
...
//                    + " c1:" + c1 );
		 ...
		 * This method is called on the two cubes resulting from a split. <br>
		 * We search the counts[] from min to max for the leftmost non-null entry. That
		 * is the new min. Then we search counts[] from max to min for the rightmost
		 * non-null entry. That is the new max. <br>
		 * This requires, that {@link #computeCounts } really computes <i>all</i>
		 * counts-values (and does not stop after the necessary number of points for a
		 * split is found, as it was done in the previous version of this class).
		 *
...
//            }
		...
		/**
		 * Splits the image according to the parameters. It tries to find a location
		 * where half the pixels are on one side and half the pixels are on the other.
		 */
...
//            }
		...
		/**
		 * create an array, which contains the number of pixels for each point along the
		 * splitChannel (between min and max of this cube).
		 *
		 * @param splitChannel one of RED | GRN | BLU
		 * @param c0           one of the other channels
		 * @param c1           the third channel
		 * @return an int[ 255 ] where only int[ min .. max ] contain valid counts.
		 */
...
//                if (( tcount > 0 ) && (tcount >= half)) break;  // fix 35683
		 ...
		 * 
		 * @return the min/max-boundarys of the rgb-channels and pixel-count of this
		 *         Cube.
		 */
		@Override
		public String toString() {
			return "Cube: [" + min[RED] + '-' + max[RED] + "] [" + min[GRN] + '-' + max[GRN] + "] [" + min[BLU] + '-'
					+ max[BLU] + "] n:" + count;
		}
		...
		public int averageColor() {
			...
			return ((rgb[RED] << 16) & 0x00FF0000) | ((rgb[GRN] << 8) & 0x0000FF00) | ((rgb[BLU]) & 0x000000FF);
		}
	...
	/**
	 * create an array of rgb-colors from the cubes-array. The color of each cube is
	 * computed as the sum of all colors in the cube, where each pixel is weighted
	 * according to it's count.
	 *
...
//        logRGB( r, g, b );
	...
	/**
	 * step 2: convert the result of step 1 to an Cube[][] which is more efficient
	 * in the following iterations. All slots in the result are filled with at least
	 * an empty array - thus we avoid tests for null. <br>
	 * Note: the converted slots in colors are no longer needed and removed.
	 *
	 * @param colors the data-structure to convert. Note that it is empty after
	 *               conversion!
	...
	/**
	 * Converts the input image (must be TYPE_INT_RGB or TYPE_INT_ARGB) to an
	 * indexed image. Generating an adaptive palette with number of colors
	 * specified.
	 * 
...
//        System.out.println("Bits: " + bits + " Cubes: " + nCubes);
		...
		if (bits == 3)
			bits = 4;
		ColorModel cm = new IndexColorModel(bits, nCubes, rgbTbl[0], rgbTbl[1], rgbTbl[2]);
		SampleModel sm;
