...
/**
 * This class is responsible for breaking up a block of tiles into a set of
...
public class TileBlock {
	 ...
	 * @param occupied Which entries in the block are already computed.
	 */
	...
	/**
	 * Really nice to string that outlines what tiles are filled and what region
	 * this block covers. Really useful for debugging the TileBlock stuff.
	 */
	@Override
	...
	/**
	 * Returnes an optimized list of TileBlocks to generate that tries to minimize
	 * the work to benefit ratio, for the set of blocks defined by this block.
	 */
		...
		for (int y = yOff; y < yOff + h; y++)
			for (int x = xOff; x < xOff + w; x++) {
				if (!filled[x + y * occW]) {
					...
					items.add(new TileBlock(occX, occY, occW, occH, occupied, x, y, cw, ch));
					x += (cw - 1);
				}
			}
