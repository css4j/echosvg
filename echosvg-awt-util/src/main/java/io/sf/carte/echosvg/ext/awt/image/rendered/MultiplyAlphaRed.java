...
public class MultiplyAlphaRed extends AbstractRed {
	...
	/**
	 * Multiply the alpha of one image with a mask image. The size of the resultant
	 * image is the intersection of the two image bounds. If you want the end image
	 * to be the size of one or the other please use the PadRed operator.
	 *
	...
	public MultiplyAlphaRed(CachableRed src, CachableRed alpha) {
		super(makeList(src, alpha), makeBounds(src, alpha), fixColorModel(src), fixSampleModel(src),
	...
	public boolean is_INT_PACK_BYTE_COMP(SampleModel srcSM, SampleModel alpSM) {
		...
		if (srcSM.getDataType() != DataBuffer.TYPE_INT)
			return false;
		if (alpSM.getDataType() != DataBuffer.TYPE_BYTE)
			return false;
	...
	public WritableRaster INT_PACK_BYTE_COMP_Impl(WritableRaster wr) {
		...
		final int alpBase = (alpDB.getOffset()
				+ csm.getOffset(rgn.x - r.getSampleModelTranslateX(), rgn.y - r.getSampleModelTranslateY()));
		...
		final int srcBase = (srcDB.getOffset()
				+ sppsm.getOffset(rgn.x - wr.getSampleModelTranslateX(), rgn.y - wr.getSampleModelTranslateY()));
		...
		if (cm.isAlphaPremultiplied()) {
			...
			for (int y = 0; y < rgn.height; y++) {
				...
				while (sp < end) {
					...
					srcPixels[sp] = ((((((pix >>> 24)) * a) & 0xFF00) << 16)
							| (((((pix >>> 16) & 0xFF) * a) & 0xFF00) << 8) | (((((pix >>> 8) & 0xFF) * a) & 0xFF00))
							| (((((pix) & 0xFF) * a) & 0xFF00) >> 8));
					sp++;
				}
			}
	...
	public WritableRaster copyData(WritableRaster wr) {
		...
		subWr = wr.createWritableChild(wr.getMinX(), wr.getMinY(), wr.getWidth(), wr.getHeight(), wr.getMinX(),
				wr.getMinY(), bands);
	...
	public static ColorModel fixColorModel(CachableRed src) {
		...
		return new ComponentColorModel(cs, bits, true, false, Transparency.TRANSLUCENT, DataBuffer.TYPE_BYTE);
	}
}
