...
public class GaussianBlurRed8Bit extends AbstractRed {
	...
	/**
	 * Construct a blurred version of <code>src</code>, by blurring with a gaussian
	 * kernel with standard Deviation of <code>stdDev</code> pixels.
	 * 
	...
	public GaussianBlurRed8Bit(CachableRed src, double stdDevX, double stdDevY, RenderingHints rh) {
		...
		init(src, myBounds, cm, sm, src.getTileGridXOffset() + xinset, src.getTileGridYOffset() + yinset, null);
	...
	/**
	 * Calculate the number of surround pixels required for a given standard
	 * Deviation.
	 */
	...
	/**
	 * Calculate the number of surround pixels required for a given standard
	 * Deviation. Also takes into account rendering quality hint.
	 */
	...
	/*
	 * Here we compute the data for the one-dimensional kernel of length
	 * '2*(radius-1) + 1'
	 *
	...
	private float[] computeQualityKernelData(int len, double stdDev) {
		...
		for (int i = 0; i < len; i++) {
			kernelData[i] = (float) (Math.pow(Math.E, -(i - mid) * (i - mid) / (2 * stdDev * stdDev))
					/ (SQRT2PI * stdDev));
			sum += kernelData[i];
		}
	...
	private WritableRaster boxFilterH(Raster src, WritableRaster dest, int skipX, int skipY, int boxSz, int loc) {
		...
		final int srcOff = (srcDB.getOffset() + srcSPPSM.getOffset(src.getMinX() - src.getSampleModelTranslateX(),
				src.getMinY() - src.getSampleModelTranslateY()));
		final int dstOff = (dstDB.getOffset() + dstSPPSM.getOffset(dest.getMinX() - dest.getSampleModelTranslateX(),
		...
		/*
		 * System.out.println("Info: srcOff: " + srcOff + " x: " + skipX + " y: " +
		 * skipY + " w: " + w + " h: " + h + " boxSz " + boxSz + " srcStride: " +
		 * srcScanStride);
		 */
		...
		for (int y = skipY; y < (h - skipY); y++) {
			...
			dp += skipX + loc;
			prev = destPixels[dp] = (((sumA * scale) & 0xFF000000) | (((sumR * scale) & 0xFF000000) >>> 8)
					| (((sumG * scale) & 0xFF000000) >>> 16) | (((sumB * scale) & 0xFF000000) >>> 24));
			dp++;
			k = 0;
			while (sp < rowEnd) {
				curr = buffer[k];
	...
	protected static ColorModel fixColorModel(CachableRed src) {
		...
		return new DirectColorModel(cs, 8 * b, masks[0], masks[1], masks[2], masks[3], true, DataBuffer.TYPE_INT);
	}
}
