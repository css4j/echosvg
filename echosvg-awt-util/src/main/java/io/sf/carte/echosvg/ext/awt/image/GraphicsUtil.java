...
/**
 * Set of utility methods for Graphics. These generally bypass broken methods in
 * Java2D or provide tweaked implementations.
 *
...
public class GraphicsUtil {
	...
	/**
	 * Draws <code>ri</code> into <code>g2d</code>. It does this be requesting tiles
	 * from <code>ri</code> and drawing them individually in <code>g2d</code> it
	 * also takes care of some colorspace and alpha issues.
	 * 
	...
	/**
	 * Draws <code>cr</code> into <code>g2d</code>. It does this be requesting tiles
	 * from <code>ri</code> and drawing them individually in <code>g2d</code> it
	 * also takes care of some colorspace and alpha issues.
	 * 
	 * @param g2d The Graphics2D to draw into.
	...
	public static void drawImage(Graphics2D g2d, CachableRed cr) {
		...
		if (cr instanceof BufferedImageCachableRed) {
			...
			if (g2dCS.equals(srcCM.getColorSpace()) && drawCM.equals(srcCM)) {
		...
		try {
			...
			if (useDrawRenderedImage) {
				...
				wr = ((WritableRaster) r).createWritableChild(clipR.x, clipR.y, clipR.width, clipR.height, 0, 0, null);
				...
				for (int y = yt0; y < yt1; y++, yloc += th) {
					...
					for (int x = xt0; x < xt1; x++, xloc += xStep) {
						...
						WritableRaster twr;
						twr = wr.createWritableChild(0, 0, iR.width, iR.height, iR.x, iR.y, null);
	 ...
	 * after taking into account a particular <code>RenderContext</code>.
	 * <p>
	 *
	 * This method also attempts to unwind the rendering chain a bit. So it knows
	 * about certain operations (like affine, pad, composite), rather than applying
	 * each of these operations in turn it accounts for their affects through
	 * modifications to the Graphics2D. This avoids generating lots of intermediate
	 * images.
	 *
	...
	/**
	 * Draws a <code>Filter</code> (<code>RenderableImage</code>) into a Graphics
	 * 2D.
	 * <p>
	 *
	 * This method also attempts to unwind the rendering chain a bit. So it knows
	...
	/**
	 * This is a wrapper around the system's BufferedImage.createGraphics that
	 * arranges for bi to be stored in a Rendering hint in the returned Graphics2D.
	...
	public static BufferedImage getDestination(Graphics2D g2d) {
		...
		Reference<BufferedImage> o = (Reference<BufferedImage>) g2d
		...
		if (WARN_DESTINATION && (gd.getType() == GraphicsDevice.TYPE_IMAGE_BUFFER) && (g2d.getRenderingHint(
				RenderingHintsKeyExt.KEY_TRANSCODING) != RenderingHintsKeyExt.VALUE_TRANSCODING_PRINTING))
	 ...
	 */
	public static final ColorModel Linear_sRGB = new DirectColorModel(ColorSpace.getInstance(ColorSpace.CS_LINEAR_RGB),
			24, 0x00FF0000, 0x0000FF00, 0x000000FF, 0x0, false, DataBuffer.TYPE_INT);
	/**
	...
	public static final ColorModel Linear_sRGB_Pre = new DirectColorModel(
			ColorSpace.getInstance(ColorSpace.CS_LINEAR_RGB), 32, 0x00FF0000, 0x0000FF00, 0x000000FF, 0xFF000000, true,
			DataBuffer.TYPE_INT);
	/**
	...
	public static final ColorModel Linear_sRGB_Unpre = new DirectColorModel(
			ColorSpace.getInstance(ColorSpace.CS_LINEAR_RGB), 32, 0x00FF0000, 0x0000FF00, 0x000000FF, 0xFF000000, false,
			DataBuffer.TYPE_INT);
	 ...
	 * colorspace. If <code>src</code> is already in linear sRGB then this method
	 * does nothing and returns <code>src</code>. Otherwise it creates a transform
	 * that will convert <code>src</code>'s output to linear sRGB and returns that
	 * CacheableRed.
	 *
	 ...
	 * colorspace. If <code>src</code> is already in sRGB then this method does
	 ...
	 * <p>
	 *
	 * In cases where <code>ri</code> is not already a CacheableRed it wraps
	 * <code>ri</code> with a helper class. The wrapped CacheableRed "Pretends" that
	 * it has no sources since it has no way of inteligently handling the
	 * dependency/dirty region calls if it exposed the source.
	 * 
	...
	/**
	 * An internal optimized version of copyData designed to work on Integer packed
	 * data with a SinglePixelPackedSampleModel. Only the region of overlap between
	 * src and dst is copied.
	 *
	 * Calls to this should be preflighted with is_INT_PACK_Data on both src and
	 * dest (requireAlpha can be false).
	 *
	 ...
	 * src and dst is copied. <code>Src</code> and <code>Dst</code> must have
	 * compatible SampleModels.
	 *
	 * @param src The source of the data
	 * @param dst The destination for the data.
	 */
	...
	/**
	 * Creates a new raster that has a <b>copy</b> of the data in <code>ras</code>.
	 * This is highly optimized for speed. There is no provision for changing any
	 * aspect of the SampleModel.
	 *
	 ...
	 * <p>
	 *
	 * You can specify a new location for the returned WritableRaster, this is
	 * especially useful for constructing BufferedImages which require the Raster to
	 * be at (0,0).
	 *
	 * This method should only be used if you need a WritableRaster due to an
	 * interface (such as to construct a BufferedImage), but have no intention of
	...
	/**
	 * Coerces data within a bufferedImage to match newAlphaPreMult, Note that this
	 * can not change the colormodel of bi so you
	 *
	...
	/**
	 * Copies data from one bufferedImage to another paying attention to the state
	 ...
	 * 
	 * if (count == 33) { String label = "sub [" + x + ", " + y + "]: ";
	 * org.ImageDisplay.showImage (label, subBI); org.ImageDisplay.printImage
	 * (label, subBI, new Rectangle(75-iR.x, 90-iR.y, 32, 32));
	 * 
	 * }
	 * 
	 * 
	 * // if ((count++ % 50) == 10) // org.ImageDisplay.showImage("foo: ", subBI);
	 * 
	 * 
	 * Graphics2D realG2D = g2d; while (realG2D instanceof
	 * sun.java2d.ProxyGraphics2D) { realG2D =
	 * ((sun.java2d.ProxyGraphics2D)realG2D).getDelegate(); } if (realG2D instanceof
	 * sun.awt.image.BufferedImageGraphics2D) { count++; if (count == 34) {
	 * RenderedImage ri; ri =
	 * ((sun.awt.image.BufferedImageGraphics2D)realG2D).bufImg; //
	 * g2d.setComposite(SVGComposite.OVER); // org.ImageDisplay.showImage("Bar: " +
	 * count, cr); org.ImageDisplay.printImage("Bar: " + count, cr, new
	 * Rectangle(75, 90, 32, 32));
	 * 
	 * org.ImageDisplay.showImage ("Foo: " + count, ri);
	 * org.ImageDisplay.printImage("Foo: " + count, ri, new Rectangle(75, 90, 32,
	 * 32));
	 * 
	 * System.out.println("BI: " + ri); System.out.println("BISM: " +
	 * ri.getSampleModel()); System.out.println("BICM: " + ri.getColorModel());
	 * System.out.println("BICM class: " + ri.getColorModel().getClass());
	 * System.out.println("BICS: " + ri.getColorModel().getColorSpace());
	 * System.out.println ("sRGB CS: " +
	 * ColorSpace.getInstance(ColorSpace.CS_sRGB)); System.out.println("G2D info");
	 * System.out.println("\tComposite: " + g2d.getComposite());
	 * System.out.println("\tTransform" + g2d.getTransform());
	 * java.awt.RenderingHints rh = g2d.getRenderingHints(); java.util.Set keys =
	 * rh.keySet(); java.util.Iterator iter = keys.iterator(); while
	 * (iter.hasNext()) { Object o = iter.next();
	 * 
	 * System.out.println("\t" + o.toString() + " -> " + rh.get(o).toString()); }
	 * 
	 * ri = cr; System.out.println("RI: " + ri); System.out.println("RISM: " +
	 * ri.getSampleModel()); System.out.println("RICM: " + ri.getColorModel());
