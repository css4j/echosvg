...
public final class BumpMap {
	...
	/**
	 * Stores the normals for this bumpMap. scaleX and scaleY are the user space to
	 * device space scales.
	 */
	...
	public double[][][] getNormalArray(final int x, final int y, final int w, final int h) {
		...
		Rectangle srcBound = new Rectangle(texture.getMinX(), texture.getMinY(), texture.getWidth(),
				texture.getHeight());
		...
		final int offset = (db.getOffset()
				+ sppsm.getOffset(srcRect.x - r.getSampleModelTranslateX(), srcRect.y - r.getSampleModelTranslateY()));
		...
		if (yloc == srcRect.y) {
			...
			for (; xloc < xEnd; xloc++) {
				...
				n[0] = -thirdSurfaceScaleX * ((2 * crnc + nrnc) - (2 * crpc + nrpc));
				n[1] = -halfSurfaceScaleY * ((nrpc + 2 * nrcc + nrnc) - (crpc + 2 * crcc + crnc));
			...
			if ((xloc < x + w) && (xloc == srcRect.x + srcRect.width - 1)) {
				...
				n[0] = -twoThirdSurfaceScaleX * ((2 * crcc + nrcc) - (2 * crpc + nrpc));
				n[1] = -twoThirdSurfaceScaleY * ((2 * nrcc + nrpc) - (2 * crcc + crpc));
		...
		for (; yloc < yEnd; yloc++) {
			...
			if (xloc != srcRect.x) {
				...
				n[0] = -halfSurfaceScaleX * ((prnc + 2 * crnc + nrnc) - (prcc + 2 * crcc + nrcc));
				n[1] = -thirdSurfaceScaleY * ((2 * prcc + prnc) - (2 * crcc + crnc));
			...
			for (; xloc < xEnd; xloc++) {
				...
				n[0] = -quarterSurfaceScaleX * ((prnc + 2 * crnc + nrnc) - (prpc + 2 * crpc + nrpc));
				n[1] = -quarterSurfaceScaleY * ((nrpc + 2 * nrcc + nrnc) - (prpc + 2 * prcc + prnc));
			...
			if ((xloc < x + w) && (xloc == srcRect.x + srcRect.width - 1)) {
				...
				n[0] = -halfSurfaceScaleX * ((prcc + 2 * crcc + nrcc) - (prpc + 2 * crpc + nrpc));
				n[1] = -thirdSurfaceScaleY * ((nrpc + 2 * nrcc) - (prpc + 2 * prcc));
		...
		if ((yloc < y + h) && (yloc == srcRect.y + srcRect.height - 1)) {
			...
			for (; xloc < xEnd; xloc++) {
				...
				n[0] = -thirdSurfaceScaleX * ((2 * crnc + prnc) - (2 * crpc + prpc));
				n[1] = -halfSurfaceScaleY * ((crpc + 2 * crcc + crnc) - (prpc + 2 * prcc + prnc));
			...
			if ((xloc < x + w) && (xloc == srcRect.x + srcRect.width - 1)) {
				...
				n[0] = -twoThirdSurfaceScaleX * ((2 * crcc + prcc) - (2 * crpc + prpc));
				n[1] = -twoThirdSurfaceScaleY * ((2 * crcc + crpc) - (2 * prcc + prpc));
