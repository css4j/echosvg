...
public class SVGComposite implements Composite {
	...
	public abstract static class AlphaPreCompositeContext implements CompositeContext {
		...
		protected abstract void precompose(Raster src, Raster dstIn, WritableRaster dstOut);

		@Override
		public void compose(Raster src, Raster dstIn, WritableRaster dstOut) {
			ColorModel srcPreCM = srcCM;
			if (!srcCM.isAlphaPremultiplied())
				srcPreCM = GraphicsUtil.coerceData((WritableRaster) src, srcCM, true);

			ColorModel dstPreCM = dstCM;
			if (!dstCM.isAlphaPremultiplied())
				dstPreCM = GraphicsUtil.coerceData((WritableRaster) dstIn, dstCM, true);
	...
	public abstract static class AlphaPreCompositeContext_INT_PACK extends AlphaPreCompositeContext {
		...
		protected abstract void precompose_INT_PACK(final int width, final int height, final int[] srcPixels,
				final int srcAdjust, int srcSp, final int[] dstInPixels, final int dstInAdjust, int dstInSp,
		...
		protected void precompose(Raster src, Raster dstIn, WritableRaster dstOut) {
			...
			final int dstInBase = (dstInDB.getOffset() + dstInSPPSM.getOffset(x0 - dstIn.getSampleModelTranslateX(),
			...
			final int dstOutBase = (dstOutDB.getOffset() + dstOutSPPSM.getOffset(x0 - dstOut.getSampleModelTranslateX(),
			...
			precompose_INT_PACK(w, h, srcPixels, srcAdjust, srcBase, dstInPixels, dstInAdjust, dstInBase, dstOutPixels,
					dstOutAdjust, dstOutBase);
		}
	}
		...
		public void precompose(Raster src, Raster dstIn, WritableRaster dstOut) {
			...
			for (int y = y0; y < y1; y++) {
				...
				while (srcSP < end) {
					...
					dstPix[dstSP] = srcPix[srcSP] + ((dstPix[dstSP] * dstM + pt5) >>> 24);
					++srcSP;
					++dstSP;
					dstPix[dstSP] = srcPix[srcSP] + ((dstPix[dstSP] * dstM + pt5) >>> 24);
					++srcSP;
	...
	public static class OverCompositeContext_INT_PACK extends AlphaPreCompositeContext_INT_PACK {
		...
		@Override
		public void precompose_INT_PACK(final int width, final int height, final int[] srcPixels, final int srcAdjust,
				int srcSp, final int[] dstInPixels, final int dstInAdjust, int dstInSp, final int[] dstOutPixels,
				final int dstOutAdjust, int dstOutSp) {
			...
			for (int y = 0; y < height; y++) {
				...
				while (dstOutSp < end) {
					...
					dstOutPixels[dstOutSp++] = (((srcP & 0xFF000000)
							+ (((((dstInP >>> 24)) * dstM + pt5) & 0xFF000000)))
							| ((srcP & 0x00FF0000) + (((((dstInP >> 16) & 0xFF) * dstM + pt5) & 0xFF000000) >>> 8))
							| ((srcP & 0x0000FF00) + (((((dstInP >> 8) & 0xFF) * dstM + pt5) & 0xFF000000) >>> 16))
							| ((srcP & 0x000000FF) + (((((dstInP) & 0xFF) * dstM + pt5)) >>> 24)));
				}
				srcSp += srcAdjust;
				dstInSp += dstInAdjust;
		...
		public void precompose_INT_PACK(final int width, final int height, final int[] srcPixels, final int srcAdjust,
			...
			for (int y = 0; y < height; y++) {
				...
				while (dstOutSp < end) {
					...
					dstM = (255 - (srcP >>> 24)) * norm;
					dstOutPixels[dstOutSp++] = (((srcP & 0x00FF0000)
	 ...
	 * unpremultiplied. This avoids having to multiply the alpha on the the source
	 * then divide it out again.
	 */
		...
		OverCompositeContext_INT_PACK_UNPRE(ColorModel srcCM, ColorModel dstCM) {
			...
			if (srcCM.isAlphaPremultiplied())
				throw new IllegalArgumentException(
						"OverCompositeContext_INT_PACK_UNPRE is only for" + "sources with unpremultiplied alpha");
		}
		...
		public void precompose_INT_PACK(final int width, final int height, final int[] srcPixels, final int srcAdjust,
			...
			for (int y = 0; y < height; y++) {
				...
				while (dstOutSp < end) {
					...
					dstOutPixels[dstOutSp++] = (((((srcP & 0xFF000000) + ((dstP >>> 24)) * dstM + pt5) & 0xFF000000))
							| (((((srcP >> 16) & 0xFF) * srcM + ((dstP >> 16) & 0xFF) * dstM + pt5) & 0xFF000000) >>> 8)
							| (((((srcP >> 8) & 0xFF) * srcM + ((dstP >> 8) & 0xFF) * dstM + pt5) & 0xFF000000) >>> 16)
							| (((((srcP) & 0xFF) * srcM + ((dstP) & 0xFF) * dstM + pt5)) >>> 24));
				}
				srcSp += srcAdjust;
				dstInSp += dstInAdjust;
				dstOutSp += dstOutAdjust;
		...
		public void precompose_INT_PACK(final int width, final int height, final int[] srcPixels, final int srcAdjust,
			...
			for (int y = 0; y < height; y++) {
				...
				while (dstOutSp < end) {
					...
					srcP = srcPixels[srcSp++];
					dstOutPixels[dstOutSp++] = ((((((srcP >>> 24)) * srcM + pt5) & 0xFF000000))
		...
		public void precompose_INT_PACK(final int width, final int height, final int[] srcPixels, final int srcAdjust,
			...
			for (int y = 0; y < height; y++) {
				...
				while (dstOutSp < end) {
					...
					dstOutPixels[dstOutSp++] = ((dstP & 0xFF000000)
							| (((((srcP >> 16) & 0xFF) * srcM + ((dstP >> 16) & 0xFF) * dstM + pt5) & 0xFF000000) >>> 8)
							| (((((srcP >> 8) & 0xFF) * srcM + ((dstP >> 8) & 0xFF) * dstM + pt5) & 0xFF000000) >>> 16)
							| (((((srcP) & 0xFF) * srcM + ((dstP) & 0xFF) * dstM + pt5)) >>> 24));
				}
		...
		public void precompose_INT_PACK(final int width, final int height, final int[] srcPixels, final int srcAdjust,
			...
			for (int y = 0; y < height; y++) {
				...
				while (dstOutSp < end) {
					...
					dstOutPixels[dstOutSp++] = ((((((srcP >>> 24)) * srcM + ((dstP >>> 24)) * dstM + pt5) & 0xFF000000))
							| (((((srcP >> 16) & 0xFF) * srcM + ((dstP >> 16) & 0xFF) * dstM + pt5) & 0xFF000000) >>> 8)
							| (((((srcP >> 8) & 0xFF) * srcM + ((dstP >> 8) & 0xFF) * dstM + pt5) & 0xFF000000) >>> 16)
							| (((((srcP) & 0xFF) * srcM + ((dstP) & 0xFF) * dstM + pt5)) >>> 24));
				}
		...
		public void precompose(Raster src, Raster dstIn, WritableRaster dstOut) {
			...
			for (y = y0; y < y1; y++) {
				...
				for (i = 0; i < srcPix.length; i++) {
					max = 0;
					for (b = 1; b < bands; b++, i++) {
						val = (int) ((kk1 * srcPix[i] * dstPix[i]) + k2 * srcPix[i] + k3 * dstPix[i] + kk4);
						if ((val & 0xFFFFFF00) != 0)
							if ((val & 0x80000000) != 0)
								val = 0;
							else
		...
		public void precompose_INT_PACK(final int width, final int height, final int[] srcPixels, final int srcAdjust,
			...
			for (int y = 0; y < height; y++) {
				...
				while (dstOutSp < end) {
					srcP = srcPixels[srcSp++];
					dstP = dstInPixels[dstInSp++];
					a = (int) ((srcP >>> 24) * (dstP >>> 24) * k1 + (srcP >>> 24) * k2 + (dstP >>> 24) * k3 + k4);
					if ((a & 0xFFFFFF00) != 0)
						if ((a & 0x80000000) != 0)
							a = 0;
						else
					...
					r = (int) (((srcP >> 16) & 0xFF) * ((dstP >> 16) & 0xFF) * k1 + ((srcP >> 16) & 0xFF) * k2
							+ ((dstP >> 16) & 0xFF) * k3 + k4);
					if ((r & 0xFFFFFF00) != 0)
						if ((r & 0x80000000) != 0)
							r = 0;
					...
					g = (int) (((srcP >> 8) & 0xFF) * ((dstP >> 8) & 0xFF) * k1 + ((srcP >> 8) & 0xFF) * k2
							+ ((dstP >> 8) & 0xFF) * k3 + k4);
					if ((g & 0xFFFFFF00) != 0)
						if ((g & 0x80000000) != 0)
							g = 0;
					...
					b = (int) ((srcP & 0xFF) * (dstP & 0xFF) * k1 + (srcP & 0xFF) * k2 + (dstP & 0xFF) * k3 + k4);
					if ((b & 0xFFFFFF00) != 0)
						if ((b & 0x80000000) != 0)
							b = 0;
						else
		...
		public void precompose(Raster src, Raster dstIn, WritableRaster dstOut) {
			...
			for (int y = y0; y < y1; y++) {
				...
				while (sp < end) {
					...
					dstPix[sp] = ((srcPix[sp] * srcM + dstPix[sp] * dstM + srcPix[sp] * dstPix[sp]) * norm
							+ pt5) >>> 24;
					++sp;
		...
		public void precompose_INT_PACK(final int width, final int height, final int[] srcPixels, final int srcAdjust,
			...
			for (int y = 0; y < height; y++) {
				...
				while (dstOutSp < end) {
					...
					dstOutPixels[dstOutSp++] = (((((srcR * srcM + dstR * dstM + srcR * dstR) * norm + pt5)
							& 0xFF000000) >>> 8)
							| ((((srcG * srcM + dstG * dstM + srcG * dstG) * norm + pt5) & 0xFF000000) >>> 16)
							| ((((srcB * srcM + dstB * dstM + srcB * dstB) * norm + pt5)) >>> 24)
		...
		public void precompose(Raster src, Raster dstIn, WritableRaster dstOut) {
			...
			for (int y = y0; y < y1; y++) {
				...
				while (sp < end) {
					...
					dstPix[sp] = (iSrcPix + iDstPix - ((iDstPix * iSrcPix * norm + pt5) >>> 24));
					++sp;
					iSrcPix = srcPix[sp];
					iDstPix = dstPix[sp];
					dstPix[sp] = (iSrcPix + iDstPix - ((iDstPix * iSrcPix * norm + pt5) >>> 24));
		...
		public void precompose_INT_PACK(final int width, final int height, final int[] srcPixels, final int srcAdjust,
			...
			for (int y = 0; y < height; y++) {
				...
				while (dstOutSp < end) {
					...
					dstOutPixels[dstOutSp++] = (((srcR + dstR - ((srcR * dstR * norm + pt5) >>> 24)) << 16)
		...
		public void precompose_INT_PACK(final int width, final int height, final int[] srcPixels, final int srcAdjust,
			...
			for (int y = 0; y < height; y++) {
				...
				while (dstOutSp < end) {
					...
					dstOutPixels[dstOutSp++] = ((dstA << 24) | (dstR << 16) | (dstG << 8) | dstB);
				}
				srcSp += srcAdjust;
				dstInSp += dstInAdjust;
				dstOutSp += dstOutAdjust;
