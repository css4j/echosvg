...
public class ColorMatrixRed extends AbstractRed {
	...
	public ColorMatrixRed(CachableRed src, float[][] matrix) {
		...
		init(src, src.getBounds(), cm, sm, src.getTileGridXOffset(), src.getTileGridYOffset(), null);
	}

	@Override
	public WritableRaster copyData(WritableRaster wr) {
		// System.out.println("Getting data for : " + wr.getWidth() + "/" +
		// wr.getHeight() + "/" + wr.getMinX() + "/" + wr.getMinY());
		...
		// System.out.println("Source was : " + wr.getWidth() + "/" + wr.getHeight()+
		...
		final int offset = (dbf.getOffset()
				+ sppsm.getOffset(minX - wr.getSampleModelTranslateX(), minY - wr.getSampleModelTranslateY()));
		...
		final float a00 = matrix[0][0] / 255f, a01 = matrix[0][1] / 255f, a02 = matrix[0][2] / 255f,
				a03 = matrix[0][3] / 255f, a04 = matrix[0][4] / 255f;
		final float a10 = matrix[1][0] / 255f, a11 = matrix[1][1] / 255f, a12 = matrix[1][2] / 255f,
				a13 = matrix[1][3] / 255f, a14 = matrix[1][4] / 255f;
		final float a20 = matrix[2][0] / 255f, a21 = matrix[2][1] / 255f, a22 = matrix[2][2] / 255f,
				a23 = matrix[2][3] / 255f, a24 = matrix[2][4] / 255f;
		final float a30 = matrix[3][0] / 255f, a31 = matrix[3][1] / 255f, a32 = matrix[3][2] / 255f,
				a33 = matrix[3][3] / 255f, a34 = matrix[3][4] / 255f;
		...
		for (i = 0; i < h; i++) {
			for (j = 0; j < w; j++) {
				...
				/*
				 * dr = dr > 255 ? 255 : dr < 0 ? 0 : dr; dg = dg > 255 ? 255 : dg < 0 ? 0 : dg;
				 * db = db > 255 ? 255 : db < 0 ? 0 : db; da = da > 255 ? 255 : da < 0 ? 0 : da;
				 */
		...
		// System.out.println("Result is : " + wr.getWidth() + "/" + wr.getHeight()+ "/"
		// + wr.getMinX() + "/" + wr.getMinY());
		return wr;
	}
