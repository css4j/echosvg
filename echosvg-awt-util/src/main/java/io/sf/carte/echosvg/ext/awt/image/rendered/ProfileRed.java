...
public class ProfileRed extends AbstractRed {
	private static final ColorSpace sRGBCS = ColorSpace.getInstance(ColorSpace.CS_sRGB);
	...
	/**
	 * This method will turn the input image in an sRGB image as follows. If there
	 * is no colorSpace defined, then the input image is simply converted to
	 * singlePixelPacked sRGB if needed. If there is a colorSpace defined, the the
	 * image data is 'interpreted' as being in that space, instead of that of the
	 * image's colorSpace.
	 *
	 * Here is how the input image is processed: a. It is converted to using a
	 * ComponentColorModel b. Its data is extracted, ignoring it's ColorSpace c. A
	 * new ComponentColorModel is built for the replacing colorSpace Note that if
	 * the number of components in the input image and the number of components in
	 * the replacing ColorSpace do not match, it is not possible to apply the
	 * conversion. d. A new BufferedImage is built, using the new
	 * ComponentColorModel and the data from the original image converted to the
	 * ComponentColorModel built in a. The alpha channel is excluded from that new
	 * BufferedImage. e. The BufferedImage created in d. is converted to sRGB using
	 * ColorConvertOp f. The alpha channel information is integrated back into the
	 * image.
	 *
	 * IMPORTANT NOTE: The code uses a BandedSampleModel in c.) and d.) and discard
	 * the alpha channel during the color conversions (it is restored in f.)),
	 * because of bugs in the interleaved model with alpha. The BandedSampleModel
	 * did not cause any bug as of JDK 1.3.
	 */
	@Override
	public WritableRaster copyData(WritableRaster argbWR) {
		try {
			...
			/**
			 * Check that the number of color components match in the input image and in the
			 * replacing profile.
			 */
			...
			/**
			 * If the source data is not a ComponentColorModel using a BandedSampleModel, do
			 * the conversion now.
			 */
			if (!(imgCM instanceof ComponentColorModel) || !(img.getSampleModel() instanceof BandedSampleModel)
					|| (imgCM.hasAlpha() && imgCM.isAlphaPremultiplied())) {
				ComponentColorModel imgCompCM = new ComponentColorModel(imgCS, // Same ColorSpace as img
				...
				WritableRaster wr = Raster.createBandedRaster(DataBuffer.TYPE_BYTE, argbWR.getWidth(),
						argbWR.getHeight(), imgCompCM.getNumComponents(), new Point(0, 0));

				BufferedImage imgComp = new BufferedImage(imgCompCM, wr, imgCompCM.isAlphaPremultiplied(), null);

				BufferedImage srcImg = new BufferedImage(imgCM, srcWR.createWritableTranslatedChild(0, 0),
			...
			/**
			 * Now, the input image is using a component color model. We can therefore
			 * create an image with the new profile, using a ComponentColorModel as well,
			 * because we know the number of components match (this was checked at the
			 * begining of this routine).
			 */
			ComponentColorModel newCM = new ComponentColorModel(colorSpace, // **** New ColorSpace ****
			...
			srcWR = Raster.createBandedRaster(data, argbWR.getWidth(), argbWR.getHeight(), argbWR.getWidth(),
					new int[] { 0, 1, 2 }, new int[] { 0, 0, 0 }, new Point(0, 0));
			BufferedImage newImg = new BufferedImage(newCM, srcWR, newCM.isAlphaPremultiplied(), null);

			/**
			 * Now, convert the image to sRGB
			 */
			ComponentColorModel sRGBCompCM = new ComponentColorModel(ColorSpace.getInstance(ColorSpace.CS_sRGB),
					new int[] { 8, 8, 8 }, false, false, Transparency.OPAQUE, DataBuffer.TYPE_BYTE);

			WritableRaster wr = Raster.createBandedRaster(DataBuffer.TYPE_BYTE, argbWR.getWidth(), argbWR.getHeight(),
			...
			if (imgCM.hasAlpha()) {
				...
				srcWR = Raster.createBandedRaster(argbData, argbWR.getWidth(), argbWR.getHeight(), argbWR.getWidth(),
						new int[] { 0, 1, 2, 3 }, new int[] { 0, 0, 0, 0 }, new Point(0, 0));
				sRGBCompCM = new ComponentColorModel(ColorSpace.getInstance(ColorSpace.CS_sRGB),
