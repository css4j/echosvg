...
public class PathLength {
	...
	/**
	 * Array where the index is the index of the original path segment and the value
	 * is the index of the first of the flattened segments in {@link #segments} that
	 * corresponds to that original path segment.
	 */
	...
	/**
	 * Returns the point that is the given proportion along the path segment given
	...
	public int findUpperIndex(float length) {
		...
		for (;;) {
			...
			if (ps.getSegType() != PathIterator.SEG_MOVETO || ub == segments.size() - 1) {
				break;
			}
			ub++;
		}
	...
	/**
	 * A {@link PathIterator} that returns only the next path segment from another
	 * {@link PathIterator}.
	 */
	protected static class SingleSegmentPathIterator implements PathIterator {
		...
		/**
		 * Sets the path iterator to use and the initial SEG_MOVETO command to return
		 * before it.
		 */
	...
	/**
	 * A single path segment in the flattened version of the path. This is a local
	 * helper class. PathSegment-objects are stored in the
	 * {@link PathLength#segments} - list. This is used as an immutable
	 * value-object.
	 */
	protected static class PathSegment {
		 ...
		 * @param idx     The index of the original path segment this flattened segment
		 *                is a part of
		 */
