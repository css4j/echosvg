 ...
 */
package io.sf.carte.echosvg.ext.awt;
...
abstract class MultipleGradientPaintContext implements PaintContext {
	...
	private static ColorModel lrgbmodel_NA = new DirectColorModel(ColorSpace.getInstance(ColorSpace.CS_LINEAR_RGB), 24,
			0xff0000, 0xFF00, 0xFF, 0x0, false, DataBuffer.TYPE_INT);

	private static ColorModel srgbmodel_NA = new DirectColorModel(ColorSpace.getInstance(ColorSpace.CS_sRGB), 24,
			0xff0000, 0xFF00, 0xFF, 0x0, false, DataBuffer.TYPE_INT);

	/** Color model used if some gradient colors are transparent */
	private static ColorModel lrgbmodel_A = new DirectColorModel(ColorSpace.getInstance(ColorSpace.CS_LINEAR_RGB), 32,
			0xff0000, 0xFF00, 0xFF, 0xFF000000, false, DataBuffer.TYPE_INT);

	private static ColorModel srgbmodel_A = new DirectColorModel(ColorSpace.getInstance(ColorSpace.CS_sRGB), 32,
			0xff0000, 0xFF00, 0xFF, 0xFF000000, false, DataBuffer.TYPE_INT);
	 ...
	 * in it, this may be because of multiple stops at the same location or use of
	 * the REPEATE mode.
	 */
	protected boolean hasDiscontinuity = false;

	/**
	 * Size of gradients array for scaling the 0-1 index when looking up colors the
	 * fast way.
	 */
	protected int fastGradientArraySize;

	/**
	 * Array which contains the interpolated color values for each interval, used by
	 * calculateSingleArrayGradient(). It is protected for possible direct access by
	 * subclasses.
	 */
	...
	/**
	 * Constructor for superclass. Does some initialization, but leaves most of the
	 * heavy-duty math for calculateGradient(), so the subclass may do some other
	 * manipulation beforehand if necessary. This is not possible if this
	 * computation is done in the superclass constructor which always gets called
	 * first.
	 **/
	protected MultipleGradientPaintContext(ColorModel cm, Rectangle deviceBounds, Rectangle2D userBounds,
			AffineTransform t, RenderingHints hints, float[] fractions, Color[] colors,
			MultipleGradientPaint.CycleMethodEnum cycleMethod, MultipleGradientPaint.ColorSpaceEnum colorSpace)
	...
	/**
	 * This function is the meat of this class. It calculates an array of gradient
	 * colors based on an array of fractions and color values at those fractions.
	 */
	protected final void calculateGradientFractions(Color[] loColors, Color[] hiColors) {
		...
		// ??? the following comments and the name Imin suggest, that we search for
		// something small
		...
		if (estimatedSize > MAX_GRADIENT_ARRAY_SIZE) {
			...
			if ((cycleMethod == MultipleGradientPaint.REPEAT)
					&& (gradients[0][0] != gradients[gradients.length - 1][GRADIENT_SIZE_INDEX]))
				hasDiscontinuity = true;
		} else {
			// fast method
	...
	/**
	 * We assume, that we always generate valid colors. When this is valid, we can
	 * compose the color-value by ourselves and use the faster Color-ctor, which
	 * does not check the incoming values.
	 *
	...
	private static Color interpolateColor(int[] workTbl, Color inColor) {
		...
		int newColorValue = ((workTbl[(oldColor >> 24) & 0xff] & 0xff) << 24)
				| ((workTbl[(oldColor >> 16) & 0xff] & 0xff) << 16) | ((workTbl[(oldColor >> 8) & 0xff] & 0xff) << 8)
	 ...
	 * FAST LOOKUP METHOD
	 *
	 * This method calculates the gradient color values and places them in a single
	 * int array, gradient[]. It does this by allocating space for each interval
	 * based on its size relative to the smallest interval in the array. The
	 * smallest interval is allocated 255 interpolated values (the maximum number of
	 * unique in-between colors in a 24 bit color system), and all other intervals
	 * are allocated size = (255 * the ratio of their size to the smallest
	 * interval).
	 *
	 * This scheme expedites a speedy retrieval because the colors are distributed
	 * along the array according to their user-specified distribution. All that is
	 * needed is a relative index from 0 to 1.
	 *
	 * The only problem with this method is that the possibility exists for the
	 * array size to balloon in the case where there is a disproportionately small
	 * gradient interval. In this case the other intervals will be allocated huge
	 * space, but much of that data is redundant. We thus need to use the space
	...
	private void calculateSingleArrayGradient(Color[] loColors, Color[] hiColors, float Imin) {
		...
		gradientAverage = (((aveA & 0xFF0000) << 8) | ((aveR & 0xFF0000)) | ((aveG & 0xFF0000) >> 8)
				| ((aveB & 0xFF0000) >> 16));
		...
		for (int[] gradient1 : gradients) {
			System.arraycopy(gradient1, 0, gradient, curOffset, gradient1.length);
	 ...
	 * SLOW LOOKUP METHOD
	 *
	 * This method calculates the gradient color values for each interval and places
	 * each into its own 255 size array. The arrays are stored in gradients[][].
	 * (255 is used because this is the maximum number of unique colors between 2
	 * arbitrary colors in a 24 bit color system)
	 *
	 * This method uses the minimum amount of space (only 255 * number of
	 * intervals), but it aggravates the lookup procedure, because now we have to
	 * find out which interval to select, then calculate the index within that
	 * interval. This causes a significant performance hit, because it requires this
	 * calculation be done for every point in the rendering loop.
	 *
	...
	/**
	 * Yet another helper function. This one linearly interpolates between 2 colors,
	 * filling up the output array.
	 *
	 * @param rgb1   the start color
	 * @param rgb2   the end color
	 * @param output the output array of colors... assuming this is not null or
	 *               length 0.
	 */
	private void interpolate(int rgb1, int rgb2, int[] output) {
		...
		// we could also save the shift when we use a different and-mask and a different
		// left-shift,
		...
		for (int i = 1; i < nSteps; i++) {
			float fI = i;
			output[i] = ((a1 + ((((int) (fI * tempA)) + 1) >> 1) & 0xff) << 24)
	...
	/**
	 * Helper function to index into the gradients array. This is necessary because
	 * each interval has an array of colors with uniform size 255. However, the
	 * color intervals are not necessarily of uniform length, so a conversion is
	 * required.
	 *
	 * @param position the unmanipulated position. want to map this into the range 0
	 *                 to 1
	 *
	...
	protected final int indexIntoGradientsArrays(float position) {
		...
		if (cycleMethod == MultipleGradientPaint.NO_CYCLE) {
			...
			if (isSimpleLookup) {
				...
				for (int i = 0; i < gradientsLength; i++) {
					...
					if (position < fractions[i + 1]) { // this is the array we want
						...
						int index = (int) delta;
						if ((index + 1 < gradients[i].length) || (i + 1 < gradientsLength))
			...
			return ((((((c1 >> 8) & 0xFF0000) + ((((c2 >>> 24)) - ((c1 >>> 24))) * w)) & 0xFF0000) << 8) |

					(((((c1) & 0xFF0000) + ((((c2 >> 16) & 0xFF) - ((c1 >> 16) & 0xFF)) * w)) & 0xFF0000)) |

					(((((c1 << 8) & 0xFF0000) + ((((c2 >> 8) & 0xFF) - ((c1 >> 8) & 0xFF)) * w)) & 0xFF0000) >> 8) |
	 ...
	 * each interval has an array of colors with uniform size 255. However, the
	 * color intervals are not necessarily of uniform length, so a conversion is
	 * required. This version also does anti-aliasing by averaging the gradient over
	 * position+/-(sz/2).
	 *
	 * @param position the unmanipulated position. want to map this into the range 0
	 *                 to 1
