...
public class TileRed extends AbstractRed implements TileGenerator {
	...
	public TileRed(RenderedImage tile, Rectangle tiledRegion, int xStep, int yStep, RenderingHints hints) {
		...
		// io.sf.carte.echosvg.test.gvt.ImageDisplay.showImage("Tile: ", tile);
		...
		if (16.1 * smSz > stepSz) {
			...
			raster = Raster.createWritableRaster(sm, new Point(tile.getMinX(), tile.getMinY()));
		}
		...
		// complie.
		init((CachableRed) null, tiledRegion, cm, sm, tile.getMinX(), tile.getMinY(), null);

		if (raster != null) {
			WritableRaster fromRaster = raster.createWritableChild(tile.getMinX(), tile.getMinY(), xStep, yStep,
	...
	public WritableRaster copyData(WritableRaster wr) {
		...
		for (int y = ty0; y <= ty1; y++)
			for (int x = tx0; x <= tx1; x++) {
				...
				r = r.createChild(r.getMinX(), r.getMinY(), r.getWidth(), r.getHeight(), r.getMinX() + xOff,
						r.getMinY() + yOff, null);
				if (is_INT_PACK)
	...
	public WritableRaster fillRasterFrom(WritableRaster wr, RenderedImage src) {
		// System.out.println("Getting Raster : " + count + " " + wr.getMinX() + "/" +
		// wr.getMinY() + "/" + wr.getWidth() + "/" + wr.getHeight());
		// System.out.println("Tile : " + tile.getMinX() + "/" + tile.getMinY() + "/" +
		// tile.getWidth() + "/" + tile.getHeight());

		ColorModel cm = getColorModel();
		BufferedImage bi = new BufferedImage(cm, wr.createWritableTranslatedChild(0, 0), cm.isAlphaPremultiplied(),
				null);
		...
		/*
		 * g.setTransform(new AffineTransform()); g.setPaint(colors[count++]); count %=
		 * colors.length;
		 * 
		 * g.fillRect(0, 0, maxX, maxY);
		 */
	...
	protected void fillOutRaster_INT_PACK(WritableRaster wr) {
		...
		final int base = (db.getOffset()
				+ sppsm.getOffset(x0 - wr.getSampleModelTranslateX(), y0 - wr.getSampleModelTranslateY()));
		int step = xStep;
	...
	/**
	 * This function 'fixes' the source's sample model. right now it just ensures
	 * that the sample model isn't much larger than my width.
	 */
