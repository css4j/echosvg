...
/**
 * This is a Grid based implementation of the TileStore. This makes it pretty
 * quick, but it can use a fair amount of memory for large tile grids.
 *
...
public class TileGrid implements TileStore {
	...
	public void setTile(int x, int y, Raster ras) {
		...
		if (DEBUG)
			System.out.println("Setting: (" + (x + minTileX) + ", " + (y + minTileY) + ")");
	}
	...
	public Raster getTile(int x, int y) {
		...
		if (DEBUG)
			System.out.println("Fetching: (" + (x + minTileX) + ", " + (y + minTileY) + ")");
		if (COUNT)
			synchronized (TileGrid.class) {
				requests++;
			}
		...
		if (ras == null) {
			if (DEBUG)
				System.out.println("Generating: (" + (x + minTileX) + ", " + (y + minTileY) + ")");
			if (COUNT)
				synchronized (TileGrid.class) {
					misses++;
				}
