...
public class Any2LsRGBRed extends AbstractRed {
	...
	/**
	 * Lookup tables for RGB lookups. The linearToSRGBLut is used when noise values
	 * are considered to be on a linearScale. The linearToLinear table is used when
	 * the values are considered to be on the sRGB scale to begin with.
	 */
	...
	public WritableRaster copyData(WritableRaster wr) {
		...
		if (srcCM == null) {
			...
			if (!dstCM.hasAlpha()) {
				...
				SampleModel dstSMNoA = new SinglePixelPackedSampleModel(dstSM.getDataType(), dstSM.getWidth(),
						dstSM.getHeight(), dstSM.getScanlineStride(), new int[] { masks[0], masks[1], masks[2] });
			...
			/*
			 * System.out.println("src: " + srcBI.getWidth() + "x" + srcBI.getHeight());
			 * System.out.println("dst: " + dstBI.getWidth() + "x" + dstBI.getHeight());
			 */
	...
	protected static SampleModel fixSampleModel(CachableRed src) {
		...
		if (alpha)
			return new SinglePixelPackedSampleModel(DataBuffer.TYPE_INT, sm.getWidth(), sm.getHeight(),
