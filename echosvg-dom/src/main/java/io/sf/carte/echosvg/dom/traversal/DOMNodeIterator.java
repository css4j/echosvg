...
public class DOMNodeIterator implements NodeIterator {
	 ...
	 * @param exp  Whether the children of entity reference nodes are visible to the
	 *             iterator.
	 */
	...
	public Node nextNode() {
		switch (state) {
		case INVALID:
			throw document.createDOMException(DOMException.INVALID_STATE_ERR, "detached.iterator", null);
		case BACKWARD:
		case INITIAL:
			state = FORWARD;
		...
		for (;;) {
			...
			if ((whatToShow & (1 << referenceNode.getNodeType() - 1)) != 0) {
				if (filter == null || filter.acceptNode(referenceNode) == NodeFilter.FILTER_ACCEPT) {
					return referenceNode;
				}
			}
		}
	...
	public void nodeToBeRemoved(Node removedNode) {
		...
		Node node;
		for (node = referenceNode; node != null && node != root; node = node.getParentNode()) {
			if (node == removedNode) {
				break;
			}
		}
		...
		if (state == BACKWARD) {
			...
			if (node.getNodeType() != Node.ENTITY_REFERENCE_NODE || expandEntityReferences) {
			...
			if (n.getNodeType() != Node.ENTITY_REFERENCE_NODE || expandEntityReferences) {
				Node t;
	...
	protected void unfilteredNextNode() {
		...
		// Go to the first child
		if (referenceNode.getNodeType() != Node.ENTITY_REFERENCE_NODE || expandEntityReferences) {
