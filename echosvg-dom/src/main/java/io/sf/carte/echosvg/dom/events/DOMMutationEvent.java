...
public class DOMMutationEvent extends AbstractEvent implements MutationEvent {
	...
	/**
	 * DOM: <code>relatedNode</code> is used to identify a secondary node related to
	 * a mutation event. For example, if a mutation event is dispatched to a node
	 * indicating that its parent has changed, the <code>relatedNode</code> is the
	 * changed parent. If an event is instead dispatch to a subtree indicating a
	 * node was changed within it, the <code>relatedNode</code> is the changed node.
	 */
	@Override
	...
	/**
	 * DOM: <code>newValue</code> indicates the new value of the <code>Attr</code>
	 * node in DOMAttrModified events, and of the <code>CharacterData</code> node in
	...
	/**
	 * DOM: The <code>initMutationEvent</code> method is used to initialize the
	 * value of a <code>MutationEvent</code> created through the
	 * <code>DocumentEvent</code> interface. This method may only be called before
	 * the <code>MutationEvent</code> has been dispatched via the
	 ...
	 */
	public void initMutationEventNS(String namespaceURIArg, String typeArg, boolean canBubbleArg, boolean cancelableArg,
			Node relatedNodeArg, String prevValueArg, String newValueArg, String attrNameArg, short attrChangeArg) {
