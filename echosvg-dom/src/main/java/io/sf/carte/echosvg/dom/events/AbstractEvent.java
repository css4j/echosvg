...
public abstract class AbstractEvent implements Event, OriginalEvent, Cloneable {
	...
	/**
	 * Whether the event propagation must be stopped after the current event
	 * listener group has been completed.
	 */
	...
	/**
	 * The number of nodes in the document this event will visit during capturing,
	 * bubbling and firing at the target. A value of 0 means to let the event be
	 * captured and bubble all the way to the document node. This field is used to
	 * handle events which should not cross sXBL shadow scopes without stopping or
	 * retargetting.
	 */
	protected int bubbleLimit = 0;

	/**
	 * DOM: The <code>type</code> property represents the event name as a string
	...
	/**
	 * DOM: The <code>target</code> property indicates the <code>EventTarget</code>
	 * whose <code>EventListeners</code> are currently being processed.
	 */
	@Override
	...
	/**
	 * DOM: The <code>eventPhase</code> property indicates which phase of event flow
	...
	/**
	 * DOM: The <code>bubbles</code> property indicates whether or not an event is a
	 * bubbling event. If the event can bubble the value is true, else the value is
	 * false.
	 */
	@Override
	...
	/**
	 * DOM: The <code>cancelable</code> property indicates whether or not an event
	 * can have its default action prevented. If the default action can be prevented
	 * the value is true, else the value is false.
	 */
	@Override
	...
	/**
	 * DOM: Used to specify the time (in milliseconds relative to the epoch) at
	 * which the event was created. Due to the fact that some systems may not
	 * provide this information the value of <code>timeStamp</code> may be returned.
	...
	/**
	 * DOM: The <code>stopPropagation</code> method is used prevent further
	 * propagation of an event during event flow. If this method is called by any
	 * <code>EventListener</code> the event will cease propagating through the tree.
	 * The event will complete dispatch to all listeners on the current
	 * <code>EventTarget</code> before event flow stops. This method may be used
	...
	/**
	 * DOM: If an event is cancelable, the <code>preventDefault</code> method is
	 * used to signify that the event is to be canceled, meaning any default action
	 * normally taken by the implementation as a result of the event will not occur.
	 * If, during any stage of event flow, the <code>preventDefault</code> method is
	 * called the event is canceled. Any default action associated with the event
	 * will not occur. Calling this method for a non-cancelable event has no effect.
	 * Once <code>preventDefault</code> has been called it will remain in effect
	 * throughout the remainder of the event's propagation. This method may be used
	 * during any stage of event flow.
	 */
	@Override
	...
	/**
	 * <b>DOM</b>: Returns whether <code>preventDefault</code> has been called on
	 * this object.
	 */
	 ...
	 */
	public List<Runnable> getDefaultActions() {
	...
	/**
	 * <b>DOM</b>: Stops propagation of this event immediately, even to listeners in
	 * the current group.
	 */
	...
	/**
	 * DOM: The <code>initEvent</code> method is used to initialize the value of
	 * interface. This method may only be called before the <code>Event</code> has
	 * been dispatched via the <code>dispatchEvent</code> method, though it may be
	 * called multiple times during that phase if necessary. If called multiple
	 * times the final invocation takes precedence. If called from a subclass of
	 ...
	 *                      event type.. The string must be an XML name . Any new
	 *                      event type must not begin with any upper, lower, or
	...
	/**
	 * Clones this event and sets the originalEvent field of the new event to be
	 * equal to this event.
	 */
