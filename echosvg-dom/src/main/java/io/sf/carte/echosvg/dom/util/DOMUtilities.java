...
public class DOMUtilities extends XMLUtilities implements XMLConstants {
	...
	private static final class NSMap {
		...
		/**
		 * The namespace name that the prefix maps to. The empty string is used to
		 * represent no namespace.
		 */
		private String ns;
		...
		/**
		 * The next number to use when generating prefixes. A prefix of the form
		 * <code>"a" + number</code> is generated when serializing a node whose
		 * namespace URI does not correspond to a prefix in scope.
		 */
		private int nextPrefixNumber;

		/**
		 * Constructs a new namespace prefix mapping object with the XML and XMLNS
		 * namespaces predeclared.
		 */
		public static NSMap create() {
			return new NSMap().declare(XMLConstants.XML_PREFIX, XMLConstants.XML_NAMESPACE_URI)
					.declare(XMLConstants.XMLNS_PREFIX, XMLConstants.XMLNS_NAMESPACE_URI);
		}
		...
		/**
		 * Declares a new prefix mapping by returning a new <code>NSMap</code> object
		...
		/**
		 * Returns the namespace URI that the specified prefix maps to, or
		 * <code>null</code> if the prefix has not been declared.
		 */
		 ...
		 * namespace URI. If the specified namespace is the default namespace (i.e., it
		 * has an empty string prefix mapping to it), then the empty string is returned.
		 * If there is no appropriate prefix, <code>null</code> is returned.
		 */
		...
		/**
		 * Returns the prefix appropriate for an attribute that maps to specified
		 * namespace URI. If there is no appropriate prefix, <code>null</code> is
		 * returned.
		 */
		...
		case Node.ELEMENT_NODE: {
			...
			if (n.hasAttributes()) {
				...
				for (int i = 0; i < len; i++) {
					...
					if (ans != null && !("xmlns".equals(prefix) || name.equals("xmlns"))) {
						if (prefix != null && !ans.equals(m.getNamespace(prefix)) || prefix == null) {
	...
	public static void writeNode(Node n, Writer w) throws IOException {
		if (n.getNodeType() == Node.DOCUMENT_NODE) {
			...
			writeNode(n, w, NSMap.create(), d == null ? false : "1.1".equals(d.getXmlVersion()));
		}
	}

	/**
	 * Returns the quote character to use when quoting the specified string. If the
	 * string contains both single and double quotes, then 0 will be returned.
	 */
	...
	/**
	 * Serializes the given DOM node using {@link #writeNode(Node,Writer)} and
	 * returns the XML as a String.
	 *
	 * @param n The Node to serialize.
	 * @return A String containing the XML serialization of the Node, or an empty
	 *         String if there was a problem during serialization.
	 */
	...
	/**
	 * Returns the given content value transformed to replace invalid characters
	 * with entities.
	 */
	 ...
	 *
	 * @param ancestorNodes The potential ancestor nodes
	...
	public static boolean isParentOf(Node node, Node parentNode) {
		if (node == null || parentNode == null || node.getParentNode() != parentNode) {
			return false;
		}
		return true;
	}
	 ...
	 * parentNode.
	 *
	 * @param children   The given node list
	 ...
	 * @return <code>true</code> if the node can have children, <code>false</code>
	 *         otherwise
	 */
	 ...
	 *
	 * @param text               The given XML string
	 * @param doc                The given document
	 * @param uri                The document URI
	 * @param prefixes           The prefixes map with (prefix, namespaceURI) pairs
	 * @param wrapperElementName null: Ignore the wrapper element and prefixes map
	 *                           and try to parse the text as a whole document
	 *                           otherwise: Wrap the given text with the wrapper
	 *                           element with prefixes specified from the prefixes
	 *                           map
	 * @param documentFactory    What document factory to use when parsing the text
	 * @return The document fragment or null on error.
	 */
	public static Node parseXML(String text, Document doc, String uri, Map<String, String> prefixes,
			String wrapperElementName, SAXDocumentFactory documentFactory) {
	...
	/**
	 * Tests whether the given string is a valid prefix. This method assume that
	 * isValidName(s) is true.
	 */
	...
	/**
	 * Gets the prefix from the given qualified name. This method assume that
	 * isValidName(s) is true.
	 */
	...
	/**
	 * Gets the local name from the given qualified name. This method assume that
	 * isValidName(s) is true.
	 */
	...
	/**
	 * Parses a 'xml-stylesheet' processing instruction data section and puts the
	 * pseudo attributes in the given table.
	 */
	public static void parseStyleSheetPIData(String data, HashMap<String, String> table) {
		...
		while (i < data.length()) {
			...
			int m = c % 32;
			if ((NAME_FIRST_CHARACTER[d] & (1 << m)) == 0) {
				throw new DOMException(DOMException.INVALID_CHARACTER_ERR, "Wrong name initial:  " + c);
			}
			...
			if (i >= data.length()) {
				throw new DOMException(DOMException.SYNTAX_ERR, "Wrong xml-stylesheet data: " + data);
			}
			// Skip whitespaces
			while (i < data.length()) {
				c = data.charAt(i);
	...
	/**
	 * String constants representing DOM modifier strings for various all key lock
	 * combinations.
	 */
	protected static final String[] LOCK_STRINGS = { "", "CapsLock", "NumLock", "NumLock CapsLock", "Scroll",
			"Scroll CapsLock", "Scroll NumLock", "Scroll NumLock CapsLock", "KanaMode", "KanaMode CapsLock",
			"KanaMode NumLock", "KanaMode NumLock CapsLock", "KanaMode Scroll", "KanaMode Scroll CapsLock",
			"KanaMode Scroll NumLock", "KanaMode Scroll NumLock CapsLock" };

	/**
	 * String constants representing DOM modifier strings for various all shift
	 * modifier combinations.
	 */
	protected static final String[] MODIFIER_STRINGS = { "", "Shift", "Control", "Control Shift", "Meta", "Meta Shift",
			"Control Meta", "Control Meta Shift", "Alt", "Alt Shift", "Alt Control", "Alt Control Shift", "Alt Meta",
			"Alt Meta Shift", "Alt Control Meta", "Alt Control Meta Shift", "AltGraph", "AltGraph Shift",
			"AltGraph Control", "AltGraph Control Shift", "AltGraph Meta", "AltGraph Meta Shift",
			"AltGraph Control Meta", "AltGraph Control Meta Shift", "Alt AltGraph", "Alt AltGraph Shift",
			"Alt AltGraph Control", "Alt AltGraph Control Shift", "Alt AltGraph Meta", "Alt AltGraph Meta Shift",
			"Alt AltGraph Control Meta", "Alt AltGraph Control Meta Shift" };
	...
	/**
	 * Returns whether the given element has a particular attribute and that it
	 * exists due to being specified explicitly, rather than being defaulted from a
	 * DTD.
	 */
