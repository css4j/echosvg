...
public class EventSupport {
	 ...
	 * If an <code>EventListener</code> is added to an <code>EventTarget</code>
	 * which is currently processing an event the new listener will not be triggered
	 * by the current event. <br>
	 ...
	 * discarded. They do not cause the <code>EventListener</code> to be called
	 * twice and since they are discarded they do not need to be removed with the
	 ...
	 *
	 * @param useCapture If true, <code>useCapture</code> indicates that the user
	 ...
	 *                   dispatched to any <code>EventTargets</code> beneath them in
	 *                   the tree. Events which are bubbling upward through the tree
	 *                   will not trigger an <code>EventListener</code> designated
	 *                   to use capture.
	 */
	...
	/**
	 * Registers an event listener for the given namespaced event type in the
	 * specified group.
	 */
	public void addEventListenerNS(String namespaceURI, String type, EventListener listener, boolean useCapture,
			Object group) {
	 ...
	 * an <code>EventListener</code> is removed from an <code>EventTarget</code>
	 * while it is processing an event, it will complete its current actions but
	 * will not be triggered again during any later stages of event flow. <br>
	 * If an <code>EventListener</code> is removed from an <code>EventTarget</code>
	 * which is currently processing an event the removed listener will still be
	 * triggered by the current event. <br>
	 * Calling <code>removeEventListener</code> with arguments which do not identify
	 * any currently registered <code>EventListener</code> on the
	 ...
	 *                   removed was registered as a capturing listener or not. If a
	 *                   listener was registered twice, one with capture and one
	 *                   without, each must be removed separately. Removal of a
	 *                   capturing listener does not affect a non-capturing version
	...
	/**
	 * Moves all of the event listeners from this EventSupport object to the given
	 * EventSupport object. Used by
	 * {@link io.sf.carte.echosvg.dom.AbstractDocument#renameNode(org.w3c.dom.Node,String,String)}.
	 */
	 ...
	 * target of the event is the <code> EventTarget</code> on which
	 ...
	 *
	 * @return The return value of <code>dispatchEvent</code> indicates whether any
	 *         of the listeners which handled the event called
	 *         <code>preventDefault</code>. If <code>preventDefault</code> was
	 *         called the value is false, else the value is true.
	 *
	 * @exception EventException UNSPECIFIED_EVENT_TYPE_ERR: Raised if the
	 ...
	 *                           <code>null</code> or an empty string will also
	...
	public boolean dispatchEvent(NodeEventTarget target, Event evt) throws EventException {
		...
		if (!(evt instanceof AbstractEvent)) {
			throw createEventException(DOMException.NOT_SUPPORTED_ERR, "unsupported.event", new Object[] {});
		}
		...
		for (NodeEventTarget node : ancestors) {
			...
			fireEventListeners(node, e, true, stoppedGroups, toBeStoppedGroups);
		...
		fireEventListeners(target, e, false, stoppedGroups, toBeStoppedGroups);
		stoppedGroups.addAll(toBeStoppedGroups);
		toBeStoppedGroups.clear();
		...
		if (e.getBubbles()) {
			...
			for (int i = ancestors.length - 1; i >= 0; i--) {
				...
				e.setCurrentTarget(node);
				fireEventListeners(node, e, false, stoppedGroups, toBeStoppedGroups);
				stoppedGroups.addAll(toBeStoppedGroups);
				toBeStoppedGroups.clear();
			}
		}
	 ...
	 */
	protected void fireEventListeners(NodeEventTarget node, AbstractEvent e, EventListenerList.Entry[] listeners,
			HashSet<Object> stoppedGroups, HashSet<Object> toBeStoppedGroups) {
		if (listeners == null) {
			return;
		}
	 ...
	 */
	protected void fireEventListeners(NodeEventTarget node, AbstractEvent e, boolean useCapture,
			HashSet<Object> stoppedGroups, HashSet<Object> toBeStoppedGroups) {
		String type = e.getType();
		...
		fireEventListeners(node, e, listeners, stoppedGroups, toBeStoppedGroups);
	}
	...
	/**
	 * Returns whether this node target has an event listener for the given event
	 * namespace URI and type.
	 */
	...
	/**
	 * Returns a list event listeners depending on the specified event type and
	 * phase.
	 * 
	...
	public EventListenerList getEventListeners(String type, boolean useCapture) {
		HashMap<String, EventListenerList> listeners = useCapture ? capturingListeners : bubblingListeners;
		if (listeners == null) {
			return null;
		}
	...
	/**
	 * Creates an EventException. Overrides this method if you need to create your
	 * own RangeException subclass.
	 * 
