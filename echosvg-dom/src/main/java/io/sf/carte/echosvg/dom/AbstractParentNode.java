...
public abstract class AbstractParentNode extends AbstractNode {
	...
	public Node insertBefore(Node newChild, Node refChild) throws DOMException {
		if ((refChild != null) && ((childNodes == null) || (refChild.getParentNode() != this)))
			throw createDOMException(DOMException.NOT_FOUND_ERR, "child.missing",
					new Object[] { (int) refChild.getNodeType(), refChild.getNodeName() });
		...
		if (newChild.getNodeType() == DOCUMENT_FRAGMENT_NODE) {
			...
			ExtendedNode n = childNodes.insert((ExtendedNode) newChild, (ExtendedNode) refChild);
	...
	public Node replaceChild(Node newChild, Node oldChild) throws DOMException {
		if ((childNodes == null) || (oldChild.getParentNode() != this))
			throw createDOMException(DOMException.NOT_FOUND_ERR, "child.missing",
					new Object[] { (int) oldChild.getNodeType(), oldChild.getNodeName() });
	...
	public void normalize() {
		...
		if (p != null) {
			...
			while (n != null) {
				if (p.getNodeType() == TEXT_NODE && n.getNodeType() == TEXT_NODE) {
	...
	/**
	 * <b>DOM</b>: Implements
	 * {@link org.w3c.dom.Element#getElementsByTagNameNS(String,String)}.
	 */
	...
	protected void fireDOMSubtreeModifiedEvent() {
		...
		if (doc.getEventsEnabled()) {
			...
			ev.initMutationEventNS(XMLConstants.XML_EVENTS_NAMESPACE_URI, "DOMSubtreeModified", true, // canBubbleArg
					false, // cancelableArg
					null, // relatedNodeArg
					null, // prevValueArg
					null, // newValueArg
	...
	protected void fireDOMNodeInsertedEvent(Node node) {
		...
		if (doc.getEventsEnabled()) {
			DOMMutationEvent ev = (DOMMutationEvent) doc.createEvent("MutationEvents");
			ev.initMutationEventNS(XMLConstants.XML_EVENTS_NAMESPACE_URI, "DOMNodeInserted", true, // canBubbleArg
					false, // cancelableArg
					this, // relatedNodeArg
					null, // prevValueArg
					null, // newValueArg
	...
	protected void fireDOMNodeRemovedEvent(Node node) {
		...
		if (doc.getEventsEnabled()) {
			DOMMutationEvent ev = (DOMMutationEvent) doc.createEvent("MutationEvents");
			ev.initMutationEventNS(XMLConstants.XML_EVENTS_NAMESPACE_URI, "DOMNodeRemoved", true, // canBubbleArg
					false, // cancelableArg
					this, // relatedNodeArg
					null, // prevValueArg
					null, // newValueArg
	...
	/**
	 * Checks the validity of a node to be inserted, and removes it from the
	 * document if needed.
	 */
	protected void checkAndRemove(Node n, boolean replace) {
		...
		if (n.getOwnerDocument() != getCurrentDocument())
			throw createDOMException(DOMException.WRONG_DOCUMENT_ERR, "node.from.wrong.document",
					new Object[] { (int) getNodeType(), getNodeName() });
		if (this == n)
			throw createDOMException(DOMException.HIERARCHY_REQUEST_ERR, "add.self", new Object[] { getNodeName() });
		...
		for (Node pn = this; pn != null; pn = pn.getParentNode()) {
			if (pn == n)
				throw createDOMException(DOMException.HIERARCHY_REQUEST_ERR, "add.ancestor",
						new Object[] { (int) getNodeType(), getNodeName() });
		}
	...
	public class ElementsByTagNameNS implements NodeList {
		...
		private void initialize(Node node) {
			if (node.getNodeType() == ELEMENT_NODE) {
				...
				if (nsMatch(namespaceURI, node.getNamespaceURI()) && (localName.equals("*") || localName.equals(nm))) {
					append(node);
				}
			}
			for (Node n = node.getFirstChild(); n != null; n = n.getNextSibling()) {
	...
	protected class ChildNodes implements NodeList, Serializable {
		...
		public ExtendedNode insert(ExtendedNode n, ExtendedNode r) {
			...
			throw createDOMException(DOMException.NOT_FOUND_ERR, "child.missing",
					new Object[] { (int) r.getNodeType(), r.getNodeName() });
		}
		...
		public ExtendedNode replace(ExtendedNode n, ExtendedNode o) {
			...
			if ((ps.getNextSibling() == o) && (ns.getPreviousSibling() == o)
					&& (ps.getParentNode() == o.getParentNode()) && (ns.getParentNode() == o.getParentNode())) {
			...
			throw createDOMException(DOMException.NOT_FOUND_ERR, "child.missing",
					new Object[] { (int) o.getNodeType(), o.getNodeName() });
		}
		...
		public ExtendedNode remove(ExtendedNode n) {
			...
			if ((ps.getNextSibling() == n) && (ns.getPreviousSibling() == n)
					&& (ps.getParentNode() == n.getParentNode()) && (ns.getParentNode() == n.getParentNode())) {
