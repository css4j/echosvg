...
public class DocumentEventSupport {
	...
	{
		// DOM 3 event names:
		eventFactories.put(EVENT_TYPE.toLowerCase(), new SimpleEventFactory());
		eventFactories.put(MUTATION_EVENT_TYPE.toLowerCase(), new MutationEventFactory());
		eventFactories.put(MUTATION_NAME_EVENT_TYPE.toLowerCase(), new MutationNameEventFactory());
		eventFactories.put(MOUSE_EVENT_TYPE.toLowerCase(), new MouseEventFactory());
		eventFactories.put(KEYBOARD_EVENT_TYPE.toLowerCase(), new KeyboardEventFactory());
		eventFactories.put(UI_EVENT_TYPE.toLowerCase(), new UIEventFactory());
		eventFactories.put(TEXT_EVENT_TYPE.toLowerCase(), new TextEventFactory());
		eventFactories.put(CUSTOM_EVENT_TYPE.toLowerCase(), new CustomEventFactory());
		// DOM 2 event names:
		eventFactories.put(EVENT_DOM2_TYPE.toLowerCase(), new SimpleEventFactory());
		eventFactories.put(MUTATION_EVENT_DOM2_TYPE.toLowerCase(), new MutationEventFactory());
		eventFactories.put(MOUSE_EVENT_DOM2_TYPE.toLowerCase(), new MouseEventFactory());
		eventFactories.put(KEY_EVENT_DOM2_TYPE.toLowerCase(), new KeyEventFactory());
		eventFactories.put(UI_EVENT_DOM2_TYPE.toLowerCase(), new UIEventFactory());
	}

	/**
	 * Creates a new Event depending on the specified parameter.
	 *
	 * @param eventType The <code>eventType</code> parameter specifies the type of
	 ...
	 *                  wishing to synthesize some kind of <code>UIEvent</code>
	 *                  would call <code>createEvent</code> with the parameter
	 *                  "UIEvent". The <code>initUIEvent</code> method could then be
	 *                  called on the newly created <code>UIEvent</code> to set the
	 *                  specific type of UIEvent to be dispatched and set its
	 *                  context information.The <code>createEvent</code> method is
	 *                  used in creating <code>Event</code>s when it is either
	 *                  inconvenient or unnecessary for the user to create an
	 *                  <code>Event</code> themselves. In cases where the
	 *                  implementation provided <code>Event</code> is insufficient,
	 *                  users may supply their own <code>Event</code>
	 ...
	 *                         not support the type of <code>Event</code> interface
	 *                         requested
	 */
	public Event createEvent(String eventType) throws DOMException {
		EventFactory ef = eventFactories.get(eventType.toLowerCase());
		if (ef == null) {
			throw new DOMException(DOMException.NOT_SUPPORTED_ERR, "Bad event type: " + eventType);
		}
