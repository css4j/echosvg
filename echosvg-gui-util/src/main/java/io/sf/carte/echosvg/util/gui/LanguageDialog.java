...
public class LanguageDialog extends JDialog implements ActionMap {
	 ...
	 * @throws MissingListenerException if the action is not found todo does it
	 *                                  throw ?? seems to return null
	 */
	@Override
	...
	public static class Panel extends JPanel implements ActionMap {
		...
		public Panel() {
			...
			listeners.put("AddLanguageButtonAction", new AddLanguageButtonAction());
			listeners.put("RemoveLanguageButtonAction", new RemoveLanguageButtonAction());
			listeners.put("UpLanguageButtonAction", new UpLanguageButtonAction());
			listeners.put("DownLanguageButtonAction", new DownLanguageButtonAction());
			listeners.put("ClearLanguageButtonAction", new ClearLanguageButtonAction());
			...
			sp.setBorder(BorderFactory.createCompoundBorder(BorderFactory
					.createTitledBorder(BorderFactory.createEmptyBorder(), resources.getString("Languages.title")),
			...
			languageList.addListSelectionListener(new LanguageListSelectionListener());

			// The user languages list
			constraints.setGridBounds(2, 0, 1, 1);

			JScrollPane sp2 = new JScrollPane();
			sp2.setBorder(BorderFactory.createCompoundBorder(BorderFactory
					.createTitledBorder(BorderFactory.createEmptyBorder(), resources.getString("User.title")),
					BorderFactory.createLoweredBevelBorder()));
			...
			this.add(new JLabel(resources.getString("InfoLabel.text")), constraints);
			...
			p.add(removeLanguageButton = bf.createJButton("RemoveLanguageButton"));
			...
			p.add(clearLanguageButton = bf.createJButton("ClearLanguageButton"));
		...
		/**
		 * Allows to pre-initialize icons used by the <code>Panel</code> constructor. It
		 * is not neccessary to call it and it should be called only once. This method
		 * is safe to be called by another thread than the event thread as it doesn't
		 * manipulate Swing <code>JComponent</code> instances.
		 */
		public static synchronized void initCountryIcons() {
			...
			if (iconMap == null) {
				...
				st = new StringTokenizer(resources.getString("Country.list"), " ");
				while (st.hasMoreTokens()) {
					computeCountryIcon(LanguageDialog.Panel.class, st.nextToken());
				}
			}
		}
