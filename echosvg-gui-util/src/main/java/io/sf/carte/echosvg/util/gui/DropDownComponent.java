...
public class DropDownComponent extends JPanel {
	 ...
	 *
	 * @param mainButton the components main button
	 */
	...
	public ScrollablePopupMenu getPopupMenu() {
		if (popupMenu == null) {
			...
			popupMenu.addPropertyChangeListener("enabled", new PropertyChangeListener() {
				@Override
			...
			popupMenu.addListener(new ScrollablePopupMenuAdapter() {
	...
	private class DropDownListener extends MouseAdapter {
		...
		public void mousePressed(MouseEvent e) {
			if (popupMenu.isShowing() && isDropDownEnabled) {
				...
				popupMenu.showMenu((Component) e.getSource(), DropDownComponent.this);
			}
		}
			...
			this.addMouseListener(new MouseAdapter() {
				...
				public void mouseEntered(MouseEvent e) {
					if (DefaultScrollablePopupMenuItem.this.isEnabled()) {
						setSelected(true);
						parent.selectionChanged(DefaultScrollablePopupMenuItem.this, true);
					}
				}
				...
				public void mouseExited(MouseEvent e) {
					if (DefaultScrollablePopupMenuItem.this.isEnabled()) {
						setSelected(false);
						parent.selectionChanged(DefaultScrollablePopupMenuItem.this, false);
					}
				}
	...
	public interface ScrollablePopupMenuModel {
		...
		/**
		 * Processes the showing of the pop up menu. Invoked before showing the pop up
		 * menu
		 */
		void processBeforeShowed();

		/**
		 * Processes the showing of the pop up menu. Invoked after showing the pop up
		 * menu
		 */
	...
	public static class ScrollablePopupMenu extends JPopupMenu {
		 ...
		 */
		private static final String RESOURCES = "io.sf.carte.echosvg.util.gui.resources.ScrollablePopupMenuMessages";
		 ...
		 *
		 * @param invoker      The popup menu invoker component
		 * @param refComponent The dropdown component that containts this menu
		 */
		...
		private int getPreferredHeight() {
			if (scrollPane.getPreferredSize().height < preferredHeight) {
				int heightOffset = 10;
				return scrollPane.getPreferredSize().height + ((Component) footer).getPreferredSize().height
						+ heightOffset;
			}
			return preferredHeight + ((Component) footer).getPreferredSize().height;
		}
		 ...
		 *
		 * @param event The associated ScrollablePopupMenuEvent event
		 */
		...
		/**
		 * Fires the itemsWereRemove event, when the items are removed from this pop up
		 * menu.
		 *
		 * @param event The associated ScrollablePopupMenuEvent event
		 */
	...
	/**
	 * The ScrollablePopupMenu listener. Handles the events that ScrollablePopupMenu
	 * fires
	 */
