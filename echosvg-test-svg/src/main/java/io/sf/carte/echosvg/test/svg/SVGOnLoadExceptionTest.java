...
public class SVGOnLoadExceptionTest extends AbstractTest {
	...
	/**
	 * Error when a BridgeException occured, as expected, but with an unexpected
	 * error code
	 */
	...
	public void setId(String id) {
		...
		if (id != null) {
			...
			fileName = "test-resources/io/sf/carte/echosvg/" + id + ".svg";
	...
	public TestReport runImpl() throws Exception {
		...
		try {
			if (!restricted) {
				...
				try {
					return AccessController.doPrivileged(new PrivilegedExceptionAction<TestReport>() {
						@Override
	...
	protected TestReport testImpl() {
		...
		if (expectedExceptionClass == null) {
			...
			if (RAN.equals(s)) {
				...
				report.addDescriptionEntry(ENTRY_KEY_UNEXPECTED_RESULT, s);
			}
		}
		if (report == null) {
			report = reportError(ERROR_EXCEPTION_DID_NOT_OCCUR);
			report.addDescriptionEntry(ENTRY_KEY_EXPECTED_EXCEPTION, expectedExceptionClass);
		}
		return report;
	}
	...
	protected TestReport handleException(Exception e) {
		if (!isMatch(e.getClass(), expectedExceptionClass)) {
			...
			report.addDescriptionEntry(ENTRY_KEY_UNEXPECTED_EXCEPTION, e.getClass().getName());
			report.addDescriptionEntry(ENTRY_KEY_EXPECTED_EXCEPTION, expectedExceptionClass);
			return report;
		} else {
			if (!ERROR_CODE_NO_CHECK.equals(expectedErrorCode) && e instanceof BridgeException) {
				if (!expectedErrorCode.equals(((BridgeException) e).getCode())) {
					TestReport report = reportError(ERROR_UNEXPECTED_ERROR_CODE);
					report.addDescriptionEntry(ENTRY_KEY_UNEXPECTED_ERROR_CODE, ((BridgeException) e).getCode());
					report.addDescriptionEntry(ENTRY_KEY_EXPECTED_ERROR_CODE, expectedErrorCode);
					return report;
				}
			}
