...
import io.sf.carte.echosvg.xml.XMLScanner;
...
public class PrettyPrinter {
	...
	protected void printXMLDecl() throws TranscoderException, XMLException, IOException {
		if (xmlDeclaration == null) {
			if (type == LexicalUnits.XML_DECL_START) {
				...
				output.printXMLDecl(space1, space2, space3, version, versionDelim, space4, space5, space6, encoding,
						encodingDelim, space7, space8, space9, standalone, standaloneDelim, space10);
	...
	protected void printDoctype() throws TranscoderException, XMLException, IOException {
		...
		default:
			if (type == LexicalUnits.DOCTYPE_START) {
				...
				output.printDoctypeStart(space1, root, space2, externalId, space3, string1, string1Delim, space4,
				...
				if (doctypeOption == DOCTYPE_CHANGE) {
					...
					output.printDoctypeStart(new char[] { ' ' }, new char[] { 's', 'v', 'g' }, new char[] { ' ' },
							externalId, new char[] { ' ' }, string1, '"', new char[] { ' ' }, string2, '"', null);
	...
	protected String printElement() throws TranscoderException, XMLException, IOException {
		...
		while (type == LexicalUnits.S) {
			...
			if (type == LexicalUnits.NAME) {
				...
				if (type != LexicalUnits.STRING && type != LexicalUnits.FIRST_ATTRIBUTE_FRAGMENT) {
					throw fatalError("string", null);
				}
				...
				attributes.add(new OutputManager.AttributeInfo(space, attName, space1, space2, new String(sb),
						valueDelim, hasEntityRef));
				space = null;
			}
		}
	...
	boolean allowSpaceAtStart(String tagName) {
		return true;
		/**
		 * This would be a real hack for SVG. This should be driven by a configuration
		 * paramater as well as needing to be really namespace aware...
		 */
	...
	protected void printContent(boolean spaceAtStart) throws TranscoderException, XMLException, IOException {
		...
		content: for (;;) {
			...
			case LexicalUnits.CHARACTER_DATA:
				preceedingSpace = output.printCharacterData(getCurrentValue(), spaceAtStart, preceedingSpace);
				scanner.clearBuffer();
				type = scanner.next();
				spaceAtStart = false;
				break;
			...
			case LexicalUnits.START_TAG:
				...
				break;
			case LexicalUnits.CHARACTER_REFERENCE:
				output.printCharacterEntityReference(getCurrentValue(), spaceAtStart, preceedingSpace);
				scanner.clearBuffer();
				type = scanner.next();
				spaceAtStart = false;
				preceedingSpace = false;
	...
	protected void printNotation() throws TranscoderException, XMLException, IOException {
		...
		output.printNotation(space1, name, space2, externalId, space3, string1, string1Delim, space4, string2,
				string2Delim, space5);
