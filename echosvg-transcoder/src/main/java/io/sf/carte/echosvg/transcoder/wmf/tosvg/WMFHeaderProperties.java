...
public class WMFHeaderProperties extends AbstractWMFReader {
	...
	protected boolean readRecords(DataInputStream is) throws IOException {
		...
		while (functionId > 0) {
			...
			case WMFConstants.META_SETMAPMODE: {
				...
				if (mapmode == WMFConstants.MM_ANISOTROPIC)
					isotropic = false;
			}
				break;
			...
			case WMFConstants.META_CREATEPENINDIRECT: {
				...
				if (recSize == 6)
					readShort(is); // if size greater than 5
			...
			case WMFConstants.META_EXTTEXTOUT: {
				...
				read += (lenText + 1) / 2;
				/*
				 * must do this because WMF strings always have an even number of bytes, even if
				...
				int lfHeight = (int) getVerticalAlignmentValue(layout, currentVertAlign);
			...
			case WMFConstants.META_CREATEFONTINDIRECT: {
				...
				int d = 0;
				while ((d < face.length())
						&& ((Character.isLetterOrDigit(face.charAt(d))) || (Character.isWhitespace(face.charAt(d)))))
					d++;
				if (d > 0)
			...
			case WMFConstants.META_PATBLT: {
				...
				int top = readShort(is);
				if (penObject >= 0)
					resizeBounds(left, top);
				if (penObject >= 0)
					resizeBounds(left + width, top + height);
			}
				break;
			...
			case WMFConstants.META_DIBBITBLT: {
				...
				readShort(is); // sx
				readShort(is); // hdc
				float height = readShort(is) * (float) inch / PIXEL_PER_INCH * getVpHFactor();
				float width = readShort(is) * (float) inch / PIXEL_PER_INCH * getVpWFactor() * scaleXY;
				float dy = inch / PIXEL_PER_INCH * getVpHFactor() * readShort(is);
				float dx = inch / PIXEL_PER_INCH * getVpWFactor() * readShort(is) * scaleXY;
				resizeImageBounds((int) dx, (int) dy);
		...
		// sets the width, height, etc of the image if the file does not have an APM (in
		// this case it is retrieved
		// from the viewport)
		if (!isAldus) {
			width = vpW;
			height = vpH;
	...
	/**
	 * @return the width of the Rectangle bounding the figures enclosed in the
	 *         Metafile, in pixels
	 */
	...
	/**
	 * @return the height of the Rectangle bounding the figures enclosed in the
	 *         Metafile, in pixels.
	 */
	...
	/**
	 * @return the X offset of the Rectangle bounding the figures enclosed in the
	 *         Metafile.
	 */
	...
	/**
	 * @return the Y offset of the Rectangle bounding the figures enclosed in the
	 *         Metafile.
	 */
	...
	private void resetBounds() {
		...
		if (_iright != -1) {
			...
			if ((_bright == -1) || (_iright > _bright))
				_bright = _iright;
			if ((_bleft == -1) || (_ileft < _bleft))
				_bleft = _ileft;
			if ((_btop == -1) || (_itop < _btop))
				_btop = _itop;
			if ((_bbottom == -1) || (_ibottom > _bbottom))
				_bbottom = _ibottom;
		}

		if ((_bleft != -1) && (_bright != -1))
			_bwidth = _bright - _bleft;
		if ((_btop != -1) && (_bbottom != -1))
			_bheight = _bbottom - _btop;
	}

	/**
	 * resize Bounds for each primitive encountered. Only elements that are in the
	 * overall width and height of the Metafile are kept.
	 */
	...
	/**
	 * resize Bounds for each image primitive encountered. Only elements that are in
	 * the overall width and height of the Metafile are kept.
	 */
	 ...
	 * Shape. There will be no resizing if one of the following properties is true :
	 * <ul>
	 * <li>the brush and the pen objects are < 0 (null objects)</li>
	 * <li>the color of the geometric Shape is white, and no other Shapes has
	 * occured</li>
	 * </ul>
	 */
	private void paint(int brushObject, int penObject, Shape shape) {
		if ((brushObject >= 0) || (penObject >= 0)) {
			Color col;
			if (brushObject >= 0)
				col = getColorFromObject(brushObject);
			else
				col = getColorFromObject(penObject);
	...
	private float getVerticalAlignmentValue(TextLayout layout, int vertAlign) {
		if (vertAlign == WMFConstants.TA_BASELINE)
