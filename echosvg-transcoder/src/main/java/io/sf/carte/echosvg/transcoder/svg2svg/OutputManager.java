 ...
 */
package io.sf.carte.echosvg.transcoder.svg2svg;
...
public class OutputManager {
	 ...
	 */
	public void printXMLDecl(char[] space1, char[] space2, char[] space3, char[] version, char versionDelim,
			char[] space4, char[] space5, char[] space6, char[] encoding, char encodingDelim, char[] space7,
			char[] space8, char[] space9, char[] standalone, char standaloneDelim, char[] space10) throws IOException {
		printString("<?xml");
	 ...
	 */
	public void printDoctypeStart(char[] space1, char[] root, char[] space2, String externalId, char[] space3,
			char[] string1, char string1Delim, char[] space4, char[] string2, char string2Delim, char[] space5)
			throws IOException {
		if (prettyPrinter.getFormat()) {
			printString("<!DOCTYPE");
			...
			if (space2 != null) {
				...
				if (space4 != null) {
					if (string2 != null) {
						if (column + string2.length + 3 > prettyPrinter.getDocumentWidth()) {
							printNewline();
	...
	public void printEntityReference(char[] name, boolean first) throws IOException {
		if ((prettyPrinter.getFormat()) && (xmlSpace.get(0) != Boolean.TRUE) && first) {
			printNewline();
			printString(margin.toString());
		}
		printCharacter('&');
	...
	public void printCharacterEntityReference(char[] code, boolean first, boolean preceedingSpace) throws IOException {
		if ((prettyPrinter.getFormat()) && (xmlSpace.get(0) != Boolean.TRUE)) {
	 ...
	 */
	public void printElementStart(char[] name, List<AttributeInfo> attributes, char[] space) throws IOException {
		...
		if (prettyPrinter.getFormat()) {
			...
			if (it.hasNext()) {
				...
				if (ai.isAttribute("xml:space")) {
					xmlSpace.set(0, (ai.value.equals("preserve") ? Boolean.TRUE : Boolean.FALSE));
				}
			...
			while (it.hasNext()) {
				...
				if (lineAttributes || len + column > prettyPrinter.getDocumentWidth()) {
					printNewline();
					printString(margin.toString());
	...
	public void printElementEnd(char[] name, char[] space) throws IOException {
		...
		if (name != null) {
			if (prettyPrinter.getFormat()) {
				if (xmlSpace.get(0) != Boolean.TRUE && (line != startingLines.get(0)
	 ...
	 */
	public void printNotation(char[] space1, char[] name, char[] space2, String externalId, char[] space3,
			char[] string1, char string1Delim, char[] space4, char[] string2, char string2Delim, char[] space5)
			throws IOException {
		...
		loop: while (i < text.length) {
			...
			if (startsWithSpace) {
				...
				if ((endCol >= prettyPrinter.getDocumentWidth() - 1)
						&& ((margin.length() + sb.length() < prettyPrinter.getDocumentWidth() - 1)
	...
	public static class AttributeInfo {
		...
		/**
		 * Tells whether the name of the attribute represented by this class equals the
		 * given string.
		 */
