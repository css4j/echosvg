...
import io.sf.carte.echosvg.ext.awt.image.codec.util.SimpleRenderedImage;
...
class PNGImage extends SimpleRenderedImage {
	...
	private static final String[] colorTypeNames = { "Grayscale", "Error", "Truecolor", "Index",
	...
	public PNGImage(InputStream stream, PNGDecodeParam decodeParam) throws IOException {
		...
		try {
			...
			if (magic != 0x89504e470d0a1a0aL) {
				String msg = PropertyUtil.getString("PNGImage.unknown.magic.number");
				throw new RuntimeException(msg);
			}
		} catch (Exception e) {
			e.printStackTrace();
			String msg = PropertyUtil.getString("PNGImage.error.reading.magic.number");
		...
		do {
			try {
				...
				e.printStackTrace();
				String msg = PropertyUtil.getString("PNGImage.decode.error");
				throw new RuntimeException(msg, e);
			}
		} while (true);
	...
	private static String getChunkType(DataInputStream distream) {
		try {
			...
			String typeString = "" + (char) ((type >> 24) & 0xff) + (char) ((type >> 16) & 0xff)
	...
	private static ColorModel createComponentColorModel(SampleModel sm, byte[] iccProfileData) {
		ICC_Profile prof = ICC_Profile.getInstance(iccProfileData);
		...
		if (type == DataBuffer.TYPE_BYTE) {
			switch (bands) {
			case 1:
				cm = new ComponentColorModel(cs, GrayBits8, false, false, Transparency.OPAQUE,
						DataBuffer.TYPE_BYTE);
				break;
			case 2:
				cm = new ComponentColorModel(cs, GrayAlphaBits8, true, false, Transparency.TRANSLUCENT,
						DataBuffer.TYPE_BYTE);
				break;
			case 3:
				cm = new ComponentColorModel(cs, RGBBits8, false, false, Transparency.OPAQUE,
						DataBuffer.TYPE_BYTE);
				break;
			case 4:
				cm = new ComponentColorModel(cs, RGBABits8, true, false, Transparency.TRANSLUCENT,
						DataBuffer.TYPE_BYTE);
				break;
			}
		} else if (type == DataBuffer.TYPE_USHORT) {
			switch (bands) {
			case 1:
				cm = new ComponentColorModel(cs, GrayBits16, false, false, Transparency.OPAQUE,
						DataBuffer.TYPE_USHORT);
				break;
			case 2:
				cm = new ComponentColorModel(cs, GrayAlphaBits16, true, false, Transparency.TRANSLUCENT,
						DataBuffer.TYPE_USHORT);
				break;
			case 3:
				cm = new ComponentColorModel(cs, RGBBits16, false, false, Transparency.OPAQUE,
						DataBuffer.TYPE_USHORT);
				break;
			case 4:
				cm = new ComponentColorModel(cs, RGBABits16, true, false, Transparency.TRANSLUCENT,
						DataBuffer.TYPE_USHORT);
				break;
			}
		} else if (type == DataBuffer.TYPE_INT) {
			switch (bands) {
			case 1:
				cm = new ComponentColorModel(cs, GrayBits32, false, false, Transparency.OPAQUE,
						DataBuffer.TYPE_INT);
				break;
			case 2:
				cm = new ComponentColorModel(cs, GrayAlphaBits32, true, false, Transparency.TRANSLUCENT,
						DataBuffer.TYPE_INT);
				break;
			case 3:
				cm = new ComponentColorModel(cs, RGBBits32, false, false, Transparency.OPAQUE,
						DataBuffer.TYPE_INT);
				break;
			case 4:
				cm = new ComponentColorModel(cs, RGBABits32, true, false, Transparency.TRANSLUCENT,
						DataBuffer.TYPE_INT);
				break;
			}
		}
	...
	private void parse_iCCP_chunk(PNGChunk chunk) {
		...
		while ((b = chunk.getByte(textIndex++)) != 0) {
			profileName.append((char) b);
		}

		// Compression method
		b = chunk.getByte(textIndex++);
		if (b != 0) {
			throw new RuntimeException(PropertyUtil.getString("PNGImage.unknown.compression.method"));
		}
	...
	private void parse_pHYs_chunk(PNGChunk chunk) {
		...
		if (emitProperties) {
			...
			properties.put("pixel_aspect_ratio", (float) xPixelsPerUnit / yPixelsPerUnit);
			if (unitSpecifier == 1) {
	...
	private WritableRaster createRaster(int width, int height, int bands, int scanlineStride, int bitDepth) {
		...
		if ((bitDepth < 8) && (bands == 1)) {
			...
			return Raster.createPackedRaster(dataBuffer, width, height, bitDepth, origin);
		} else if (bitDepth <= 8) {
			dataBuffer = new DataBufferByte(height * scanlineStride);
		} else {
