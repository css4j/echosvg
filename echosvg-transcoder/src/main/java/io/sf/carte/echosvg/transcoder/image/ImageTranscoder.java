 ...
 * a &lt;switch&gt; SVG element for example),
 * <code>KEY_USER_STYLESHEET_URI</code> to fix the URI of a user stylesheet, and
 * <code>KEY_MM_PER_PIXEL</code> to specify the number of millimeters in each
 * pixel .
 *
...
public abstract class ImageTranscoder extends SVGAbstractTranscoder {
	 ...
	 */
	public abstract void writeImage(BufferedImage img, TranscoderOutput output) throws TranscoderException;
	 ...
	 * display with a white background instead of a black background. <br>
	 * However, note that the modified image will display differently over a white
	 * background in a viewer that supports transparency. <br>
	 * Not all Transcoders use this key (in particular some formats can't preserve
	 * the alpha channel at all in which case this is not used).</td>
	 * </tr>
	 * </table>
	 */
