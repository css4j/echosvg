...
public abstract class AbstractWMFReader {
	...
	// the sign values for X and Y, will be modified depending on the VIEWPORT
	// values
	...
	/**
	 * get the left units in the WMF Metafile. This value is given in the Aldus
	 * Placable Metafile.
	 */
	...
	/**
	 * get the right units in the WMF Metafile. This value is given in the Aldus
	 * Placable Header.
	 */
	...
	/**
	 * get the top units in the WMF Metafile. This value is given in the Aldus
	 * Placable Header.
	 */
	...
	/**
	 * get the width units in the WMF Metafile. This value is given in the Aldus
	 * Placable Header.
	 */
	...
	/**
	 * get the height units in the WMF Metafile. This value is given in the Aldus
	 * Placable Header.
	 */
	...
	/**
	 * get the bottom units in the WMF Metafile. This value is given in the Aldus
	 * Placable Header.
	 */
	...
	/**
	 * get the number of Metafile units per inch in the WMF Metafile. This value is
	 * given in the Aldus Placable Header.
	 */
	...
	/**
	 * get the Rectangle defining the viewport of the WMF Metafile, in Metafile
	 * units. This viewport is defined in the Aldus Placable Header, by its left,
	 * top, bottom, right components.
	 * 
	 * @see #getRightUnits()
	 * @see #getLeftUnits()
	 * @see #getTopUnits()
	...
	/**
	 * Return the sign of X coordinates. It is equal to 1 by default, but can be -1
	 * if all X coordinates are inversed.
	 */
	public int getXSign() {
		return xSign;
	}
	...
	/**
	 * Return the sign of Y coordinates. It is equal to 1 by default, but can be -1
	 * if all Y coordinates are inversed.
	 */
	public int getYSign() {
		return ySign;
	}
	 ...
	 * read (see {@link #read(DataInputStream)}). The behavior of this method is
	 * left to the subclass. Each Metafile record is composed of :
	 * <ul>
	 * <li>the size of the Record in int (32 bits)</li>
	 * <li>the function ID for the Record on a short word (16 bits)</li>
	 * <li>the function parameters, according to the WMF Metafile specification. the
	 * remaining size in short words (16 bits) for the parameters is equal to the
	 * total size for the record minus 3 short words (= 16 + 32 bits)</li>
	 * </ul>
	 *
	 * <p>
	 * Example :
	 ...
	 * </ul>
	 * If the file contains an APM (aldus placeable header), this method read these
	 ...
	 * </ul>
	 * <p>
	 * Then it calls the {@link #readRecords(DataInputStream)} abstract method,
	...
	public void read(DataInputStream is) throws IOException {
		...
		if (dwIsAldus == WMFConstants.META_ALDUS_APM) {
			...
			// read the beginning of the header, the first int corresponds to the first two
			// parameters
	...
	/**
	 * Adds a GdiObject to the internal handle table. Wmf files specify the index as
	 * given in EMF records such as EMRCREATEPENINDIRECT whereas WMF files always
	 * use 0.
	 *
