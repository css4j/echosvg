...
/**
 * This class generate Paints from WMF hatch definitions. All generated Paints
...
public final class TextureFactory {
	...
	/**
	 * Get the unique instance of the class, setting the scale of the pattern. TODO
	 * : scale is not handled for now
	 */
	...
	/**
	 * Get a texture from a WMF hatch definition (in black Color). This texture will
	 * be cached, so the Paint will only be created once.
	 */
	...
	public Paint getTexture(int textureId, Color foreground) {
		...
		if (textures.containsKey(_ctexture)) {
			...
			if (paint != null)
				textures.put(_ctexture, paint);
			return paint;
		}
	}

	/**
	 * Get a texture from a WMF hatch definition, with a foreground and a background
	 * color. This texture will be cached, so the Paint will only be created once.
	 */
	...
	/**
	 * Called internally if the Paint does not exist in the cache and must be
	 * created.
	 */
