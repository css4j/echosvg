...
public class WMFPainter extends AbstractWMFPainter {
	...
	private static BasicStroke solid = new BasicStroke(1.0f, BasicStroke.CAP_BUTT, BasicStroke.JOIN_ROUND);

	private static BasicStroke textSolid = new BasicStroke(1.0f, BasicStroke.CAP_BUTT, BasicStroke.JOIN_ROUND);
	...
	public WMFPainter(WMFRecordStore currentStore, int xOffset, int yOffset, float scale) {
		...
		float xfactor = (float) currentStore.getVpW() / currentStore.getWidthPixels() * currentStore.getWidthUnits()
				/ currentStore.getWidthPixels();
		float yfactor = (float) currentStore.getVpH() / currentStore.getHeightPixels() * currentStore.getHeightUnits()
				/ currentStore.getHeightPixels();
		...
		if (!currentStore.isReading()) {
			...
			for (int iRec = 0; iRec < numRecords; iRec++) {
				...
				case WMFConstants.META_CREATEFONTINDIRECT: {
					...
					int d = 0;
					while ((d < face.length()) && ((Character.isLetterOrDigit(face.charAt(d)))
				...
				case WMFConstants.META_POLYPOLYGON: {
					...
					/*
					 * need to do this for POLYPOLYGON, because only GeneralPaths can handle filling
					 * for complex WMF shapes, so we need to get all the Polygons and then convert
					 * them to a GeneralPath
					 */
					if (brushObject >= 0) {
				...
				case WMFConstants.META_EXTTEXTOUT:
					try {
						...
						drawString(flag, g2d, getCharacterIterator(g2d, sr, wmfFont, currentHorizAlign), x, y, layout,
								wmfFont, currentHorizAlign);
						if (clipped) {
							g2d.setClip(clip);
						}
					} catch (Exception e) {
				...
				case WMFConstants.META_DRAWTEXT:
					try {
						...
						drawString(-1, g2d, getCharacterIterator(g2d, sr, wmfFont), x, y, layout, wmfFont,
								currentHorizAlign);
					} catch (Exception e) {
					}
					break;
				...
				case WMFConstants.META_CHORD: {
					...
					switch (mr.functionId) {
					case WMFConstants.META_ARC:
						arc = new Arc2D.Double(left, top, right - left, bottom - top, startAngle, extentAngle,
								Arc2D.OPEN);
						g2d.draw(arc);
						break;
					case WMFConstants.META_PIE:
				...
				case WMFConstants.META_PATBLT: {
					...
					if (ok) {
						...
						/*
						 * avoid to draw a huge rectangle on beginning of the drawing This is caused by
						 * the fact that a WMF file always seems to contain a META_PATBLT rectangle at
						 * its beginning.
						 */
				...
				case WMFConstants.META_DIBSTRETCHBLT: {
					...
					if (img != null) {
						g2d.drawImage(img, (int) dx, (int) dy, (int) (dx + widthDst), (int) (dy + heightDst), sx, sy,
								sx + width, sy + height, bkgdColor, observer);
					}
				}
					break;
				case WMFConstants.META_STRETCHDIB: {
					...
					if (img != null) {
						if (opaque) {
							g2d.drawImage(img, (int) dx, (int) dy, (int) (dx + widthDst), (int) (dy + heightDst), sx,
									sy, sx + width, sy + height, bkgdColor, observer);
						} else {
							// g2d.setComposite(AlphaComposite.SrcOver);
							g2d.drawImage(img, (int) dx, (int) dy, (int) (dx + widthDst), (int) (dy + heightDst), sx,
									sy, sx + width, sy + height, observer);
						}
					}
				}
					break;
				case WMFConstants.META_DIBBITBLT: {
					...
					if (mr instanceof MetaRecord.ByteRecord) {
						...
						if (img != null) {
							...
							if (opaque) {
								g2d.drawImage(img, (int) dx, (int) dy, (int) (dx + width), (int) (dy + height), sx, sy,
										sx + withSrc, sy + heightSrc, bkgdColor, observer);
							} else {
								// g2d.setComposite(AlphaComposite.SrcOver);
								g2d.drawImage(img, (int) dx, (int) dy, (int) (dx + width), (int) (dy + height), sx, sy,
										sx + withSrc, sy + heightSrc, observer);
							}
						}
					} else {
						if (opaque) {
	 ...
	 * </ul>
	 * <p>
	 * The other characteristics of the text, deriving from the WMF Font, must have
	 * been collected before to constitute the AttributedCharacterIterator.
	 * </p>
	 */
	private void drawString(int flag, Graphics2D g2d, AttributedCharacterIterator ati, float x, float y,
			TextLayout layout, WMFFont wmfFont, int align) {
	...
	private void fillTextBackground(int align, int flag, Graphics2D g2d, float x, float y, float width,
			TextLayout layout) {
	...
	private void setStroke(Graphics2D g2d, int penStyle, float penWidth, float scale) {
		...
		float _scale = (float) Platform.getScreenResolution() / currentStore.getMetaFileUnitsPerInch();
	...
	private int getHorizontalAlignement(int align) {
		...
		if (v >= 6)
			return WMFConstants.TA_CENTER;
		else if (v >= 2)
			return WMFConstants.TA_RIGHT;
		else
			return WMFConstants.TA_LEFT;
	}
	...
	private Paint getStoredPaint(WMFRecordStore currentStore, int object) {
		if (object >= 0) {
			...
			if (gdiObj.obj instanceof Paint)
				...
				return getPaint((byte[]) gdiObj.obj);
		} else
			return null;
	}
	...
	private float getVerticalAlignmentValue(TextLayout layout, int vertAlign) {
		if (vertAlign == WMFConstants.TA_BOTTOM)
			return -layout.getDescent();
		else if (vertAlign == WMFConstants.TA_TOP)
