...
public abstract class TimingParser extends AbstractParser {
	...
	protected Object[] parseTimingSpecifier() throws ParseException, IOException {
		...
		Object[] ret = null;
		if (current == '+' || (current == '-' && !escaped) || (current >= '0' && current <= '9')) {
	...
	protected String parseName() throws ParseException, IOException {
		...
		} while (XMLUtilities.isXMLNameCharacter((char) current) && (midEscaped || (current != '-' && current != '.')));
		return sb.toString();
	}
	...
	protected Object[] parseIDValue(boolean escaped) throws ParseException, IOException {
		String id = parseName();
		if ((id.equals("accessKey") && useSVG11AccessKeys || id.equals("accesskey")) && !escaped) {
			if (current != '(') {
				reportUnexpectedCharacterError(current);
			}
			current = reader.read();
			...
			return new Object[] { TIME_ACCESSKEY, offset, key };
			...
			return new Object[] { TIME_ACCESSKEY_SVG12, offset, keyName.toString() };
			...
			skipSpaces();
			if (current != ')') {
				reportError("character.unexpected", new Object[] { current });
			}
			current = reader.read();
			...
			if (current == '.') {
				...
				if ((id2.equals("begin") || id2.equals("end")) && !escaped) {
					...
					return new Object[] { TIME_SYNCBASE, offset, id, id2 };
					...
					return new Object[] { TIME_REPEAT, offset, id, repeatIteration };
					...
					current = reader.read();
					return new Object[] { TIME_MEDIA_MARKER, id, markerName };
				} else {
					skipSpaces();
					float offset = 0;
					if (current == '+' || current == '-') {
					...
					return new Object[] { TIME_EVENTBASE, offset, id, id2 };
				}
			} else {
				skipSpaces();
				float offset = 0;
				...
				return new Object[] { TIME_EVENTBASE, offset, null, id };
			}
		}
	}
