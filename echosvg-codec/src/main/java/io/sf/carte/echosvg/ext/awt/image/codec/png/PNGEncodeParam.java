...
public abstract class PNGEncodeParam implements ImageEncodeParam {
	 ...
	 *
	 * @throws IllegalStateException if the physical dimension information is not
	 *                               set.
	 */
	 ...
	 * unset, an <code>IllegalStateException</code> will be thrown.
	 *
	 * @throws IllegalStateException if the suggested palette information is not
	 *                               set.
	 */
	 ...
	 * </pre>
	 *
	 * @param currRow       The current row as an array of <code>byte</code>s of
	 *                      length at least
	 *                      <code>bytesPerRow + bytesPerPixel</code>. The pixel data
	 *                      starts at index <code>bytesPerPixel</code>; the initial
	 *                      <code>bytesPerPixel</code> bytes are zero.
	 * @param prevRow       The current row as an array of <code>byte</code>s The
	 *                      pixel data starts at index <code>bytesPerPixel</code>;
	 *                      the initial <code>bytesPerPixel</code> bytes are zero.
	 * @param scratchRows   An array of 5 <code>byte</code> arrays of length at
	 *                      least <code>bytesPerRow +
	 *        bytesPerPixel</code>, useable to hold temporary results. The filtered
	 *                      row will be returned as one of the entries of this
	 *                      array. The returned filtered data should start at index
	 *                      <code>bytesPerPixel</code>; The initial
	 *                      <code>bytesPerPixel</code> bytes are not used.
	 * @param bytesPerRow   The number of bytes in the image row. This value will
	 *                      always be greater than 0.
	 * @param bytesPerPixel The number of bytes representing a single pixel, rounded
	 *                      up to an integer. This is the 'bpp' parameter described
