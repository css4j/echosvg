 ...
 *
 * <p>
 * In addition to the familiar methods from <code>InputStream</code>, the
 ...
 * <code>getFilePointer()</code>. Some subclasses of <code>SeekableStream</code>
 * guarantee the ability to seek backwards while others may not offer this
 * feature in the interest of providing greater efficiency for those users who
 * do not require it.
 *
 * <p>
 * The <code>DataInput</code> interface is supported as well. This included the
 * <code>skipBytes()</code> and <code>readFully()</code> methods and a variety
 * of <code>read</code> methods for various data types.
 *
 * <p>
 * A number of concrete subclasses of <code>SeekableStream</code> are supplied
 * in the <code>com.sun.media.jai.codec</code> package.
 *
 * <p>
 ...
 *
 * <p>
 * The <code>FileSeekableStream</code> class wraps a <code>File</code> or
 * <code>RandomAccessFile</code>. It forwards requests to the real underlying
 * file. It performs a limited amount of caching in order to avoid excessive I/O
 * costs.
 *
 * <p>
 * The <code>SegmentedSeekableStream</code> class performs a different sort of
 * function. It creates a <code>SeekableStream</code> from another
 ...
 *
 * <p>
 * A convenience methods, <code>wrapInputStream</code> is provided to construct
 * a suitable <code>SeekableStream</code> instance whose data is supplied by a
 * given <code>InputStream</code>. The caller, by means of the
...
public abstract class SeekableStream extends InputStream implements DataInput {
	...
	/**
	 * Returns a <code>SeekableStream</code> that will read from a given
	 * <code>InputStream</code>, optionally including support for seeking backwards.
	 * This is a convenience method that avoids the need to instantiate specific
	 * subclasses of <code>SeekableStream</code> depending on the current security
	 * model.
	 *
	 * @param is               An <code>InputStream</code>.
	 * @param canSeekBackwards <code>true</code> if the ability to seek backwards in
	...
	/**
	 * Returns the file position to its position at the time of the immediately
	 * previous call to the <code>mark()</code> method.
	 */
	@Override
	...
	/**
	 * Returns <code>true</code> if marking is supported. Marking is automatically
	 * supported for <code>SeekableStream</code> subclasses that support seeking
	 * backeards. Subclasses that do not support seeking backwards but do support
	 * marking must override this method.
	 */
	@Override
	 ...
	 * <code>seek(pos)</code> with an offset <code>pos</code> smaller than the
	 * current offset, as returned by <code>getFilePointer</code>.
	 */
	 ...
	 *
	 * <p>
	 * If <code>canSeekBackwards()</code> returns <code>false</code>, then setting
	 * <code>pos</code> to an offset smaller than the current value of
	 * <code>getFilePointer()</code> will have no effect.
	 *
	 * @param pos the offset position, measured in bytes from the beginning of the
	 *            stream, at which to set the stream pointer.
	...
	/**
	 * Reads <code>b.length</code> bytes from this stream into the byte array,
	 * starting at the current stream pointer. This method reads repeatedly from the
	 * stream until the requested number of bytes are read. This method blocks until
	 * the requested number of bytes are read, the end of the stream is detected, or
	 * an exception is thrown.
	 *
	 * @param b the buffer into which the data is read.
	 * @exception EOFException if this stream reaches the end before reading all the
	 *                         bytes.
	 * @exception IOException  if an I/O error occurs.
	 */
	@Override
	...
	/**
	 * Reads exactly <code>len</code> bytes from this stream into the byte array,
	 * starting at the current stream pointer. This method reads repeatedly from the
	 * stream until the requested number of bytes are read. This method blocks until
	 * the requested number of bytes are read, the end of the stream is detected, or
	 * an exception is thrown.
	...
	/**
	 * Attempts to skip over <code>n</code> bytes of input discarding the skipped
	 * bytes.
	 * <p>
	 *
	 * This method may skip over some smaller number of bytes, possibly zero. This
	 * may result from any of a number of conditions; reaching end of stream before
	 * <code>n</code> bytes have been skipped is only one possibility. This method
	 * never throws an <code>EOFException</code>. The actual number of bytes skipped
	 * is returned. If <code>n</code> is negative, no bytes are skipped.
	 *
	...
	/**
	 * Reads a <code>boolean</code> from this stream. This method reads a single
	 * byte from the stream, starting at the current stream pointer. A value of
	 * <code>0</code> represents <code>false</code>. Any other value represents
	 * <code>true</code>. This method blocks until the byte is read, the end of the
	...
	/**
	 * Reads a signed eight-bit value from this stream. This method reads a byte
	 * from the stream, starting from the current stream pointer. If the byte read
	...
	/**
	 * Reads an unsigned eight-bit number from this stream. This method reads a byte
	 * from this stream, starting at the current stream pointer, and returns that
	 * byte.
	 * <p>
	 * This method blocks until the byte is read, the end of the stream is detected,
	 * or an exception is thrown.
	 *
	 * @return the next byte of this stream, interpreted as an unsigned eight-bit
	 *         number.
	 * @exception EOFException if this stream has reached the end.
	 * @exception IOException  if an I/O error occurs.
	 */
	...
	/**
	 * Reads a signed 16-bit number from this stream. The method reads two bytes
	 * from this stream, starting at the current stream pointer. If the two bytes
	 * read, in order, are <code>b1</code> and <code>b2</code>, where each of the
	 * two values is between <code>0</code> and <code>255</code>, inclusive, then
	 ...
	 *
	 * @return the next two bytes of this stream, interpreted as a signed 16-bit
	 *         number.
	 * @exception EOFException if this stream reaches the end before reading two
	 *                         bytes.
	 * @exception IOException  if an I/O error occurs.
	 */
	@Override
	...
	/**
	 * Reads a signed 16-bit number from this stream in little-endian order. The
	 * method reads two bytes from this stream, starting at the current stream
	 * pointer. If the two bytes read, in order, are <code>b1</code> and
	 * <code>b2</code>, where each of the two values is between <code>0</code> and
	...
	/**
	 * Reads an unsigned 16-bit number from this stream. This method reads two bytes
	 * from the stream, starting at the current stream pointer. If the bytes read,
	 * in order, are <code>b1</code> and <code>b2</code>, where
	 * <code>0&nbsp;&lt;=&nbsp;b1, b2&nbsp;&lt;=&nbsp;255</code>, then the result is
	 * equal to: <blockquote>
	 * 
	 * <pre>
	 * (b1 &lt;&lt; 8) | b2
	 ...
	 * detected, or an exception is thrown.
	 *
	 * @return the next two bytes of this stream, interpreted as an unsigned 16-bit
	 *         integer.
	 * @exception EOFException if this stream reaches the end before reading two
	 *                         bytes.
	 * @exception IOException  if an I/O error occurs.
	...
	/**
	 * Reads an unsigned 16-bit number from this stream in little-endian order. This
	 * method reads two bytes from the stream, starting at the current stream
	 * pointer. If the bytes read, in order, are <code>b1</code> and
	 * <code>b2</code>, where
	 * <code>0&nbsp;&lt;=&nbsp;b1, b2&nbsp;&lt;=&nbsp;255</code>, then the result is
	 * equal to: <blockquote>
	 * 
	...
	/**
	 * Reads a Unicode character from this stream. This method reads two bytes from
	 * the stream, starting at the current stream pointer. If the bytes read, in
	...
	/**
	 * Reads a Unicode character from this stream in little-endian order. This
	 * method reads two bytes from the stream, starting at the current stream
	 * pointer. If the bytes read, in order, are <code>b1</code> and
	 * <code>b2</code>, where
	 * <code>0&nbsp;&lt;=&nbsp;b1,&nbsp;b2&nbsp;&lt;=&nbsp;255</code>, then the
	...
	/**
	 * Reads a signed 32-bit integer from this stream. This method reads 4 bytes
	 * from the stream, starting at the current stream pointer. If the bytes read,
	...
	/**
	 * Reads a signed 32-bit integer from this stream in little-endian order. This
	 * method reads 4 bytes from the stream, starting at the current stream pointer.
	...
	/**
	 * Reads an unsigned 32-bit integer from this stream. This method reads 4 bytes
	 * from the stream, starting at the current stream pointer. If the bytes read,
	 * in order, are <code>b1</code>, <code>b2</code>, <code>b3</code>, and
	 * <code>b4</code>, where
	 * <code>0&nbsp;&lt;=&nbsp;b1, b2, b3, b4&nbsp;&lt;=&nbsp;255</code>, then the
	 ...
	 * This method reads 4 bytes from the stream, starting at the current stream
	...
	/**
	 * Reads a signed 64-bit integer from this stream. This method reads eight bytes
	 * from the stream, starting at the current stream pointer. If the bytes read,
	 ...
	 *         <code>long</code>.
	 * @exception EOFException if this stream reaches the end before reading eight
	 *                         bytes.
	 * @exception IOException  if an I/O error occurs.
	 */
	@Override
	...
	/**
	 * Reads a signed 64-bit integer from this stream in little-endian order. This
	 * method reads eight bytes from the stream, starting at the current stream
	 * pointer. If the bytes read, in order, are <code>b1</code>, <code>b2</code>,
	 * <code>b3</code>, <code>b4</code>, <code>b5</code>, <code>b6</code>,
	 * <code>b7</code>, and <code>b8,</code> where: <blockquote>
	 * 
	 * <pre>
	 *     0 &lt;= b1, b2, b3, b4, b5, b6, b7, b8 &lt;=255,
	 * </pre>
	 ...
	 * <code>int</code> value, starting at the current stream pointer, as if by the
	 * <code>readInt</code> method and then converts that <code>int</code> to a
	...
	/**
	 * Reads a <code>float</code> from this stream in little-endian order. This
	 * method reads an <code>int</code> value, starting at the current stream
	 * pointer, as if by the <code>readInt</code> method and then converts that
	 ...
	 * <code>long</code> value, starting at the current stream pointer, as if by the
	 * <code>readLong</code> method and then converts that <code>long</code> to a
	...
	/**
	 * Reads a <code>double</code> from this stream in little-endian order. This
	 * method reads a <code>long</code> value, starting at the current stream
	 * pointer, as if by the <code>readLong</code> method and then converts that
	...
	/**
	 * Reads the next line of text from this stream. This method successively reads
	 * bytes from the stream, starting at the current stream pointer, until it
	 * reaches a line terminator or the end of the stream. Each byte is converted
	 * into a character by taking the byte's value for the lower eight bits of the
	 * character and setting the high eight bits of the character to zero. This
	 * method does not, therefore, support the full Unicode character set.
	 *
	 * <p>
	 * A line of text is terminated by a carriage-return character
	 * (<code>'&#92;r'</code>), a newline character (<code>'&#92;n'</code>), a
	 * carriage-return character immediately followed by a newline character, or the
	 * end of the stream. Line-terminating characters are discarded and are not
	 * included as part of the string returned.
	 *
	 * <p>
	 ...
	 *
	 * @return the next line of text from this stream, or null if end of stream is
	...
	/**
	 * Reads in a string from this stream. The string has been encoded using a
	 * modified UTF-8 format.
	 * <p>
	 * The first two bytes are read, starting from the current stream pointer, as if
	 * by <code>readUnsignedShort</code>. This value gives the number of following
	 * bytes that are in the encoded string, not the length of the resulting string.
	 * The following bytes are then interpreted as bytes encoding characters in the
	 * UTF-8 format and are converted into characters.
	 * <p>
	 ...
	 * @exception IOException            if an I/O error occurs.
	 * @exception UTFDataFormatException if the bytes do not represent valid UTF-8
