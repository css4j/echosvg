...
import io.sf.carte.echosvg.gvt.event.SelectionAdapter;
...
public class JGVTComponent extends JComponent {
	...
	/**
	 * Whether the text should be selectable if eventEnabled is false, this flag is
	 * ignored.
	 */
	protected boolean selectableText;

	/**
	 * Whether the JGVTComponent should adhere to 'Unix' text selection semantics
	 * where as soon as text is selected it is copied to the clipboard. If users
	 * want Mac/Windows behaviour they need to handle selections them selves.
	 */
	 ...
	 *                       key events.
	 * @param selectableText Whether the text should be selectable. if eventEnabled
	 *                       is false, this flag is ignored.
	 */
	...
	/**
	 * Turn off all 'interactor' objects (pan, zoom, etc) if 'b' is true, turn them
	 * on if 'b' is false.
	 */
	...
	/**
	 * If 'b' is true text selections will copied to the clipboard immediately. If
	 * 'b' is false then nothing will be done when selections are made (the
	 * application is responsable for copying the selection in response to user
	 * actions).
	 */
	...
	/**
	 * Returns true if the canvas will copy selections to the clipboard when they
	 * are completed.
	 */
	...
	protected void initializeEventHandling() {
		if (eventsEnabled) {
			...
			if (selectableText) {
				textSelectionManager = createTextSelectionManager(eventDispatcher);
	...
	/**
	 * Returns the current Text selection manager for the Component. Users can
	 * register with this to be notifed of changes in the text selection.
	 */
	...
	public void immediateRepaint() {
		if (EventQueue.isDispatchThread()) {
			...
			if (doubleBufferedRendering)
				repaint(visRect.x, visRect.y, visRect.width, visRect.height);
			else
	...
	/**
	 * Sets the painting transform. A null transform is the same as an identity
	 * transform. The next repaint will use the given transform.
	 */
	...
	/**
	 * Sets the rendering transform. Calling this method causes a rendering to be
	 * performed.
	 */
	...
	/**
	 * Sets whether this component should use double buffering to render SVG
	 * documents. The change will be effective during the next rendering.
	 */
	...
	/**
	 * Flush any cached image data (preliminary interface, may be removed or
	...
	/**
	 * Flush a rectangle of cached image data (preliminary interface, may be removed
	...
	protected void renderGVTTree() {
		...
		gvtTreeRenderer = new GVTTreeRenderer(renderer, renderingTransform, doubleBufferedRendering, s, visRect.width,
				visRect.height);
		...
		for (GVTTreeRendererListener gvtTreeRendererListener : gvtTreeRendererListeners) {
			gvtTreeRenderer.addGVTTreeRendererListener(gvtTreeRendererListener);
		}
	...
	/**
	 * Computes the initial value of the transform used for rendering. Return true
	 * if a repaint is required, otherwise false.
	 */
	...
	protected class Listener
		...
		public void mouseReleased(java.awt.event.MouseEvent e) {
			if ((checkClick) && hadDrag) {
				...
				if ((dx * dx + dy * dy < MAX_DISP) && (cTime - startTime) < CLICK_TIME) {
					...
					//
					MouseEvent click = new MouseEvent(e.getComponent(), MouseEvent.MOUSE_CLICKED, e.getWhen(),
		...
		public void mouseWheelMoved(MouseWheelEvent e) {
			/*
			 * selectInteractor(e); if (interactor != null) { interactor.mouseWheelMoved(e);
			 * deselectInteractor(); } else
			 */ if (eventDispatcher != null) {
		...
		protected void selectInteractor(InputEvent ie) {
			if (!disableInteractions && !suspendInteractions && interactor == null && gvtRoot != null) {
