...
import io.sf.carte.echosvg.swing.gvt.AbstractPanInteractor;
import io.sf.carte.echosvg.swing.gvt.AbstractResetTransformInteractor;
import io.sf.carte.echosvg.swing.gvt.AbstractRotateInteractor;
import io.sf.carte.echosvg.swing.gvt.AbstractZoomInteractor;
import io.sf.carte.echosvg.swing.gvt.Interactor;
 ...
 * <code>JSVGCanvas</code> does not provided additional functionalities compared
 * to the <code>JSVGComponent</code> but simply provides an API conformed to the
...
public class JSVGCanvas extends JSVGComponent {
	...
	/**
	 * This flag bit indicates whether or not the zoom interactor is enabled. True
	 * means the zoom interactor is functional.
	 */
	private boolean isZoomInteractorEnabled = true;

	/**
	 * This flag bit indicates whether or not the image zoom interactor is enabled.
	 * True means the image zoom interactor is functional.
	 */
	private boolean isImageZoomInteractorEnabled = true;

	/**
	 * This flag bit indicates whether or not the pan interactor is enabled. True
	 * means the pan interactor is functional.
	 */
	private boolean isPanInteractorEnabled = true;

	/**
	 * This flag bit indicates whether or not the rotate interactor is enabled. True
	 * means the rotate interactor is functional.
	 */
	...
	/**
	 * Keeps track of the last known mouse position over the canvas. This is used
	 * for displaying tooltips at the right location.
	 */
	protected LocationListener locationListener = new LocationListener();

	/**
	 * Mapping of elements to listeners so they can be removed, if the tooltip is
	 * removed.
	 */
	...
	/**
	 * Sets the URI to the specified uri. If the input 'newURI' string is null, then
	 * the canvas will display an empty document.
	 *
	...
	protected void installSVGDocument(SVGDocument doc) {
		if (toolTipDocs != null) {
			for (SVGDocument ttdoc : toolTipDocs.keySet()) {
				...
				if (root == null)
					continue;
				root.removeEventListenerNS(XMLConstants.XML_EVENTS_NAMESPACE_URI, SVGConstants.SVG_EVENT_MOUSEOVER,
						toolTipListener, false);
				root.removeEventListenerNS(XMLConstants.XML_EVENTS_NAMESPACE_URI, SVGConstants.SVG_EVENT_MOUSEOUT,
						toolTipListener, false);
			}
			toolTipDocs = null;
		}
...
//     }
	 ...
	 * transform. Before the rendering transform is applied the method translates
	 * the center of the display to 0,0 so scale and rotate occur around the middle
	 * of the display.
	 */
	...
	/**
	 * A swing action to apply a zoom factor to the canvas. This can be used to zoom
	...
	protected Interactor zoomInteractor = new AbstractZoomInteractor() {
		...
		public boolean startInteraction(InputEvent ie) {
			...
			return ie.getID() == MouseEvent.MOUSE_PRESSED && (mods & InputEvent.BUTTON1_DOWN_MASK) != 0
	...
	protected Interactor imageZoomInteractor = new AbstractImageZoomInteractor() {
		...
		public boolean startInteraction(InputEvent ie) {
			int mods = ie.getModifiersEx();
			return ie.getID() == MouseEvent.MOUSE_PRESSED && (mods & InputEvent.BUTTON3_DOWN_MASK) != 0
	...
	protected Interactor resetTransformInteractor = new AbstractResetTransformInteractor() {
		...
		public boolean startInteraction(InputEvent ie) {
			int mods = ie.getModifiersEx();
			return ie.getID() == MouseEvent.MOUSE_CLICKED && (mods & InputEvent.BUTTON3_DOWN_MASK) != 0
					&& (mods & InputEvent.SHIFT_DOWN_MASK) != 0 && (mods & InputEvent.CTRL_DOWN_MASK) != 0;
		}
	};
	 ...
	 * default <code>BridgeUserAgent</code>. A tooltip will be displayed wheneven
	 * the mouse lingers over an element which has a &lt;title&gt; or a &lt;desc&gt;
	 * child element.
	 */
	protected class CanvasUserAgent extends BridgeUserAgent
		 ...
		 * &lt;title&gt; element, a &lt;desc&gt; element or both. <br>
		 * Because these elements can appear in any order, here is the algorithm used to
		 * build the tool tip:<br>
		 * <ul>
		 * <li>If a &lt;title&gt; is passed to <code>handleElement</code> the method
		 * checks if there is a &gt;desc&gt; peer. If there is one, nothing is done
		 * (because the desc will do it). If there in none, the tool tip is set to the
		 * value of the &lt;title&gt; element content.</li>
		 * <li>If a &lt;desc&gt; is passed to <code>handleElement</code> the method
		 * checks if there is a &lt;title&gt; peer. If there is one, the content of that
		 * peer is pre-pended to the content of the &lt;desc&gt; element.</li>
		 * </ul>
		 */
		@Override
		public void handleElement(Element elt, Object data) {
			...
			if (elt.getLocalName().equals(SVGConstants.SVG_TITLE_TAG)) {
				if (data == Boolean.TRUE)
					titlePeer = elt;
				descPeer = getPeerWithTag(parent, SVGConstants.SVG_NAMESPACE_URI, SVGConstants.SVG_DESC_TAG);
			} else if (elt.getLocalName().equals(SVGConstants.SVG_DESC_TAG)) {
				if (data == Boolean.TRUE)
					descPeer = elt;
				titlePeer = getPeerWithTag(parent, SVGConstants.SVG_NAMESPACE_URI, SVGConstants.SVG_TITLE_TAG);
			}
				...
				if (o != null) {
					...
					EventQueue.invokeLater(new Runnable() {
						...
						public void run() {
							...
							MouseEvent e = new MouseEvent(JSVGCanvas.this, MouseEvent.MOUSE_MOVED,
									System.currentTimeMillis(), 0, locationListener.getLastX(),
		...
		/**
		 * Converts line breaks to HTML breaks and encodes special entities. Poor way of
		 * replacing '&lt;', '&gt;' and '&amp;' in content.
		 */
		...
		private void replace(StringBuilder sb, char c, String r) {
		...
		/**
		 * Checks if there is a peer element of a given type. This returns the first
		 * occurence of the given type or null if none is found.
		 */
		...
		/**
		 * Returns a boolean defining whether or not there is a peer of <code>elt</code>
		...
		public void setToolTip(Element elt, String toolTip) {
			...
			if (toolTipDocs.put(doc, MAP_TOKEN) == null) {
				...
				root.addEventListenerNS(XMLConstants.XML_EVENTS_NAMESPACE_URI, SVGConstants.SVG_EVENT_MOUSEOVER,
						toolTipListener, false, null);
				// On mouseout, it removes the tooltip
				root.addEventListenerNS(XMLConstants.XML_EVENTS_NAMESPACE_URI, SVGConstants.SVG_EVENT_MOUSEOUT,
						toolTipListener, false, null);
			}
	...
	/**
	 * Checks if the specified event time and element are the same as the last tool
	 * tip event.
	 */
	...
	/**
	 * Helper class. Simply keeps track of the last known mouse position over the
	 * canvas.
	 */
	 ...
	 * <code>MouseEvent</code> when the toolTip is set to a non-null value, so as to
	...
	protected class ToolTipRunnable implements Runnable {
		...
		public void run() {
			...
			MouseEvent e;
			if (theToolTip != null) {
				e = new MouseEvent(JSVGCanvas.this, MouseEvent.MOUSE_ENTERED, System.currentTimeMillis(), 0,
						locationListener.getLastX(), locationListener.getLastY(), 0, false);
				ToolTipManager.sharedInstance().mouseEntered(e);
				e = new MouseEvent(JSVGCanvas.this, MouseEvent.MOUSE_MOVED, System.currentTimeMillis(), 0,
						locationListener.getLastX(), locationListener.getLastY(), 0, false);
				ToolTipManager.sharedInstance().mouseMoved(e);
			} else {
				e = new MouseEvent(JSVGCanvas.this, MouseEvent.MOUSE_MOVED, System.currentTimeMillis(), 0,
