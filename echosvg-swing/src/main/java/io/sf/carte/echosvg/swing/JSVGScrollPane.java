 ...
 * <p>
 * Reimplementation, rather than implementing the Scrollable interface, provides
 * several advantages. The main advantage is the ability to control more
 * precisely ScrollBar events; fewer JSVGCanvas updates are required when
...
public class JSVGScrollPane extends JPanel {
	...
	public JSVGScrollPane(JSVGCanvas canvas) {
		...
		cornerBox = Box.createRigidArea(
				new Dimension(vertical.getPreferredSize().width, horizontal.getPreferredSize().height));
	...
	/**
	 * Scrollbar listener factory method so subclasses can override the default
	 * SBListener behaviour.
	 */
	...
	class SVGScrollDocumentLoaderListener extends SVGDocumentLoaderAdapter {
		...
		public void documentLoadingCompleted(SVGDocumentLoaderEvent e) {
			...
			root.addEventListenerNS(XMLConstants.XML_EVENTS_NAMESPACE_URI, SVGConstants.SVG_SVGZOOM_EVENT_TYPE,
					new EventListener() {
						@Override
	...
	/**
	 * Resets this object (for reloads), releasing any cached data and recomputing
	 * scroll extents.
	 */
	public void reset() {
		viewBox = null;
	...
	/**
	 * Sets the translation portion of the transform based upon the current scroll
	 * bar position
	 */
	 ...
	 * events. It doesn't coalesce as many events as it should, but it helps
	 * considerably.
	 * </p>
	 */
	protected class SBListener implements ChangeListener {
		...
		public synchronized void stateChanged(ChangeEvent e) {
			...
			int val = ((isVertical) ? vertical.getValue() : horizontal.getValue());
			...
			if (brm.getValueIsAdjusting()) {
				if (!inDrag) {
					...
					AffineTransform at;
					if (isVertical) {
						at = AffineTransform.getTranslateInstance(0, startValue - val);
					} else {
						at = AffineTransform.getTranslateInstance(startValue - val, 0);
					}
	...
	protected class ScrollListener extends ComponentAdapter
		...
		public void gvtRenderingCompleted(GVTTreeRendererEvent e) {
			...
			if ((newview.getX() != viewBox.getX()) || (newview.getY() != viewBox.getY())
					|| (newview.getWidth() != viewBox.getWidth()) || (newview.getHeight() != viewBox.getHeight())) {
				viewBox = newview;
				resizeScrollBars();
			}
		}
	...
	/**
	 * Derives the SVG Viewbox from the SVG root element. Caches it. Assumes that it
	 * will not change.
	 *
	 */
	...
	/**
	 * Called when the scale size changes. The scale factor (1.0 == original size).
	 * By default, this method does nothing, but may be overidden to display a scale
