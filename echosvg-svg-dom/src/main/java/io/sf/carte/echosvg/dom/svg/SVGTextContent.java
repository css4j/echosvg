...
public interface SVGTextContent {
	 ...
	 * sequence of glyphs), then each of the inseparable characters will return the
	 * same extent.
	 *
	 * @param charnum The index of the character, where the first character has an
	 *                index of 0.
	 ...
	 * 'letter-spacing' and 'word-spacing' and adjustments due to attributes x, y,
	 * dx and dy. If multiple consecutive characters are rendered inseparably (e.g.,
	 * as a single glyph or a sequence of glyphs), then each of the inseparable
	 ...
	 * of any inter- character adjustments to prepare for the next character, such
	 * as properties 'kerning', 'letter-spacing' and 'word-spacing' and adjustments
	 * due to attributes x, y, dx and dy. If multiple consecutive characters are
	 * rendered inseparably (e.g., as a single glyph or a sequence of glyphs), then
	 * each of the inseparable characters will return the end position for the last
	 * glyph.
	 *
	 * @param charnum The index of the character, where the first character has an
	 *                index of 0.
	 ...
	 * multiple glyph(s) are used to render the given character and the glyphs each
	 * have different rotations (e.g., due to text-on-a-path), the user agent shall
	 * return an average value (e.g., the rotation angle at the midpoint along the
	 * path for all glyphs used to render this character). The rotation value
	 * represents the rotation that is supplemental to any rotation due to
	 * properties 'glyph-orientation-horizontal' and 'glyph-orientation-vertical';
	 * thus, any glyph rotations due to these properties are not included into the
	 * returned rotation value. If multiple consecutive characters are rendered
	 * inseparably (e.g., as a single glyph or a sequence of glyphs), then each of
	 * the inseparable characters will return the same rotation value.
	 *
	 * @param charnum The index of the character, where the first character has an
	 *                index of 0.
	 ...
	 *                specifies more characters than are available, then the
	 *                substring will consist of all characters starting with charnum
