...
public abstract class AbstractSVGMatrix implements SVGMatrix {
	...
	public SVGMatrix multiply(SVGMatrix secondMatrix) {
		AffineTransform at = new AffineTransform(secondMatrix.getA(), secondMatrix.getB(), secondMatrix.getC(),
				secondMatrix.getD(), secondMatrix.getE(), secondMatrix.getF());
	...
	public SVGMatrix inverse() throws SVGException {
		try {
			...
			throw new SVGOMException(SVGException.SVG_MATRIX_NOT_INVERTABLE, e.getMessage());
		}
	}
	...
	public SVGMatrix skewX(float angleDeg) {
		AffineTransform tr = (AffineTransform) getAffineTransform().clone();
		tr.concatenate(AffineTransform.getShearInstance(Math.tan(Math.toRadians(angleDeg)), 0));
		return new SVGOMMatrix(tr);
	}
