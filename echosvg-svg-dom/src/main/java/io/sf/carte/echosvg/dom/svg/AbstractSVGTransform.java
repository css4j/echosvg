...
public abstract class AbstractSVGTransform implements SVGTransform {
	...
	/**
	 * The angle of the transformation, if this transformation is a rotation or a
	 * skew. This is stored to avoid extracting the angle from the transformation
	 * matrix.
	 */
	protected float angle;

	/**
	 * The x coordinate of the center of the rotation, if this transformation is a
	 * rotation.
	 */
	protected float x;

	/**
	 * The y coordinate of the center of the rotation, if this transformation is a
	 * rotation.
	 */
	protected float y;

	/**
	 * Creates and returns a new {@link SVGMatrix} for exposing the transformation
	 * as a matrix.
	 * 
	...
	public void setMatrix(SVGMatrix matrix) {
		...
		affineTransform = new AffineTransform(matrix.getA(), matrix.getB(), matrix.getC(), matrix.getD(), matrix.getE(),
				matrix.getF());
	}
	...
	public void setSkewX(float angle) {
		...
		affineTransform = AffineTransform.getShearInstance(Math.tan(Math.toRadians(angle)), 0.0);
		this.angle = angle;
	}
