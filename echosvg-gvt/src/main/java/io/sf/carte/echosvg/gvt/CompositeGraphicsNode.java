 ...
 *
 * <br>
 * Note: this class is a 'little bit aware of' other threads, but not really
 * threadsafe.
 *
...
public class CompositeGraphicsNode extends AbstractGraphicsNode implements List<GraphicsNode> {
	...
	/**
	 * This flag indicates if this node has BackgroundEnable = 'new'. If so
	 * traversal of the gvt tree can halt here.
	 */
	protected Rectangle2D backgroundEnableRgn = null;

	/**
	 * Internal Cache: Geometry bounds for this node, not taking into account any of
	 * its children rendering attributes into account
	 */
	 ...
	 * fire a graphicsNodeChange event because this doesn't really effect us (it
	 * effects our children through CSS inheritence).
	 *
	...
	public void primitivePaint(Graphics2D g2d) {
		...
		// Thread.currentThread() is potentially expensive, so reuse my instance in
		// hasBeenHalted()
	...
	/**
	 * Invalidates the cached geometric bounds. This method is called each time an
	 * attribute that affects the bounds of this node changed.
	 */
	@Override
	...
	/**
	 * Transforms a Rectangle 2D by an affine transform. It assumes the transform is
	 * only scale/translate so there is no loss of precision over transforming the
	 * source geometry.
	 */
	public static Rectangle2D getTransformedBBox(Rectangle2D r2d, AffineTransform t) {
		...
		return new Rectangle2D.Float((float) (x * sx + t.getTranslateX()), (float) (y * sy + t.getTranslateY()),
	 ...
	 * its rendering attributes into account. That is, exclusive of any clipping,
	...
	public Rectangle2D getGeometryBounds() {
		if (geometryBounds == null) {
			...
			while (geometryBounds == null && i < count) {
				geometryBounds = children[i++].getTransformedGeometryBounds(IDENTITY);
			}
	...
	/**
	 * Returns the bounds of the area covered by this node, without taking any of
	 * its rendering attribute into accoun. That is, exclusive of any clipping,
	...
	public Rectangle2D getSensitiveBounds() {
		...
		while (sensitiveBounds == null && i < count) {
			sensitiveBounds = children[i++].getTransformedSensitiveBounds(IDENTITY);
		}
	...
	/**
	 * Returns true if the specified Point2D is inside the boundary of this node,
	 * false otherwise.
	 *
	...
	/**
	 * Returns true if this composite graphics node does not contain graphics node,
	 * false otherwise.
	 */
	@Override
	...
	/**
	 * Returns true if this composite graphics node contains the specified graphics
	...
	/**
	 * Returns an array containing all of the graphics node in the children list of
	 * this composite graphics node in the correct order.
	 */
	@Override
	...
	/**
	 * Returns an array containing all of the graphics node in the children list of
	 * this composite graphics node in the correct order. If the children list fits
	...
	public <T> T[] toArray(T[] a) {
		if (a.length < count) {
			...
			return (T[]) Arrays.copyOf(children, count, a.getClass());
		}
	 ...
	 * @exception IndexOutOfBoundsException if the index is out of range
	 * @exception IllegalArgumentException  if the node is not an instance of
	 *                                      GraphicsNode
	 */
	@Override
	 ...
	 *
	 * @param node the graphics node to add
	 ...
	 * children list. Shifts the graphics node currently at that position (if any)
	 * and any subsequent graphics nodes to the right (adds one to their indices).
	 *
	 * @param index the position at which the specified graphics node is to be
	 *              inserted.
	 * @param node  the graphics node to be inserted.
	 * @exception IndexOutOfBoundsException if the index is out of range
	 * @exception IllegalArgumentException  if the node is not an instance of
	 *                                      GraphicsNode
	 */
	...
	/**
	 * <b>Not supported</b> - Throws <code>UnsupportedOperationException</code>
	 * exception.
	 */
	@Override
	...
	/**
	 * Removes the first instance of the specified graphics node from the children
	 * list.
	 *
	 ...
	 * Shifts any subsequent graphics nodes to the left (subtracts one from their
	 * indices).
	 *
	...
	/**
	 * Returns true if this composite graphics node contains all the graphics node
	...
	/**
	 * Returns the index in the children list of the specified graphics node or -1
	 * if the children list does not contain this graphics node.
	 *
	...
	/**
	 * Returns an iterator over the children of this graphics node, starting at the
	 * specified position in the children list.
	 *
	 * @param index the index of the first graphics node to return from the children
	 *              list
	 */
	@Override
	...
	/**
	 * Increases the capacity of the children list, if necessary, to ensure that it
	 * can hold at least the number of graphics nodes specified by the minimum
	 * capacity argument.
	 *
	 ...
	 */
	private class Itr implements Iterator<GraphicsNode> {
		 ...
		 * Reset to -1 if this graphics node is deleted by a call to remove.
		 */
		int lastRet = -1;
	 ...
	 */
	private class ListItr extends Itr implements ListIterator<GraphicsNode> {
		...
		public GraphicsNode previous() {
			try {
				GraphicsNode previous = get(--cursor);
				checkForComodification();
				lastRet = cursor;
				return previous;
			} catch (IndexOutOfBoundsException e) {
		...
		@Override
		public void set(GraphicsNode o) {
			if (lastRet == -1) {
				throw new IllegalStateException();
			}
			checkForComodification();
		...
		@Override
		public void add(GraphicsNode o) {
			checkForComodification();
			try {
