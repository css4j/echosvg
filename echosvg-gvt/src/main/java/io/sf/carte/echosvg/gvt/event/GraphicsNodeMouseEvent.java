...
public class GraphicsNodeMouseEvent extends GraphicsNodeInputEvent {
	...
	/**
	 * The id for the "mouseMoved" event. This MouseMotionEvent occurs when the
	 * mouse position changes.
	 */
	public static final int MOUSE_MOVED = MOUSE_FIRST + 3;

	/**
	 * The id for the "mouseEntered" event. This MouseEvent occurs when the mouse
	 * cursor enters a graphics node's area.
	 */
	public static final int MOUSE_ENTERED = MOUSE_FIRST + 4;

	/**
	 * The id for the "mouseExited" event. This MouseEvent occurs when the mouse
	 * cursor leaves a graphics node's area.
	 */
	public static final int MOUSE_EXITED = MOUSE_FIRST + 5;

	/**
	 * The id for the "mouseDragged" event. This MouseEvent occurs when the mouse
	 * position changes while the "drag" modifier is active (for example, the shift
	 * key).
	 */
	public static final int MOUSE_DRAGGED = MOUSE_FIRST + 6;

	/**
	 * The graphics node mouse events x coordinate. The x value is relative to the
	 * graphics node that fired the event.
	 */
	float x;

	/**
	 * The graphics node mouse events y coordinate. The y value is relative to the
	 * graphics node that fired the event.
	 */
	float y;
	...
	/**
	 * Additional information. For a MOUSE_EXITED, this will contain the destination
	 * node, for a MOUSE_ENTERED the last node and for a MOUSE_DRAGGED the node
	 ...
	 * @see #getRelatedNode
	 */
	public GraphicsNodeMouseEvent(GraphicsNode source, int id, long when, int modifiers, int lockState, int button,
			float x, float y, int clientX, int clientY, int screenX, int screenY, int clickCount,
	...
	/**
	 * Returns the related node for this <code>GraphicsNodeMouseEvent</code>. For a
	 * <code>MOUSE_ENTERED</code> event it is the previous node target, for a
	 * <code>MOUSE_EXITED</code> event it is the next node target and for a
	 * <code>MOUSE_DRAGGED</code> event it is the node under the mouse pointer.
	 * Otherwise the value is <code>null</code>.
	 */
