 ...
 * corresponding to the mouse event coordinate). Searches for containment are
 * performed from the EventDispatcher's "root" node.
 * </p>
 *
...
public class AWTEventDispatcher
	...
	/**
	 * The base AffineTransform for InputEvent-to-GraphicsNodeEvent coordinates as
	 * determined by setBaseTransform().
	 */
	...
	/**
	 * Sets the root node for MouseEvent dispatch containment searches and field
	 * selections.
	 * 
	...
	/**
	 * Returns the root node for MouseEvent dispatch containment searches and field
	 * selections.
	 */
	@Override
	...
	public void setBaseTransform(AffineTransform t) {
		if ((baseTransform != t) && ((baseTransform == null) || (!baseTransform.equals(t))))
	...
	/**
	 * Returns the base transform applied to MouseEvent coordinates prior to
	 * dispatch.
	 */
	@Override
	...
	/**
	 * Dispatches the specified AWT mouse event down to the GVT tree. The mouse
	 * event is mutated to a GraphicsNodeMouseEvent.
	 * 
	...
	/**
	 * Dispatches the specified AWT mouse wheel event down to the GVT tree. The
	 * mouse wheel event is mutated to a GraphicsNodeMouseWheelEvent.
	 * 
	 * @param evt the mouse event to propagate
	 */
	@Override
	...
	/**
	 * Dispatches the specified AWT key event down to the GVT tree. The mouse event
	 * is mutated to a GraphicsNodeKeyEvent.
	 * 
	...
	/**
	 * Adds the specified 'global' GraphicsNodeMouseListener which is notified of
	 * all MouseEvents dispatched.
	 * 
	...
	/**
	 * Removes the specified 'global' GraphicsNodeMouseListener which is notified of
	 * all MouseEvents dispatched.
	 * 
	...
	/**
	 * Adds the specified 'global' GraphicsNodeMouseWheelListener which is notified
	 * of all MouseWheelEvents dispatched.
	 * 
	 * @param l the listener to add
	 */
	@Override
	...
	/**
	 * Adds the specified 'global' GraphicsNodeKeyListener which is notified of all
	 * KeyEvents dispatched.
	 * 
	 * @param l the listener to add
	 */
	@Override
	...
	/**
	 * Removes the specified 'global' GraphicsNodeKeyListener which is notified of
	 * all KeyEvents dispatched.
	 * 
	 * @param l the listener to remove
	 */
	@Override
	...
	/**
	 * Returns an array of listeners that were added to this event dispatcher and of
	 * the specified type.
	 * 
	...
	public EventListener[] getListeners(Class<?> listenerType) {
		...
		Object array = Array.newInstance(listenerType, glisteners.getListenerCount(listenerType));
	...
	protected void dispatchKeyEvent(KeyEvent evt) {
		...
		GraphicsNode target = currentKeyEventTarget == null ? root : currentKeyEventTarget;
		processKeyEvent(new GraphicsNodeKeyEvent(target, evt.getID(), evt.getWhen(), evt.getModifiersEx(),
				getCurrentLockState(), evt.getKeyCode(), evt.getKeyChar(), evt.getKeyLocation()));
	}
	...
	protected void dispatchMouseEvent(MouseEvent evt) {
		...
		if (lastHit != node) {
			...
			if (lastHit != null) {
				gvtevt = new GraphicsNodeMouseEvent(lastHit, MouseEvent.MOUSE_EXITED, evt.getWhen(),
						evt.getModifiersEx(), currentLockState, evt.getButton(), (float) gnp.getX(), (float) gnp.getY(),
						(int) Math.floor(p.getX()), // evt.getX() ??
						(int) Math.floor(p.getY()), // evt.getY() ??
						screenPos.x, screenPos.y, evt.getClickCount(), node);
				processMouseEvent(gvtevt);
				// lastHit.processMouseEvent(gvtevt);
			}
			// post a MOUSE_ENTERED event
			if (node != null) {
				gvtevt = new GraphicsNodeMouseEvent(node, MouseEvent.MOUSE_ENTERED, evt.getWhen(), evt.getModifiersEx(),
						currentLockState, evt.getButton(), (float) gnp.getX(), (float) gnp.getY(),
						(int) Math.floor(p.getX()), (int) Math.floor(p.getY()), screenPos.x, screenPos.y,
						evt.getClickCount(), lastHit);
				processMouseEvent(gvtevt);
		...
		if (node != null) {
			gvtevt = new GraphicsNodeMouseEvent(node, evt.getID(), evt.getWhen(), evt.getModifiersEx(),
					currentLockState, evt.getButton(), (float) gnp.getX(), (float) gnp.getY(),
					(int) Math.floor(p.getX()), (int) Math.floor(p.getY()), screenPos.x, screenPos.y,
					evt.getClickCount(), null);
			...
			gvtevt = new GraphicsNodeMouseEvent(root, evt.getID(), evt.getWhen(), evt.getModifiersEx(),
					currentLockState, evt.getButton(), (float) gnp.getX(), (float) gnp.getY(),
					(int) Math.floor(p.getX()), (int) Math.floor(p.getY()), screenPos.x, screenPos.y,
					evt.getClickCount(), null);
	...
	protected void dispatchMouseWheelEvent(MouseWheelEvent evt) {
		if (lastHit != null) {
			processMouseWheelEvent(new GraphicsNodeMouseWheelEvent(lastHit, evt.getID(), evt.getWhen(),
					evt.getModifiersEx(), getCurrentLockState(), evt.getWheelRotation()));
		}
	}

	/**
	 * Processes the specified event by firing the 'global' listeners attached to
	...
	protected void processMouseEvent(GraphicsNodeMouseEvent evt) {
		if (glisteners != null) {
			GraphicsNodeMouseListener[] listeners = (GraphicsNodeMouseListener[]) getListeners(
	...
	protected void processMouseWheelEvent(GraphicsNodeMouseWheelEvent evt) {
		if (glisteners != null) {
			GraphicsNodeMouseWheelListener[] listeners = (GraphicsNodeMouseWheelListener[]) getListeners(
					GraphicsNodeMouseWheelListener.class);
	...
	/**
	 * Dispatches a graphics node key event to by firing the 'global' listeners
	...
	public void processKeyEvent(GraphicsNodeKeyEvent evt) {
		if ((glisteners != null)) {
			GraphicsNodeKeyListener[] listeners = (GraphicsNodeKeyListener[]) getListeners(
					GraphicsNodeKeyListener.class);
	...
	/**
	 * Associates all InputEvents of type <code>e.getID()</code> with "incrementing"
	 * of the currently selected GraphicsNode.
	 */
	@Override
	...
	/**
	 * Associates all InputEvents of type <code>e.getID()</code> with "decrementing"
	 * of the currently selected GraphicsNode. The notion of "currently selected"
	 * GraphicsNode is used for dispatching KeyEvents.
	 */
	@Override
