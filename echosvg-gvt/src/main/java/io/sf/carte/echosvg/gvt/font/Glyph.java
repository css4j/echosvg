...
public class Glyph {
	 ...
	 */
	public Glyph(String unicode, List<String> names, String orientation, String arabicForm, String lang,
			Point2D horizOrigin, Point2D vertOrigin, float horizAdvX, float vertAdvY, int glyphCode, TextPaintInfo tpi,
	...
	/**
	 * Returns the orientation of this glyph. Indicates what
	 * inline-progression-direction this glyph can be used in. Should be either "h"
	 * for horizontal only, "v" for vertical only, or empty which indicates that the
	 * glyph can be used in both.
	 *
	...
	/**
	 * Returns which of the four possible arabic forms this glyph represents. This
	 * is only used for arabic glyphs.
	 *
	...
	/**
	 * Returns the glyphs unique code with resect to its font. This will be the
	 * index into the font's list of glyphs.
	 *
	...
	public GVTGlyphMetrics getGlyphMetrics() {
		if (metrics == null) {
			...
			metrics = new GVTGlyphMetrics(getHorizAdvX(), getVertAdvY(),
					new Rectangle2D.Double(gb.getX() - position.getX(), gb.getY() - position.getY(), gb.getWidth(),
							gb.getHeight()),
	 ...
	 *
	 * @param hkern The horizontal kerning value to apply when calculating the glyph
	 *              metrics.
	 * @param vkern The horizontal vertical value to apply when calculating the
	 *              glyph metrics.
	 * @return The kerned glyph metics
	 */
	public GVTGlyphMetrics getGlyphMetrics(float hkern, float vkern) {
		return new GVTGlyphMetrics(getHorizAdvX() - hkern, getVertAdvY() - vkern, getGeometryBounds(),
				GlyphMetrics.COMPONENT);
	...
	public Rectangle2D getBounds2D() {
		...
		if ((bounds != null) && TextPaintInfo.equivilent(tpi, cacheTPI))
			return bounds;
		...
		if (bounds == null)
			bounds = new Rectangle2D.Double(position.getX(), position.getY(), 0, 0);
	 ...
	 * glyph transforms will have been applied.
	 *
