...
public abstract class AbstractGraphicsNode implements GraphicsNode {
	...
	/**
	 * The compositing operation to be used when a graphics node is painted on top
	 * of another one.
	 */
	...
	/**
	 * The rendering hints that control the quality to use when rendering this
	 * graphics node.
	 */
	...
	/**
	 * Returns a canonical WeakReference to this GraphicsNode. This is suitable for
	 * use as a key value in a hash map
	 */
	@Override
	 ...
	 *
	 * @return VISIBLE_PAINTED | VISIBLE_FILL | VISIBLE_STROKE | VISIBLE | PAINTED |
	...
	/**
	 * Returns the concatenated transform of this node. That is, this node's
	...
	/**
	 * Maps the specified key to the specified value in the rendering hints of this
	 * node.
	 *
	...
	/**
	 * Copies all of the mappings from the specified Map to the rendering hints of
	 * this node.
	 *
	...
	/**
	 * Returns the GraphicsNodeRable for this node. This GraphicsNodeRable is the
	 * Renderable (Filter) before any of the filter operations have been applied.
	 */
	@Override
	...
	/**
	 * Returns the GraphicsNodeRable for this node. This GraphicsNodeRable is the
	 * Renderable (Filter) after all of the filter operations have been applied.
	 */
	@Override
	...
	public void paint(Graphics2D g2d) {
		...
		if (paintNeeded) {
			...
			if ((clip != null) && clip.getUseAntialiasedClip()) {
				antialiasedClip = isAntialiasedClip(g2d.getTransform(), g2d.getRenderingHints(), clip.getClipPath());
			}
			...
			if (!useOffscreen) {
				...
				io.sf.carte.echosvg.ext.awt.image.GraphicsUtil.drawImage(g2d, filteredImage);
	...
	protected boolean isAntialiasedClip(AffineTransform usr2dev, RenderingHints hints, Shape clip) {
		...
		if (!(clip instanceof Rectangle2D && usr2dev.getShearX() == 0 && usr2dev.getShearY() == 0))
			return true;
	...
	public void fireGraphicsNodeChangeStarted(GraphicsNode changeSrc) {
		if (changeStartedEvent == null)
			changeStartedEvent = new GraphicsNodeChangeEvent(this, GraphicsNodeChangeEvent.CHANGE_STARTED);
	...
	public void fireGraphicsNodeChangeCompleted() {
		if (changeCompletedEvent == null) {
			changeCompletedEvent = new GraphicsNodeChangeEvent(this, GraphicsNodeChangeEvent.CHANGE_COMPLETED);
		}
	...
	/**
	 * Returns the bounds of this node in user space. This includes primitive paint,
	 * filtering, clipping and masking.
	 */
	@Override
	...
	/**
	 * Returns the bounds of this node after applying the input transform (if any),
	 * concatenated with this node's transform (if any).
	 *
	...
	/**
	 * This method makes sure that neither the width nor height of the rectangle is
	 * zero. But it tries to make them very small relatively speaking.
	 */
