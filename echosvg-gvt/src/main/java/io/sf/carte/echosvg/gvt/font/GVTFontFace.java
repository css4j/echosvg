...
public class GVTFontFace implements SVGConstants {
	 ...
	 */
	public GVTFontFace(String familyName, float unitsPerEm, String fontWeight, String fontStyle, String fontVariant,
			String fontStretch, float slope, String panose1, float ascent, float descent, float strikethroughPosition,
			float strikethroughThickness, float underlinePosition, float underlineThickness, float overlinePosition,
	...
	/**
	 * Constructs an SVGFontFace with default values for all the font-face
	 * attributes other than familyName
	 */
	public GVTFontFace(String familyName) {
		this(familyName, 1000, SVG_FONT_FACE_FONT_WEIGHT_DEFAULT_VALUE, SVG_FONT_FACE_FONT_STYLE_DEFAULT_VALUE,
				SVG_FONT_FACE_FONT_VARIANT_DEFAULT_VALUE, SVG_FONT_FACE_FONT_STRETCH_DEFAULT_VALUE, 0,
				SVG_FONT_FACE_PANOSE_1_DEFAULT_VALUE, 800, 200, 300, 50, -75, 50, 800, 50);
	}
		...
		if (ffname.length() > family.length()) {
			...
			if (idx + family.length() < ffname.length()) {
				...
				default:
					return false;
				case ' ':
					loop: for (int i = idx + family.length() + 1; i < ffname.length(); i++) {
						switch (ffname.charAt(i)) {
						default:
							return false;
						case ' ':
	...
	/**
	 * Returns the maximum unaccented height of the font within the font coordinate
	 * system.
	 */
	...
	/**
	 * Returns the maximum unaccented depth of the font within the font coordinate
	 * system.
	 */
