...
/**
 * Handles the processing of arabic text. In particular it determines the form
 * each arabic char should take. It also contains methods for substituting plain
 * arabic glyphs with their shaped forms. This is needed when the arabic text is
 * rendered using an AWT font.
 *
...
public final class ArabicTextHandler {
	 ...
	 * attribute, i&#x2e;e&#x2e; initial|medial|terminal|isolated, to each arabic
	 * char.
	 *
	...
	public static AttributedString assignArabicForms(AttributedString as) {
		...
		if (numChars >= 3) {
			...
			char c = aci.next();
			int i = 1;
			for (char nextChar = aci
					.next(); nextChar != CharacterIterator.DONE; prevChar = c, c = nextChar, nextChar = aci
							.next(), i++) {
		...
		if (charOrder != null) {
			...
			if (charOrder[0] != aci.getBeginIndex()) {
				...
				if (x != null && !x.isNaN()) {
					reorderedAS.addAttribute(GVTAttributedCharacterIterator.TextAttribute.X, Float.NaN, charOrder[0],
							charOrder[0] + 1);
					reorderedAS.addAttribute(GVTAttributedCharacterIterator.TextAttribute.X, x, 0, 1);
				}
				if (y != null && !y.isNaN()) {
					reorderedAS.addAttribute(GVTAttributedCharacterIterator.TextAttribute.Y, Float.NaN, charOrder[0],
							charOrder[0] + 1);
		...
		for (int c = aci.first(); c != CharacterIterator.DONE; c = aci.next(), idx++) {
	...
	/**
	 * Returns true if the char is a standard arabic char. (ie. within the range
	 * U+0600 - U+6FF)
	 *
	...
	public static boolean arabicCharTransparent(char c) {
		...
		if ((charVal <= 0x0655) || (charVal == 0x0670) || (charVal >= 0x06D6 && charVal <= 0x06E4)
				|| (charVal >= 0x06E7 && charVal <= 0x06E8) || (charVal >= 0x06EA)) {
			return true;
		}
		return false;
	}
	...
	/**
	 * Returns true if the character shapes to the right. Note that duel shaping
	 * characters also shape to the right and so will return true.
	 *
	 * @param c The character to test.
	...
	private static boolean arabicCharShapesRight(char c) {
		int charVal = c;
		if ((charVal >= 0x0622 && charVal <= 0x0625) || (charVal == 0x0627) || (charVal == 0x0629)
				|| (charVal >= 0x062F && charVal <= 0x0632) || (charVal == 0x0648)
				|| (charVal >= 0x0671 && charVal <= 0x0673) || (charVal >= 0x0675 && charVal <= 0x0677)
				|| (charVal >= 0x0688 && charVal <= 0x0699) || (charVal == 0x06C0)
				|| (charVal >= 0x06C2 && charVal <= 0x06CB) || (charVal == 0x06CD) || (charVal == 0x06CF)
	...
	private static boolean arabicCharShapesDuel(char c) {
		...
		if ((charVal == 0x0626) || (charVal == 0x0628) || (charVal >= 0x062A && charVal <= 0x062E)
				|| (charVal >= 0x0633 && charVal <= 0x063A) || (charVal >= 0x0641 && charVal <= 0x0647)
				|| (charVal >= 0x0649 && charVal <= 0x064A) || (charVal >= 0x0678 && charVal <= 0x0687)
				|| (charVal >= 0x069A && charVal <= 0x06BF) || (charVal == 0x6C1) || (charVal == 0x6CC)
				|| (charVal == 0x6CE) || (charVal >= 0x06D0 && charVal <= 0x06D1)
	...
	/**
	 * Returns true if character shapes to the left. Note that duel shaping
	 * characters also shape to the left and so will return true.
	 *
	 * @param c The character to test.
	 ...
	 * @param form Indicates the required arabic form. (isolated = 1, final = 2,
	 *             initial = 3, medial = 4)
	 *
	...
	/**
	 * Where possible substitues plain arabic glyphs with their shaped forms. This
	 * is needed when the arabic text is rendered using an AWT font. Simple arabic
	 * ligatures will also be recognised and replaced by a single character so the
	 * length of the resulting string may be shorter than the number of characters
	 * in the aci.
	 *
	 * @param aci Contains the text to process. Arabic form attributes should
	 *            already be assigned to each arabic character.
			...
			if (charStartsLigature(c) && (i + 1 < end)) {
				...
				if (form != null && nextForm != null) {
					if (form.equals(ARABIC_TERMINAL) && nextForm.equals(ARABIC_INITIAL)) {
						...
						int substChar = ArabicTextHandler.getSubstituteChar(c, nextChar, ARABIC_ISOLATED);
						if (substChar > -1) {
						...
						int substChar = ArabicTextHandler.getSubstituteChar(c, nextChar, ARABIC_TERMINAL);
						if (substChar > -1) {
							substString.append((char) substChar);
							i++;
							continue;
						...
						int substChar = ArabicTextHandler.getSubstituteChar(c, nextChar, ARABIC_MEDIAL);
						if (substChar > -1) {
							substString.append((char) substChar);
							i++;
							continue;
	...
	public static boolean charStartsLigature(char c) {
		int charVal = c;
		if (charVal == 0x064B || charVal == 0x064C || charVal == 0x064D || charVal == 0x064E || charVal == 0x064F
				|| charVal == 0x0650 || charVal == 0x0651 || charVal == 0x0652 || charVal == 0x0622 || charVal == 0x0623
				|| charVal == 0x0625 || charVal == 0x0627) {
			return true;
		}
		return false;
	}
	...
	public static boolean isLigature(char c) {
		...
		if ((charVal <= 0xFE72) || (charVal == 0xFE74) || (charVal >= 0xFE76 && charVal <= 0xFE7F)
				|| (charVal >= 0xFEF5)) {
			return true;
		}
		return false;
