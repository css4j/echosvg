...
public interface Renderer {
	 ...
	 *
	 * @param area the region to be repainted, in the current user space coordinate
	 *             system.
	 */
	void repaint(Shape area);

	/**
	 * Repaints the associated GVT tree at least in areas under the list of
	 * <code>areas</code>.
	 *
	...
	/**
	 * Returns a copy of the transform from the current user space (as defined by
	 * the top node of the GVT tree) to the device space (1 unit = 1/72nd of an inch
	 * / 1 pixel, roughly speaking
	 */
	...
	/**
	 * Cause the renderer to ask to be removed from external reference lists,
	 * de-register as a listener to events, etc. This is so that in the absence of
	 * other existing references, it can be removed by the garbage collector.
	 */
	void dispose();
