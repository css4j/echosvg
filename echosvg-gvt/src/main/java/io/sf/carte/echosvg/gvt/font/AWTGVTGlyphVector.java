...
public class AWTGVTGlyphVector implements GVTGlyphVector {
	 ...
	 * @param scaleFactor The scale factor to apply to the glyph vector. IMPORTANT:
	 *                    This is only required because the GlyphVector class
	 *                    doesn't handle font sizes less than 1 correctly. By using
	 *                    the scale factor we can use a GlyphVector created by a
	 *                    larger font and then scale it down to the correct size.
	...
	public int[] getGlyphCodes(int beginGlyphIndex, int numEntries, int[] codeReturn) {
		return awtGlyphVector.getGlyphCodes(beginGlyphIndex, numEntries, codeReturn);
	}
	...
	/**
	 * Calculates the logical bounds for each glyph. The logical bounds are what is
	 * used for highlighting the glyphs when selected.
	 */
	private void computeGlyphLogicalBounds() {
		...
		for (int i = 0; i < getNumGlyphs(); i++) {
			...
			float glyphWidth = (float) (glyphMetrics.getHorizontalAdvance() / scaleFactor);
			float glyphHeight = (float) (glyphMetrics.getVerticalAdvance() / scaleFactor);
			Rectangle2D glyphBounds = new Rectangle2D.Double(glyphX, glyphY, glyphWidth, glyphHeight);
		...
		if (logicalBounds.getHeight() < maxHeight * 1.5) {
			...
			for (int i = 0; i < getNumGlyphs(); i++) {
				...
				tempLogicalBounds[i] = new Rectangle2D.Double(x, logicalBounds.getMinY(), width,
			...
			for (int i = 0; i < getNumGlyphs(); i++) {
				...
				tempLogicalBounds[i] = new Rectangle2D.Double(logicalBounds.getMinX(), y, logicalBounds.getWidth(),
						height);
			}
		}
	...
	public GVTGlyphMetrics getGlyphMetrics(int glyphIndex) {
		...
		AWTGlyphGeometryCache.Value v = AWTGVTFont.getGlyphGeometry(gvtFont, c, awtGlyphVector, glyphIndex, glyphPos);
		Rectangle2D gmB = v.getBounds2D();
		// -- end glyph cache code --

		Rectangle2D bounds = new Rectangle2D.Double(gmB.getX() * scaleFactor, gmB.getY() * scaleFactor,
		...
		// glyphVector.
		float adv = (float) (defaultGlyphPositions[glyphIndex + 1].getX() - defaultGlyphPositions[glyphIndex].getX());
		glyphMetrics[glyphIndex] = new GVTGlyphMetrics((float) (adv * scaleFactor), (ascent + descent), bounds,
	...
	public Shape getGlyphOutline(int glyphIndex) {
		if (glyphOutlines[glyphIndex] == null) {
			...
			char c = ci.setIndex(ci.getBeginIndex() + glyphIndex);
			ci.setIndex(ci.getBeginIndex());
			AWTGlyphGeometryCache.Value v = AWTGVTFont.getGlyphGeometry(gvtFont, c, awtGlyphVector, glyphIndex,
					glyphPos);
			...
			// positioning in EchoSVG is definitely due to the change in
			...
			//
			/*
			 * if (outlinesPositioned()) { Point2D glyphPos =
			 * defaultGlyphPositions[glyphIndex]; tr.translate(-glyphPos.getX(),
			 * -glyphPos.getY()); }
			 */
			tr.scale(scaleFactor, scaleFactor);
	...
	public Shape getGlyphVisualBounds(int glyphIndex) {
		if (glyphVisualBounds[glyphIndex] == null) {
			/*
			 * Shape glyphOutline = awtGlyphVector.getGlyphOutline(glyphIndex); Rectangle2D
			 * glyphBounds = glyphOutline.getBounds2D();
			 */
			// -- start glyph cache code --
			Point2D glyphPos = defaultGlyphPositions[glyphIndex];
			char c = ci.setIndex(ci.getBeginIndex() + glyphIndex);
	...
	public void performDefaultLayout() {
		...
		for (; i < getNumGlyphs(); i++) {
			...
			/*
			 * if (ArabicTextHandler.arabicCharTransparent(c)) { int j; shiftLeft +=
			 * getGlyphMetrics(i).getHorizontalAdvance(); for (j=i+1; j<getNumGlyphs(); j++)
			 * { char c2 = ci.setIndex(j+ci.getBeginIndex()); if
			 * (!ArabicTextHandler.arabicCharTransparent(c2)) break; shiftLeft +=
			 * getGlyphMetrics(j).getHorizontalAdvance(); } if (j != getNumGlyphs()) {
			 * Point2D glyphPosBase = defaultGlyphPositions[j]; double rEdge =
			 * glyphPosBase.getX()+getGlyphMetrics(j).getHorizontalAdvance(); rEdge -=
			 * shiftLeft; for (int k=i; k<j; k++) { glyphTransforms [k] = null;
			 * glyphVisualBounds [k] = null; glyphLogicalBounds[k] = null; glyphOutlines [k]
			 * = null; glyphMetrics [k] = null; x =
			 * (float)rEdge-getGlyphMetrics(k).getHorizontalAdvance(); y = (float)
			 * (defaultGlyphPositions[k].getY() * scaleFactor); if (glyphPositions[k] ==
			 * null) { glyphPositions[k] = new Point2D.Float(x,y); } else {
			 * glyphPositions[k].x = x; glyphPositions[k].y = y; } } i = j-1; } } else {
			 */
		...
		Point2D glyphPos = defaultGlyphPositions[i];
		glyphPositions[i] = new Point2D.Float((float) ((glyphPos.getX() * scaleFactor) - shiftLeft),
			...
			for (int i = 0; i < numGlyphs; i++) {
				...
				if (at != null) {
					...
					if ((type & ~AffineTransform.TYPE_TRANSLATION) == 0) {
						// Just translation
					} else if (glyphVectorTransformWorks && ((type & typeGTrans) == 0) && ((type & typeGRot) == 0)) {
