...
import io.sf.carte.echosvg.ext.awt.geom.ExtendedShape;
import io.sf.carte.echosvg.ext.awt.geom.ShapeExtender;
...
public class MarkerShapePainter implements ShapePainter {
	...
	/**
	 * Returns the area covered by this shape painter (even if not painted). This is
	 * always null for Markers.
	 */
	@Override
	 ...
	 * painted). This is always null for Markers.
	 */
	@Override
	...
	/**
	 * Returns true if pt is in the sensitive area. This is always false for
	 * Markers.
	 */
	@Override
	...
	/**
	 * Returns the marker that shall be drawn at every other vertex (not the first
	...
	/**
	 * Sets the marker that shall be drawn at every other vertex (not the first or
	 * the last one) of the given shape.
	 *
	...
	/**
	 * Builds a proxy <code>GraphicsNode</code> for the input <code>Marker</code> to
	 * be drawn at the start position
	 */
	protected ProxyGraphicsNode buildStartMarkerProxy() {
		...
		AffineTransform markerTxf = computeMarkerTransform(startMarker, markerPosition, rotation);
	...
	protected ProxyGraphicsNode buildEndMarkerProxy() {
		...
		if (Double.isNaN(rotation)) {
			rotation = computeRotation(lastButOne, lastButOneSegType, last, lastSegType, null, 0);
		}

		// Now, compute the marker's proxy transform
		AffineTransform markerTxf = computeMarkerTransform(endMarker, markerPosition, rotation);
	...
	/**
	 * Builds a proxy <code>GraphicsNode</code> for the input <code>Marker</code> to
	 * be drawn at the middle positions
	 */
	protected ProxyGraphicsNode[] buildMiddleMarkerProxies() {
		...
		while (!iter.isDone()) {
			...
			proxies.add(createMiddleMarker(prev, prevSegType, curr, currSegType, next, nextSegType));
	 ...
	 */
	private ProxyGraphicsNode createMiddleMarker(double[] prev, int prevSegType, double[] curr, int currSegType,
		...
		if (Double.isNaN(rotation)) {
			rotation = computeRotation(prev, prevSegType, curr, currSegType, next, nextSegType);
		}

		// Now, compute the marker's proxy transform
		AffineTransform markerTxf = computeMarkerTransform(middleMarker, markerPosition, rotation);
	 ...
	 */
	private double computeRotation(double[] prev, int prevSegType, double[] curr, int currSegType, double[] next,
			int nextSegType) {

		// Compute in slope, i.e., the slope of the segment
		// going into the current point
		double[] inSlope = computeInSlope(prev, prevSegType, curr, currSegType);

		// Compute out slope, i.e., the slope of the segment
		// going out of the current point
		double[] outSlope = computeOutSlope(curr, currSegType, next, nextSegType);
	...
	private double[] computeInSlope(double[] prev, int prevSegType, double[] curr, int currSegType) {
		...
		case ExtendedPathIterator.SEG_ARCTO: {
			...
			Arc2D arc = ExtendedGeneralPath.computeArc(prevEndPoint.getX(), prevEndPoint.getY(), curr[0], curr[1],
					curr[2], large, goLeft, curr[5], curr[6]);
	...
	private double[] computeOutSlope(double[] curr, int currSegType, double[] next, int nextSegType) {
		...
		case ExtendedPathIterator.SEG_ARCTO: {
			...
			Arc2D arc = ExtendedGeneralPath.computeArc(currEndPoint.getX(), currEndPoint.getY(), next[0], next[1],
					next[2], large, goLeft, next[5], next[6]);
	...
	/**
	 * Computes the transform for the input marker, so that it is positioned at the
	 * given position with the specified rotation
	 */
	private AffineTransform computeMarkerTransform(Marker marker, Point2D markerPosition, double rotation) {
		...
		/*
		 * AffineTransform txf =
		 * AffineTransform.getTranslateInstance(markerPosition.getX() - ref.getX(),
