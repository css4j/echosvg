 ...
 * the options of the SVGGraphics2D constructor. This is the same test as Font
 * testing with regards to the Java 2D API code, except that it validates text
 * to shapes convertion.
 *
...
public class Font2 implements Painter {
	...
	public void paint(Graphics2D g) {
		...
		for (int fontSize : fontSizes) {
			Font font = new Font(defaultFont.getFamily(), Font.PLAIN, fontSize);
			g.setFont(font);
		...
		for (int i = 0; i < fontStyles.length; i++) {
			Font font = new Font(defaultFont.getFamily(), fontStyles[i], 20);
			g.setFont(font);
		...
		// Logical fonts
		//
		Font[] logicalFonts = { new Font("dialog", Font.PLAIN, 14), new Font("dialoginput", Font.BOLD, 14),
				new Font("monospaced", Font.ITALIC, 14), new Font("serif", Font.PLAIN, 14),
