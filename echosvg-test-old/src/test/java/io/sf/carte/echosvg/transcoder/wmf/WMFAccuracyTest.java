 ...
 * @version $Id$
 */
public class WMFAccuracyTest extends AbstractTest implements SVGConstants {

	public static final String GENERATOR_REFERENCE_BASE = "test-references/io/sf/carte/echosvg/transcoder/wmf";
	...
	/**
	 * Error when an error occurs while generating SVG with the
	 * <code>SVGGraphics2D</code> {0} = painter class name if painter not null. Null
	 * otherwise {1} = exception class name {2} = exception message {3} = exception
	 * stack trace.
	 */
	public static final String ERROR_CANNOT_GENERATE_SVG = "WMFAccuracyTest.error.cannot.generate.svg";

	/**
	 * Error when the reference SVG file cannot be opened {0} = URI of the reference
	...
	/**
	 * Error when there is an IOException while comparing the reference SVG file
	 * with the newly generated SVG content {0} = URI of the reference image {1} =
	 * IOException message.
	 */
	...
	public void setFile(String id) {
		...
		refURL = resolveURL(GENERATOR_REFERENCE_BASE + "/" + dirNfile[1] + SVG_EXTENSION);
		saveSVG = new File(GENERATOR_REFERENCE_BASE + "/" + CANDIDATE_REF_DIR + "/" + dirNfile[1] + SVG_EXTENSION);
	...
	public TestReport runImpl() throws Exception {
		...
		try {
			...
			msg = Messages.formatMessage(ERROR_CANNOT_GENERATE_SVG,
					new String[] { wmfURL.toString(), e.getClass().getName(), e.getMessage(), trace.toString() });
		...
		try {
			...
			msg = Messages.formatMessage(ERROR_CANNOT_OPEN_REFERENCE_SVG_FILE,
					new Object[] { refURLStr, e.getMessage() });
			report.setDescription(new TestReport.Entry[] { new TestReport.Entry(desc, msg) });
			report.setPassed(false);
		...
		try {
			...
			report.addDescriptionEntry(fmtMsg(ENTRY_KEY_LINE_NUMBER), line);
			report.addDescriptionEntry(fmtMsg(ENTRY_KEY_COLUMN_NUMBER), cn);
			report.addDescriptionEntry(fmtMsg(ENTRY_KEY_COLUMN_EXPECTED_VALUE), expectedChar);
			report.addDescriptionEntry(fmtMsg(ENTRY_KEY_COLUMN_FOUND_VALUE), foundChar);
			report.addDescriptionEntry(fmtMsg(ENTRY_KEY_REFERENCE_LINE), refStr);
	...
	/**
	 * Saves the byte array in the "saveSVG" file if that file's parent directory
	 * exists.
	 */
