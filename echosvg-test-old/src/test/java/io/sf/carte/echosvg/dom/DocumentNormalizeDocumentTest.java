...
public class DocumentNormalizeDocumentTest extends DOM3Test {
	...
	public TestReport runImpl() throws Exception {
		...
		if (!(e.getFirstChild().getNodeType() == Node.TEXT_NODE && e.getFirstChild().getNodeValue().equals("abcdefghi")
		...
		e.appendChild(doc.createTextNode("ghi"));
		doc.normalizeDocument();
		if (!(e.getFirstChild().getNodeType() == Node.TEXT_NODE && e.getFirstChild().getNodeValue().equals("abcghi")
				&& e.getFirstChild() == e.getLastChild())) {
			if (report == null) {
				report = reportError("Document.normalizeDocument test failed");
			}
		...
		e.appendChild(doc.createTextNode("    "));
		doc.normalizeDocument();
		if (!(e.getFirstChild().getNodeType() == Node.ELEMENT_NODE && e.getFirstChild().getNodeName().equals("g")
				&& e.getFirstChild() == e.getLastChild())) {
			if (report == null) {
				report = reportError("Document.normalizeDocument test failed");
			}
		...
		if (!(a != null && a.getNodeName().equals("xmlns:ns")
		...
		if (!(a != null && a.getNodeName().equals("xmlns:ns") && a.getNodeValue().equals("http://www.example.org/ns2")
		...
		if (!(a != null && a.getNodeValue().equals("http://www.example.org/ns1") && a2 != null
