...
public class AttributedCharacterIterator implements Painter {
	...
	public void paint(Graphics2D g) {
		...
		/*
		 * TextLayout aLayout = new TextLayout("A", font, frc); Shape aShape =
		 * aLayout.getOutline(null);
		 * 
		 * ShapeGraphicAttribute aReplacement = new ShapeGraphicAttribute(aShape,
		 * GraphicAttribute.ROMAN_BASELINE, true);
		 * styledText.addAttribute(TextAttribute.CHAR_REPLACEMENT, aReplacement, 0, 1);
		 * 
		 * 
		 * // Create a BufferedImage to decorate the Shape { TextLayout aLayout = new
		 * TextLayout("A", font, frc); Shape aShape = aLayout.getOutline(null);
		 * Rectangle bounds = aShape.getBounds();
		 * 
		 * int blurWidth = 6; BufferedImage image = new BufferedImage(bounds.width +
		 * blurWidth*4, bounds.height + blurWidth*4, BufferedImage.TYPE_INT_ARGB);
		 * Graphics2D g2 = image.createGraphics(); int w = image.getWidth(), h =
		 * image.getHeight(); g2.setRenderingHint(RenderingHints.KEY_ANTIALIASING,
		 * RenderingHints.VALUE_ANTIALIAS_ON); g2.setPaint(Color.black);
		 * g2.translate(-bounds.x + (w - bounds.width)/2, -bounds.y + (h -
		 * bounds.height)/2); g2.fill(aShape); g2.setStroke(new
		 * BasicStroke(blurWidth/2)); g2.draw(aShape); g2.dispose();
		 * 
		 * float k[] = new float[blurWidth*blurWidth]; for(int i=0; i<k.length; i++)
		 * k[i] = 1/(float)k.length; Kernel kernel = new Kernel(blurWidth, blurWidth,
		 * k); ConvolveOp blur = new ConvolveOp(kernel); image = blur.filter(image,
		 * null); g2 = image.createGraphics();
		 * g2.setRenderingHint(RenderingHints.KEY_ANTIALIASING,
		 * RenderingHints.VALUE_ANTIALIAS_ON); g2.translate(-bounds.x + (w -
		 * bounds.width)/2, -bounds.y + (h - bounds.height)/2);
		 * g2.setComposite(AlphaComposite.Clear); g2.fill(aShape);
		 * 
		 * image = image.getSubimage(blurWidth, blurWidth, image.getWidth() -
		 * 2*blurWidth, image.getHeight() - 2*blurWidth);
		 * 
		 * ImageGraphicAttribute aImageReplacement = new ImageGraphicAttribute(image,
		 * GraphicAttribute.ROMAN_BASELINE, blurWidth, blurWidth + bounds.height);
		 * styledText.addAttribute(TextAttribute.CHAR_REPLACEMENT, aImageReplacement, 0,
		 * 1); }
		 */
		 ...
		 * 
		 * Rectangle bounds = layout.getBounds().getBounds(); bounds.width += 50;
		 * bounds.height += 50;
		 * 
