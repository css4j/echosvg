 ...
 * information associated with keyboard devices. Each keyboard event references
 * a key using an identifier. Keyboard events are commonly directed at the
 * element that has the focus.
 * <p>
 * The <code>KeyboardEvent</code> interface provides convenient attributes for
 * some common modifiers keys: <code>KeyboardEvent.ctrlKey</code>,
 * <code>KeyboardEvent.shiftKey</code>, <code>KeyboardEvent.altKey</code>,
 * <code>KeyboardEvent.metaKey</code>. These attributes are equivalent to use
 * the method <code>KeyboardEvent.getModifierState(keyIdentifierArg)</code> with
 * "Control", "Shift", "Alt", or "Meta" respectively.
 * <p>
 * To create an instance of the <code>KeyboardEvent</code> interface, use the
 * <code>DocumentEvent.createEvent("KeyboardEvent")</code> method call.
 * <p>
 * See also the
...
public interface KeyboardEvent extends UIEvent {
	 ...
	 * key, and did not originate from the numeric keypad (or did not originate with
	 * a virtual key corresponding to the numeric keypad). Example: the 'Q' key on a
	 ...
	 * possible location for this key). Example: the left Shift key on a PC 101 Key
	 * US keyboard.
	 */
	int DOM_KEY_LOCATION_LEFT = 0x01;
	/**
	 * The key activation is in the right key location (there is more than one
	 * possible location for this key). Example: the right Shift key on a PC 101 Key
	 * US keyboard.
	 */
	 ...
	 * identifiers are defined in Appendix A.2 "". Implementations that are unable
	 * to identify a key must use the key identifier <code>"Unidentified"</code>.
	 */
	String getKeyIdentifier();

	/**
	 * The <code>keyLocation</code> attribute contains an indication of the location
	 * of they key on the device, as described in .
	 */
	int getKeyLocation();
	 ...
	 * <p >
	 * <b>Note:</b> The Option key modifier on Macintosh systems must be represented
	 ...
	 * <p >
	 * <b>Note:</b> The Command key modifier on Macintosh systems must be
	 ...
	 * .
	 * 
	 * @param keyIdentifierArg A modifier key identifier. Common modifier keys are
	 ...
	 *                         <p >
	 *                         <b>Note:</b> If an application wishes to distinguish
	 *                         between right and left modifiers, this information
	 *                         could be deduced using keyboard events and
	...
	/**
	 * The <code>initKeyboardEvent</code> method is used to initialize the value of
	 * a <code>KeyboardEvent</code> object and has the same behavior as
	 * <code>UIEvent.initUIEvent()</code>. The value of <code>UIEvent.detail</code>
	 * remains undefined.
	 * 
	 * @param typeArg          Refer to the <code>UIEvent.initUIEvent()</code>
	 ...
	 *                         method for a description of this parameter.
	 * @param viewArg          Refer to the <code>UIEvent.initUIEvent()</code>
	 *                         method for a description of this parameter.
	 * @param keyIdentifierArg Specifies <code>KeyboardEvent.keyIdentifier</code>.
	 * @param keyLocationArg   Specifies <code>KeyboardEvent.keyLocation</code> .
	 * @param modifiersList    A <a href=
	 *                         'http://www.w3.org/TR/2004/REC-xml-20040204/#NT-S'>white
	 *                         space</a> separated list of modifier key identifiers
	...
	void initKeyboardEvent(String typeArg, boolean canBubbleArg, boolean cancelableArg, AbstractView viewArg,
			String keyIdentifierArg, int keyLocationArg, String modifiersList);

	/**
	 * The <code>initKeyboardEventNS</code> method is used to initialize the value
	 * of a <code>KeyboardEvent</code> object and has the same behavior as
	 ...
	 *                         <code>KeyboardEvent.initKeyboardEvent()</code> method
	 ...
	 *                         <code>"Control Alt"</code> will activated the control
	 *                         and alt modifiers.
	 */
	void initKeyboardEventNS(String namespaceURI, String typeArg, boolean canBubbleArg, boolean cancelableArg,
			AbstractView viewArg, String keyIdentifierArg, int keyLocationArg, String modifiersList);
