...
public class SimpleTestReportProcessor implements TestReportProcessor {
	...
	/**
	 * Recursively prints out the entries of the input report and its children
	 * reports, if any.
	 */
	@Override
	public void processReport(TestReport report) throws TestException {
		try {
			...
			throw new TestException(INTERNAL_ERROR,
					new Object[] { e.getClass().getName(), e.getMessage(), sw.toString() }, e);
		}
	}
	...
	public void processReport(TestReport report, String prefix, PrintWriter out) {
		String status = report.hasPassed() ? Messages.formatMessage(MESSAGES_TEST_SUITE_STATUS_TEST_PASSED, null)
		...
		out.println(Messages.formatMessage(MESSAGES_TEST_SUITE_STATUS,
				new Object[] { report.getTest().getName(), status }));

		if (!report.hasPassed()) {
			out.println(Messages.formatMessage(MESSAGES_TEST_SUITE_ERROR_CODE, new Object[] { report.getErrorCode() }));
		}
