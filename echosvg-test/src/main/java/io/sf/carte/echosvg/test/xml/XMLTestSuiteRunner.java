...
/**
 * This class can be used to build and run a <code>TestSuite</code> from an XML
 * description following the "XML Test Run" and "XML Test Suite" formats, whose
 * constants are defined in the <code>XTRunConstants</code> and
 * <code>XTSConstants</code> interfaces.
 *
 * This class takes a "Test Run" XML description as an input. That description
 * contains: <br>
 * + pointers to a number of "Test Suite" XML descriptions, which contain the
 * definition of the set of <code>Tests</code> to be run and their
 * configuration.<br>
 * + a description of the set of <code>TestReportProcessor</code> and their
 * configuration that should be used to process the reports generated by the
 * various <code>TestSuites</code>.<br>
 *
...
public class XMLTestSuiteRunner implements XTRunConstants, XTSConstants {
	/**
	 * Displayed when no test or testSuite matching the input id was found. {0} :
	 * unmatched id set
	 */
	 ...
	 * description. {0} : the &lt;testReportProcessor&gt; "className" attribute
	 * value {1} : exception's class name {2} : exception's message {3} :
	...
	/**
	 * An error happened while running the <code>TestSuite</code> {0} :
	 * <code>TestSuite</code> name {1} : <code>TestSuite</code> class name. {1} :
	 * exception's class name. {2} : exception's message {3} : exception's stack
	 * trace.
	 */
	public static final String TEST_SUITE_EXCEPTION = "xml.XMLTestSuiteRunner.test.suite.exception";

	/**
	 * An error happened while processing the <code>TestReport</code> generated by
	 * the <code>TestSuite</code> {0} : <code>TestReportProcessor</code> class name.
	 * {1} : exception's class name. {2} : exception's message {3} : exception's
	 * stack trace.
	 */
	...
	/**
	 * Test filter which only accepts tests with ids matching the ones passed to its
	 * constructor.
	 */
	public static class IdBasedTestFilter implements TestFilter {
		...
		/**
		 * Accept a test if one of the ids is found (i.e., an exact match or a
		 * substring) in the <code>Test</code>'s qualified id. <code>TestSuite</code>s
		 * are accepted if they have children and rejected if they have none.
		 */
		@Override
		public Test filter(Test t) {
		...
		protected boolean isRequestedId(String id) {
			for (String id1 : ids) {
				...
				if (id1.lastIndexOf(id) == 0) {
					// System.out.println("accepting " + id + ". It is (or is the a parent of) " +
					// ids[i]);
				...
				if (id.lastIndexOf(id1) != -1) {
					// System.out.println("accepting " + id + " it is (or is a child of) the
					// requested " + ids[i]);
					unmatchedIds.remove(id1);
					return true;
				}
	...
	/**
	 * Builds an array of <code>TestReportProcessor</code> from the input element,
	...
	protected TestReportProcessor buildProcessor(Element element) throws TestException {
		try {
			...
			throw new TestException(CANNOT_CREATE_TEST_REPORT_PROCESSOR, new Object[] {
					element.getAttribute(XR_CLASS_ATTRIBUTE), e.getClass().getName(), e.getMessage(), sw.toString() },
					e);
		}
	}
	 ...
	 * that element is a &lt;testRun&gt; instance. The element is scanned for
	 * children &lt;testSuite&gt; elements which is loaded into a <code>Test</code>
	 * and composited into a <code>TestSuite</code>
	 */
	protected DefaultTestSuite buildTestRunTestSuite(Element element) throws TestException {
		...
		Element[] testSuites = getChildrenByTagName(element, XTRun_TEST_SUITE_TAG);

		int n = testSuites != null ? testSuites.length : 0;
		for (int i = 0; i < n; i++) {
			String suiteHref = testSuites[i].getAttribute(XTRun_HREF_ATTRIBUTE);
	...
	/**
	 * Runs the test suite described by the input Document object. If the input ids
	 * array is null or of zero length, then all the tests will be run. Otherwise,
	 * only the tests identified by the array will be run.
	 */
	...
	protected TestReport runTest(Test test) throws TestException {
		try {
			...
			PrintWriter pw = new PrintWriter(sw);
			e.printStackTrace(pw);
			throw new TestException(TEST_SUITE_EXCEPTION, new Object[] { test.getName(), test.getClass().getName(),
	...
	protected void processReport(TestReport report, TestReportProcessor[] processors) throws TestException {
		...
		try {
			...
			PrintWriter pw = new PrintWriter(sw);
			e.printStackTrace(pw);
			throw new TestException(TEST_REPORT_PROCESSING_EXCEPTION, new Object[] { processors[i].getClass().getName(),
					e.getClass().getName(), e.getMessage(), sw.toString() }, e);
		}
	}
	...
	protected TestReport run(Element testRunElement, String[] ids) throws TestException {
		...
		//
		Test testRun = buildTestRunTestSuite(testRunElement);
		...
		//
		TestReportProcessor[] processors = extractTestReportProcessor(testRunElement);
	...
	/**
	 * Displayed when the input argument does not represent an existing file to
	 * notify the user that the argument is going to be interpreted as a URI.
	 */
	...
	/**
	 * Displayed when the input document cannot be parsed. {0} : uri of the invalid
	 * document. {1} : exception generated while parsing {2} : exception message
	 */
	...
	public static void main(String[] args) {
		...
		try {
			...
			System.err.println(Messages.formatMessage(INVALID_DOCUMENT,
					new Object[] { uriStr, e.getClass().getName(), e.getMessage() }));
			System.exit(0);
		}
