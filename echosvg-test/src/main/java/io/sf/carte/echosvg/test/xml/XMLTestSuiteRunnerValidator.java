 ...
 * that it runs the tests specified by the command line parameter and only these
 * tests. <br>
 * The test uses an dummy &lt;testRun&gt; which contains: <br>
 * - testRun, id="all" - testSuite, href="testSuiteA.xml" <br>
...
public class XMLTestSuiteRunnerValidator extends DefaultTestSuite {
	 ...
	 * a given input contains a list of report ids and no more (i.e., that the
	 * expected test reports were generated and no more). <br>
	 ...
	 * <br>
	 * - 1 target test-suite: "all.A". A test-report with "A1", "A2",
	 * "duplicatedId", "AG", "AG.AG1" and "AG.AG2" should be produced.<br>
	 * - 1 target test-suite and 2 tests: "all.B and all.A.A1 and all.A.A2. A
	 * test-report for "all.B.B1", "all.B.B2", "all.B.B3", "all.B.duplicatedId",
	 * "all.A.A1" and "all.A.A2" should be produced. <br>
	 * - 1 target testGroup: "AG". A test-report with "A.AG", "A.AG.AG1" and
	 * "A.AG.AG2" should be produced.<br>
	 * <br>
	 * In addition, the following test with non-qualified ids: <br>
	 * - 1 target test id: "duplicatedId" should be produced and pass for
	 * "all.A.duplicatedId" and "all.B.duplicatedId".
	 */
	public XMLTestSuiteRunnerValidator() {
		Object[][] config = {
				{ "", new String[] { "all", "all.A", "all.A.A1", "all.A.A2", "all.A.duplicatedId",
						"all.A.duplicatedId.1", "all.A.duplicatedId.2", "all.A.AG", "all.A.AG.AG1", "all.A.AG.AG2",
						"all.B", "all.B.B1", "all.B.B2", "all.B.B3", "all.B.duplicatedId" } },

				{ "all.B.B3", new String[] { "all", "all.B", "all.B.B3" } },

				{ "all.A", new String[] { "all", "all.A", "all.A.A1", "all.A.A2", "all.A.duplicatedId",
						"all.A.duplicatedId.1", "all.A.duplicatedId.2", "all.A.AG", "all.A.AG.AG1", "all.A.AG.AG2" } },

				{ "all.B all.A.A1 all.A.A2",
						new String[] { "all", "all.B", "all.B.B1", "all.B.B2", "all.B.B3", "all.B.duplicatedId",
								"all.A", "all.A.A1", "all.A.A2" } },

				{ "duplicatedId",
						new String[] { "all", "all.A", "all.A.duplicatedId", "all.A.duplicatedId.1",
								"all.A.duplicatedId.2", "all.B", "all.B.duplicatedId" } },

				{ "AG", new String[] { "all", "all.A", "all.A.AG", "all.A.AG.AG1", "all.A.AG.AG2" } } };
	...
	static class XMLTestSuiteRunnerTest extends AbstractTest {
		...
		/**
		 * @param config an array of two objects: a String containing the "config" to
		 *               pass to the XMLTestSuiteRunner and an array of Strings
		 *               containing the expected ids from the XMLTestSuiteRunner
		 *               operation.
		 */
		...
		public TestReport runImpl() throws Exception {
			...
			TestReport runReport = runner.run(doc, args);
			...
			if (idNotExpected == null) {
				if (idSet.isEmpty()) {
					...
					report.addDescriptionEntry(ENTRY_KEY_CONFIGURATION, arrayToString(args));
					report.addDescriptionEntry(ENTRY_KEY_EXPECTED_RESULT, arrayToString(idsArray));
					report.addDescriptionEntry(ENTRY_KEY_ACTUAL_RESULT, reportIdsToString(runReport));
					report.addDescriptionEntry(ENTRY_KEY_TEST_IDS_NOT_RUN, arrayToString(idSet.toArray()));
				...
				report.addDescriptionEntry(ENTRY_KEY_EXPECTED_RESULT, arrayToString(idsArray));
				report.addDescriptionEntry(ENTRY_KEY_ACTUAL_RESULT, reportIdsToString(runReport));
				report.addDescriptionEntry(ENTRY_KEY_TEST_ID_NOT_EXPECTED, idNotExpected);
				report.setPassed(false);
				return report;
			}
		}
		...
		/**
		 * Validates that the input <code>TestReport</code> contains only the expected
		 * identifiers. The following code is by no means optimized, but it gets the job
		 * done.
		 */
			...
			if (report instanceof TestSuiteReport) {
				...
				if (childReports != null) {
					for (TestReport childReport : childReports) {
						String idNotExpected = checkTestReport(childReport, idSet);
						if (idNotExpected != null) {
