 ...
 * at creation time. <br>
 * The <code>XMLTestReportProcessor</code> can optionally notify a report
 * consumer of the XML file it created.
 *
...
public class XMLTestReportProcessor implements TestReportProcessor, XTRConstants, XMLConstants {
	/**
	 * An <code>XMLReportConsumer</code> is notified every time a new report is
	 * generated by an <code>XMLTestReportProcessor</code>
	 */
	public interface XMLReportConsumer {
		 ...
		 * @param reportDirectory base directory where any resource relative to the
		 *                        report processing should be stored.
		 */
	 ...
	 */
	public static final String XML_TEST_REPORT_DEFAULT_DIRECTORY = Messages
			.formatMessage("XMLTestReportProcessor.config.xml.test.report.default.directory", null);

	/**
	 * Directory where the XML report is created
	 */
	public static final String XML_REPORT_DIRECTORY = Messages
			.formatMessage("XMLTestReportProcessor.xml.report.directory", null);

	/**
	 * Directory where resources (e.g., images) referenced by the XML report are
	 * copied.
	 */
	public static final String XML_RESOURCES_DIRECTORY = Messages
			.formatMessage("XMLTestReportProcessor.xml.resources.directory", null);

	/**
	 * Test report name
	 */
	public static final String XML_TEST_REPORT_NAME = Messages
			.formatMessage("XMLTestReportProcessor.config.xml.test.report.name", null);

	/**
	 * The XMLReportConsumer instance is notified whenever this object generates a
	 * new report.
	 */
	...
	/**
	 * @param consumer consumer for the XML report generated by this object. May be
	 *                 null.
	 */
	...
	/**
	 * Recursively processes the input <code>TestReport</code> and any of its
	 * children.
	 */
	@Override
	public void processReport(TestReport report) throws TestException {
		...
		/**
		 * First, create the directories for the report and report resources
		 */
		...
		try {
			...
			/**
			 * Create a new document and build the root <testReport> element. Then, process
			 * the TestReports recursively.
			 */
			...
			if (report.getTest() instanceof TestSuite) {
				document = impl.createDocument(XTR_NAMESPACE_URI, XTR_TEST_SUITE_REPORT_TAG, null);
			} else {
				document = impl.createDocument(XTR_NAMESPACE_URI, XTR_TEST_REPORT_TAG, null);
			}

			Element root = document.getDocumentElement();

			root.setAttribute(XTR_DATE_ATTRIBUTE, reportDate);
	...
	/**
	 * By default, the report directory is given by a configuration variable. Each
	 * test run will create a sub directory with the current date and time as the
	 * same. All the resources created by the report processor are then put into
	 * that "dated" directory.
	 */
	public void initializeReportDirectories() throws TestException {
		...
		String dirName = "" + c.get(Calendar.YEAR) + "." + makeTwoDigits(c.get(Calendar.MONTH) + 1) + "."
				+ makeTwoDigits(c.get(Calendar.DAY_OF_MONTH)) + "-" + makeTwoDigits(c.get(Calendar.HOUR_OF_DAY)) + "h"
				+ makeTwoDigits(c.get(Calendar.MINUTE)) + "m" + makeTwoDigits(c.get(Calendar.SECOND)) + "s";
	...
	/**
	 * By default, the report resources directory is given by a configuration
	 * variable.
	 */
	...
	/**
	 * Recursively processes the input <code>TestReport</code> adding the report
	 * information to the input element.
	 */
	protected void processReport(TestReport report, Element reportElement, Document reportDocument) throws IOException {
		...
		reportElement.setAttribute(XTR_TEST_NAME_ATTRIBUTE, report.getTest().getName());

		String id = report.getTest().getQualifiedId();
		if (!"".equals(id)) {
			reportElement.setAttribute(XTR_ID_ATTRIBUTE, id);
		}

		String status = report.hasPassed() ? XTR_PASSED_VALUE : XTR_FAILED_VALUE;

		reportElement.setAttribute(XTR_STATUS_ATTRIBUTE, status);

		String className = report.getTest().getClass().getName();

		reportElement.setAttribute(XTR_CLASS_ATTRIBUTE, className);

		if (!report.hasPassed()) {
			reportElement.setAttribute(XTR_ERROR_CODE_ATTRIBUTE, report.getErrorCode());
		}
		...
		if (n > 0) {
			Element descriptionElement = reportDocument.createElementNS(null, XTR_DESCRIPTION_TAG);

			reportElement.appendChild(descriptionElement);

			for (int i = 0; i < n; i++) {
				processEntry(entries[i], descriptionElement, reportDocument);

			}
		}
	}

	protected void processEntry(TestReport.Entry entry, Element descriptionElement, Document reportDocument)
			throws IOException {
		...
		if (value instanceof TestReport) {
			...
			if (report.getTest() instanceof TestSuite) {
				reportElement = reportDocument.createElementNS(XTR_NAMESPACE_URI, XTR_TEST_SUITE_REPORT_TAG);
			} else {
				reportElement = reportDocument.createElementNS(XTR_NAMESPACE_URI, XTR_TEST_REPORT_TAG);
			}

			descriptionElement.appendChild(reportElement);
			processReport((TestReport) entry.getValue(), reportElement, reportDocument);
		} else if (value instanceof URL) {
			Element entryElement = reportDocument.createElementNS(XTR_NAMESPACE_URI, XTR_URI_ENTRY_TAG);

			descriptionElement.appendChild(entryElement);

			entryElement.setAttribute(XTR_KEY_ATTRIBUTE, key.toString());

			entryElement.setAttribute(XTR_VALUE_ATTRIBUTE, value.toString());
			...
			Element entryElement = reportDocument.createElementNS(XTR_NAMESPACE_URI, XTR_FILE_ENTRY_TAG);

			descriptionElement.appendChild(entryElement);

			entryElement.setAttribute(XTR_KEY_ATTRIBUTE, key.toString());

			entryElement.setAttribute(XTR_VALUE_ATTRIBUTE, tmpFileCopy.toURI().toURL().toString());

		} else {

			Element entryElement = reportDocument.createElementNS(XTR_NAMESPACE_URI, XTR_GENERIC_ENTRY_TAG);
	...
	/**
	 * Untility method. Creates a file in the resources directory for the given
	 * name. If a file in that directory does not exist yet, then it is used.
	 * Otherwise, a file with the same name with a digit suffix is created. For
	 * example, if "myFile.png" is requested, then "myFile.png" is created or
	 * "myFile&lt;n&gt;.png" where &lt;n&gt; will be one or several digits.
	 */
	...
	protected File createResourceFileForName(String fileName, int instance) {
		...
		if (!r.exists()) {
			...
			return createResourceFileForName(fileName, instance + 1);
		}
	}
	...
	protected File serializeReport(Element reportElement) throws IOException {
		...
		//
		File reportFile = new File(xmlDirectory, XML_TEST_REPORT_NAME);

		FileWriter fw = new FileWriter(reportFile);

		serializeElement(reportElement, "", fw);
		...
		if (children != null && children.getLength() > 0) {
			...
			for (int i = 0; i < n; i++) {
				...
				if (child.getNodeType() == Node.ELEMENT_NODE) {
					serializeElement((Element) child, prefix + XML_TAB, writer);
				}
			}
			writer.write(prefix);
	...
	/**
	 * Poor way of replacing '&lt;', '&gt;', '"', '&amp;' and ''' in attribute
	 * values.
	 */
