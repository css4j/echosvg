...
/**
 * This class loads an XML document describing a test suite into a
...
public class XMLTestSuiteLoader implements XTSConstants {
	...
	/**
	 * An error happened while loading a test suite document. {0} : the
	 * &lt;testSuite&gt; href value. {1} : the exception's class name {2} :
	 * exception's message {3} : exception's stack trace
	 */
	public static final String TEST_SUITE_LOADING_EXCEPTION = "xml.XMLTestSuiteLoader.error.test.suite.loading.exception";

	/**
	 * An error happened while processing a <code>Test</code> description. {0} : the
	 * &lt;test&gt; "className" attribute value {1} : exception's class name {2} :
	 * exception's message {3} : exception's stack trace
	 */
	...
	protected static Document loadTestSuiteDocument(String testSuiteURI) throws TestException {
		...
		try {
			...
			throw new TestException(TEST_SUITE_LOADING_EXCEPTION,
					new Object[] { testSuiteURI, e.getClass().getName(), e.getMessage(), sw.toString() }, e);
	...
	/**
	 * Builds a <code>TestSuite</code> from an input element. This method assumes
	 * that element is a &lt;testSuite&gt; instance, as the input document should
	 * have been validated when loaded.
	 */
	protected static TestSuite buildTestSuite(Element element, TestSuite parent) throws TestException {
		...
		String suiteId = element.getAttribute(XTS_ID_ATTRIBUTE);
	...
	protected static Test buildTest(Element element) throws TestException {
		try {
			...
			String id = element.getAttribute(XTS_ID_ATTRIBUTE);
			t.setId(id);
			return t;
		} catch (Exception e) {
			StringWriter sw = new StringWriter();
			PrintWriter pw = new PrintWriter(sw);
			e.printStackTrace(pw);
			throw new TestException(CANNOT_CREATE_TEST, new Object[] { element.getAttribute(XR_CLASS_ATTRIBUTE),
					e.getClass().getName(), e.getMessage(), sw.toString() }, e);
		}
	}
