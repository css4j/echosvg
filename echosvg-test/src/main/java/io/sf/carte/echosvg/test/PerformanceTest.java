 ...
 * testing.
 *
 * Derived classes need only implement the <code>runOp</code> and, optionally,
 * the <code>runRef</code> methods.
 *
 * The <code>setReferenceScore</code> method is used to specify the last
 * recorded score for the performance test and the
 * <code>setAllowedScoreDeviation</code> method is used to specify the allowed
 * deviation from the reference score.
 *
...
public abstract class PerformanceTest extends AbstractTest {
	...
	/**
	 * Force implementations to only implement <code>runOp</code> and other
	...
	/**
	 * This implementation of runImpl runs the reference operation (with
	 * <code>runRef</code>), then runs the operation (with <code>runOp</code>) and
	 * checks whether or not the score is within the allowed deviation of the
	 * reference score.
	 *
	 * @see #runRef
	 * @see #runOp
	...
	/**
	 * Runs the reference operation. By default, this runs the same BufferedImage
