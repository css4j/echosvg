...
public class PerformanceTestValidator extends AbstractTest {
	...
	public TestReport runImpl() throws Exception {
		...
		if (!r.hasPassed()) {
			...
			result.addDescriptionEntry("regression.percentage",
					"" + 100 * (score - p.getLastScore()) / p.getLastScore());
			return result;
		}
	...
	static class SimplePerformanceTest extends PerformanceTest {
		...
		public void runOp() {
			...
			/*
			 * Vector v = new Vector(); for (int i=0; i<5000; i++) { v.addElement("" + i); }
			 * 
			 * for (int i=0; i<5000; i++) { if (v.contains("" + i)) { v.remove("" + i); } }
			 */
		}
	}
}
