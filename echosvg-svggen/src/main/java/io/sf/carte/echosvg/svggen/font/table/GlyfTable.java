...
public class GlyfTable implements Table {
	...
	protected GlyfTable(DirectoryEntry de, RandomAccessFile raf) throws IOException {
		...
		raf.read(buf);
		/*
		 * TableMaxp t_maxp = (TableMaxp) td.getEntryByTag(maxp).getTable(); TableLoca
		 * t_loca = (TableLoca) td.getEntryByTag(loca).getTable(); descript = new
		 * TableGlyfDescript[t_maxp.getNumGlyphs()]; for (int i = 0; i <
		 * t_maxp.getNumGlyphs(); i++) { raf.seek(tde.getOffset() +
