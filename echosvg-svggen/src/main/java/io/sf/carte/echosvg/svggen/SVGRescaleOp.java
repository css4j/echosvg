 ...
 * The SVG filter corresponding to a RescaleOp is an feComponentTransfer, with a
 * type set to 'linear', the slopes equal to the RescapeOp scaleFactors and the
 * intercept equal to the RescapeOp offsets.
 *
...
public class SVGRescaleOp extends AbstractSVGFilterConverter {
	 ...
	 * @return a description of the SVG filter corresponding to rescaleOp. The
	 *         definition of the feComponentTransfer filter in put in
	 *         feComponentTransferDefSet
	 */
	public SVGFilterDescriptor toSVG(RescaleOp rescaleOp) {
		...
		if (filterDesc == null) {
			...
			Element feComponentTransferDef = domFactory.createElementNS(SVG_NAMESPACE_URI,
			...
			if (offsets.length == 1) {
				...
				if (offsets.length == 4) {
					...
					feFuncA.setAttributeNS(null, SVG_SLOPE_ATTRIBUTE, doubleString(scaleFactors[3]));
					feFuncA.setAttributeNS(null, SVG_INTERCEPT_ATTRIBUTE, doubleString(offsets[3]));
				}
			}
...
//            filterAttrBuf.append(URL_SUFFIX);
			...
			String filterAttrBuf = URL_PREFIX + SIGN_POUND + filterDef.getAttributeNS(null, SVG_ID_ATTRIBUTE)
					+ URL_SUFFIX;
