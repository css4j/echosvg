...
public class SVGFont extends AbstractSVGConverter {
	...
	public static final float REGULAR = TextAttribute.WEIGHT_REGULAR;
	public static final float SEMIBOLD = TextAttribute.WEIGHT_SEMIBOLD;
	public static final float MEDIUM = TextAttribute.WEIGHT_MEDIUM;
	public static final float DEMIBOLD = TextAttribute.WEIGHT_DEMIBOLD;
	public static final float BOLD = TextAttribute.WEIGHT_BOLD;
	public static final float HEAVY = TextAttribute.WEIGHT_HEAVY;
	public static final float EXTRABOLD = TextAttribute.WEIGHT_EXTRABOLD;
	public static final float ULTRABOLD = TextAttribute.WEIGHT_ULTRABOLD;

	public static final float POSTURE_REGULAR = TextAttribute.POSTURE_REGULAR;
	public static final float POSTURE_OBLIQUE = TextAttribute.POSTURE_OBLIQUE;

	/**
	 * Contains threshold value for the various Font styles. If a given style is in
	 * an interval, then it is mapped to the style at the top of that interval.
	 * 
	 * @see #styleToSVG
	 */
	 ...
	 */
	static final String[] svgStyles = { /* POSTURE_REGULAR */ SVG_NORMAL_VALUE,
			/* POSTURE_OBLIQUE */ SVG_ITALIC_VALUE };

	/**
	 * Contains threshold values for the various Font weights. If a given weight is
	 * in an interval, then it is mapped to the weight at the top of the interval.
	 * 
	 * @see #weightToSVG
	 */
	static final float[] fontWeights = { EXTRA_LIGHT + (LIGHT - EXTRA_LIGHT) / 2f, LIGHT + (DEMILIGHT - LIGHT) / 2f,
			DEMILIGHT + (REGULAR - DEMILIGHT) / 2f, REGULAR + (SEMIBOLD - REGULAR) / 2f,
			SEMIBOLD + (MEDIUM - SEMIBOLD) / 2f, MEDIUM + (DEMIBOLD - MEDIUM) / 2f, DEMIBOLD + (BOLD - DEMIBOLD) / 2f,
			BOLD + (HEAVY - BOLD) / 2f, HEAVY + (EXTRABOLD - HEAVY) / 2f, EXTRABOLD + (ULTRABOLD - EXTRABOLD), };

	/**
	 * SVG Weights corresponding to the fontWeights
	 */
	static final String[] svgWeights = { /* EXTRA_LIGHT */ SVG_100_VALUE, /* LIGHT */ SVG_200_VALUE,
			/* DEMILIGHT */ SVG_300_VALUE, /* REGULAR */ SVG_NORMAL_VALUE, /* SEMIBOLD */ SVG_500_VALUE,
			/* MEDIUM */ SVG_500_VALUE, /* DEMIBOLD */ SVG_600_VALUE, /* BOLD */ SVG_BOLD_VALUE,
			/* HEAVY */ SVG_800_VALUE, /* EXTRABOLD */ SVG_800_VALUE, /* ULTRABOLD */ SVG_900_VALUE };
	...
	/**
	 * Records that the specified font has been used to draw the text string. This
	 * is so we can keep track of which glyphs are required for each SVG font that
	 * is generated.
	 */
...
//        }
	...
	/**
	 * Creates a new Font that is of the common font size used for generating SVG
	 * fonts. The new Font will be the same as the specified font, with only its
	...
	/**
	 * @param font Font object which should be converted to a set of SVG attributes
	...
	public SVGFontDescriptor toSVG(Font font, FontRenderContext frc) {
		...
		if (fontDesc != null) {
			...
			fontDef = domFactory.createElementNS(SVG_NAMESPACE_URI, SVG_FONT_TAG);

			//
			// create the font-face element
			//
			Element fontFace = domFactory.createElementNS(SVG_NAMESPACE_URI, SVG_FONT_FACE_TAG);
			...
			fontFace.setAttributeNS(null, SVG_FONT_FAMILY_ATTRIBUTE, svgFontFamilyString);
			fontFace.setAttributeNS(null, SVG_FONT_WEIGHT_ATTRIBUTE, fontWeight);
			fontFace.setAttributeNS(null, SVG_FONT_STYLE_ATTRIBUTE, fontStyle);
			fontFace.setAttributeNS(null, SVG_UNITS_PER_EM_ATTRIBUTE, "" + COMMON_FONT_SIZE);
			fontDef.appendChild(fontFace);

			//
			// create missing glyph element
			//
			Element missingGlyphElement = domFactory.createElementNS(SVG_NAMESPACE_URI, SVG_MISSING_GLYPH_TAG);
		...
		for (int i = textUsingFont.length() - 1; i >= 0; i--) {
			...
			if (!foundGlyph) {
				// need to create one
				Element glyphElement = domFactory.createElementNS(SVG_NAMESPACE_URI, SVG_GLYPH_TAG);
	...
	public static String familyToSVG(Font font) {
		...
		String logicalFontFamily = logicalFontMap.get(font.getName().toLowerCase());
	...
	/**
	 * @param font whose weight should be converted to an SVG string value. Note
	 *             that there is loss of precision for semibold and extrabold.
	 */
	...
	/**
	 * this helper-class implements a set of characters. it stores all used
	 * characters in a font.
	 *
	 * <br>
	 * implementation: we keep a sorted list of integers. This allows to use binary
	 * search for lookup and insert. The use of <code>int</code> instead of
	 * <code>char</code> allows us to handle surrogate characters as well.
	 */
	private static class CharListHelper {

		/**
		 * the number of slots actually used. must always be 0 &lt;= nUsed &lt;=
		 * charList.length
		 */
		private int nUsed = 0;
		...
		/**
		 * reset the string of recently added characters - used after glyphs were
		 * created for them.
		 */
		void clearNewChars() {
		...
		/**
		 * test, if the character is contained in the charList. If not, insert c into
		 * charList. charList is kept sorted for efficient search.
		 * 
		 * @param c
