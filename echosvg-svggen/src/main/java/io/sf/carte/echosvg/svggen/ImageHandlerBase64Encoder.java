...
public class ImageHandlerBase64Encoder extends DefaultImageHandler {
	...
	protected void handleEmptyImage(Element imageElement) {
		imageElement.setAttributeNS(XLINK_NAMESPACE_URI, XLINK_HREF_QNAME, DATA_PROTOCOL_PNG_PREFIX);
		imageElement.setAttributeNS(null, SVG_WIDTH_ATTRIBUTE, "0");
		imageElement.setAttributeNS(null, SVG_HEIGHT_ATTRIBUTE, "0");
	}

	/**
	 * This version of handleHREF encodes the input image into a PNG image whose
	 * bytes are then encoded with Base64. The resulting encoded data is used to set
	 * the url on the input imageElement, using the data: protocol.
	 */
	@Override
