 ...
 * properties. Here is how individual hints are converted. + RENDERING -&gt;
 * sets all other hints to initial value. + FRACTIONAL_METRICS -&gt; sets
 * initial values for text-rendering and shape-rendering. + ALPHA_INTERPOLATION
 * -&gt; Not mapped + ANTIALIASING -&gt; shape-rendering and text-rendering +
 * COLOR_RENDERING -&gt; color-rendering + DITHERING -&gt; not mapped +
 * INTERPOLATION -&gt; image-rendering + TEXT_ANTIALIASING -&gt; text-rendering
 *
...
public class SVGRenderingHints extends AbstractSVGConverter {
	...
	/**
	 * @param hints RenderingHints object which should be converted to a set of SVG
	 *              attributes.
