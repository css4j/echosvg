...
public class SVGFont implements XMLConstants, SVGConstants, ScriptTags, FeatureTags {
	 ...
	 */
	private static String CONFIG_USAGE = "SVGFont.config.usage";

	/**
	 * Defines the start of the generated SVG document {0} SVG public ID {1} SVG
	 * system ID
	 */
	private static String CONFIG_SVG_BEGIN = "SVGFont.config.svg.begin";

	/**
	 * Defines the SVG start fragment that exercise the generated Font.
	 */
	private static String CONFIG_SVG_TEST_CARD_START = "SVGFont.config.svg.test.card.start";
	...
	protected static String getSVGFontFaceElement(Font font) {
		...
		sb.append(XML_OPEN_TAG_START).append(SVG_FONT_FACE_TAG).append(EOL).append(XML_TAB)
				...
				.append(XML_TAB).append(SVG_ALPHABETIC_ATTRIBUTE).append(XML_EQUAL_QUOT).append(baseline)
				.append(XML_CHAR_QUOT).append(XML_OPEN_TAG_END_NO_CHILDREN).append(EOL);
	...
	/**
	 * Returns a &lt;font&gt;&#x2e;&#x2e;&#x2e;&lt;/font&gt; block, defining the
	 * specified font.
	 *
		...
		try {
			...
			for (int i = first; i <= last; i++) {
				...
				if (glyphIndex > 0) {
					...
					ps.println(getGlyphAsSVG(font, font.getGlyph(glyphIndex), glyphIndex, horiz_advance_x, initialSubst,
							medialSubst, terminalSubst, (32 <= i && i <= 127) ? encodeEntities(String.valueOf((char) i))
	...
	protected static String getGlyphAsSVG(Font font, Glyph glyph, int glyphIndex, int defaultHorizAdvanceX,
		...
		if (glyphIndex == 0) {
			...
			// Unicode value
			sb.append(XML_OPEN_TAG_START).append(SVG_GLYPH_TAG).append(XML_SPACE).append(SVG_UNICODE_ATTRIBUTE)
		...
		if (horiz_advance_x != defaultHorizAdvanceX) {
			sb.append(XML_SPACE).append(SVG_HORIZ_ADV_X_ATTRIBUTE).append(XML_EQUAL_QUOT).append(horiz_advance_x)
					.append(XML_CHAR_QUOT);
	...
	protected static String getGlyphAsSVG(Font font, Glyph glyph, int glyphIndex, int defaultHorizAdvanceX,
		...
		if (arabInitGlyphIndex != glyphIndex) {
			sb.append(getGlyphAsSVG(font, font.getGlyph(arabInitGlyphIndex), arabInitGlyphIndex, defaultHorizAdvanceX,
					// " arabic-form=\"initial\"",
					(XML_SPACE + SVG_ARABIC_FORM_ATTRIBUTE + XML_EQUAL_QUOT + SVG_INITIAL_VALUE + XML_CHAR_QUOT),
					code));
			// sb.append("\r\n");
			sb.append(EOL);
			substituted = true;
		...
		if (arabMediGlyphIndex != glyphIndex) {
			sb.append(getGlyphAsSVG(font, font.getGlyph(arabMediGlyphIndex), arabMediGlyphIndex, defaultHorizAdvanceX,
					// " arabic-form=\"medial\"",
					(XML_SPACE + SVG_ARABIC_FORM_ATTRIBUTE + XML_EQUAL_QUOT + SVG_MEDIAL_VALUE + XML_CHAR_QUOT), code));
			// sb.append("\r\n");
			sb.append(EOL);
			substituted = true;
		}
		...
		if (arabTermGlyphIndex != glyphIndex) {
			sb.append(getGlyphAsSVG(font, font.getGlyph(arabTermGlyphIndex), arabTermGlyphIndex, defaultHorizAdvanceX,
					// " arabic-form=\"terminal\"",
					(XML_SPACE + SVG_ARABIC_FORM_ATTRIBUTE + XML_EQUAL_QUOT + SVG_TERMINAL_VALUE + XML_CHAR_QUOT),
					code));
			// sb.append("\r\n");
			sb.append(EOL);
			substituted = true;
		...
		if (substituted) {
			sb.append(getGlyphAsSVG(font, glyph, glyphIndex, defaultHorizAdvanceX,
					// " arabic-form=\"isolated\"",
					(XML_SPACE + SVG_ARABIC_FORM_ATTRIBUTE + XML_EQUAL_QUOT + SVG_ISOLATED_VALUE + XML_CHAR_QUOT),
					code));
		} else {
	 ...
	 * firstIndex, count)); firstIndex = i + 1; count = 0; } } return sb.toString();
	 * }
	 * 
	 * protected static void writeTextAsSVGFragment(PrintStream ps, Font f, int
	 * size, String text) { CmapFormat cmapFmt =
	 * f.getCmapTable().getCmapFormat(Table.platformMicrosoft, Table.encodingUGL);
	 * int x = 0; for (short i = 0; i < text.length(); i++) { int glyphIndex =
	 * cmapFmt.mapCharCode((short)text.charAt(i)); Glyph glyph =
	 * f.getGlyph(glyphIndex); if (glyph != null) { ps.println(translateSVG(x, 0,
	 * getGlyphAsSVGPath(glyph))); x += glyph.getAdvanceWidth(); } } }
	 */
	protected static void writeSvgBegin(PrintStream ps) {
		...
		ps.println(Messages.formatMessage(CONFIG_SVG_BEGIN, new Object[] { SVG_PUBLIC_ID, SVG_SYSTEM_ID }));
	...
	protected static void writeSvgTestCard(PrintStream ps, String fontFamily) {
		...
		/*
		 * ps.println("<g style=\"font-family: '" + fontFamily +
		 * "'; font-size:18;fill:black\">"); ps.
		 * println("<text x=\"20\" y=\"60\"> !&quot;#$%&amp;&apos;()*+,-./0123456789:;&lt;&gt;?</text>"
		 * ); ps.
		 * println("<text x=\"20\" y=\"120\">@ABCDEFGHIJKLMNOPQRSTUVWXYZ[\\]^_</text>");
		 ...
		 * &#x8c;&#x8d;&#x8e;&#x8f;&#x90;&#x91;&#x92;&#x93;&#x94;&#x95;&#x96;&#x97;&#x98
		 * ;&#x99;&#x9a;&#x9b;&#x9c;&#x9d; &#x9e;&#x9f;</text>");
		 * ps.println("<text x=\"20\" y=\"300\">&#xa0;&#xa1;&#xa2;&#xa3;&#xa4;&#xa5;&#
		 * xa6;&#xa7;&#xa8;&#xa9;&#xaa;&#xab;
		 * &#xac;&#xad;&#xae;&#xaf;&#xb0;&#xb1;&#xb2;&#xb3;&#xb4;&#xb5;&#xb6;&#xb7;&#xb8
		 * ;&#xb9;&#xba;&#xbb;&#xbc;&#xbd; &#xbe;&#xbf;</text>");
		 * ps.println("<text x=\"20\" y=\"360\">&#xc0;&#xc1;&#xc2;&#xc3;&#xc4;&#xc5;&#
		 * xc6;&#xc7;&#xc8;&#xc9;&#xca;&#xcb;
		 * &#xcc;&#xcd;&#xce;&#xcf;&#xd0;&#xd1;&#xd2;&#xd3;&#xd4;&#xd5;&#xd6;&#xd7;&#xd8
		 * ;&#xd9;&#xda;&#xdb;&#xdc;&#xdd; &#xde;&#xdf;</text>");
		 * ps.println("<text x=\"20\" y=\"420\">&#xe0;&#xe1;&#xe2;&#xe3;&#xe4;&#xe5;&#
		 * xe6;&#xe7;&#xe8;&#xe9;&#xea;&#xeb;
		 * &#xec;&#xed;&#xee;&#xef;&#xf0;&#xf1;&#xf2;&#xf3;&#xf4;&#xf5;&#xf6;&#xf7;&#xf8
		 * ;&#xf9;&#xfa;&#xfb;&#xfc;&#xfd; &#xfe;&#xff;</text>"); ps.println("</g>");
		 */
	}
	...
	public static void main(String[] args) {
		try {
			...
			if (path != null) {
				...
				writeFontAsSVGFragment(ps, font, id, (low != null ? Integer.parseInt(low) : -1),
						(high != null ? Integer.parseInt(high) : -1), (autoRange != null), (ascii != null));
				writeSvgDefsEnd(ps);
				if (testCard != null) {
					String fontFamily = font.getNameTable().getRecord(Table.nameFontFamilyName);
