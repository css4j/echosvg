 ...
 *
 * There are two rules that control how children nodes are added to the group
 * managed by this class:
 *
 * + Children node are added to the group as long as there is no more than n
 * graphic context overrides needed to describe the children style. A graphic
 * context override happens when style attributes need to be added to a child
 * node to reflect the state of the graphic context at the time the child was
 * added. Note that the opacity is never reflected in a group node and
 * therefore, is not accounted for in the number of overrides. The number of
 * overrides can be configured and defaults to 2. + Children nodes are added to
 * the current group as long as the associated GraphicContext's transform stack
 * is valid.
 *
 * When children nodes can no longer be added, the group is considered complete
 * and the associated DOMTreeManager is notified of the availability of a
 * completed group. Then, a new group is started. <br>
 * The DOMTreeManager is also notified every thime a new element is added to the
 * current group. This is needed to let the DOMTreeManager handle group managers
 * that would be used concurrently.
 *
...
public class DOMGroupManager implements SVGSyntax, ErrorConstants {
	 ...
	 * @param domTreeManager DOMTreeManager instance this group manager cooperates
	 *                       with.
	 */
	...
	public void addElement(Element element, short method) {
		...
		if (!currentGroup.hasChildNodes()) {
			...
			domTreeManager.getStyleHandler().setStyle(element, deltaGC.getGraphicElementContext(),
					domTreeManager.getGeneratorContext());
	...
	/**
	 * Analyses the Map to define how many attributes constitute overrides. Only
	 * differences in the group context are considered overrides.
	 */
	...
	/**
	 * Processes the transform attribute value corresponding to a given transform
	 * stack
	 */
	...
	/**
	 * Processes the difference between two graphic contexts. The values in gc that
	 * are different from the values in referenceGc will be present in the delta.
	 * Other values will no.
	 */
	static SVGGraphicContext processDeltaGC(SVGGraphicContext gc, SVGGraphicContext referenceGc) {
		Map<String, String> groupDelta = processDeltaMap(gc.getGroupContext(), referenceGc.getGroupContext());
		 ...
		 * gc.getTransformStack().length); System.err.println("reference stack length :
		 * " + referenceGc.getTransformStack().length); System.err.println("delta stack
		 * length : " + deltaTransformStack.length);
		 */

		/*
		 * TransformStackElement gcStack[] = gc.getTransformStack(); for(int i=0;
		 * i<gcStack.length; i++) System.err.println("gcStack[" + i + "] = " +
		 * gcStack[i].toString());
		 * 
		 * TransformStackElement refStack[] = referenceGc.getTransformStack(); for(int
		 * i=0; i<refStack.length; i++) System.err.println("refStack[" + i + "] = " +
		 * refStack[i].toString());
		 * 
	...
	/**
	 * Processes the difference between two Maps. The code assumes that the input
	 * Maps have the same key sets. Values in map that are different from values in
	 * referenceMap are place in the returned delta Map.
	 */
	static Map<String, String> processDeltaMap(Map<String, String> map, Map<String, String> referenceMap) {
		...
		for (String key : map.keySet()) {
			...
			if (!value.equals(refValue)) {
				/*
				 * if(key.equals(SVG_TRANSFORM_ATTRIBUTE)){ // Special handling for the
				 * transform attribute. // At this point in the processing, the transform // in
				 * map has to be a substring of the one in // referenceMap. see the addElement
				 * member. value = value.substring(refValue.length()).trim(); }
				 */
