...
public abstract class SwingSVGPrettyPrint implements SVGSyntax {
	...
	public static void print(JComponent cmp, SVGGraphics2D svgGen) {
		...
		cmpGroup.setAttributeNS(null, "id",
				svgGen.getGeneratorContext().getIDGenerator().generateID(cmp.getClass().getName()));
	...
	/**
	 * WARNING: The following code does some special case processing depending on
	 * the class of the input JComponent. This is needed because there is no generic
	 * way I could find to determine whether a component should be painted or not.
	 */
	private static void paintBorder(JComponent cmp, SVGGraphics2D svgGen) {
		Border border = cmp.getBorder();
		if (border != null) {
			if ((cmp instanceof AbstractButton) || (cmp instanceof JPopupMenu) || (cmp instanceof JToolBar)
					|| (cmp instanceof JMenuBar) || (cmp instanceof JProgressBar)) {
