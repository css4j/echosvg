 ...
 * new Nodes to the SVG DOM Tree. This class handles a set of DOMGroupManager
 * objects that can all append to the top level group managed by this class.
 * This allows multiple SVGGraphics2D instances, created from the same
 * SVGGraphics2D through the various create methods, to append to the same SVG
 * document and keep the rendering order correct.
 *
 * The root node managed by this DOMTreeManager contains two children: a top
 * level group node and a top level defs node. The top level defs node contains
 * the definition of common SVG entities such as the various AlphaComposite
 * rules. Note that other defs can also be created under the top level group,
 * for example to represent gradient or pattern paints. <br>
 * [svg] | +-- [defs] Contain generic definitions +-- [g] Top level group | +--
 * [defs] Contains definitions specific to rendering +-- [g] Group 1 +-- ... +--
 * [g] Group n
 *
...
public class DOMTreeManager implements SVGSyntax, ErrorConstants {
	...
	/**
	 * Maximum of Graphic Context attributes overrides in children of the current
	 * group.
	 */
	int maxGCOverrides;
	 ...
	 * @param maxGCOverrides   defines how many overrides are allowed in children
	 *                         nodes of the current group.
	 */
	...
	/**
	 * @param groupManager new DOMGroupManager to add to the list of managers that
	 *                     collaborate with this tree manager.
	 */
	...
	/**
	 * @param groupManager DOMGroupManager to remove from the list of managers that
	 *                     collaborate with this tree manager
	 */
	...
	/**
	 * When a group is appended to the tree by this call, all the other group
	 * managers are requested to start new groups, in order to preserve the Z-order.
	 *
	...
	public void appendGroup(Element group, DOMGroupManager groupManager) {
		...
		synchronized (groupManagers) {
			...
			for (DOMGroupManager gm : groupManagers) {
				if (gm != groupManager)
	...
	public List<Element> getDefinitionSet() {
		...
		//
		List<Element> defSet = gcConverter.getDefinitionSet();
	 ...
	 * a new topLevelGroup.
	 *
	 * @param includeDefinitionSet if true, the definition set is included and the
	 *                             converters are reset (i.e., they start with an
