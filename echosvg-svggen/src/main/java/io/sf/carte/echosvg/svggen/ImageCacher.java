...
public abstract class ImageCacher implements SVGSyntax, ErrorConstants {
	...
	/**
	 * Checks if the image is already in the cache, and adds it if not. Returns a
	...
	public String lookup(ByteArrayOutputStream os, int width, int height, SVGGeneratorContext ctx)
		...
		if (href == null) {
			...
			ImageCacheEntry newEntry = createEntry(checksum, data, width, height, ctx);
			list.add(newEntry);
			href = newEntry.href;
		}
	 ...
	 * information should actually be stored.
	 *
	...
	/**
	 * Determines if two images are equal. Interpretation of the objects referred to
	 * by <code>o1</code> and <code>o2</code> is entirely implementation-dependent.
	 *
	 ...
	 * @param ctx      the SVGGeneratorContext
	 */
	abstract ImageCacheEntry createEntry(int checksum, Object data, int width, int height, SVGGeneratorContext ctx)
			throws SVGGraphics2DIOException;
	...
	/**
	 * Instances of this class are created to keep track of the set of images
	 * processed by the ImageHandler. Each entry corresponds to one unique member of
	 * this set.
	 */
	...
	public static class Embedded extends ImageCacher {
		...
		private void addToTree(String id, String href, int width, int height, SVGGeneratorContext ctx) {
			...
			imageElement.setAttributeNS(null, SVG_ID_ATTRIBUTE, id);
			...
			imageElement.setAttributeNS(XMLConstants.XLINK_NAMESPACE_URI, XLINK_HREF_QNAME, href);
			// imageDefs.appendChild(imageElement);
			domTreeManager.addOtherDef(imageElement);
		}

		/**
		 * Returns the top level defs section dedicated to cached embedded images,
		 * creating one if necessary. This one very simply creates a new defs section
		 * for each image, causing them to be spread throughout the entire SVG tree. A
		 * nicer implementation would group all imageDefs sections into one.
		 */
		/*
		 * private Element getImageDefs(Document domFactory, SVGGeneratorContext ctx) {
		 * 
		...
		boolean imagesMatch(Object o1, Object o2) throws SVGGraphics2DIOException {
			...
			try {
				...
				while (bytesRead != imageLen) {
					bytesRead += imageStream.read(imageBytes, bytesRead, imageLen - bytesRead);
				}
