 ...
 * they handle. This class stores images in an configurable directory. The
 * xlink:href value the class generates is made of a configurable url root and
 * the name of the file created by this handler.
 *
...
public abstract class AbstractImageHandlerEncoder extends DefaultImageHandler {
	...
	private static Graphics2D createGraphics(BufferedImage buf) {
		if (!initDone) {
			try {
				...
				// happen only if EchoSVG extensions are not there
			} finally {
				initDone = true;
			}
		}
	 ...
	 *                 an SVGGraphics2DRuntimeException is thrown.
	...
	public AbstractImageHandlerEncoder(String imageDir, String urlRoot) throws SVGGraphics2DIOException {
		...
		if (urlRoot != null)
			...
			try {
				...
				throw new SVGGraphics2DIOException(ERR_CANNOT_USE_IMAGE_DIR + e.getMessage(), e);
			}
		}
	}
	...
	private void saveBufferedImageToFile(Element imageElement, BufferedImage buf, SVGGeneratorContext generatorContext)
			throws SVGGraphics2DIOException {
		...
		// Update HREF
		imageElement.setAttributeNS(XLINK_NAMESPACE_URI, XLINK_HREF_QNAME, urlRoot + "/" + imageFile.getName());
	}
	 ...
	 */
	public abstract void encodeImage(BufferedImage buf, File imageFile) throws SVGGraphics2DIOException;
