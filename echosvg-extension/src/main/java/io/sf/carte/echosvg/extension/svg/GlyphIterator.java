...
public class GlyphIterator {
	...
	/**
	 * @return The current advance for the line, this is the 'visual width' of the
	 *         current line.
	 */
	public float getAdv() {
		return adv;
	...
	/**
	 * @return The origin of the glyph vector (the point all glyphs are layed out
	 *         with respect to).
	 */
	...
	/**
	 * @return The current adjustment for the line. This is the ammount that needs
	 *         to be subracted from the following line to get it back to the start
	 *         of the next line.
	 */
	public float getAdj() {
		return adj;
	...
	public boolean isBreakChar() {
		switch (ch) {
		case GlyphIterator.ZERO_WIDTH_SPACE:
			return true;
		case GlyphIterator.ZERO_WIDTH_JOINER:
			return false;
		case GlyphIterator.SOFT_HYPHEN:
			return true;
		case ' ':
		case '\t':
			return true;
	...
	public void nextChar() {
		if ((ch == SOFT_HYPHEN) || (ch == ZERO_WIDTH_SPACE) || (ch == ZERO_WIDTH_JOINER)) {
	...
	protected void updateLineMetrics(int end) {
		GVTLineMetrics glm = font.getLineMetrics(aci, fontStart, end, frc);
	...
	public LineInfo newLine(Point2D.Float loc, float lineWidth, boolean partial, Point2D.Float verticalAlignOffset) {
		...
		for (int ci = lineIdx; ci <= idx; ci++) {
			...
			gv.setGlyphPosition(ci, new Point2D.Float(gp[2 * ci] - leftShift, gp[2 * ci + 1]));
		}
		leftShiftIdx = null;
		leftShiftAmt = null;
		...
		while (!done()) {
			...
			if ((ch == ZERO_WIDTH_SPACE) || (ch == ZERO_WIDTH_JOINER))
				gv.setGlyphVisible(idx, false);
		...
		LineInfo ret = new LineInfo(loc, aci, gv, lineIdx, lineInfoIdx, lineInfoAdj, lineInfoAdv, lineInfoChW,
	...
	/**
	 * Get the visual advance associated with the current glyph. This is the
	 * distance from the location of the glyph to the rightmost part of the glyph.
	 */
