...
public class FlowExtGlyphLayout extends GlyphLayout {
	 ...
	 *                     text to wrap. There is one aci per text chunk (which maps
	 *                     to flowPara elements. Used to access font, paragraph, and
	 *                     line break info.
	 * @param chunkLayouts A List of List of GlyphLayout objects. There is a List of
	 *                     GlyphLayout objects for each flowPara element. There is a
	 *                     GlyphLayout for approximately each sub element in the
	 *                     flowPara element.
	 * @param flowRects    A List of Rectangle2D representing the regions to flow
	 *                     text into.
	 */
	public static void textWrapTextChunk(AttributedCharacterIterator[] acis, List<List<TextSpanLayout>> chunkLayouts,
		...
		for (int chunk = 0; clIter.hasNext(); chunk++) {
			...
			width = (float) (currentRegion.getWidth() - (leftMargin + rightMargin));
			height = (float) currentRegion.getHeight();
	...
	/**
	 * Updates the specified verticalAlignmentOffset using the current alignment
	 * rule and the heights of the flow rect and the maximum descent of the text.
	 * This method gets for called every line, but only the value that is calculated
	 * for the last line of the flow rect is used by the glyph rendering. This is
	 * achieved by creating a new verticalAlignOffset object everytime a new flow
	 * rect is encountered, thus a single verticalAlignmentOffset is shared for all
	 * {@link LineInfo} objects created for a given flow rect. The value is
	 * calculated by determining the left over space in the flow rect and scaling
	 * that value by 1.0 to align to the bottom, 0.5 for middle and 0.0 for top.
	 *
	 * @param verticalAlignOffset the {@link java.awt.geom.Point2D.Float} object
	 *                            that is storing the alignment offset.
	 * @param region              the {@link RegionInfo} object that we are
	 *                            rendering into.
	 * @param maxDescent          the very lowest point this line reaches.
	 */
	public static void updateVerticalAlignOffset(Point2D.Float verticalAlignOffset, RegionInfo region,
			float maxDescent) {
		float freeSpace = (float) region.getHeight() - maxDescent;
		verticalAlignOffset.setLocation(0, region.getVerticalAlignment() * freeSpace);
	}
