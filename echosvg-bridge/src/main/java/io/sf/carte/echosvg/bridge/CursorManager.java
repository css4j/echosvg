...
public class CursorManager implements SVGConstants, ErrorConstants {
	 ...
	 * does not handle user-defined cursors, so it always uses the cursor at the end
	 * of the list
	 */
	...
	public Cursor convertBuiltInCursor(Element e, String cursorStr) {
		...
		if (cursorStr.charAt(0) == 'a') {
			...
			if (SVGConstants.SVG_NAMESPACE_URI.equals(nameSpaceURI)) {
				...
				} else if (SVGConstants.SVG_TEXT_TAG.equals(tag) || SVGConstants.SVG_TSPAN_TAG.equals(tag)
	...
	/**
	 * Converts the input ParsedURL into a Filter and transforms the input hotSpot
	 * point (in image space) to cursor space
	 */
	...
	/**
	 * Simple inner class which holds the information describing a cursor, i.e., the
	...
	/**
	 * Simple extension of the SoftReferenceCache that offers typed interface (Kind
