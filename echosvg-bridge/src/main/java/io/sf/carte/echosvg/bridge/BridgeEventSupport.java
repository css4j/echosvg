...
public abstract class BridgeEventSupport implements SVGConstants {
	 ...
	 */
	protected static void storeEventListenerNS(BridgeContext ctx, EventTarget e, String n, String t, EventListener l,
			boolean c) {
	...
	protected static class Listener implements GraphicsNodeMouseListener, GraphicsNodeKeyListener {
		...
		protected void dispatchKeyEvent(String eventType, GraphicsNodeKeyEvent evt) {
			...
			keyEvt.initKeyEvent(eventType, true, true, evt.isControlDown(), evt.isAltDown(), evt.isShiftDown(),
					evt.isMetaDown(), mapKeyCode(evt.getKeyCode()), evt.getKeyChar(), null);
		...
		/**
		 * The java KeyEvent keyCodes and the DOMKeyEvent keyCodes map except for the
		 * VK_ENTER code (which has a different value in DOM and the VK_KANA_LOCK and
		 * VK_INPUT_METHOD_ON_OFF which have no DOM equivalent.
		 */
		...
		public void mouseEntered(GraphicsNodeMouseEvent evt) {
			...
			dispatchMouseEvent("mouseover", targetElement, relatedElement, clientXY, evt, true);
		}
		...
		protected void dispatchMouseEvent(String eventType, GraphicsNodeMouseEvent evt, boolean cancelable) {
			...
			Element targetElement = getEventTarget(node, evt.getPoint2D());
			Element relatedElement = getRelatedElement(evt);
			dispatchMouseEvent(eventType, targetElement, relatedElement, clientXY, evt, cancelable);
		}
		...
		protected void dispatchMouseEvent(String eventType, Element targetElement, Element relatedElement,
			...
			/*
			 * if (relatedElement != null) { System.out.println ("dispatching "+eventType+
			 * " target:"+targetElement.getLocalName()+
			 * " relatedElement:"+relatedElement.getLocalName()); } else {
			 * System.out.println ("dispatching "+eventType+
			 * " target:"+targetElement.getLocalName());
			 * 
			 * }
			 */
			...
			mouseEvt.initMouseEventNS(XMLConstants.XML_EVENTS_NAMESPACE_URI, eventType, true, cancelable, null,
					evt.getClickCount(), screenXY.x, screenXY.y, clientXY.x, clientXY.y, (short) (evt.getButton() - 1),
					(EventTarget) relatedElement, modifiers);
