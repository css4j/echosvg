 ...
 * @version $Id$
 */
public abstract class AbstractSVGFilterPrimitiveElementBridge extends AnimatableGenericSVGBridge
	...
	/**
	 * Returns the input source of the specified filter primitive element defined by
	 * its 'in' attribute.
	 *
	 ...
	 */
	protected static Filter getIn(Element filterElement, Element filteredElement, GraphicsNode filteredNode,
	...
	/**
	 * Returns the input source of the specified filter primitive element defined by
	 * its 'in2' attribute. The 'in2' attribute is assumed to be required if the
	 * subclasses ask for it.
	 *
	 * @param filterElement   the filter primitive element
	 * @param filteredElement the element on which the filter is referenced
	 * @param filteredNode    the graphics node on which the filter is applied
	 ...
	 * @param ctx             the bridge context
	 */
	protected static Filter getIn2(Element filterElement, Element filteredElement, GraphicsNode filteredNode,
			Filter inputFilter, Map<String, Filter> filterMap, BridgeContext ctx) {
	 ...
	 * @param ctx             the bridge context
	 */
	static Filter getFilterSource(Element filterElement, String s, Element filteredElement, GraphicsNode filteredNode,
			Map<String, Filter> filterMap, BridgeContext ctx) {
	...
	/**
	 * This is a bit of a hack but we set the flood bounds to -floatmax/2 ->
	 * floatmax/2 (should cover the area ok).
	 */
	static final Rectangle2D INFINITE_FILTER_REGION = new Rectangle2D.Float(-Float.MAX_VALUE / 2, -Float.MAX_VALUE / 2,
			Float.MAX_VALUE, Float.MAX_VALUE);

	/**
	 * Converts on the specified filter primitive element, the specified attribute
	 * that represents an integer and with the specified default value.
	 *
	 * @param filterElement the filter primitive element
	...
	/**
	 * Converts on the specified filter primitive element, the specified attribute
	 * that represents a float and with the specified default value.
	 *
	 * @param filterElement the filter primitive element
	 * @param attrName      the name of the attribute
	 * @param defaultValue  the default value of the attribute
	 * @param ctx           the BridgeContext to use for error information
	 */
	protected static float convertNumber(Element filterElement, String attrName, float defaultValue,
			BridgeContext ctx) {
