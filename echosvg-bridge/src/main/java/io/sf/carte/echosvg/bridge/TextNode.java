...
public class TextNode extends AbstractGraphicsNode implements Selectable {
	...
	/**
	 * Location of this text node (inherited, independent of explicit X and Y
	 * attributes applied to children).
	 */
	 ...
	 * AttributedCharacterIterator. Before Char indicates if the Marker should be
	 * considered before or after char.
	 */
	...
	public Object getSelection() {
		...
		if ((ranges != null) && (ranges.length > 1)) {
			...
			o = new AttributedCharacterSpanIterator(aci, ranges[0], ranges[1] + 1);
		}
		return o;
	}
	...
	public Shape getHighlightShape() {
		Shape highlightShape = textPainter.getHighlightShape(beginMark, endMark);
	...
	/**
	 * Defines where the text of a <code>TextNode</code> can be anchored relative to
	 * its location.
	 */
	public static final class Anchor implements java.io.Serializable {
		...
		/**
		 * The anchor which enables the rendered characters to be aligned such that the
		 * start of the text string is at the initial current text location.
		 */
		public static final Anchor START = new Anchor(ANCHOR_START);

		/**
		 * The anchor which enables the rendered characters to be aligned such that the
		 * middle of the text string is at the initial current text location.
		 */
		public static final Anchor MIDDLE = new Anchor(ANCHOR_MIDDLE);

		/**
		 * The anchor which enables the rendered characters to be aligned such that the
		 * end of the text string is at the initial current text location.
		 */
