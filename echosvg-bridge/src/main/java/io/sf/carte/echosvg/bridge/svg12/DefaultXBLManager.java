...
public class DefaultXBLManager implements XBLManager, XBLConstants {
	...
	public void startProcessing() {
		...
		NodeList nl = document.getElementsByTagNameNS(XBL_NAMESPACE_URI, XBL_DEFINITION_TAG);
		...
		nl = document.getElementsByTagNameNS(XBL_NAMESPACE_URI, XBL_IMPORT_TAG);
		...
		es.addImplementationEventListenerNS(XMLConstants.XML_EVENTS_NAMESPACE_URI, "DOMNodeRemoved", docRemovedListener,
				true);
	...
	public void stopProcessing() {
		...
		for (Object defRec1 : defRecs) {
			...
			TreeSet<DefinitionRecord> defs = (TreeSet<DefinitionRecord>) definitionLists.get(defRec.namespaceURI,
					defRec.localName);
			if (defs != null) {
				while (!defs.isEmpty()) {
					defRec = defs.first();
	...
	/**
	 * Adds a definition through its referring definition element (one with a 'ref'
	 * attribute).
	 */
	...
	/**
	 * Removes a definition through its referring definition element (one with a
	 * 'ref' attribute).
	 */
	protected void removeDefinitionRef(Element defRef) {
		ImportRecord ir = imports.get(defRef);
		NodeEventTarget et = (NodeEventTarget) defRef;
		et.removeEventListenerNS(XMLConstants.XML_EVENTS_NAMESPACE_URI, "DOMAttrModified", refAttrListener, false);
		DefinitionRecord defRec = (DefinitionRecord) definitions.get(ir.node, defRef);
		removeDefinition(defRec);
	...
	protected void addImport(Element imp) {
		...
		et.addEventListenerNS(XMLConstants.XML_EVENTS_NAMESPACE_URI, "DOMAttrModified", importAttrListener, false,
				null);

		et = (NodeEventTarget) n;
		et.addEventListenerNS(XMLConstants.XML_EVENTS_NAMESPACE_URI, "DOMNodeInserted", ir.importInsertedListener,
				false, null);
		et.addEventListenerNS(XMLConstants.XML_EVENTS_NAMESPACE_URI, "DOMNodeRemoved", ir.importRemovedListener, false,
				null);
		et.addEventListenerNS(XMLConstants.XML_EVENTS_NAMESPACE_URI, "DOMSubtreeModified", ir.importSubtreeListener,
				false, null);
	...
	protected void removeImport(Element imp) {
		...
		et.removeEventListenerNS(XMLConstants.XML_EVENTS_NAMESPACE_URI, "DOMNodeInserted", ir.importInsertedListener,
				false);
		et.removeEventListenerNS(XMLConstants.XML_EVENTS_NAMESPACE_URI, "DOMNodeRemoved", ir.importRemovedListener,
				false);
		et.removeEventListenerNS(XMLConstants.XML_EVENTS_NAMESPACE_URI, "DOMSubtreeModified", ir.importSubtreeListener,
				false);

		et = (NodeEventTarget) imp;
		et.removeEventListenerNS(XMLConstants.XML_EVENTS_NAMESPACE_URI, "DOMAttrModified", importAttrListener, false);
	 ...
	 * affect elements with the specified QName. This may or may not actually cause
	 * a new binding to come in to effect, as this new definition element may be
	 * added earlier in the document than another already in effect.
	 *
	 ...
	 *                     this definition is being added, or null if the binding is
	...
	protected void addDefinition(String namespaceURI, String localName, XBLOMDefinitionElement def, Element imp) {
		...
		defRec = new DefinitionRecord(namespaceURI, localName, def, template, imp);
		defs.add(defRec);
	...
	/**
	 * Removes an xbl:definition element from the list of definitions that could
	 * possibly affect elements with the specified QName. This will only cause a new
	 * binding to come in to effect if it is currently active.
	 */
	...
	protected void bind(Element e) {
		...
		if (e instanceof BindableElement) {
			DefinitionRecord defRec = getActiveDefinition(e.getNamespaceURI(), e.getLocalName());
	...
	protected void rebind(String namespaceURI, String localName, Element e) {
		...
		if (e instanceof BindableElement && namespaceURI.equals(e.getNamespaceURI())
	...
	/**
	 * Sets the given definition as the active one for a particular bindable
	 * element.
	 */
	...
	protected void setXblShadowTree(BindableElement elt, XBLOMShadowTreeElement newShadow) {
		XBLOMShadowTreeElement oldShadow = (XBLOMShadowTreeElement) getXblShadowTree(elt);
		if (oldShadow != null) {
			...
			oldShadow.removeEventListenerNS(XMLConstants.XML_EVENTS_NAMESPACE_URI, "DOMSubtreeModified",
					docSubtreeListener, false);
		}
		if (newShadow != null) {
			newShadow.addEventListenerNS(XMLConstants.XML_EVENTS_NAMESPACE_URI, "DOMSubtreeModified",
	...
	protected XBLOMShadowTreeElement cloneTemplate(XBLOMTemplateElement template) {
		XBLOMShadowTreeElement clone = (XBLOMShadowTreeElement) template.getOwnerDocument()
				.createElementNS(XBL_NAMESPACE_URI, XBL_SHADOW_TREE_TAG);
	...
	/**
	 * Updates the xblPreviousSibling and xblNextSibling properties of the given XBL
	 * node.
	 */
	...
	/**
	 * Returns the content element that caused the given node to be present in the
	 * flattened tree.
	 */
	...
	/**
	 * Determines the number of nodes events should bubble if the mouse pointer has
	...
	/**
	 * Returns the ContentManager that handles the shadow tree the given node
	 * resides in.
	 */
	...
	/**
	 * Adds the specified ContentSelectionChangedListener to the global listener
	 * list.
	 */
	public void addContentSelectionChangedListener(ContentSelectionChangedListener l) {
		contentSelectionChangedListenerList.add(ContentSelectionChangedListener.class, l);
	}

	/**
	 * Removes the specified ContentSelectionChangedListener from the global
	 * listener list.
	 */
	public void removeContentSelectionChangedListener(ContentSelectionChangedListener l) {
		contentSelectionChangedListenerList.remove(ContentSelectionChangedListener.class, l);
	}
	...
	/**
	 * Called by the ContentManager of a shadow tree to indicate some selected nodes
	 * have changed.
	 */
	...
	/**
	 * Returns whether the given definition element is the active one for its
	 * element name.
	 */
	protected boolean isActiveDefinition(XBLOMDefinitionElement def, Element imp) {
		...
		return defRec == getActiveDefinition(defRec.namespaceURI, defRec.localName);
	}
	...
	protected static class DefinitionRecord implements Comparable<DefinitionRecord> {
		...
		public int compareTo(DefinitionRecord rec) {
			...
			if (importElement == null) {
				...
				n2 = rec.definition;
			} else if (definition.getOwnerDocument() == rec.definition.getOwnerDocument()) {
				n1 = definition;
				n2 = rec.definition;
			} else {
				n1 = (AbstractNode) importElement;
	...
	protected class ImportRecord {
		...
		/**
		 * The DOM attribute mutation listener for definitions accessed through this
		 * import.
		 */
		...
		/**
		 * The DOM subtree modified listener for templates of definitions accessed
		 * through this import.
		 */
			...
			if (target instanceof XBLOMDefinitionElement) {
				...
				if (target instanceof BindableElement) {
					...
					for (Node n = ((Node) target).getParentNode(); n != null; n = n.getParentNode()) {
						if (n instanceof BindableElement && getRecord(n).definitionElement != null) {
							return;
						}
					}
					bind((Element) target);
	...
	protected class TemplateMutationListener implements EventListener {
		...
		public void handleEvent(Event evt) {
			...
			DefinitionRecord defRec = (DefinitionRecord) definitions.get(n, importElement);
			if (defRec == null) {
				return;
			}
	...
	protected class DefNodeInsertedListener implements EventListener {
		...
		public void handleEvent(Event evt) {
			...
			DefinitionRecord defRec = (DefinitionRecord) definitions.get(parent, importElement);
			if (defRec == null) {
				return;
			}
		...
		public void handleEvent(Event evt) {
			...
			if (mevt.getAttrName().equals(XBL_REF_ATTRIBUTE)) {
				...
				if (mevt.getNewValue().length() == 0) {
					...
					String ns = def.getElementNamespaceURI();
					String ln = def.getElementLocalName();
					addDefinition(ns, ln, (XBLOMDefinitionElement) defRef, null);
				} else {
	...
	protected class XBLRecord {
		...
		/**
		 * The bound element that owns this shadow tree, if this node is an
	...
	protected class XblChildNodes implements NodeList {
		...
		protected void update() {
			...
			Node last = null;
			Node m = shadowTree == null ? record.node.getFirstChild() : shadowTree.getFirstChild();
			while (m != null) {
				...
				if (!XBL_NAMESPACE_URI.equals(n.getNamespaceURI())) {
					...
					if (cm != null) {
						...
						for (int i = 0; i < selected.getLength(); i++) {
							prev = collectXblChildNodes(selected.item(i), prev);
						}
					}
				}
			} else {
		...
		/**
		 * Mark the xblNextSibling and xblPreviousSibling variables on each node in the
		 * list as invalid, then invalidate the NodeList.
		 */
	...
	protected class XblScopedChildNodes extends XblChildNodes {
		...
		protected void update() {
			size = 0;
			Node shadowTree = getXblShadowTree(record.node);
			Node n = shadowTree == null ? record.node.getFirstChild() : shadowTree.getFirstChild();
			while (n != null) {
