...
public class GlyphLayout implements TextSpanLayout {
	...
	/**
	 * Creates the specified text layout using the specified
	 * AttributedCharacterIterator and rendering context.
	 *
	 * @param aci     the AttributedCharacterIterator whose text is to be laid out
	...
	public GlyphLayout(AttributedCharacterIterator aci, int[] charMap, Point2D offset, FontRenderContext frc) {
		...
		AltGlyphHandler altGlyphHandler = (AltGlyphHandler) this.aci
				.getAttribute(GVTAttributedCharacterIterator.TextAttribute.ALT_GLYPH_HANDLER);
	...
	/**
	 * Returns the current text position at the beginning of glyph layout, before
	 * the application of explicit glyph positioning attributes.
	 */
	@Override
	 ...
	 * spaces between them will be adjusted. Only the scale factor in the
	 * progression direction is used (x for horizontal text, y for vertical text ).
	 * 
	...
	public boolean isLeftToRight() {
		aci.first();
		int bidiLevel = (Integer) aci.getAttribute(GVTAttributedCharacterIterator.TextAttribute.BIDI_LEVEL);
	...
	/**
	 * This method makes certain that the layout has been completed at this point
	 * (much of the layout is done lazily).
	 */
	...
	/**
	 * Returns the rectangular bounds of the completed glyph layout, inclusive of
	 * "decoration" (underline, overline, etc.)
	 */
	@Override
	...
	/**
	 * Returns the position to used when drawing a text run after this one. It takes
	 * into account the text path layout if there is one.
	 */
	@Override
			...
			if ((glyphCharIndex >= beginCharIndex) && (glyphCharIndex <= endCharIndex) && gv.isGlyphVisible(i)) {
				...
				if (gbounds != null) {
					...
					if (pi.isDone()) {
						...
						if ((botPts[ptIdx] != null) && ((topPts[ptIdx].x != topPts[ptIdx + 1].x)
		...
		for (int i = 0; i < gv.getNumGlyphs(); i++) {
			...
			if (gbounds != null) {
				...
				if (gbounds.contains(x, y)) {
					boolean isRightHalf = (x > (gbounds2d.getX() + (gbounds2d.getWidth() / 2d)));
...
//protected
	...
	protected Shape getOverlineShape() {
		...
		return overlineStroke.createStrokedShape(new Line2D.Double(logicalBounds.getMinX() + overlineThickness / 2.0,
				offset.getY() + y, logicalBounds.getMaxX() - overlineThickness / 2.0, offset.getY() + y));
	}
	...
	protected Shape getUnderlineShape() {
		...
		return underlineStroke.createStrokedShape(new Line2D.Double(logicalBounds.getMinX() + underlineThickness / 2.0,
				offset.getY() + y, logicalBounds.getMaxX() - underlineThickness / 2.0, offset.getY() + y));
	}
	...
	protected Shape getStrikethroughShape() {
		...
		return strikethroughStroke
				.createStrokedShape(new Line2D.Double(logicalBounds.getMinX() + strikethroughThickness / 2.0,
						offset.getY() + y, logicalBounds.getMaxX() - strikethroughThickness / 2.0, offset.getY() + y));
	}

	/**
	 * Explicitly lays out each of the glyphs in the glyph vector. This will handle
	 * any glyph position adjustments such as dx, dy and baseline offsets. It will
	 * also handle vertical layouts.
	 */
	protected void doExplicitGlyphLayout() {

		this.gv.performDefaultLayout();

		float baselineAscent = vertical ? (float) gv.getLogicalBounds().getWidth()
		...
		while (i < numGlyphs) {
			...
			if (i == 0) {
				...
				if (glyphOrientationAuto && (verticalFirstOffset == 0f) && !isLatinChar(ch)) {
					float advY = gm.getVerticalAdvance();
					float asc = metrics.getAscent();
					float dsc = metrics.getDescent();
					verticalFirstOffset = asc + (advY - (asc + dsc)) / 2;
				...
				if (vertical) {
					...
					if (glyphOrientationAuto) {
						if (isLatinChar(ch)) {
							...
							ox -= (float) ((glyphBounds.getMaxX() - gp[2 * i]) - glyphBounds.getWidth() / 2);
						}
					} else {
			...
			if (ArabicTextHandler.arabicCharTransparent(ch)) {
				...
				if (vertical) {
					...
					if (glyphOrientationAngle == 0) {
						...
						// need to translate so that the spacing
						// between chars is correct
						gv.setGlyphTransform(i, AffineTransform.getTranslateInstance(advanceX, 0));
					} else {
						// 90, 270
		...
		advance = new Point2D.Float((float) (curr_x_pos - offset.getX()), (float) (curr_y_pos - offset.getY()));
	...
	protected void adjustTextSpacing() {
		...
		aci.first();
		Boolean customSpacing = (Boolean) aci.getAttribute(GVTAttributedCharacterIterator.TextAttribute.CUSTOM_SPACING);
		if ((customSpacing != null) && customSpacing) {
			advance = doSpacing((Float) aci.getAttribute(GVTAttributedCharacterIterator.TextAttribute.KERNING),
					(Float) aci.getAttribute(GVTAttributedCharacterIterator.TextAttribute.LETTER_SPACING),
					(Float) aci.getAttribute(GVTAttributedCharacterIterator.TextAttribute.WORD_SPACING));
	 ...
	 *
	 * @param kern          The kerning adjustment to apply to the space between
	 *                      each char.
	...
	protected Point2D doSpacing(Float kern, Float letterSpacing, Float wordSpacing) {
		...
		Point2D lastCharAdvance = new Point2D.Double(advance.getX() - (gv.getGlyphPosition(numGlyphs - 1).getX() - x),
		...
		try {
			...
			if (vertical) {
				...
				lastCharAdvance.setLocation(lastCharAdvance.getX() + kernVal + letterSpacingVal,
		...
		double advX = gv.getGlyphPosition(numGlyphs - 1).getX() - gv.getGlyphPosition(0).getX();
		double advY = gv.getGlyphPosition(numGlyphs - 1).getY() - gv.getGlyphPosition(0).getY();
	 ...
	 *
	 * @param stretchGlyphs if true xScale, yScale will be applied to each glyphs
	 *                      transform.
	 */
	protected void applyStretchTransform(boolean stretchGlyphs) {
		...
		advance = new Point2D.Float((float) (advance.getX() * xScale), (float) (advance.getY() * yScale));
		// Basic layout is now messed up...
		layoutApplied = false;
	}
	...
	/**
	 * Returns true if the specified character is within one of the Latin unicode
	 * character blocks.
	 *
	...
	protected boolean isLatinChar(char c) {
		...
		if (block == Character.UnicodeBlock.BASIC_LATIN || block == Character.UnicodeBlock.LATIN_1_SUPPLEMENT
				...
				|| block == Character.UnicodeBlock.LATIN_EXTENDED_A || block == Character.UnicodeBlock.LATIN_EXTENDED_B
				|| block == Character.UnicodeBlock.ARABIC || block == Character.UnicodeBlock.ARABIC_PRESENTATION_FORMS_A
	...
	/**
	 * Returns the value of the vertical glyph orientation angle. This will be one
	 * of 0, 90, 180 or 270.
	 */
