...
public class BaseScriptingEnvironment {
	 ...
	 */
	public static boolean isDynamicElement(Element elt, BridgeContext ctx, List<BridgeExtension> bridgeExtensions) {
	 ...
	 */
	public io.sf.carte.echosvg.bridge.Window getWindow(Interpreter interp, String lang) {
		io.sf.carte.echosvg.bridge.Window w = windowObjects.get(interp);
		if (w == null) {
			w = interp == null ? new Window(null, null) : createWindow(interp, lang);
	 ...
	 */
	public io.sf.carte.echosvg.bridge.Window getWindow() {
	...
	/**
	 * Executes the specified &lt;script&gt; element, if it hasn't been executed
	 * already.
	 */
		...
		if (type.equals(SVGConstants.SVG_SCRIPT_TYPE_JAVA)) {
			try {
				...
				if (sh != null) {
					...
					initializer = (EventListenerInitializer) cll.loadClass(sh).getDeclaredConstructor().newInstance();
		...
		try {
			...
			if (href.length() > 0) {
						...
						if (read > 0) {
							...
							if (read >= 2) {
								if (buf[0] == (byte) 0xff && buf[1] == (byte) 0xfe) {
									...
									enc = "UTF-16BE";
									pbis.skip(2);
								} else if (read >= 3 && buf[0] == (byte) 0xef && buf[1] == (byte) 0xbb
										&& buf[2] == (byte) 0xbf) {
									enc = "UTF-8";
									pbis.skip(3);
								} else if (read >= 4 && buf[0] == 0 && buf[1] == 0 && buf[2] == (byte) 0xfe
										&& buf[3] == (byte) 0xff) {
									enc = "UTF-32BE";
									pbis.skip(4);
								}
				...
				desc = Messages.formatMessage(INLINE_SCRIPT_DESCRIPTION,
						new Object[] { d.getURL(), "<" + script.getNodeName() + ">", line });
				// Inline script.
	...
	/**
	 * Checks that the script URLs and the document url are compatible. A
	 * SecurityException is thrown if loading the script is not allowed.
	 */
	...
	protected class Window implements io.sf.carte.echosvg.bridge.Window {
		 ...
		 * a new document if 'doc' is null. The implementation in this class always
		 * returns 'null'
		 * 
		 ...
		 */
		@Override
		public void getURL(String uri, io.sf.carte.echosvg.bridge.Window.URLResponseHandler h, String enc) {
		}
