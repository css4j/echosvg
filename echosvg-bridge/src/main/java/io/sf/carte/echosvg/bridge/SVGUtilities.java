...
public abstract class SVGUtilities implements SVGConstants, ErrorConstants {
	...
	/**
	 * Returns the logical parent element of the given element. The parent element
	 * of a used element is the &lt;use&gt; element which reference it.
	 */
	 ...
	 * @exception NumberFormatException if the ratio is not a valid number or
	 *                                  percentage
	 */
	...
	public static String getDescription(SVGElement elt) {
		...
		if (n != null && n.getNodeType() == Node.ELEMENT_NODE) {
			...
			if (name.equals(SVG_DESC_TAG)) {
				preserve = ((SVGLangSpace) n).getXMLspace().equals(SVG_PRESERVE_VALUE);
	...
	/**
	 * Tests whether or not the specified language specification matches the user
	 * preferences.
	 *
	...
	/**
	 * Returns the value of the specified attribute specified on the specified
	 * element or one of its ancestor. Ancestors are found using the xlink:href
	 * attribute.
	 *
	...
	public static String getChainableAttributeNS(Element element, String namespaceURI, String attrName,
		...
		for (;;) {
			...
			for (ParsedURL ref : refs) {
				if (purl.equals(ref))
					throw new BridgeException(ctx, e, ERR_XLINK_HREF_CIRCULAR_DEPENDENCIES, new Object[] { uriStr });
			}
			...
			try {
				...
				throw new BridgeException(ctx, e, secEx, ERR_URI_UNSECURE, new Object[] { uriStr });
			}
		}
	}
	 ...
	 */
	public static Point2D convertPoint(String xStr, String xAttr, String yStr, String yAttr, short unitsType,
	...
	/**
	 * Returns the mask region according to the x, y, width, height, and maskUnits
	 * attributes.
	 *
	 ...
	 * @param ctx           the bridge context
	 */
	public static Rectangle2D convertMaskRegion(Element maskElement, Element maskedElement, GraphicsNode maskedNode,
			BridgeContext ctx) {
	...
	/**
	 * Returns the pattern region according to the x, y, width, height, and
	 * patternUnits attributes.
	 *
	 ...
	 * @param ctx            the bridge context
	 */
	public static Rectangle2D convertPatternRegion(Element patternElement, Element paintedElement,
			GraphicsNode paintedNode, BridgeContext ctx) {
	...
	/**
	 * Returns an array of 2 float numbers that describes the filter resolution of
	 * the specified filter element.
	 *
	...
	public static float[] convertFilterRes(Element filterElement, BridgeContext ctx) {
		...
		String s = getChainableAttributeNS(filterElement, null, SVG_FILTER_RES_ATTRIBUTE, ctx);
		Float[] vals = convertSVGNumberOptionalNumber(filterElement, SVG_FILTER_RES_ATTRIBUTE, s, ctx);
	...
	/**
	 * This function parses attrValue for a number followed by an optional second
	 * Number. It always returns an array of two Floats. If either or both values
	 * are not provided the entries are set to null
	 */
	public static Float[] convertSVGNumberOptionalNumber(Element elem, String attrName, String attrValue,
			BridgeContext ctx) {
		...
		try {
			...
			if (tokens.hasMoreTokens()) {
				throw new BridgeException(ctx, elem, ERR_ATTRIBUTE_VALUE_MALFORMED,
						new Object[] { attrName, attrValue });
			}
		} catch (NumberFormatException nfEx) {
			throw new BridgeException(ctx, elem, nfEx, ERR_ATTRIBUTE_VALUE_MALFORMED,
	...
	/**
	 * Returns the filter region according to the x, y, width, height, dx, dy, dw,
	 * dh and filterUnits attributes.
	 *
	 ...
	 * @param ctx             the bridge context
	 */
	public static Rectangle2D convertFilterChainRegion(Element filterElement, Element filteredElement,
		 ...
		 * ctx.getSVGContext(filteredElement); if (unitsType == OBJECT_BOUNDING_BOX &&
		 * bridge != null) { Rectangle2D bbox = bridge.getBBox(); if (bbox != null &&
		 * bbox.getWidth() == 0 || bbox.getHeight() == 0) { return null; } }
		 */
		...
		if (units.length() == 0) {
			...
			unitsType = parseCoordinateSystem(filterElement, SVG12Constants.SVG_FILTER_MARGINS_UNITS_ATTRIBUTE, units,
					ctx);
		}
	...
	/**
	 * Returns a rectangle that represents the region extended by the specified
	 ...
	 * @param unitsType specifies whether the values are in userSpaceOnUse or
	...
	protected static Rectangle2D extendRegion(String dxStr, String dyStr, String dwStr, String dhStr, short unitsType,
		...
		case USER_SPACE_ON_USE:
			...
			dy = UnitProcessor.svgVerticalCoordinateToUserSpace(dyStr, SVG12Constants.SVG_MY_ATRIBUTE, uctx);
			dw = UnitProcessor.svgHorizontalCoordinateToUserSpace(dwStr, SVG12Constants.SVG_MW_ATRIBUTE, uctx);
			dh = UnitProcessor.svgVerticalCoordinateToUserSpace(dhStr, SVG12Constants.SVG_MH_ATRIBUTE, uctx);
			break;
		case OBJECT_BOUNDING_BOX:
			...
			if (bounds == null) {
				...
				dx = UnitProcessor.svgHorizontalCoordinateToObjectBoundingBox(dxStr, SVG12Constants.SVG_MX_ATRIBUTE,
						uctx);
				dx *= bounds.getWidth();

				dy = UnitProcessor.svgVerticalCoordinateToObjectBoundingBox(dyStr, SVG12Constants.SVG_MY_ATRIBUTE,
						uctx);
				dy *= bounds.getHeight();

				dw = UnitProcessor.svgHorizontalCoordinateToObjectBoundingBox(dwStr, SVG12Constants.SVG_MW_ATRIBUTE,
						uctx);
				dw *= bounds.getWidth();

				dh = UnitProcessor.svgVerticalCoordinateToObjectBoundingBox(dhStr, SVG12Constants.SVG_MH_ATRIBUTE,
						uctx);
				dh *= bounds.getHeight();
			}
			break;
	...
	public static Rectangle2D getBaseFilterPrimitiveRegion(Element filterPrimitiveElement, Element filteredElement,
			GraphicsNode filteredNode, Rectangle2D defaultRegion, BridgeContext ctx) {
		String s;
	...
	/**
	 * Returns the filter primitive region according to the x, y, width, height, and
	 * filterUnits attributes. Processing the element as the top one in the filter
	 * chain.
	 *
	 ...
	 * @param ctx                    the bridge context
	 */
	public static Rectangle2D convertFilterPrimitiveRegion(Element filterPrimitiveElement, Element filterElement,
			Element filteredElement, GraphicsNode filteredNode, Rectangle2D defaultRegion, Rectangle2D filterRegion,
			BridgeContext ctx) {
		...
		if (filterPrimitiveElement != null) {
			// 'x' attribute - default is defaultRegion.getX()
			xStr = filterPrimitiveElement.getAttributeNS(null, SVG_X_ATTRIBUTE);

			// 'y' attribute - default is defaultRegion.getY()
			yStr = filterPrimitiveElement.getAttributeNS(null, SVG_Y_ATTRIBUTE);

			// 'width' attribute - default is defaultRegion.getWidth()
			wStr = filterPrimitiveElement.getAttributeNS(null, SVG_WIDTH_ATTRIBUTE);

			// 'height' attribute - default is defaultRegion.getHeight()
			hStr = filterPrimitiveElement.getAttributeNS(null, SVG_HEIGHT_ATTRIBUTE);
		}
		...
		// settings (EchoSVG extension). The dx/dy/dw/dh padding is
		// *always* in userSpaceOnUse space.

		units = "";
		if (filterElement != null) {
			units = getChainableAttributeNS(filterElement, null,
		...
		if (units.length() == 0) {
			...
			unitsType = parseCoordinateSystem(filterElement,
					SVG12Constants.SVG_FILTER_PRIMITIVE_MARGINS_UNITS_ATTRIBUTE, units, ctx);
		}
		...
		if (filterPrimitiveElement != null) {
			// 'dx' attribute - default is 0
			dxStr = filterPrimitiveElement.getAttributeNS(null, SVG12Constants.SVG_MX_ATRIBUTE);

			// 'dy' attribute - default is 0
			dyStr = filterPrimitiveElement.getAttributeNS(null, SVG12Constants.SVG_MY_ATRIBUTE);

			// 'dw' attribute - default is 0
			dwStr = filterPrimitiveElement.getAttributeNS(null, SVG12Constants.SVG_MW_ATRIBUTE);
	 ...
	 * @param ctx                    the bridge context
	 */
	public static Rectangle2D convertFilterPrimitiveRegion(Element filterPrimitiveElement, Element filteredElement,
			GraphicsNode filteredNode, Rectangle2D defaultRegion, Rectangle2D filterRegion, BridgeContext ctx) {
		...
		return convertFilterPrimitiveRegion(filterPrimitiveElement, filterElement, filteredElement, filteredNode,
				defaultRegion, filterRegion, ctx);
	}
	...
	public static short parseCoordinateSystem(Element e, String attr, String coordinateSystem, BridgeContext ctx) {
		if (SVG_USER_SPACE_ON_USE_VALUE.equals(coordinateSystem)) {
			...
			throw new BridgeException(ctx, e, ERR_ATTRIBUTE_VALUE_MALFORMED, new Object[] { attr, coordinateSystem });
		}
	}

	/**
	 * Parses the specified coordinate system defined by the specified marker
	 * element.
	 *
	 * @param e                the element that defines the coordinate system
	 * @param attr             the attribute which contains the coordinate system
	...
	/**
	 * Returns a rectangle that represents the region defined by the specified
	 * coordinates.
	 *
	 ...
	 * @param uctx       the unit processor context (needed for userSpaceOnUse)
	 */
	protected static Rectangle2D convertRegion(String xStr, String yStr, String wStr, String hStr, short unitsType,
			GraphicsNode targetNode, UnitProcessor.Context uctx) {
	...
	/**
	 * Returns an AffineTransform to move to the objectBoundingBox coordinate
	 * system.
	 *
	 * @param Tx   the original transformation
	 * @param node the graphics node that defines the coordinate system to move into
	 */
	...
	/**
	 * Returns the specified a Rectangle2D move to the objectBoundingBox coordinate
	 * system of the specified graphics node.
	 *
	...
	public static Rectangle2D toObjectBBox(Rectangle2D r, GraphicsNode node) {
		...
		Rectangle2D bounds = node.getGeometryBounds();
		if (bounds != null) {
			return new Rectangle2D.Double(bounds.getX() + r.getX() * bounds.getWidth(),
					bounds.getY() + r.getY() * bounds.getHeight(), r.getWidth() * bounds.getWidth(),
	...
	/**
	 * Returns the value of the 'snapshotTime' attribute on the specified element as
	 * a float, or <code>0f</code> if the attribute is missing or given as
	 * <code>"none"</code>.
	 *
	...
	public static float convertSnapshotTime(Element e, BridgeContext ctx) {
		...
		try {
			...
			throw new BridgeException(null, e, pEx, ERR_ATTRIBUTE_VALUE_MALFORMED,
