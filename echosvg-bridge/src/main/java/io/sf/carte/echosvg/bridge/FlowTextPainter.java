...
import io.sf.carte.echosvg.gvt.flow.GlyphGroupInfo;
import io.sf.carte.echosvg.gvt.flow.LineInfo;
import io.sf.carte.echosvg.gvt.flow.RegionInfo;
import io.sf.carte.echosvg.gvt.flow.TextLineBreaks;
import io.sf.carte.echosvg.gvt.flow.WordInfo;
...
public class FlowTextPainter extends StrokingTextPainter {
	 ...
	 */
	private static FlowTextPainter singleton = new FlowTextPainter();

	/**
	 * Returns a unique instance of this class.
	 */
	public static FlowTextPainter getInstance() {
		return singleton;
	}
	...
	public static boolean textWrap(AttributedCharacterIterator[] acis, List<List<TextSpanLayout>> chunkLayouts,
			List<RegionInfo> flowRects, FontRenderContext frc) {
	...
	/**
	 * This returns an array of glyphs numbers for each glyph group in each word:
	 * ret[word][glyphGroup][glyphNum].
	 */
	static WordInfo[] doWordAnalysis(GVTGlyphVector gv, AttributedCharacterIterator aci, int numWords,
		...
		for (int i = 0; i < words; i++) {
			...
			if (cnt == 1) {
				...
				wordGlyphGroups[0] = new GlyphGroupInfo(gv, start, end, hide, hideLast[end], glyphPos, advAdj,
						lastAdvAdj, space);
			} else {
				int glyphGroup = 0;
				int[] glyphs = wordGlyphs[i];
				...
				for (int j = 1; j < glyphs.length; j++) {
					if (prev + 1 != glyphs[j]) {
						int end = glyphs[j - 1];
						wordGlyphGroups[glyphGroup] = new GlyphGroupInfo(gv, start, end, hide, hideLast[end], glyphPos,
								advAdj, lastAdvAdj, space);
						start = glyphs[j];
						glyphGroup++;
					}
					prev = glyphs[j];
				...
				int end = glyphs[glyphs.length - 1];
				wordGlyphGroups[glyphGroup] = new GlyphGroupInfo(gv, start, end, hide, hideLast[end], glyphPos, advAdj,
						lastAdvAdj, space);
			}
