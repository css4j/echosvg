...
public class CSSFontFace extends FontFace implements SVGConstants {
	 ...
	 */
	public CSSFontFace(List<Object> srcs, String familyName, float unitsPerEm, String fontWeight, String fontStyle,
	...
	public static CSSFontFace createCSSFontFace(CSSEngine eng, FontFaceRule ffr) {
		...
		String familyName = getStringProp(sm, eng, SVGCSSEngine.FONT_FAMILY_INDEX);
		...
		/*
		 * float unitsPerEm = getFloatProp (sm, eng, SVGCSSEngine.UNITS_PER_EM_INDEX);
		 * String slope = getFloatProp (sm, eng, SVGCSSEngine.SLOPE_INDEX); String
		 * panose1 = getStringProp (sm, eng, SVGCSSEngine.PANOSE1_INDEX); String ascent
		 * = getFloatProp (sm, eng, SVGCSSEngine.ASCENT_INDEX); String descent =
		 * getFloatProp (sm, eng, SVGCSSEngine.DESCENT_INDEX); String
		 * strikethroughPosition = getFloatProp (sm, eng,
		 * SVGCSSEngine.STRIKETHROUGH_POSITION_INDEX); String strikethroughThickness =
		 * getFloatProp (sm, eng, SVGCSSEngine.STRIKETHROUGH_THICKNESS_INDEX); String
		 * underlinePosition = getFloatProp (sm, eng,
		 * SVGCSSEngine.UNDERLINE_POSITION_INDEX); String underlineThickness =
		 * getFloatProp (sm, eng, SVGCSSEngine.UNDERLINE_THICKNESS_INDEX); String
		 * overlinePosition = getFloatProp (sm, eng,
		 * SVGCSSEngine.OVERLINE_POSITION_INDEX); String overlineThickness =
		 * getFloatProp (sm, eng, SVGCSSEngine.OVERLINE_THICKNESS_INDEX);
		 */
		return ret;
	}
