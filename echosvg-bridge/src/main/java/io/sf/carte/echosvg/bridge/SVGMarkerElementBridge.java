	...
	public Marker createMarker(BridgeContext ctx, Element markerElement, Element paintedElement) {
		...
		if (s.length() == 0) {
			...
			try {
				...
				throw new BridgeException(ctx, markerElement, nfEx, ERR_ATTRIBUTE_VALUE_MALFORMED,
						new Object[] { SVG_ORIENT_ATTRIBUTE, s });
			}
		}

		// 'stroke-width' property
		Value val = CSSUtilities.getComputedStyle(paintedElement, SVGCSSEngine.STROKE_WIDTH_INDEX);
		...
		AffineTransform preserveAspectRatioTransform = ViewBox.getPreserveAspectRatioTransform(markerElement,
				markerWidth, markerHeight, ctx);
		...
		if (CSSUtilities.convertOverflow(markerElement)) { // overflow:hidden
			...
			if (offsets == null) { // clip:auto
				markerClip = new Rectangle2D.Float(0, 0, strokeWidth * markerWidth, strokeWidth * markerHeight);
			} else { // clip:rect(<x>, <y>, <w>, <h>)
				// offsets[0] = top
				// offsets[1] = right
				// offsets[2] = bottom
				// offsets[3] = left
				markerClip = new Rectangle2D.Float(offsets[3], offsets[0],
