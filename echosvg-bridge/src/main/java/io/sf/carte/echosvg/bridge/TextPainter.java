...
public interface TextPainter {
	...
	/**
	 * Initiates a text selection on a particular AttributedCharacterIterator, using
	 * the text/font metrics employed by this TextPainter instance.
	 */
	Mark selectAt(double x, double y, TextNode node);

	/**
	 * Continues a text selection on a particular AttributedCharacterIterator, using
	 * the text/font metrics employed by this TextPainter instance.
	 */
	 ...
	 * Leading edge indicates if the mark should be considered immediately 'before'
	 * glyph or after
	 */
	 ...
	 *
	 * Note that the instances of Mark passed to this function <em>must come</em>
	 * from the same TextPainter that generated them via selectAt() and selectTo(),
	 * since the TextPainter implementation may rely on hidden implementation
	 * details of its own Mark implementation.
	 */
	int[] getSelected(Mark start, Mark finish);

	/**
	 * Get a Shape in userspace coords which encloses the textnode glyphs bounded by
	 * two Marks. Note that the instances of Mark passed to this function <em>must
	 * come</em> from the same TextPainter that generated them via selectAt() and
	 ...
	 * rendered bounds (includes stroke etc).
	 * 
	 * @param node the TextNode to measure
	 */
