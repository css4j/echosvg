...
import io.sf.carte.echosvg.ext.awt.image.renderable.GaussianBlurRable8Bit;
...
public class SVGFeGaussianBlurElementBridge extends AbstractSVGFilterPrimitiveElementBridge {
	...
	public Filter createFilter(BridgeContext ctx, Element filterElement, Element filteredElement,
		...
		if (stdDeviationXY[0] < 0 || stdDeviationXY[1] < 0) {
			throw new BridgeException(ctx, filterElement, ERR_ATTRIBUTE_VALUE_MALFORMED, new Object[] {
					SVG_STD_DEVIATION_ATTRIBUTE, String.valueOf(stdDeviationXY[0]) + stdDeviationXY[1] });
		}
	...
	/**
	 * Returns the standard deviation of the specified feGaussianBlur filter
	 * primitive element.
	 *
