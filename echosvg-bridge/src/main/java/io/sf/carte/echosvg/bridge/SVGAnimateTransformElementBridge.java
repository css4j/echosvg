...
public class SVGAnimateTransformElementBridge extends SVGAnimateElementBridge {
	...
	protected AbstractAnimation createAnimation(AnimationTarget target) {
		...
		if (element.hasAttributeNS(null, SVG_FROM_ATTRIBUTE)) {
			from = parseValue(element.getAttributeNS(null, SVG_FROM_ATTRIBUTE), type, target);
		}
		if (element.hasAttributeNS(null, SVG_TO_ATTRIBUTE)) {
			to = parseValue(element.getAttributeNS(null, SVG_TO_ATTRIBUTE), type, target);
		}
		if (element.hasAttributeNS(null, SVG_BY_ATTRIBUTE)) {
			by = parseValue(element.getAttributeNS(null, SVG_BY_ATTRIBUTE), type, target);
		}
		return new TransformAnimation(timedElement, this, parseCalcMode(), parseKeyTimes(), parseKeySplines(),
				parseAdditive(), parseAccumulate(), parseValues(type, target), from, to, by, type);
	}
	...
	protected AnimatableValue parseValue(String s, short type, AnimationTarget target) {
		...
		int j = i;
		if (i < len && type != SVGTransform.SVG_TRANSFORM_SKEWX && type != SVGTransform.SVG_TRANSFORM_SKEWY) {
			while (i < len) {
				c = s.charAt(i);
				if (c == ' ' || c == ',') {
					break;
