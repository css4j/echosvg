 ...
 * the SVG namespace. Override the <code>getNamespaceURI</code> if you want to
 * add custom <code>GraphicsNode</code> with a custom namespace.
 *
 * <p>
 * This class handles various attributes that are defined on most of the SVG
 * graphic elements as described in the SVG specification.
 * </p>
 *
 * <ul>
 * <li>clip-path</li>
...
public abstract class AbstractGraphicsNodeBridge extends AnimatableSVGBridge
	...
	/**
	 * Associates the {@link SVGContext} with the element. This method should be
	 * called even for static documents, since some bridges will need to access
	 * animated attribute values even during the first build.
	 */
	 ...
	 * responsibility of this method is to ensure that any dynamic modifications of
	...
	protected void fireShapeChangeEvent() {
		...
		evt.initEventNS(SVG_NAMESPACE_URI, "shapechange", true, false);
		try {
	...
	/**
	 * Disposes all resources related to the specified node and its subtree, and
	 * optionally removes the nodes' {@link SVGContext}.
	 */
	...
	public void handleAnimatedAttributeChanged(AnimatedLiveAttributeValue alav) {
		if (alav.getNamespaceURI() == null && alav.getLocalName().equals(SVG_TRANSFORM_ATTRIBUTE)) {
			setTransform(node, e, ctx);
	...
	/**
	 * Checks if the bounding box of the node has changed, and if so, fires a
	 * bboxchange event on the element.
	 */
	protected void checkBBoxChange() {
		if (e != null) {
			/*
			 * Rectangle2D oldBBox = bbox; Rectangle2D newBBox = getBBox(); if (oldBBox !=
			 * newBBox && newBBox != null) { if (oldBBox == null || oldBBox.getX() !=
			 * bbox.getX() || oldBBox.getY() != bbox.getY() || oldBBox.getWidth() !=
			 * bbox.getWidth() || oldBBox.getHeight() != bbox.getHeight()) {
			 */
			fireBBoxChangeEvent();
			/*
			 * } }
	...
	protected void fireBBoxChangeEvent() {
		DocumentEvent d = (DocumentEvent) e.getOwnerDocument();
		AbstractEvent evt = (AbstractEvent) d.createEvent("SVGEvents");
		evt.initEventNS(SVG_NAMESPACE_URI, "RenderedBBoxChange", true, false);
		try {
			((EventTarget) e).dispatchEvent(evt);
		} catch (RuntimeException ex) {
			ctx.getUserAgent().displayError(ex);
	...
	public Rectangle2D getBBox() {
		...
		if ((bboxShape != null) && (s == bboxShape.get()))
			return bbox;
