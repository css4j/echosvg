...
public class UpdateManager {
	...
	/**
	 * Array of resource documents' ScriptingEnvironments that should have their
	 * SVGLoad event dispatched.
	 */
	...
	public UpdateManager(BridgeContext ctx, GraphicsNode gn, Document doc) {
		...
		secondaryBridgeContexts = ctx.getChildContexts().clone();
		secondaryScriptingEnvironments = new ScriptingEnvironment[secondaryBridgeContexts.length];
	...
	/**
	 * Creates an appropriate ScriptingEnvironment and XBL manager for the given
	 * document.
	 */
	...
	/**
	 * Dispatches an 'SVGUnLoad' event to the document. This method interrupts the
	 * update manager threads. NOTE: this method must be called outside the update
	 * thread.
	 */
	public void dispatchSVGUnLoadEvent() {
		...
		updateRunnableQueue.preemptLater(new Runnable() {
			...
			public void run() {
				synchronized (UpdateManager.this) {
					AbstractEvent evt = (AbstractEvent) ((DocumentEvent) document).createEvent("SVGEvents");
					String type;
					...
					evt.initEventNS(XMLConstants.XML_EVENTS_NAMESPACE_URI, type, false, // canBubbleArg
							false); // cancelableArg
					((EventTarget) (document.getDocumentElement())).dispatchEvent(evt);
					running = false;
	 ...
	 * @param u2d    The user to device transform.
	 * @param dbr    Whether the double buffering should be used.
	 * @param cpt    If the canvas painting transform should be cleared when the
	 *               update complets
	 * @param aoi    The area of interest in the renderer space units.
	 * @param width  The offscreen buffer width.
	 * @param height The offscreen buffer height.
	 ...
	 * 
	 * @param areas                  List of areas of interest in rederer space
	 *                               units.
	 * @param clearPaintingTransform Indicates if the painting transform should be
	 *                               cleared as a result of this update.
	 */
	protected void updateRendering(List<Shape> areas, boolean clearPaintingTransform) {
		try {
			UpdateManagerEvent ev = new UpdateManagerEvent(this, repaintManager.getOffScreen(), null);
			...
			ev = new UpdateManagerEvent(this, repaintManager.getOffScreen(), l, clearPaintingTransform);
	...
	/**
	 * This tracks when the rendering first got 'out of date' with respect to the
	 * document.
	 */
	...
	/**
	 * Users of EchoSVG should essentially never call this directly from Java. If
	 * the Canvas is not updating when you change the SVG Document it is almost
	 * certainly because you are not making your changes in the RunnableQueue
	 * (getUpdateRunnableQueue()). You will have problems if you are not making all
	 * changes to the document in the UpdateManager's RunnableQueue.
	 *
	 * This method exists to implement the 'SVGSVGElement.forceRedraw()' method.
	 */
	...
	protected static class SuspensionInfo {
		...
		int index;
		/**
		 * The system time in millisec that this suspension will expire and redraws can
		 * resume (at least for this suspension.
		 */
		long resumeMilli;
	...
	/**
	 * Sets up a timer that will trigger a repaint when it fires. If create is true
	...
	int addRedrawSuspension(int max_wait_milliseconds) {
		...
		SuspensionInfo si = new SuspensionInfo(nextSuspensionIndex++, resumeTime);
