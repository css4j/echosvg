...
public class SVGPatternElementBridge extends AnimatableGenericSVGBridge implements PaintBridge, ErrorConstants {
	 ...
	 */
	@Override
	public Paint createPaint(BridgeContext ctx, Element patternElement, Element paintedElement,
			GraphicsNode paintedNode, float opacity) {

		// extract pattern content
		RootGraphicsNode patternContentNode;
		patternContentNode = (RootGraphicsNode) ctx.getElementData(patternElement);
		...
		if (viewBoxStr.length() > 0) {
			...
			String aspectRatioStr = SVGUtilities.getChainableAttributeNS(patternElement, null,
					SVG_PRESERVE_ASPECT_RATIO_ATTRIBUTE, ctx);
		...
		if (opacity != 1) {
			...
			filter = new ComponentTransferRable8Bit(filter,
					ConcreteComponentTransferFunction.getLinearTransfer(opacity, 0), // alpha
	...
	/**
	 * Returns the content of the specified pattern element. The content of the
	 * pattern can be specified as children of the patternElement or children of one
	 * of its 'ancestor' (linked with the xlink:href attribute).
	 *
