 ...
 * nodes, graphic contexts such as a <code>GraphicsNodeRenderContext</code>, and
 * the different objects required by the GVT builder to interpret a SVG DOM tree
 * such as the current viewport or the user agent.
 *
...
public class BridgeContext implements ErrorConstants, CSSContext, Closeable {
	...
	/**
	 * The interpreter cache per document. key is the language - value is a
	 * Interpreter
	 */
	protected Map<String, Interpreter> interpreterMap = new HashMap<>(7);

	/**
	 * A Map of all the font families already matched. This is to reduce the number
	 * of instances of GVTFontFamilies and to hopefully reduce the time taken to
	 * search for a matching SVG font.
	 */
	...
	/**
	 * Bridge Map: Keys are namespace URI - values are HashMap (with keys are local
	 * name and values are a Bridge instance).
	 */
	protected Map<String, HashMap<String, Bridge>> namespaceURIMap;

	/**
	 * Default bridge. When a bridge is requested for an element type that does not
	 * have a bridge, and there is no other bridge for elements in the same
	 * namespace, the default bridge is returned. This is used for custom elements,
	 * which all use the same bridge type.
	 */
	protected Bridge defaultBridge;

	/**
	 * Default bridge reserved namespaces set. Default bridges will not be created
	 * for elements that have a namespace URI present in this set.
	 */
	protected Set<String> reservedNamespaceSet;

	/**
	 * Element Data Map: This is a general location for elements to 'cache' data.
	 * Such as the graphics tree for a pattern or the Gradient arrays.
	 *
	 * This is a weak hash map and the data is referenced by SoftReference so both
	 * must be referenced elsewhere to stay live.
	 */
	...
	/**
	 * The bridge context for the primary document, if this is a bridge context for
	 * a resource document.
	 */
	...
	/**
	 * This function creates a new 'sub' BridgeContext to associated with 'newDoc'
	 * if one currently doesn't exist, otherwise it returns the BridgeContext
	 * currently associated with the document.
	 * 
	...
	/**
	 * This function creates a new BridgeContext, it mostly exists so subclasses can
	 * provide an instance of themselves when a sub BridgeContext is needed.
	 */
	...
	/**
	 * Sets the text painter that will be used by text nodes. This attributes might
	 * be used by bridges (especially SVGTextElementBridge) to set the text painter
	 * of each TextNode.
	 *
	...
	/**
	 * Sets the document this bridge context is dedicated to, to the specified
	 * document.
	 * 
	...
	/**
	 * Associates a data object with a node so it can be retrieved later. This is
	 * primarily used for caching the graphics node generated from a 'pattern'
	 * element. A soft reference to the data object is used.
	 */
	...
	/**
	 * Sets the interpreter pool used to handle scripts to the specified interpreter
	 * pool.
	 * 
	 * @param interpreterPool the interpreter pool
	 */
	...
	/**
	 * Returns the actual size of the document or null if the document has not been
	 * built yet.
	 */
	...
	/**
	 * Sets the document as a STATIC, INTERACTIVE or DYNAMIC document. Call this
	 * method before the build phase (ie. before <code>gvtBuilder.build(...)</code>)
	...
	/**
	 * Sets the document as DYNAMIC if <code>dynamic</code> is true STATIC
	 * otherwise.
	 */
	...
	/**
	 * Sets the document as INTERACTIVE if <code>interactive</code> is true STATIC
	 * otherwise.
	 */
	 ...
	 * The referenced node can be either an element given by a fragment ID, or the
	 * document node.
	 * 
	 ...
	 * The referenced element can not be a Document.
	 *
	 * @param e   the element referencing
	 ...
	 * automatically bind the graphics node to the element and the element to the
	 * graphics node.
	 *
	...
	/**
	 * Returns true if the specified element has a GraphicsNodeBridge associated to
	 * it, false otherwise.
	 *
	...
	/**
	 * Associates the specified <code>Bridge</code> object with it's namespace URI
	 * and local name.
	 *
	 * @param bridge the bridge that manages the element
	 */
	...
	/**
	 * Removes the <code>Bridge</code> object associated to the specified namespace
	 * URI and local name.
	 *
	 * @param namespaceURI the namespace URI
	 * @param localName    the local name
	...
	/**
	 * Sets the <code>Bridge</code> object to be used for foreign namespace
	 * elements.
	 *
	 * @param bridge the bridge that manages the element
	 */
	...
	/**
	 * The list of all EventListener attached by bridges that need to be removed on
	 * a dispose() call.
	 */
	...
	/**
	 * Adds EventListeners to the input document to handle the cursor property. This
	 * is not done in the addDOMListeners method because addDOMListeners is only
	 * used for dynamic content whereas cursor support is provided for all content.
	 * Also note that it is very important that the listeners be registered for the
	 * capture phase as the 'default' behavior for cursors is handled by the
	 * BridgeContext during the capture phase and the 'custom' behavior (handling of
	 * 'auto' on anchors, for example), is handled during the bubbling phase.
	 */
	public void addUIEventListeners(Document doc) {
		...
		evtTarget.addEventListenerNS(XMLConstants.XML_EVENTS_NAMESPACE_URI, SVGConstants.SVG_EVENT_MOUSEOVER,
				domMouseOverListener, true, null);
		storeEventListenerNS(evtTarget, XMLConstants.XML_EVENTS_NAMESPACE_URI, SVGConstants.SVG_EVENT_MOUSEOVER,
				domMouseOverListener, true);

		DOMMouseOutEventListener domMouseOutListener = new DOMMouseOutEventListener();
		evtTarget.addEventListenerNS(XMLConstants.XML_EVENTS_NAMESPACE_URI, SVGConstants.SVG_EVENT_MOUSEOUT,
				domMouseOutListener, true, null);
		storeEventListenerNS(evtTarget, XMLConstants.XML_EVENTS_NAMESPACE_URI, SVGConstants.SVG_EVENT_MOUSEOUT,
	...
	/**
	 * Adds EventListeners to the DOM and CSSEngineListener to the CSSEngine to
	 * handle any modifications on the DOM tree or style properties and update the
	 * GVT tree in response.
	 */
	public void addDOMListeners() {
		SVGOMDocument doc = (SVGOMDocument) document;

		domAttrModifiedEventListener = new DOMAttrModifiedEventListener();
		doc.addEventListenerNS(XMLConstants.XML_EVENTS_NAMESPACE_URI, "DOMAttrModified", domAttrModifiedEventListener,
				true, null);

		domNodeInsertedEventListener = new DOMNodeInsertedEventListener();
		doc.addEventListenerNS(XMLConstants.XML_EVENTS_NAMESPACE_URI, "DOMNodeInserted", domNodeInsertedEventListener,
				true, null);

		domNodeRemovedEventListener = new DOMNodeRemovedEventListener();
		doc.addEventListenerNS(XMLConstants.XML_EVENTS_NAMESPACE_URI, "DOMNodeRemoved", domNodeRemovedEventListener,
				true, null);

		domCharacterDataModifiedEventListener = new DOMCharacterDataModifiedEventListener();
		doc.addEventListenerNS(XMLConstants.XML_EVENTS_NAMESPACE_URI, "DOMCharacterDataModified",
	...
	protected void removeDOMListeners() {
		...
		doc.removeEventListenerNS(XMLConstants.XML_EVENTS_NAMESPACE_URI, "DOMAttrModified",
				domAttrModifiedEventListener, true);
		doc.removeEventListenerNS(XMLConstants.XML_EVENTS_NAMESPACE_URI, "DOMNodeInserted",
				domNodeInsertedEventListener, true);
		doc.removeEventListenerNS(XMLConstants.XML_EVENTS_NAMESPACE_URI, "DOMNodeRemoved", domNodeRemovedEventListener,
				true);
		doc.removeEventListenerNS(XMLConstants.XML_EVENTS_NAMESPACE_URI, "DOMCharacterDataModified",
	...
	/**
	 * Clears the list of child BridgeContexts and disposes them if there are no
	...
	/**
	 * Returns the SVGContext associated to the specified Node or null if there is
	 * none.
	 */
	...
	/**
	 * Returns the BridgeUpdateHandler associated to the specified Node or null if
	 * there is none.
	 */
	...
	/**
	 * The CSSEngineListener invoked when CSS properties are modified on a
	 * particular element.
	 */
	protected class CSSPropertiesChangedListener implements CSSEngineListener {
		...
		/**
		 * Handles CSSEngineEvent that describes the CSS properties that have changed on
	...
	protected class AnimatedAttrListener implements AnimatedAttributeListener {
		...
		/**
		 * Called to notify an object of a change to the animated value of an animated
		 * XML attribute.
		 * 
	...
	public Value getDefaultFontFamily() {
		...
		return doc.getCSSEngine().parsePropertyValue(root, CSSConstants.CSS_FONT_FAMILY_PROPERTY, str);
	}
	...
	/**
	 * Tells whether the given SVG document is Interactive. We say it is, if it has
	 * any &lt;title&gt;, &lt;desc&gt;, or &lt;a&gt; elements, of if the 'cursor'
	 * property is anything but Auto on any element.
	 */
	...
	/**
	 * used by isInteractiveDocument to check if document contains any 'interactive'
	 * elements.
	 */
	...
	public boolean checkInteractiveElement(SVGDocument doc, Element e) {
		...
		/*
		 * We would like to do this but the CSS Engine isn't setup when we want to do
		 * this.
		 * 
