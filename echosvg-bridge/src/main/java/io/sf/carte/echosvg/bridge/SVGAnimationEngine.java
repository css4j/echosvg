...
public class SVGAnimationEngine extends AnimationEngine {
	...
	/**
	 * Whether animation processing has started. This affects whether animation
	 * element bridges add their animation on to the initial bridge list, or process
	 * them immediately.
	 */
	...
	/**
	 * The initial time that will be seeked to when the animation engine starts, as
	 * set by {@link #setCurrentTime}.
	 */
	 ...
	 */
	protected Factory[] factories = { null, // TYPE_UNKNOWN
	...
	static {
		String[] eventNamesCommon = { "click", "mousedown", "mouseup", "mouseover", "mousemove", "mouseout",
				"beginEvent", "endEvent" };
		String[] eventNamesSVG11 = { "DOMSubtreeModified", "DOMNodeInserted", "DOMNodeRemoved",
				"DOMNodeRemovedFromDocument", "DOMNodeInsertedIntoDocument", "DOMAttrModified",
				"DOMCharacterDataModified", "SVGLoad", "SVGUnload", "SVGAbort", "SVGError", "SVGResize", "SVGScroll",
				"repeatEvent" };
		String[] eventNamesSVG12 = { "load", "resize", "scroll", "zoom" };
	...
	/**
	 * Adds an animation element bridge to the list of bridges that require
	 * initializing when the document is started.
	 */
	 ...
	 */
	public AnimatableValue parseAnimatableValue(Element animElt, AnimationTarget target, String ns, String ln,
			boolean isCSS, String s) {
		...
		if (factory == null) {
			...
			throw new BridgeException(ctx, animElt, "attribute.not.animatable",
	...
	public void start(long documentStartTime) {
		...
		try {
			try {
				...
				throw new BridgeException(ctx, ex.getElement().getElement(), ex.getMessage());
			}
		} catch (Exception ex) {
	...
	protected class AnimationRoot extends TimedDocumentRoot {
		...
		protected String getEventNamespaceURI(String eventName) {
			...
			if (eventName.equals("focusin") || eventName.equals("focusout") || eventName.equals("activate")
		...
		/**
		 * Returns the type of the event that corresponds to the given animation event
		 * name.
		 */
		@Override
		...
		protected void fireTimeEvent(String eventType, Calendar time, int detail) {
			AnimationSupport.fireTimeEvent((EventTarget) document, eventType, time, detail);
		}
	...
	protected static class AnimationTickRunnable implements RunnableQueue.IdleRunnable {
		...
		/**
		 * Calendar instance used for passing current time values to the animation
		 * timing system.
		 */
...
//         protected int frames;
		...
		/**
		 * The number of milliseconds to wait until the next animation tick. This is
		 * returned by {@link #getWaitTime()}.
		 */
		...
		/**
		 * The number of past tick times to keep, for computing the average time per
		 * tick.
		 */
		...
		/**
		 * The number of consecutive exceptions that have been thrown. This is used to
		 * detect when exceptions are occurring every tick, and to stop reporting them
		 * when this happens.
		 */
		...
		/**
		 * Forces an animation update, if the {@link RunnableQueue} is currently
		 * waiting.
		 */
		public void resume() {
			waitTime = 0;
...
//                         frames++;
	...
	/**
	 * Factory class for AnimatableValues for CSS properties. XXX Shorthand
	...
	protected abstract class CSSValueFactory implements Factory {
		...
		/**
		 * Creates a new AnimatableValue from a CSS {@link Value}, after computation and
		 * inheritance.
		 */
		...
		/**
		 * Computes a CSS {@link Value} and performance inheritance if the specified
		 * value is 'inherit'.
		 */
		protected Value computeValue(CSSStylableElement elt, String pn, Value v) {
			...
			if (idx != -1) {
				...
				v = vms[idx].computeValue(elt, null, cssEngine, idx, dummyStyleMap, v);
			}
			return v;
		}
	}
	...
	protected static class AnimatablePreserveAspectRatioValueFactory implements Factory {
		...
		public AnimatableValue createValue(AnimationTarget target, String ns, String ln, boolean isCSS, String s) {
			try {
				parser.parse(s);
				return new AnimatablePreserveAspectRatioValue(target, align, meetOrSlice);
		...
		/**
		 * Creates a new AnimatableValue from a CSS {@link Value}. Returns null since
		 * preserveAspectRatio values aren't used in CSS values.
		 */
		@Override
		public AnimatableValue createValue(AnimationTarget target, String pn, Value v) {
			return null;
	...
	protected static class AnimatableLengthListValueFactory implements Factory {
		...
		/**
		 * Creates a new AnimatableValue from a CSS {@link Value}. Returns null since
		 * point lists aren't used in CSS values.
		 */
		@Override
		public AnimatableValue createValue(AnimationTarget target, String pn, Value v) {
			return null;
	...
	protected static class AnimatableNumberListValueFactory implements Factory {
		...
		public AnimatableValue createValue(AnimationTarget target, String ns, String ln, boolean isCSS, String s) {
			try {
				parser.parse(s);
				return new AnimatableNumberListValue(target, producer.getFloatArray());
			} catch (ParseException e) {
				// XXX Do something better than returning null.
				return null;
			}
		...
		/**
		 * Creates a new AnimatableValue from a CSS {@link Value}. Returns null since
		 * number lists aren't used in CSS values.
		 */
		@Override
		public AnimatableValue createValue(AnimationTarget target, String pn, Value v) {
			return null;
	...
	protected static class AnimatableRectValueFactory implements Factory {
		...
		/**
		 * Creates a new AnimatableValue from a CSS {@link Value}. Returns null since
		 * rects aren't used in CSS values.
		 */
		@Override
		public AnimatableValue createValue(AnimationTarget target, String pn, Value v) {
			return null;
	...
	protected static class AnimatablePointListValueFactory implements Factory {
		...
		public AnimatableValue createValue(AnimationTarget target, String ns, String ln, boolean isCSS, String s) {
			try {
				parser.parse(s);
				return new AnimatablePointListValue(target, producer.getFloatArray());
			} catch (ParseException e) {
				// XXX Do something better than returning null.
				return null;
			}
	...
	protected static class AnimatablePathDataFactory implements Factory {
		...
		public AnimatableValue createValue(AnimationTarget target, String ns, String ln, boolean isCSS, String s) {
			try {
				parser.parse(s);
				return new AnimatablePathDataValue(target, producer.getPathCommands(), producer.getPathParameters());
			} catch (ParseException e) {
				// XXX Do something better than returning null.
				return null;
			}
	...
	protected class AnimatableNumberOrIdentFactory extends CSSValueFactory {
		...
		/**
		 * Whether numbers are actually numeric keywords, as with the font-weight
		 * property.
		 */
	...
	protected class AnimatableColorValueFactory extends CSSValueFactory {
		...
		protected AnimatableValue createAnimatableValue(AnimationTarget target, String pn, Value v) {
			...
			if (p instanceof Color) {
				Color c = (Color) p;
				return new AnimatableColorValue(target, c.getRed() / 255f, c.getGreen() / 255f, c.getBlue() / 255f);
			}
	...
	protected class AnimatablePaintValueFactory extends CSSValueFactory {
		...
		protected AnimatablePaintValue createColorPaintValue(AnimationTarget t, Color c) {
			return AnimatablePaintValue.createColorPaintValue(t, c.getRed() / 255f, c.getGreen() / 255f,
					c.getBlue() / 255f);
