...
import io.sf.carte.echosvg.anim.SimpleAnimation;
...
public class SVGAnimateElementBridge extends SVGAnimationElementBridge {
	...
	protected AbstractAnimation createAnimation(AnimationTarget target) {
		...
		return new SimpleAnimation(timedElement, this, parseCalcMode(), parseKeyTimes(), parseKeySplines(),
	...
	protected int parseCalcMode() {
		...
		if (animationType == AnimationEngine.ANIM_TYPE_CSS && !targetElement.isPropertyAdditive(attributeLocalName)
				|| animationType == AnimationEngine.ANIM_TYPE_XML
						&& !targetElement.isAttributeAdditive(attributeNamespaceURI, attributeLocalName)) {
	...
	protected float[] parseKeyTimes() {
		String keyTimesString = element.getAttributeNS(null, SVG_KEY_TIMES_ATTRIBUTE);
	...
	protected float[] parseKeySplines() {
		String keySplinesString = element.getAttributeNS(null, SVG_KEY_SPLINES_ATTRIBUTE);
