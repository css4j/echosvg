...
import io.sf.carte.echosvg.gvt.font.KerningTable;
...
public final class SVGGVTFont implements GVTFont, SVGConstants {
	 ...
	 * @param glyphUnicodes       An array containing the unicode values for all the
	 *                            glyphs this font can display.
	 * @param glyphNames          An array containing the names of all the glyphs
	 *                            this font can display.
	 * @param ctx                 The bridge context.
	 * @param glyphElements       An array containing the children glyph elements of
	 *                            the SVG font.
	 * @param missingGlyphElement The missing glyph element for this font.
	 * @param hkernElements       An array containing all hkern elements for this
	 *                            font.
	 * @param vkernElements       An array containing all vkern elements for this
	 *                            font.
	 * @param textElement         The text element that contains the text to be
	 *                            rendered using this font.
	 */
	public SVGGVTFont(float fontSize, GVTFontFace fontFace, String[] glyphUnicodes, String[] glyphNames,
			String[] glyphLangs, String[] glyphOrientations, String[] glyphForms, BridgeContext ctx,
			Element[] glyphElements, Element missingGlyphElement, Element[] hkernElements, Element[] vkernElements,
			Element textElement) {
	...
	/**
	 * Creates the kerning tables for this font. Two tables are created, horizontal
	 * and vertical. If there are not children vkern or hkern elements these tables
	 * will be empty.
	 */
	...
	/**
	 * Returns the horizontal kerning value for the specified glyph pair. This will
	 * be zero if there is no explicit horizontal kerning value for this particular
	 * glyph pair.
	 *
	...
	public float getHKern(int glyphCode1, int glyphCode2) {
		if (glyphCode1 < 0 || glyphCode1 >= glyphUnicodes.length || glyphCode2 < 0
				|| glyphCode2 >= glyphUnicodes.length) {
			return 0f;
		}
		float ret;
		ret = hKerningTable.getKerningValue(glyphCode1, glyphCode2, glyphUnicodes[glyphCode1],
				glyphUnicodes[glyphCode2]);
		return ret * scale;
	}

	/**
	 * Returns the vertical kerning value for the specified glyph pair. This will be
	 * zero if there is no explicit vertical kerning value for for this particular
	 * glyph pair.
	 *
	 * @param glyphCode1 The id of the first glyph.
	 * @param glyphCode2 The id of the second glyph.
	...
	public float getVKern(int glyphCode1, int glyphCode2) {
		...
		float ret;
		ret = vKerningTable.getKerningValue(glyphCode1, glyphCode2, glyphUnicodes[glyphCode1],
				glyphUnicodes[glyphCode2]);
		return ret * scale;
	}

	/**
	 * Returns an array of glyph codes (unique ids) of the glyphs with the specified
	 * name (there may be more than one).
	 *
	...
	/**
	 * Returns an array of glyph codes (unique ids) of the glyphs with the specified
	 * unicode value (there may be more than one).
	 *
	...
	/**
	 * Returns true if the glyph language matches the language of the text node to
	 * be rendered by this font. This will be the case if one of the languages in
	 * glyphLang matches exactly with the xml:lang attibute of the text node, or if
	 * the xml:lang attribute exactly equals a prefix of one glyph languages.
	 *
	 * @param glyphLang A comma separated list of languages that are associated with
	 *                  a glyph.
	 *
	 * @return Whether or not the glyph language matches the language of the text
	 *         node.
	 */
	private boolean languageMatches(String glyphLang) {
		...
		while (st.hasMoreTokens()) {
			...
			if (s.equals(language) || (s.startsWith(language) && s.length() > language.length()
	...
	/**
	 * Returns true if the glyph orientation matches the orientation of the text
	 * node to be rendered by this font.
	 *
	 * @param glyphOrientation The glyph orientation attribute value. Will be "h",
	 *                         "v" or empty.
	 *
	 * @return Whether or not the glyph orientation matches the text to be rendered
	 *         by this font object.
	 */
	...
	/**
	 * Returns true if the glyph form matches that of the current character in the
	 * aci.
	 *
	...
	public boolean canDisplayGivenName(String name) {
		for (int i = 0; i < glyphNames.length; i++) {
			if (glyphNames[i] != null && glyphNames[i].equals(name) && languageMatches(glyphLangs[i])
	...
	public boolean canDisplay(char c) {
		for (int i = 0; i < glyphUnicodes.length; i++) {
			if (glyphUnicodes[i].indexOf(c) != -1 && languageMatches(glyphLangs[i])
					&& orientationMatches(glyphOrientations[i])) {
				return true;
			}
		}
	...
	/**
	 * Checks whether this Font can display the characters in the specified
	 * character array starting at start and ending at limit.
	 *
	 ...
	 *
	 * @return The index of the first character it can't display or -1 if it can
	...
	/**
	 * Checks whether this Font can display the characters in the specified
	 * character iterator starting at start and ending at limit.
	 *
	...
	public int canDisplayUpTo(CharacterIterator iter, int start, int limit) {
		...
		while (c != CharacterIterator.DONE && currentIndex < limit) {
			...
			for (int i = 0; i < glyphUnicodes.length; i++) {
				if (glyphUnicodes[i].indexOf(c) == 0 && languageMatches(glyphLangs[i])
						&& orientationMatches(glyphOrientations[i])
						&& formMatches(glyphUnicodes[i], glyphForms[i], aci, currentIndex)) {
	...
	/**
	 * Checks whether or not this font can display the characters in the specified
	 * String.
	 *
	...
	/**
	 * Returns a new GVTGlyphVector object for the characters in the specified
	 * character iterator.
	 *
	 * @param frc The current font render context.
	...
	/**
	 * Returns a new GVTGlyphVector object for the glyphs in the the glyph code
	 * array.
	 *
	 * @param frc        The current font render context.
	 * @param glyphCodes An array containin the ids of the glyphs that the glyph
	 *                   vector will represent.
	 *
	 * @return The new glyph vector.
	 */
	@Override
	...
	/**
	 * Creates a new GVTFont object by replicating this font object and applying a
	 * new size to it.
	 *
	...
	public GVTFont deriveFont(float size) {
		return new SVGGVTFont(size, fontFace, glyphUnicodes, glyphNames, glyphLangs, glyphOrientations, glyphForms, ctx,
				glyphElements, missingGlyphElement, hkernElements, vkernElements, textElement);
	}
	...
	GVTLineMetrics getLineMetrics(int beginIndex, int limit) {
		if (lineMetrics != null)
			return lineMetrics;
		...
		lineMetrics = new GVTLineMetrics(ascent, Font.ROMAN_BASELINE, baselineOffsets, descent, fontHeight, fontHeight,
				limit - beginIndex, stOffset, stThickness, ulOffset, ulThickness, olOffset, olThickness);
		return lineMetrics;
	}
	...
	/**
	 * Returns a string representation of this font. This is for debugging purposes
	 * only.
	 *
