...
public class SVGAltGlyphElementBridge extends AbstractSVGBridge implements ErrorConstants {
	...
	/**
	 * Constructs an array of Glyphs that represents the specified &lt;altGlyph&gt;
	 * element at the requested size.
	 *
	 ...
	 *         unavailable.
	 */
	public Glyph[] createAltGlyphArray(BridgeContext ctx, Element altGlyphElement, float fontSize,
		...
		if (refElement.getLocalName().equals(SVG_ALT_GLYPH_DEF_TAG)) {
			...
			if (containsGlyphRefNodes) { // process the glyphRef children
				...
				NodeList glyphRefNodes = localRefElement.getElementsByTagNameNS(SVG_NAMESPACE_URI, SVG_GLYPH_REF_TAG);
				...
				NodeList altGlyphItemNodes = localRefElement.getElementsByTagNameNS(SVG_NAMESPACE_URI,
				...
				if (numAltGlyphItemNodes > 0) {
					...
					for (int i = 0; i < numAltGlyphItemNodes && !foundMatchingGlyph; i++) {
						...
						NodeList altGlyphRefNodes = altGlyphItemElement.getElementsByTagNameNS(SVG_NAMESPACE_URI,
								SVG_GLYPH_REF_TAG);
		...
		/*
		 * // reference is not to a valid element type, throw an exception throw new
		 * BridgeException(altGlyphElement, ERR_URI_BAD_TARGET, new Object[] {uri});
		 */
	...
	/**
	 * Returns a Glyph object that represents the glyph at the specified URI scaled
	 ...
	 */
	private Glyph getGlyph(BridgeContext ctx, String glyphUri, Element altGlyphElement, float fontSize,
			AttributedCharacterIterator aci) {
		...
		if ((refGlyphElement == null) || (!SVG_NAMESPACE_URI.equals(refGlyphElement.getNamespaceURI()))
		...
		if (isLocal) {
			...
			NodeList fontFaceElements = localFontElement.getElementsByTagNameNS(SVG_NAMESPACE_URI, SVG_FONT_FACE_TAG);
			...
			NodeList glyphElements = localFontElement.getElementsByTagNameNS(SVG_NAMESPACE_URI, SVG_GLYPH_TAG);
