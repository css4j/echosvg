...
public class XMLUtilities extends XMLCharacters {
	...
	/**
	 * Tests whether the given character is usable as the first character of an XML
	 * name.
	 */
	...
	/**
	 * Tests whether the given 32 bits character is valid in XML documents. Because
	 * the majority of code-points is covered by the table-lookup-test, we do it
	...
	public static boolean isXML11Character(int c) {
		return c >= 1 && c <= 0xd7ff || c >= 0xe000 && c <= 0xfffd || c >= 0x10000 && c <= 0x10ffff;
	}
	 ...
	 * 
	 * @return A bitfield of {@link #IS_XML_10_NAME} and {@link #IS_XML_10_QNAME}.
	 */
	...
	/**
	 * Creates a Reader initialized to scan the characters in the given XML
	 * document's InputStream.
	 * 
	 * @param is The input stream positionned at the beginning of an XML document.
	 * @return a Reader positionned at the beginning of the XML document It is
	 *         created from an encoding figured out from the first few bytes of the
	 *         document. As a consequence the given input stream is not positionned
	 *         anymore at the beginning of the document when this method returns.
	 */
	...
	/**
	 * Creates a reader from the given input stream and encoding. This method
	 * assumes the input stream working buffer is at least 128 byte long. The input
	 * stream is restored before this method returns. The 4 first bytes are skipped
	...
	/**
	 * Converts the given standard encoding representation to the corresponding Java
	 * encoding string.
	 * 
