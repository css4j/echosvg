...
public class XMLScanner implements Localizable {
	 ...
	 */
	protected static final String BUNDLE_CLASSNAME = "io.sf.carte.echosvg.xml.resources.Messages";
			...
			case '!':
				...
				case 'E':
					...
					case 'L':
						...
						return readIdentifier("EMENT", LexicalUnits.ELEMENT_DECLARATION_START, -1);
					case 'N':
						context = ENTITY_CONTEXT;
						return readIdentifier("TITY", LexicalUnits.ENTITY_START, -1);
					default:
						throw createXMLException("invalid.character");
					}
				...
				case 'A':
					context = ATTLIST_CONTEXT;
					return readIdentifier("TTLIST", LexicalUnits.ATTLIST_START, -1);
				case 'N':
					context = NOTATION_CONTEXT;
					return readIdentifier("OTATION", LexicalUnits.NOTATION_START, -1);
				default:
					throw createXMLException("invalid.character");
				}
			default:
	 ...
	 * public/system identifiers... The current character must be the string
	 * delimiter.
	 * 
	 * @return type.
	 */
	 ...
	 * @param ntype The lexical unit type to set if the identifier do not match or
	 *              -1 if an error must be signaled.
	 */
	...
	protected int nextInElementDeclaration() throws IOException, XMLException {
		...
		case 'E':
			return readIdentifier("MPTY", LexicalUnits.EMPTY_IDENTIFIER, LexicalUnits.NAME);

		case 'A':
			return readIdentifier("NY", LexicalUnits.ANY_IDENTIFIER, LexicalUnits.NAME);
	...
	protected int nextInAttList() throws IOException, XMLException {
		...
		case 'C':
			return readIdentifier("DATA", LexicalUnits.CDATA_IDENTIFIER, LexicalUnits.NAME);
		...
		case 'N':
			...
			case 'O':
				...
				return readIdentifier("TATION", LexicalUnits.NOTATION_IDENTIFIER, LexicalUnits.NAME);
	...
	protected int nextInEntity() throws IOException, XMLException {
		...
		case 'N':
			return readIdentifier("DATA", LexicalUnits.NDATA_IDENTIFIER, LexicalUnits.NAME);
	...
	protected int nextInEntityValue() throws IOException, XMLException {
		...
		default:
			while (current != -1 && current != attrDelimiter && current != '&' && current != '%') {
				nextChar();
			}
			switch (current) {
			case -1:
	...
	protected XMLException createXMLException(String message) {
		String m;
		try {
			m = formatMessage(message, new Object[] { reader.getLine(), reader.getColumn() });
