...
public class AnimatableAngleOrIdentValue extends AnimatableAngleValue {
	...
	public AnimatableValue getZeroValue() {
		return new AnimatableAngleOrIdentValue(target, 0, SVGAngle.SVG_ANGLETYPE_UNSPECIFIED);
	}
		...
		if (to == null) {
			...
			if (isIdent || toValue.isIdent) {
				if (interpolation >= 0.5) {
					if (res.isIdent != toValue.isIdent || res.unit != toValue.unit || res.value != toValue.value
					...
					if (res.isIdent != isIdent || res.unit != unit || res.value != value
