...
public class AnimatablePaintValue extends AnimatableColorValue {
	...
	/**
	 * Creates a new AnimatablePaintValue for a URI reference with a 'none'
	 * fallback.
	 */
	...
	/**
	 * Creates a new AnimatablePaintValue for a URI reference with a 'currentColor'
	 * fallback.
	 */
	 ...
	 */
	public static AnimatablePaintValue createURIColorPaintValue(AnimationTarget target, String uri, float r, float g,
			float b) {
		AnimatablePaintValue v = new AnimatablePaintValue(target, r, g, b);
		v.uri = uri;
	...
	public AnimatableValue interpolate(AnimatableValue result, AnimatableValue to, float interpolation,
		...
		if (paintType == PAINT_COLOR) {
			...
			if (accumulation != null) {
				...
				canInterpolate = canInterpolate && accPaint.paintType == PAINT_COLOR;
			}
			if (canInterpolate) {
		...
		if (res.paintType != newPaintType || res.uri == null || !res.uri.equals(newURI) || res.red != newRed
				|| res.green != newGreen || res.blue != newBlue) {
