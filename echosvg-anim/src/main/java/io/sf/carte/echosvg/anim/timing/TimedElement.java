...
public abstract class TimedElement implements SMILConstants {
	...
	/**
	 * Duration of this element, if {@link #durMedia} <code>= false</code>. If
	 * unspecified, it will be {@link #UNRESOLVED}.
	 */
	protected float simpleDur;

	/**
	 * Whether the simple duration of this element should be equal to the implicit
	 * duration.
	 */
	protected boolean durMedia;

	/**
	 * The number of repeats. If unspecified, it will be {@link #UNRESOLVED}.
	 */
	protected float repeatCount;

	/**
	 * The duration of repeats. If unspecified, it will be {@link #UNRESOLVED}.
	 */
	...
	/**
	 * The fill mode for this element. Uses the FILL_* constants defined in this
	 * class.
	 */
	protected int fillMode;

	/**
	 * The restart mode for this element. Uses the RESTART_* constants defined in
	 * this class.
	 */
	 ...
	 */
	protected List<InstanceTime> beginInstanceTimes = new ArrayList<>();

	/**
	 * List of end InstanceTimes.
	 */
	protected List<InstanceTime> endInstanceTimes = new ArrayList<>();
	 ...
	 * begin times.
	 */
	protected LinkedList<TimingSpecifier> beginDependents = new LinkedList<>();

	/**
	 * List of TimingSpecifiers on other elements that depend on this element's end
	 * times.
	 */
	protected LinkedList<TimingSpecifier> endDependents = new LinkedList<>();

	/**
	 * Whether the list of instance times should be checked to update the current
	 * interval.
	 */
	...
	/**
	 * Map of {@link Event} objects to {@link HashSet}s of {@link TimingSpecifier}s
	 * that caught them.
	 */
	protected Map<Event, Set<TimingSpecifier>> handledEvents = new HashMap<>();
	...
	/**
	 * Whether an instance time update message has already been propagated to this
	 * timed element.
	 */
	...
	/**
	 * Called by a TimingSpecifier of this element when a new InstanceTime is
	 * created. This will be in response to an event firing, a DOM method being
	 * called or a new Instance being created by a syncbase element.
	 */
	protected float addInstanceTime(InstanceTime time, boolean isBegin) {
		// Trace.enter(this, "addInstanceTime", new Object[] { time, new
		// Boolean(isBegin) } ); try {
		hasPropagated = true;
		List<InstanceTime> instanceTimes = isBegin ? beginInstanceTimes : endInstanceTimes;
	...
	/**
	 * Called by a TimingSpecifier of this element when an InstanceTime should be
	 * removed. This will be in response to the pruning of an Interval.
	 */
	protected float removeInstanceTime(InstanceTime time, boolean isBegin) {
		// Trace.enter(this, "removeInstanceTime", new Object[] { time, new
		// Boolean(isBegin) } ); try {
		hasPropagated = true;
		List<InstanceTime> instanceTimes = isBegin ? beginInstanceTimes : endInstanceTimes;
		int index = Collections.binarySearch(instanceTimes, time);
		for (int i = index; i >= 0; i--) {
			InstanceTime it = instanceTimes.get(i);
			if (it == time) {
	...
	/**
	 * Called by a TimingSpecifier of this element when an InstanceTime has been
	 * updated. This will be in response to a dependent syncbase change.
	 */
	protected float instanceTimeChanged(InstanceTime time, boolean isBegin) {
		// Trace.enter(this, "instanceTimeChanged", new Object[] { time, new
		// Boolean(isBegin) } ); try {
		hasPropagated = true;
		shouldUpdateCurrentInterval = true;
		float ret;
	...
	public float getSimpleDur() {
		if (durMedia) {
			...
			if (isUnresolved(repeatCount) && isUnresolved(repeatDur) && endTimes.length > 0) {
				return INDEFINITE;
			}
			return getImplicitDur();
		} else {
	...
	/**
	 * Returns whether the given time value is equal to the {@link #UNRESOLVED}
	 * value.
	 */
	...
	public float getActiveDur(float B, float end) {
		...
		if (d == 0) {
			...
			if (isUnresolved(repeatDur) && isUnresolved(repeatCount)) {
				...
				float p1 = isUnresolved(repeatCount) ? INDEFINITE : multiplyTime(d, repeatCount);
				float p2 = isUnresolved(repeatDur) ? INDEFINITE : repeatDur;
	...
	protected float minTime(float t1, float t2) {
		...
		if ((t1 == INDEFINITE || isUnresolved(t1)) && t2 != INDEFINITE && !isUnresolved(t2)) {
			return t2;
		}
		if ((t2 == INDEFINITE || isUnresolved(t2)) && t1 != INDEFINITE && !isUnresolved(t1)) {
			return t1;
		}
	 ...
	 * containers are not supported by SVG so this just returns {@link #UNRESOLVED}
	 * by default. This should be overriden in derived classes that play media,
	 * since they will have an implicit duration.
	 */
	...
	protected float notifyNewInterval(Interval interval) {
		...
		Iterator<TimingSpecifier> i = beginDependents.iterator();
		while (i.hasNext()) {
			TimingSpecifier ts = i.next();
	...
	protected float sampleAt(float parentSimpleTime, boolean hyperlinking) {
		// Trace.enter(this, "sampleAt", new Object[] { Float.valueOf(parentSimpleTime)
		// } ); try {
		isSampling = true;
		...
		for (Map.Entry<Event, Set<TimingSpecifier>> e : handledEvents.entrySet()) {
			Event evt = e.getKey();
			Set<TimingSpecifier> ts = e.getValue();
			Iterator<TimingSpecifier> j = ts.iterator();
			boolean hasBegin = false, hasEnd = false;
			while (j.hasNext() && !(hasBegin && hasEnd)) {
				TimingSpecifier t = j.next();
				if (t.isBegin()) {
					hasBegin = true;
				} else {
					hasEnd = true;
			...
			if (hasBegin && hasEnd) {
				...
				useEnd = !useBegin;
			} else if (hasBegin && (!isActive || restartMode == RESTART_ALWAYS)) {
				useBegin = true;
				useEnd = false;
		...
		if (currentInterval != null) {
			...
			if (time >= begin) {
				float d = getSimpleDur();
				while (time - lastRepeatTime >= d && lastRepeatTime + d < begin + repeatDuration) {
					lastRepeatTime += d;
					currentRepeatIteration++;
					fireTimeEvent(root.getRepeatEventName(), lastRepeatTime, currentRepeatIteration);
				}
			}
		}
		...
		while (shouldUpdateCurrentInterval || hasEnded) {
			...
			if (currentInterval == null || hasEnded) {
				if (first || hyperlinking || restartMode != RESTART_NEVER) {
					...
					Interval interval = computeInterval(first, false, beginAfter, incl);
					if (interval == null) {
						currentInterval = null;
					} else {
				...
				if (currentBegin > time) {
					...
					Interval interval = computeInterval(false, false, beginAfter, incl);
					...
					Interval interval = computeInterval(false, true, currentBegin, true);
					float newEnd = interval.getEnd();
					if (currentInterval.getEnd() != newEnd) {
						float dmt = currentInterval.setEnd(newEnd, interval.getEndInstanceTime());
						if (dmt < dependentMinTime) {
							dependentMinTime = dmt;
						}
					}
		...
		if (isActive && !isFrozen) {
			if (time - currentInterval.getBegin() >= repeatDuration) {
				...
				// Trace.print("element active, sampling at simple time " + (time -
				// lastRepeatTime));
		...
		if (isFrozen) {
			...
			if (isActive) {
				...
				// Interval previousInterval = (Interval) previousIntervals.getLast(); // does
				// this always make sense?
				t = previousInterval.getEnd() - lastRepeatTime; // at least i would use >=
				atLast = lastRepeatTime + d == previousInterval.getEnd(); // <- same here
			}
			if (atLast) {
				// Trace.print("element frozen" + (isActive ? " (but still active)" : "") + ",
				// sampling last value");
				sampledLastValue(currentRepeatIteration);
			} else {
				// Trace.print("element frozen" + (isActive ? " (but still active)" : "") + ",
				// sampling at simple time " + (t % d));
	...
	/**
	 * Sets the current interval to the one specified. This will notify dependents
	 * and fire the 'begin' and any necessary 'repeat' events.
	 * 
	 * @param time     the current sampling time
	 * @param interval the Interval object to select to be current
	 * @return the minimum time the animation engine can safely wait, as determined
	 *         by dependents of the interval
	 */
	protected float selectNewInterval(float time, Interval interval) {
		...
		if (time >= beginEventTime) {
			...
			while (time - lastRepeatTime >= d && lastRepeatTime + d < end) {
				lastRepeatTime += d;
				currentRepeatIteration++;
				fireTimeEvent(root.getRepeatEventName(), lastRepeatTime, currentRepeatIteration);
			}
	 ...
	 * @param beginAfter the earliest possible begin time for the computed interval.
	...
	protected Interval computeInterval(boolean first, boolean fixedBegin, float beginAfter, boolean incl) {
		// Trace.enter(this, "computeInterval", new Object[] { new Boolean(first), new
		// Boolean(fixedBegin), Float.valueOf(beginAfter)} ); try {
		// Trace.print("computing interval from begins=" + beginInstanceTimes + ",
		// ends=" + endInstanceTimes);
		Iterator<InstanceTime> beginIterator = beginInstanceTimes.iterator();
		Iterator<InstanceTime> endIterator = endInstanceTimes.iterator();
		...
		for (;;) {
			...
			if (endTimes.length == 0) {
				...
				if (endInstanceTimes.isEmpty()) {
					...
					if (first && !firstEnd && tempEnd == tempBegin
							|| !first && currentInterval != null && tempEnd == currentInterval.getEnd()
									&& (incl && beginAfter >= tempEnd || !incl && beginAfter > tempEnd)) {
						for (;;) {
			...
			if (!first || tempEnd > 0 || tempBegin == 0 && tempEnd == 0 || isUnresolved(tempEnd)) {
				// Trace.print("considering restart semantics");
				if (restartMode == RESTART_ALWAYS && nextBeginInstanceTime != null) {
	...
	protected void reset(boolean clearCurrentBegin) {
		Iterator<InstanceTime> i = beginInstanceTimes.iterator();
		while (i.hasNext()) {
			InstanceTime it = i.next();
			if (it.getClearOnReset()
					&& (clearCurrentBegin || currentInterval == null || currentInterval.getBeginInstanceTime() != it)) {
				i.remove();
			}
		}
	 ...
	 */
	public void parseAttributes(String begin, String dur, String end, String min, String max, String repeatCount,
			String repeatDur, String fill, String restart) {
		if (!hasParsed) {
			parseBegin(begin);
			parseDur(dur);
			parseEnd(end);
	...
	protected void parseBegin(String begin) {
		try {
			...
			throw createException("attribute.malformed", new Object[] { null, SMIL_BEGIN_ATTRIBUTE });
		}
	}
		...
		if (dur.equals(SMIL_MEDIA_VALUE)) {
			...
			if (dur.length() == 0 || dur.equals(SMIL_INDEFINITE_VALUE)) {
				...
				try {
					...
					throw createException("attribute.malformed", new Object[] { null, SMIL_DUR_ATTRIBUTE });
				}
				if (simpleDur < 0) {
					simpleDur = INDEFINITE;
				}
	...
	protected void parseEnd(String end) {
		try {
			endTimes = TimingSpecifierListProducer.parseTimingSpecifierList(TimedElement.this, false, end,
					root.useSVG11AccessKeys, root.useSVG12AccessKeys);
		} catch (ParseException ex) {
			throw createException("attribute.malformed", new Object[] { null, SMIL_END_ATTRIBUTE });
		}
	}
	...
	protected void parseRepeatCount(String repeatCount) {
		if (repeatCount.length() == 0) {
			...
			try {
				...
				throw createException("attribute.malformed", new Object[] { null, SMIL_REPEAT_COUNT_ATTRIBUTE });
			}
		}
	}
	...
	protected void parseRepeatDur(String repeatDur) {
		try {
			...
			throw createException("attribute.malformed", new Object[] { null, SMIL_REPEAT_DUR_ATTRIBUTE });
		}
	}
	...
	protected void parseFill(String fill) {
		if (fill.length() == 0 || fill.equals(SMIL_REMOVE_VALUE)) {
			...
			throw createException("attribute.malformed", new Object[] { null, SMIL_FILL_ATTRIBUTE });
		}
	}
	...
	protected void parseRestart(String restart) {
		if (restart.length() == 0 || restart.equals(SMIL_ALWAYS_VALUE)) {
			...
			throw createException("attribute.malformed", new Object[] { null, SMIL_RESTART_ATTRIBUTE });
		}
	}
	...
	/**
	 * Adds a time to the begin time instance list that will cause the element to
	 * begin immediately (if restart semantics allow it).
	 */
	...
	/**
	 * Adds a time to the begin time instance list that will cause the element to
	 * begin at some offset to the current time (if restart semantics allow it).
	 */
	...
	/**
	 * Adds a time to the end time instance list that will cause the element to end
	 * immediately (if restart semantics allow it).
	 */
	...
	/**
	 * Adds a time to the end time instance list that will cause the element to end
	 * at some offset to the current time (if restart semantics allow it).
	 */
	...
	/**
	 * Returns the begin time of the current interval, in parent simple time, or
	 * <code>Float.NaN</code> if the element is not active.
	 */
	public float getCurrentBeginTime() {
		float begin;
		if (currentInterval == null || (begin = currentInterval.getBegin()) < lastSampleTime) {
			return Float.NaN;
		}
		return begin;
	}
	...
	public boolean canBegin() {
		return currentInterval == null || isActive && restartMode != RESTART_NEVER;
	}
	...
	public float getHyperlinkBeginTime() {
		...
		if (!beginInstanceTimes.isEmpty()) {
			return beginInstanceTimes.get(0).getTime();
		}
		return Float.NaN;
	}

	/**
	 * Returns an array of the begin timing specifiers on this timed element. XXX
	 * Note that this is an experimental public API and is subject to change, when
	 * somebody gets around to thinking of a better model for exposing animation
	...
	/**
	 * Returns an array of the end timing specifiers on this timed element. XXX Note
	 * that this is an experimental public API and is subject to change, when
	 * somebody gets around to thinking of a better model for exposing animation
	 * information from the document.
	 */
	...
	/**
	 * Invoked by a {@link TimingSpecifier} to indicate that an event occurred that
	 * would create a new instance time for this timed element. These will be
	...
	/**
	 * Returns the DOM element that corresponds to this timed element, if such a DOM
	 * element exists.
	 */
	 ...
	 */
	protected static final String RESOURCES = "io.sf.carte.echosvg.anim.resources.Messages";

	/**
	 * The localizable support for the error messages.
	 */
	protected static LocalizableSupport localizableSupport = new LocalizableSupport(RESOURCES,
			TimedElement.class.getClassLoader());

	/**
	 * Implements
	 * {@link io.sf.carte.echosvg.i18n.Localizable#setLocale(java.util.Locale)}.
	 */
