...
// import io.sf.carte.echosvg.anim.timing.Trace;
...
public abstract class AbstractAnimation {
	...
	public AnimatableValue getComposedValue() {
		...
		// Trace.print("isActive == " + isActive + ", isFrozen == " + isFrozen + ",
		// isDirty == " + isDirty);
		if (!isActive && !isFrozen) {
			return null;
		}
		if (isDirty) {
			...
			composedValue = value.interpolate(composedValue, null, 0f, lowerValue, 1);
	...
	protected void markDirty() {
		isDirty = true;
		if (higherAnimation != null && !higherAnimation.willReplace() && !higherAnimation.isDirty) {
	...
	/**
	 * Called when the element is sampled at the given time. This updates the
