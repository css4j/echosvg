...
import io.sf.carte.echosvg.util.DoublyIndexedSet;
...
public abstract class TimedDocumentRoot extends TimeContainer {
	...
	/**
	 * Allows the use of accessKey() timing specifiers with a single character, as
	 * specified in SVG 1.1.
	 */
	protected boolean useSVG11AccessKeys;

	/**
	 * Allows the use of accessKey() timing specifiers with a DOM 3 key name, as
	 * specified in SVG 1.2.
	 */
	...
	/**
	 * List of {link TimegraphListener}s to be notified of changes to the timed
	...
	/**
	 * Returns the implicit duration of the element. The document root has an
	 * {@link #INDEFINITE} implicit duration.
	 */
	@Override
	...
	/**
	 * Returns the default begin time for the given child timed element. In SVG,
	 * this is always 0, since the only time container is the root SVG element,
	 * which acts like a 'par'.
	 */
	@Override
	...
	/**
	 * Fires an {@link TimegraphListener#elementAdded} event on all timegraph
	 * listeners.
	 */
	...
	/**
	 * Fires an {@link TimegraphListener#elementRemoved} event on all timegraph
	 * listeners.
	 */
	...
	/**
	 * Returns whether the specified newly created {@link Interval} should propagate
	 * its times to the given {@link TimingSpecifier}.
	 * 
	 * @param i       the Interval that has just been created
	 * @param ts      the TimingSpecifier that is a dependent of the Interval
	 * @param isBegin whether the dependency is on the begin or end time of the
	 *                Interval
	 */
	 ...
	 * interval will be re-evaluated at the next sample. This should be overridden
	 * in a concrete class so that ticks can be scheduled immediately if they are
	 * currently paused due to no animations being active.
	 */
