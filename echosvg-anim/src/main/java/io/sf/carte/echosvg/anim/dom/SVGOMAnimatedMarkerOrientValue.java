 ...
 * {@link SVGAnimatedEnumeration} for the 'marker' element's 'orient' attribute.
 *
	...
	protected class BaseSVGAngle extends SVGOMAngle {
		...
		protected void reset() {
			try {
				...
				valid = true;
				String value;
				if (baseEnumerationVal == SVGMarkerElement.SVG_MARKER_ORIENT_ANGLE) {
					value = getValueAsString();
				} else if (baseEnumerationVal == SVGMarkerElement.SVG_MARKER_ORIENT_AUTO) {
		...
		/**
		 * Parse a String value as an SVGAngle. If orient="auto", the method will parse
		 * the value "0" instead.
		 */
		@Override
