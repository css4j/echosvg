...
public class XBLEventSupport extends EventSupport {
	...
	public void addEventListenerNS(String namespaceURI, String type, EventListener listener, boolean useCapture,
		...
		if (namespaceURI == null || namespaceURI.equals(XMLConstants.XML_EVENTS_NAMESPACE_URI)) {
	...
	/**
	 * Registers an event listener that will not be stopped by the usual XBL
	 * stopping.
	 */
	...
	public void removeImplementationEventListenerNS(String namespaceURI, String type, EventListener listener,
			boolean useCapture) {
		HashMap<String, EventListenerList> listeners = useCapture ? capturingImplementationListeners
	 ...
	 * {@link io.sf.carte.echosvg.dom.AbstractDocument#renameNode(Node,String,String)}.
	 */
	@Override
...
//             System.err.println("\t-- * CAPTURING " + e.getType() + "  " + ((Node) node).getNodeName());
			...
			setEventPhase(es[i], Event.CAPTURING_PHASE);
			fireImplementationEventListeners(node, es[i], true);
			fireEventListeners(node, es[i], true, stoppedGroups, toBeStoppedGroups);
			fireHandlerGroupEventListeners(node, es[i], true, stoppedGroups, toBeStoppedGroups);
...
//         System.err.println("\t-- * AT_TARGET " + e.getType() + "  " + ((Node) target).getNodeName());
		...
		fireHandlerGroupEventListeners(node, e, false, stoppedGroups, toBeStoppedGroups);
		stoppedGroups.addAll(toBeStoppedGroups);
		toBeStoppedGroups.clear();
...
//                 System.err.println("\t-- * BUBBLING  " + e.getType() + "  " + ((Node) node).getNodeName());
				...
				fireEventListeners(node, es[i], false, stoppedGroups, toBeStoppedGroups);
				fireHandlerGroupEventListeners(node, es[i], false, stoppedGroups, toBeStoppedGroups);
				preventDefault = preventDefault || es[i].getDefaultPrevented();
				stoppedGroups.addAll(toBeStoppedGroups);
				toBeStoppedGroups.clear();
			}
...
//                 System.err.println("\t--   BUBBLING  " + e.getType() + "  " + ((Node) node).getNodeName());
	 ...
	 */
	protected void fireHandlerGroupEventListeners(NodeEventTarget node, AbstractEvent e, boolean useCapture,
	...
	/**
	 * Returns whether the given event should be stopped once it crosses a shadow
	 * scope boundary.
	 */
	...
	/**
	 * Returns an array of Event objects to be used for each event target in the
	 * event flow. The Event objects are retargetted if an sXBL shadow scope is
	 * crossed and the event is not a DOM mutation event.
	 */
	protected AbstractEvent[] getRetargettedEvents(NodeEventTarget target, NodeEventTarget[] ancestors,
			AbstractEvent e) {
